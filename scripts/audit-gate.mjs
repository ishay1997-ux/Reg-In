...
const WAIVERS = [
  ...
  // **מבטל את הנימוק הזה** וחייב למדוד מחדש.
  {
    ...
    reviewTrigger: 'סגירת המודול הבא, או הגשת הפרויקט 19/09/2026 — המוקדם מביניהם.',
  },
  {
  ...
  },
  {
    ...
    date: '09/08/2026',
    reason:
      'לולאה אינסופית ב-nanoid כשמעבירים לו מחולל-מותאם עם `size=0`. מגיעה דרך ' +
      '`shadcn → postcss → nanoid` — אותה משפחת CLI-פיתוח-בלבד כמו יתר הפטורים למעלה. ' +
      'אומת 09/08/2026: `npm ls nanoid --all` מראה **שרשרת יחידה** (shadcn@4.13.0 → ' +
      'postcss@8.5.24 → nanoid@3.3.16), ו-`grep -ric nanoid dist/` מחזיר **אפס**. ' +
      'ותנאי-הניצול אינו קיים אצלנו כלל: אנחנו לא קוראים ל-nanoid בשום מקום, ו-postcss ' +
      'אינו מעביר לו מחולל-מותאם. מקבלים את הסיכון כי הוא אינו יכול להגיע למשתמש. ' +
      '⚠️ ובכנות: `npm audit fix` מציע לתקן אותה (וגם את js-yaml ו-undici) — **נדחה במכוון** ' +
      'לסגירת המודול, כי שדרוג-תלויות באמצע בניית פזה הוא סיכון-בנייה תמורת אפס שיפור למשתמש.',
    reviewTrigger: 'סגירת המודול הבא, או הגשת הפרויקט 19/09/2026 — המוקדם מביניהם.',
  },
]
