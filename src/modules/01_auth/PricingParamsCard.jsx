...
export default function PricingParamsCard({ canEdit }) {
  ...
  async function handleSave() {
    setFormError('')
    // 🔴 **גם הודעת-ההצלחה הקודמת נמחקת בפתיחת שמירה חדשה** (26/08/2026).
    // בלי זה, שמירה מוצלחת שאחריה שמירה כושלת — **בלי לגעת בשדה בין השתיים** — הציגה את
    // שתיהן זו לצד זו: *"הנתונים נשמרו בהצלחה"* מעל *"שמירת הפרמטרים נכשלה."*
    // הירוק התאפס רק בהקלדה בשדה, **ובניגוד לשלושת אחיו במערכת אין לו טיימר כלל** — כלומר
    // הוא נשאר על המסך לנצח. ⇒ המסך הכריז "נשמר" בזמן שהודיע שנכשל, וזה בדיוק ההפך
    // מכלל-הבית *"לעולם לא 'נשמר' בשקט"*.
    setSavedOk(false)
