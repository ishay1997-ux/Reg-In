...
//   • תאריך-אירוע שעבר (§7.32) — ה-RPC דוחה
...
//
// ⚠️ **עד 31/07/2026 היה כתוב כאן שהודעת ה-RPC "מוצגת כמו-שהיא" — וזה לא היה נכון.**
// ‏`toError` שם אותה ב-`e.cause` שאיש לא רינדר, ולכן כל מסלולי-הכשל של האישור הגיעו כמשפט
// אחד ("אישור ההצעה נכשל."). מאז: `quoteServerErrorMessage` (`src/lib/quotes.js`) ממפה כל
// כשל לניסוח עברי שאומר **מה לעשות**, `api.js` מזריק אותו ל-`Error.message`, והמעטפת
// (`QuoteActionDialog`) מציגה אותו. המחרוזת הגולמית של המסד עצמה אינה מוצגת לעולם — היא
// נושאת ערכי-enum באנגלית ואת התו `%`.
