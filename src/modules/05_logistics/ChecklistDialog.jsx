    ...
    <>
      <DialogHeader>
        ...
        <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
          <span>
            <span className="text-xs text-slate-500">מצב הפרויקט:</span>{' '}
            <StatusTag
              label={statusLabel}
          ...
          <span>
            <span className="text-xs text-slate-500">מוכנות לוגיסטית:</span>{' '}
            <span className="text-sm font-semibold text-slate-800" data-testid="checklist-metric">
        ...
        {!isCancelled && !metricBlocked && subLine && (
          <span className="text-xs font-semibold text-amber-700">{subLine}</span>
        )}
      </DialogHeader>
      ...
      {locked && !isCancelled && (
        <div
          role="status"
          className="rounded-lg border border-amber-200 bg-amber-50 px-3.5 py-2.5 text-sm leading-relaxed text-amber-800"
      ...
      {emptyKind === 'noPermission' ? (
        ...
        <div>
          <p className="mb-2 text-xs text-slate-400">{SORT_LINE}</p>
...
function CancelBanner({ project, canEdit }) {
  return (
    ...
    >
      <b className="mb-0.5 block text-sm">
...
function CompletionBanner({ completion }) {
  return (
    <div
      role="status"
      className="rounded-lg border border-green-200 bg-green-50 px-3.5 py-2.5 text-sm leading-relaxed text-green-700"
      ...
      <tr data-testid={`checklist-row-${key}`}>
        ...
        <td className="px-2.5 py-2.5 align-top">
          ...
          {row.actual_qty_autofilled && !qtyDirty && (
            <span
              className="mb-1 inline-block whitespace-nowrap rounded-full bg-slate-100 px-2 py-px text-xs text-slate-500"
          ...
          {canEdit ? (
            <input
              ...
              className="h-8 w-16 rounded-lg border border-slate-200 bg-white px-2 text-center text-sm text-slate-800 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-500"
            />
          ) : (
          ...
          <span className="mt-0.5 block text-xs text-slate-500">{unit}</span>
        </td>
      ...
      <tr>
        <td className="border-b border-slate-100 px-2.5 pb-2.5" colSpan={columnCount}>
          <div className="flex items-center">
            <span className="ml-2 whitespace-nowrap text-xs text-slate-500">הערה</span>
            {canEdit ? (
              <input
                type="text"
                value={noteValue}
                ...
                className="h-[30px] w-full rounded-lg border border-slate-200 bg-white px-2.5 text-sm text-slate-800 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-500"
              />
            ) : (
              <span className="text-sm text-slate-600" data-testid={`checklist-note-text-${key}`}>
...
function ArrivalCell({
    ...
    return (
      <>
        ...
        {row.actual_arrival_date && (
          <span className="mt-0.5 block text-xs text-slate-500">הגיע בפועל</span>
        )}
      </>
    )
  }
  ...
  if (!canEdit) {
    return (
      <span className="text-sm text-slate-800" data-testid={`checklist-date-text-${testKey}`}>
  ...
  return (
    <input
      ...
      className={cn(
        'h-8 w-full min-w-0 rounded-lg border border-slate-200 bg-white px-2 text-sm text-slate-800',
