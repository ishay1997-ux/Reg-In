...
//
// ✅ **כתיבות מחזור-חיי-השיבוץ נמצאות כאן מ-09/08/2026** (צעדים 3.4+3.5) — יצירת זימונים,
// אישור-סופי עם השחרור-האוטומטי שנוסע איתו, סימוני-סטטוס, שחרור ואחראית-משמרת.
// שתי ההכרעות שחסמו אותן נסגרו: **§7.33** (`local-13` — השחרור רץ **בקוד** ולא בטריגר,
// כי טריגר אינו יכול לשלוח את המייל שנלווה לו) ו-**§7.41↳** (המפתח המשולש הופך את מרוץ
// ה-`max+1` לכשל **רועש** ⇒ מספיק ניסיון-חוזר אחד).
// ⚠️ **והכללים עצמם אינם כאן אלא ב-`src/lib/assignmentActions.js`** — שם יש להם
// בדיקות-יחידה; כאן יושב רק הדיבור עם המסד.
