...
describe('כרטיס-דיילת — העדפות של לקוחות', () => {
  it('העדפות אמיתיות: שם-הלקוח, התווית העברית והסיבה — "לא_לשלוח" לעולם לא גולמי', async () => {
    ...
    expect(screen.getByTestId('hostess-preference-7')).toHaveTextContent('אלפא אירועים')
    expect(screen.getByTestId('hostess-preference-7')).toHaveTextContent('מצוינת')
    expect(screen.getByTestId('hostess-preference-9')).toHaveTextContent('לא לשלוח שוב')
    expect(screen.getByTestId('hostess-preference-9')).toHaveTextContent('איחרה פעמיים')
  ...
  it('כשל-טעינה ⇒ "לא נטענו" + "נסי שוב", לעולם לא "טרם נרשמו"; ושאר הכרטיס מוצג', async () => {
    ...
    expect(await screen.findByTestId('hostess-preference-7')).toHaveTextContent('בסדר')
