...
// כל שורות ה-`params` נערכות כאן בלי SQL: חיפוש · רשימת-קבוצות עם מונים · פאנל
// ‏🔴 בלי מספר, במכוון (16/09/2026): כאן עמד "43", ומודול 11 הוסיף ארבע שורות ⇒ 47.
//    מספר קשיח בהערה מתיישן בשקט בכל זריעה ובכל מודול שמוסיף פרמטר, ואין לו שער שיתפוס אותו.
//    הספירה החיה: `select count(*) from params`.
...
// ✏️ 25/09/2026 — הכרעת-ישי (סבב 2, בשיחת הסגן 14:1X: *"סבב 2 … לפי המלצתך"*): המשפט הנעול של "תמחור ותזמון"
// ירד מראש הקבוצה לשורת המע"מ בלבד (`note` במרשם), מילה-במילה. הוא נכון רק למע"מ — המע"מ היחיד שנקפא בהצעה
// (`quotes.vat_rate_snapshot`); חמש מתוך שבע השורות בקבוצה משפיעות גם על הצעות קיימות (`params-copy-review.md` נ1).
