...
// `SalaryReportHistoryCard` (יצוא-נקוב — כרטיס-ההיסטוריה). שני הרכיבים חיים באותו קובץ כי שניהם
// צורכים את אותו `listSalaryReports()` והם חלק מאותה יחידת-מסירה (מדריך-המיקרו §2.4 מונה קובץ
// יחיד ל-S3).
// 🔴 **מוזג לתוך הדיאלוג `28/08/2026`, בהכרעת-ישי.** עד אז ישב הכרטיס עצמאי מתחת לדיאלוג לפי
// design-contract.md §S3 ("לא בתוך dlg-body"), ומסך-הבית (S1) מיקם וקישר ביניהם דרך פרופ
// `onGenerated` שהועבר מבחוץ. **היום הכרטיס מרונדר בתוך `SalaryReportDialog` עצמו** (`embedded`,
// ר' הערת ה-`historyTick` למטה) והפרופ `onGenerated` הוסר כליל.
//
...
export default function SalaryReportDialog({ open, onOpenChange }) {
  ...
  // ה9: ה-UNIQUE על period במסד הוא האכיפה האמיתית מאחורי "לא ניתן להפיק פעמיים אותו חודש" —
  // הוא זה שחוסם גם לחיצה-כפולה, לא רק בדיקת-הכפילות מצד-לקוח שמעליו. הניסוח למטה נמנע
  ...
  const blockedBannerText = existingForSelected
    ? existingForSelected.send_status === 'sent'
      ...
      : // ⚠️ סטייה מודעת מהנוסח הנעול: הוא מניח "נשלח ב-X" — ונכון תמיד למה שהמוקאפ מצייר,
        ...
        }). לא ניתן להפיק פעמיים אותו חודש — המערכת חוסמת זאת גם במקרה של לחיצה כפולה.`
    : ''
    ...
    >
      ...
      {accountantEmail === null && (
        ...
        >
          ...
          לא נטענה כתובת של משרד רואי-החשבון (הפרמטר &apos;מייל משרד רואי החשבון&apos;).{' '}
        ...
        <div className="overflow-x-auto">
          ...
          <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-2.5 text-xs leading-relaxed text-slate-600">
            {/* §7.69: הסכום נכון לרגע ההפקה מהפרמטר החי, וטרם אומת מול רואה-החשבון.
                ✏️ 24/09/2026 (מבקרים טריים): "הפרמטר החי" ⇐ היכן היא רואה אותו — ההגדרות. */}
            {hasTravel && (
              <p>
                נסיעות מחושבות לפי &apos;סכום נסיעות למשמרת&apos; שבהגדרות, כפי שהיה ברגע ההפקה —
                הסכום עדיין טעון אימות מול רואה-החשבון.
              </p>
            )}
            {/* ה24/ה29: נוסחת-הפיצוי אינה כוללת רכיב-בונוס, ואין נסיעות למשמרת שבוטלה — ולכן
                "—" ולא 0.00 ₪ (אפס-כן-חושב אינו אותו דבר כמו רכיב-שלא-קיים). */}
            <p>
              <b>בונוס/נסיעות בשורות-פיצוי-ביטול:</b> "{NOT_APPLICABLE}" ולא 0.00 ₪ — נוסחת-הפיצוי
              אינה כוללת רכיב-בונוס, ואין נסיעות למשמרת שבוטלה.
            </p>
            {omittedFromFile > 0 && (
            ...
            {/* ה19: המקור הוא הטבלה המוגנת hostess_bank_details, שקריאה ממנה מוגבלת להרשאת
                'כספים'. הניסוח למנהלת-הכספים למטה נותן את האמון בלי שם-טבלה/קוד-החלטה. */}
            <p>
              <b>🔒 פרטי-הבנק</b> מוצגים מהמאגר המוגן — נגישים בהרשאת 'כספים' בלבד.
            </p>
          </div>
        </div>
      )}
...
// ---------------------------------------------------------------------------
// SalaryReportHistoryCard — כרטיס-ההיסטוריה. עד `28/08/2026` היה זה card עצמאי מתחת לדיאלוג
// (design-contract §S3: "לא בתוך dlg-body"); מ-`28/08/2026`, בהכרעת-ישי, הוא מרונדר **בתוך**
// `SalaryReportDialog` (ר' `embedded` ו-`historyTick` שם) — הפרופ `onGenerated` שקישר בין
// השניים כשהכרטיס ישב מחוץ לדיאלוג הוסר כליל.
// ---------------------------------------------------------------------------

// `refreshToken`: כל ערך שמשתנה גורם לרענון. עד `28/08/2026` S1 (מסך-הכספים) היה מעביר מונה
// שהוא מגדיל ב-`onGenerated` של הדיאלוג, בעוד הכרטיס ישב מחוצה לו ולא ידע עליו. **מ-`28/08/2026`,
// בהכרעת-ישי, הקורא היחיד הוא `SalaryReportDialog` עצמו** — הוא מגדיל `historyTick` אחרי הפקה
// מוצלחת ומעביר אותו לכאן (ר' הערת ה-`historyTick` למעלה). הפרופ נשאר גנרי; הצרכן החיצוני נעלם.
