...
function Tiles({ tiles, onOpenTarget }) {
  ...
  return (
    // 🧱 **רשת ברוחב-כרטיס אחיד — הכרעת-ישי 23/09/2026 לילה** (*"זה לא מצחיק כרטיסים בגדלים שונים?"* ·
    // *"אני רוצה להיות מקצועי בסוף"*). ב-flex-wrap כרטיס יתום בשורה שנייה נמתח לכמעט רוחב כפול,
    // ורוחב-הכרטיס השתנה מדוח לדוח. ‏`auto-fill` ולא `auto-fit`: שלושה כרטיסים תופסים שלוש עמודות
    // ונשאר רווח — **המטרה של הכרעת 08/08 ("פלקס ולעולם לא grid": לא למתוח כרטיס בודד לרוחב מלא)
    // נשמרת**, ואין גלילה-אופקית ואין מספר-עמודות קבוע. רק הנוסח השתנה.
    // 📏 ‏240px ולא 180: ב-180 שש עמודות צרות ב-1536, ושורות-ההשוואה נשברו לשתיים (נמדד בבדיקת-הצפיפות,
    // 18 חריגות). ‏240 = חמש עמודות, שורה אחת לכל השוואה, ובדיוק רוחב אריח-הצד של הגרף (מ9).
    <div
      className="mb-4 grid grid-cols-[repeat(auto-fill,minmax(240px,1fr))] gap-3"
      data-testid="report-tiles"
    >
      {tiles.map((tile) => (
        ...
        <div
          ...
          // ✏️ 23/09/2026 לילה — ברשת רוחב העמודה אחיד ⇒ גבולות-הרוחב (min/max/basis) של ה-flex ירדו.
