...
export const SMART_MATCH_CLARIFICATION = 'השליחה בשיבוץ החכם נעשית בידי מנהלת הגיוס.'
// ‏ζ (צעד 3.0): תג אחראית-המשמרת בלי ★ — הגליף כבר אומר "התרשמות" ב-RatingStars.
export const SHIFT_LEAD_LABEL = 'אחראית משמרת'
// 🧹 איפוס-שלב-4 (08–09/09/2026): SCOPE_NOTE ("שינוי הכמות נרשם כשינוי-תכולה…") ו-
// RAW_STATUS_NOTE ("כאן מוצג הסטטוס הגולמי…") הוסרו מהבסיס — שתיהן ⑤/③-מכניזם
// (R27/R28: SCOPE_NOTE כפלה את מה ש-ScopeChangeDialog כבר אומר בפועל; RAW_STATUS_NOTE
// נשען על ז'רגון-בנאים "גולמי/נגזר", B8) ונרשמו כמועמדות-שכבה בדוח הכתיבה.
...
export function assignmentMeaning({
  ...
  switch (status) {
    ...
    case 'released':
      // R7: "אנחנו ויתרנו עליה" (הנוסח הקודם, כמו גם SmartMatchPage.jsx:364) הוא גוף-ראשון-
      // רבים שאינו הודאה-בתקלה — אסור לפי כלל-הפנייה. הניסוח כאן שומר על אותה הבחנה
      // (שחרור = לא ביוזמתה) בלי "אנחנו".
      return 'שוחררה מהשיבוץ — לא ביוזמתה, ולא נספר לרעתה בשום מקום.'
...
// "9 שורות בהיסטוריה, 6 דיילות על המסך" — הקיפול של MAX(assignment_number) גלוי במספרים.
// 🧹 "במסד" (מקורי) הוחלף ב"בהיסטוריה" — ז'רגון-מפתחים (B8/R10): "מסד" הוא מונח-DB
// שאינה רואה; "בהיסטוריה" מתאר את הטבלה שהיא בדיוק פתחה עכשיו.
export function historyFootnote(rowCount, hostessCount) {
  const rowsText = rowCount === 1 ? 'שורה אחת בהיסטוריה' : `${rowCount} שורות בהיסטוריה`
...
// שורת-העובדה כשאין שינויים — הכמות החיה + הבטחת-הקיפאון של ההצעה.
// 🧹 המשפט האמצעי של המוקאפ ("כל שינוי יופיע כאן בשורה משלו — מה השתנה · בכמה · מי
// ביצע · מתי · והסיבה שנרשמה") הוסר — תיאור-מראש של שורה שהיא עדיין לא ראתה (R28: היא לא
// עושה שום דבר אחרת בזכותו; ברגע שתהיה שורה ראשונה, ScopeChangesSection מדגים אותה בעצמה).
export function scopeFactSentence({ required, approvedOnText } = {}) {
  const since = approvedOnText ? ` (${approvedOnText})` : ''
  return `הכמות עומדת על ${Number(required) || 0} מאז אישור ההצעה${since}. ההצעה עצמה נשארת קפואה ואינה משתנה לעולם.`
}
