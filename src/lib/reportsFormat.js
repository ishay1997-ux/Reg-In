...
/**
 * *"ערך שאינו מספר סופי הוא **חוסר**, ולעולם לא אפס"* — הבית **היחיד** של הכלל הזה
 * במודול 11 (‏T1, אודיט-הסגירה 17/09/2026).
 *
 * 🔴 **למה כאן, ולמה לא `toFiniteNumber` של `pricing.js`:** הן נראות זהות ואינן —
 * ‏`toFiniteNumber` פוסלת `''` בלבד, ולכן `Number('  ')` שלה הוא **0**. כאן מחרוזת
 * שכולה רווחים היא **שורה חסרה** ולא סף-אפס, וזו בדיוק ההבחנה ש-§7.83 נשען עליה:
 * ‏`מקדם_אמינות_אדום` שערכו רווחים היה נקרא כסף 0 ⇒ *"אין דיילות בסיכון"* על מסך שפשוט
 * אינו יודע. ⇒ **שתי פונקציות, שני נימוקים, ולא דגל על אחת מהן.**
 * ⚠️ **והיא ישבה בשלושה עותקים** (`reportsCustomers` · `reportsExecutive` ·
 * `reportsHostesses`), שאחד מהם כבר סטה טקסטואלית. שלושתם מקודדים כלל אחד ⇒ בית אחד.
 * 🚫 **הפסילה קודמת להמרה**, כי `Number(null)` · `Number('')` · `Number('  ')` הם כולם 0.
 */
