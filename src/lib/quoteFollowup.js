...
// שוב על 5xx כל עוד לא עבר תקציב של 90 — כמו `classify-feedback` (הכרעת-הסגן 25/09/2026). ניסיון מתחיל
// רק לפני ה-deadline, ולכן המקרה הגרוע הוא `BUDGET_MS + PROVIDER_TIMEOUT_MS` = 150 שניות, ועוד 10 ל-cold-start
// ולרשת. ⇒ השרת תמיד עונה (טיוטה או "הניסוח נכשל") לפני שהחלון מוותר; בדיקה ב-`quoteFollowup.test.js`
// קוראת את שני המספרים מהשרת ונכשלת אם זה מפסיק להיות נכון. ⚠️ **היעד ≤10 שניות (התוכנית §6ה-ה) לא
