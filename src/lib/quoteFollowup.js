...
// ‏160 שניות: השרת (`supabase/functions/draft-followup/index.ts`) מחכה לספק 60 שניות בכל ניסיון ומנסה
// שוב על 5xx כל עוד לא עבר תקציב של 90 — כמו `classify-feedback` (הכרעת-הסגן 25/09/2026). תקרת כל ניסיון
// נחתכת למה שנשאר מהתקציב (`providerRetry.ts`), ולכן השרת עונה עד `BUDGET_MS` (90) ועוד זמן-המסד; ‏160 משאירות
// מרווח רחב ל-cold-start ולרשת. ⇒ השרת תמיד עונה (טיוטה או "הניסוח נכשל") לפני שהחלון מוותר; בדיקה
// ב-`quoteFollowup.test.js` קוראת את `BUDGET_MS` מהשרת ונכשלת אם זה מפסיק להיות נכון. ⚠️ **היעד ≤10 שניות (התוכנית §6ה-ה) לא
// השתנה** — זו תקרה לאבחון, והכפתור מוסתר (`FOLLOWUP_AI_AVAILABLE`) עד שנמדדת טיוטה אמיתית.
export const FOLLOWUP_DRAFT_TIMEOUT_MS = 160_000

// 🔌 **המתג של הכפתור "נסחי מייל מעקב".** (היסטוריה: כובה 24/09, הודלק 25/09 — ר' למטה.) הכרעת-הסגן 24/09/2026: *"אם הקריאה החיה המדודה לא
...
// ✏️ 25/09/2026 — **דלוק.** ישי בצ'אט הסגן, 14:0X: *"מאשר הכל לפי המלצתך"* (ההמלצה: להדליק). בדיקות-ה-e2e
// מיירטות את `draft-followup` בכל בדיקה (`e2e/quote-followup.spec.js` `routeDraft` · `e2e/accessibility.spec.js`).
export const FOLLOWUP_AI_AVAILABLE = true
