...
describe('validateParamValue — min/max/decimals של השורה נאכפים, לא רק מוצהרים (A2)', () => {
  ...
  it.each([
    ['אחוז_מעמ', '17.555', 'הזיני מספר בין 0 ל-100, עד שתי ספרות אחרי הנקודה'],
    ['משקולת_קרבה', '0.333333', 'הזיני מספר בין 0 ל-1, עד שתי ספרות אחרי הנקודה'],
    ['סף_שביעות_רצון', '99', 'הזיני מספר שלם בין 1 ל-5'],
  ])(
...
// 🔴 **התקרות שנוספו 03/09/2026, ומה שהבדיקה הזו באמת שומרת עליו.** ‏20 מתוך 22 הפרמטרים
// המספריים שהיו בלי תקרה קיבלו אחת, כמגן-הקלדה בלבד: **כל 20 הערכים החיים נמדדו מול המסד
// ונמצאו מתחת לתקרה שלהם**, כלומר אף ערך בשימוש אינו נחסם. **השניים שנשארו בלי תקרה נשארו
// כך בהכרעת-ישי, לא בשכחה** — ‏`סף_לקוח_רדום_ימים` (חלון-רדימוּת של שנתיים הוא בחירה עסקית
// לגיטימית לחברה שלקוחותיה עושים כנס פעם בשנה) ו-`תקרת_שבועות_הוגנות` (אין עוגן בעולם,
// והחסם האמיתי הוא על המכפלה `שיעור × תקרה` ולא על התקרה לבדה).
// 🔑 **ולכן הבדיקה נועלת את שני הכיוונים:** שסשן עתידי לא יוסיף להם תקרה בלי שישי הכריע,
// ושלא ייווצר פרמטר מספרי חדש בלי תקרה בלי ששמו יופיע כאן במפורש.
describe('תקרות מספריות — כל 22 מוגנות; אף אחת אינה פתוחה עוד', () => {
  // 🔴 **היסטוריה שחייבת לשרוד את התרוקנות הרשימה (03/09/2026).** עד היום ישבו כאן שני שמות,
  // ולא בשכחה: **ישי שמר אותם לעצמו במפורש** כשאישר את 20 האחרות. הם נסגרו באותו יום
  // ב**הכרעת-קלוד-בהאצלה בהרשאת-ישי** *("תדונו ותחליטו לי וזהו" · "מבחינתי מאשר מה שתחליטו")*,
  // בדיון בין שני סשנים, וכל אחד עם נימוק משלו:
  // ‏· `סף_לקוח_רדום_ימים` ⇒ **730** — לקוח שעושה כנס פעם בשנה יכול לחזור אחרי יותר משנה,
  //   ולכן 365 היה עלול לחסום בחירה עסקית לגיטימית ולא רק שגיאת-הקלדה.
  // ‏· `תקרת_שבועות_הוגנות` ⇒ **104** — 🚫 **ולא כלל-רוחב על המכפלה `שיעור × תקרה`**, שהיה
  //   ההצעה הראשונה כאן ונפסלה: זו מכונה חדשה לבעיה שאיש לא דיווח עליה, מול הכלל של ישי
  //   *"אין סיבה להוסיף עבודה גם אם זה שיפור נחמד"*. אם הבונוס אי-פעם באמת יגבר על התיק
  //   האמיתי של דיילת בשיבוץ חי — **אז** החסם המוצרי מרוויח את מקומו.
  // 🔑 **בלי הפסקה הזו, בעוד שלושה שבועות שתי התקרות ייראו כאילו תמיד היו כאן**, והעובדה
  // שישי שמר אותן לעצמו — והוויתור המפורש שלו — נעלמת יחד עם הרשימה שהחזיקה אותה.
  const OPEN_BY_RULING = []
  const NUMERIC_KINDS = ['int', 'decimal', 'percent', 'weight']
  const numeric = PARAM_REGISTRY.filter((entry) => NUMERIC_KINDS.includes(entry.kind))

  it('כל פרמטר מספרי נושא תקרה — הרשימה הפתוחה ריקה', () => {
    const uncapped = numeric.filter((entry) => entry.max == null).map((entry) => entry.name)
    expect(uncapped.sort()).toEqual([...OPEN_BY_RULING].sort())
  })

  // 🔬 מגן-ההקלדה עצמו: אותו ערך חי עובר, ופי-עשרה ממנו נדחה. בלי החצי השני הבדיקה
  // הייתה מוכיחה רק ש-`max` קיים, לא שהוא אוכף משהו.
  it.each([
    ['תנאי_תשלום_ימים', '30', '300'],
    ['שעות_תוקף_זימון', '48', '480'],
    ['שער_מרחק_קמ', '80', '800'],
    ['קבוע_ריסון_m', '3', '30'],
  ])('%s — %s עובר, %s נדחה', (name, live, typo) => {
    const entry = getParamEntry(name)
    expect(validateParamValue(entry, live).ok).toBe(true)
    expect(validateParamValue(entry, typo).ok).toBe(false)
  })
})
...
describe('נוסחי ההגדרות — מה שנאמר על המסך נכון', () => {
  it('🔴 תוקף הצעה: ה-↳ אומר שהצעות קיימות נדחות בלילה (עבודת-הלילה נבדקה חי)', () => {
    expect(getParamEntry('ימי_תוקף_הצעה').affects).toContain('פגה אוטומטית בלילה')
