...
// `42501` (הרשאה) · `P0002` (הצעה לא נמצאה) · `P0001` (**11 אתרי-RAISE שונים**, נמדד
// 31/07/2026 על שתי המיגרציות). כל ההבחנה בתוך P0001 היא על הטקסט.
// 🔗 **המקור הפעיל: `supabase/migrations/20260731085335_module3_vat_and_expiry_param_guards.sql`**
// (‏`approve_quote_and_create_project`) **+ `20260723115000_module3_lock_and_conversion_rpc.sql`**
// (‏`replace_quote_lines` + טריגר-הנעילה). ⛔ **מיגרציה עתידית שתשכתב ניסוח של RAISE חייבת
// לעדכן את התחיליות כאן** — אחרת המיפוי חוזר בשקט ל-fallback ואף בדיקה לא תיפול. זה כבר קרה
// כמעט: סבב A שכתב את הפונקציה כולה ב-31/07 בבוקר. שורת-האזהרה המקבילה: `docs/db_roadmap.md` §6.
