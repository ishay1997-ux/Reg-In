...
export function validateQuoteForm(form, lines, todayIso) {
  ...
  if (!form.eventDate) {
    ...
    errors.eventDate = 'תאריך האירוע כבר עבר — בחרי תאריך עתידי.'
  }
...
const SERVER_MESSAGE_RULES = [
  ...
  },
  {
    prefix: 'הצעה נעולה:',
    build: () =>
      'ההצעה נעולה לשינויים — עריכה או דחייה אפשריות רק כשההצעה בסטטוס "בתהליך", ולכן יש לרענן את המסך.',
  },
...
export function quoteServerErrorMessage(error) {
  ...
  if (code === 'P0002') {
    return 'ההצעה לא נמצאה, או שאין לך הרשאה אליה — רענני את רשימת ההצעות.'
  }
...
export const QUOTE_SEND_FAILED_MESSAGE =
  'ההצעה לא נשלחה — לחצי "שלחי במייל" שוב, או הורידי את הקובץ ושלחי ידנית.'
