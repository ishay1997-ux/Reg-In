...
export function compensationPercent({
  ...
  // 🛡️ **שכבה שנייה במכוון:** מסך-ההגדרות כבר חוסם את השמירה (`cancellationOrderOk`), והכלל
  // הזה חוסם את הפירוש גם כשהמצב ההפוך נכנס בעקיפה — Table Editor, PATCH ישיר, מיגרציה עתידית.
  // `null` כאן הוא אותו חוזה בדיוק שכבר חל על פרמטר חסר בשורה שמעל: המסך מציג מצב-שגיאה, לא מספר.
  //
  // 🔴 **וגבול-ההגנה, כי הנוסח הקודם כאן טען יותר ממה שהקוד עושה** (תוקן באודיט-הסגירה,
  // 03/09/2026, אחרי שנמדד): הוא נכתב "חוסם את הפירוש **מכל מקור**", וזה **לא נכון** —
  // הוא חוסם את הפירוש **בצד-הלקוח בלבד.** אותו מדרג חי גם במסד, בגוף
  // `20260828102653_module8_h6_gate_fee_proposal_on_finance_permission.sql:90-96` (ושוב ב-:105),
  // **בלי שום בדיקת-היפוך** — כלומר בצמד הפוך המסך אומר "לא יודע" בעוד ה-RPC ממשיך להחזיר
  // אחוז שגוי לתוך הצעת-שכר-הטרחה ולתוך דו"ח-השכר. **הנגישות היום היא מנכ"ל דרך Table Editor
  // בלבד** (מסך-ההגדרות חוסם), ולכן זה נרשם כחוב ולא כתקלה חיה — אבל שומר-מסד הוא החלטת-תכנון
  // בפני עצמה ולא תיקון-לילה. 🚫 **הלקח: טענה שגויה בהערה גרועה מהיעדר הערה** — הסשן הבא קורא
  // אותה כחומה שנבדקה. ההשוואה למרשם+RPC של שכר-המינימום נמחקה כאן בכוונה: **שם** באמת יש
  // שתי שכבות, וכאן יש אחת.
