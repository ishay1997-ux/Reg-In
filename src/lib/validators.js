...
// ⚠️ תוספת בלבד לקובץ המשותף עם מודולים 2 ו-3 — אין נגיעה בוולידטורים שמעל השורה הזו
// (אותה מוסכמת-shared-surface שמודול 3 פעל לפיה ב-29/07).

// ת"ז ישראלית — **ספרת-ביקורת בתקן**, לא "9 ספרות".
// 🔑 למה זה מצדיק אלגוריתם ולא regex, וזו לא קפדנות: ת"ז היא **המפתח העסקי היחיד שמונע
// כפילות-אדם במאגר** — ‏§7.65 ויתר במכוון על UNIQUE על האימייל *בהסתמך עליה* ("הת"ז כבר
// מונעת כפילות-אדם"). מספר שהוקלד שגוי עובר בדיקת-אורך בקלות, נשמר, ומייצר דיילת-רפאים
// שנייה לאותו אדם — ואז שתיהן מדורגות בנפרד ב-Smart Match עם חצי היסטוריה כל אחת.
// 🚨 והמסד **אינו** שומר עלינו כאן: יש שם `unique` על הערך, לא בדיקה של תקינותו.
export function isValidIsraeliId(value) {
  if (typeof value !== 'string' && typeof value !== 'number') return false
  // המשתמשת מדביקה מהטלפון — מקפים ורווחים הם עיצוב, לא נתון.
  const digits = String(value).replace(/[\s-]/g, '')
  if (!/^\d{1,9}$/.test(digits)) return false

  const padded = digits.padStart(9, '0')
  // ⚠️ מלכודת אמיתית באלגוריתם: סכומן של תשע אפסים הוא 0, ו-0 מתחלק ב-10 —
  // כלומר "000000000" עובר את חשבון הביקורת. נפסל במפורש.
  if (padded === '000000000') return false

  // התקן: משקל 1 לספרה במקום אי-זוגי ו-2 לזוגי; מכפלה דו-ספרתית מוחלפת בסכום ספרותיה
  // (שקול לחיסור 9), והמספר תקין כשהסכום מתחלק ב-10.
  let sum = 0
  for (let i = 0; i < 9; i += 1) {
    const product = Number(padded[i]) * ((i % 2) + 1)
