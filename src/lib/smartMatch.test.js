...
describe('🎯 rankCandidates — העוגן של כנס לקוחות שנתי, 22/08/2026', () => {
  ...
  // ✏️ 25/09/2026 (אודיט השיבוץ-החכם, פער 1): הבדיקה הזו נקראה "סדר-התצוגה" — והייתה ירוקה
  // בזמן שהמסך הציג **דנה ← מיכל ← נועה**, כי היא בודקת את סדר `rankCandidates` ולא את המסך.
  // ⇒ שמה תוקן; את **המסך** בודק `SmartMatchPage.order.test.jsx`, על אותו מקרה בדיוק.
  it('🎯 סדר-ההמלצה (`rankCandidates`): נועה ← מיכל ← דנה', () => {
    expect(rankCandidates(POOL, context).map((r) => r.full_name)).toEqual(['נועה', 'מיכל', 'דנה'])
  })

  it('✏️ `C` נסכם על החלון הבסיסי כשהשכבה מספקת אותו — לא על החלון המורחב של דיילת', () => {
    // אותו עוגן, אבל לנועה הורחב החלון: 7/6 שלה כוללים 4 תשובות ישנות שאינן בחלון הבסיסי.
    const widened = POOL.map((h) =>
      h.full_name === 'נועה' ? { ...h, baseAnswered: 3, baseConfirmed: 3 } : h,
    )
    // (3+6+1+2+3 אישורים) ÷ (3+12+1+5+5 תשובות) = 15 ÷ 26 — ולא 18 ÷ 30.
    expect(companyResponsivenessAverage(widened)).toBeCloseTo(15 / 26, 10)
  })
