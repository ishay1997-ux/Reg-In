...
//   · **`medianOf` אינה כאן כלל** — המקור הוא `./reportsHostesses`, ומי שצריך חציון
//     מייבא משם ישירות (כך עושה גם `./reportsCustomers`). ✏️ **16/09/2026: הקובץ הזה
//     ייצא אותה מחדש כנוחוּת, ו-`knip` תפס את הייצוא כמת** — איש לא ייבא אותו, כי
//     היעד הטבעי הוא המקור. מצביע-נוחוּת שאיש אינו עובר בו הוא רק מקום שני להסתכל בו.
//     ‏`percentile_cont(0.5)` ב-SQL הוא התאום של `medianOf`: שניהם ממצעים את שני
//     האמצעיים במספר-ערכים זוגי.
...
/**
 * ‏**איזו תווית-דלי נושאת ערך נתון** — התאום ב-JS של עוגן קו-הייחוס שמ6 מחזיר
 * ‏(`v_param_bucket` ב-`report_m06_staffing`), והבדיקות שלידו הן האורקל שלו.
 *
 * 🔴 **למה זה קיים בכלל, וזה לא נוחוּת:** ציר-ה-X של ההיסטוגרמה **קטגוריאלי** — דליים
 * כמחרוזות — ולכן `<ReferenceLine x={50}>` אינו נמצא בתחום-הסקאלה ו**אינו מצויר**.
 * העוגן היחיד שהרכיב מזהה הוא תווית-הדלי, ולכן הפרמטר החי חייב להיתרגם לתווית לפני
 * שהוא נשלח. ערך מחוץ לכל דלי, או ערך שאינו מספר, מחזיר `null` — ואז **אין קו**,
 * ולא קו במקום שרירותי.
 */
export function ratioBucketLabelFor(value) {
  const n = finiteNumber(value)
  if (n === null) return null
  const bucket = RATIO_HISTOGRAM_BUCKETS.find(
    (b) => (b.min === null || n >= b.min) && (b.max === null || n < b.max),
  )
