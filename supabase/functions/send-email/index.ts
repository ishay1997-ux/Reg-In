...
// ✅ **`invoice` ו-`salary_report` נוספו 27/08/2026 (מ8), ובסדר הנכון:** המיגרציה
// `module8_email_log_finance_entities` הוחלה **קודם** ואומתה (ה-CHECK מונה שישה ערכים),
// ורק אחריה נפרסה הגרסה הזו. הכיוון אינו סימטרי — פריסה לפני המיגרציה ⇒ המייל יוצא,
// כתיבת-היומן נדחית, **והכשל נבלע** (ר' למטה). 🚧 **מ11 עדיין לא כאן** ויוסיף את שלו
// באותו סדר בדיוק.
...
const ENTITY_MODULE: Record<string, string> = {
  ...
  // 🔴 **שני ערכים למ8, ושניהם מול 'כספים' — לא מול 'הצעות מחיר' ולא מול 'דיילות'.**
  //    ‏`invoice` הוא מסמך-גבייה ו-`salary_report` הוא דוח לרו"ח; שניהם פעולות של
  //    מנהלת-הכספים. **למנהלת-הכספים יש `view` בלבד על 'הצעות מחיר'** ⇒ מיחזור של
  //    `quote` היה מפיל כל חשבונית שהיא שולחת ב-403 שקט — בדיוק המלכודת שמ6 תיעדה
  //    שורה מעליי, על אותה משבצת.
  invoice: 'כספים',
  salary_report: 'כספים',
}
...
const ENTITY_REQUIRES_ATTACHMENT: Record<string, boolean> = {
  ...
  project: false,
  project_report: true,
  // 🔴 **שניהם `true`, וזה לא הידור-יתר:** חשבונית בלי הקובץ אינה חשבונית, ודוח-שכר
  //    בלי האקסל אינו דוח — הרו"ח והלקוח מקבלים מייל שנראה תקין ואין בו כלום.
  //    ‏(‏R4-F12: *"שני ערכים חדשים … שניהם attachment-חובה"*.)
  invoice: true,
  salary_report: true,
}
