-- =============================================================================
-- מודול 11 · מיגרציה I1 · פורמטים, יחידות-מידה, יחידות-ציר והערות-גרף — תיקון-קדימה אחד
-- =============================================================================
-- 🎯 **מה זה:** מיגרציית תיקון-קדימה אחת, חוצת-לשוניות, על **שלוש-עשרה** פונקציות-דוח:
--    ‏`report_m03_trends` (הנהלה) · `report_m07_finance_overview` ·
--    ‏`report_m08_profitability` · `report_m09_aging` · `report_m12_equipment` (כספים) ·
--    ‏`report_m14_hostess_overview` · `report_m15_reliability` · `report_m16_quality_cost` ·
--    ‏`report_m17_fairness` (דיילות) · `report_m19_customers_overview` ·
--    ‏`report_m20_satisfaction` · `report_m21_drifting` · `report_m22_notes` (לקוחות).
--    **כל שינוי הוא תוספת, או `format`/תווית, או ערך של מפתח חדש.** אין טבלה חדשה, אין
--    עמודה, אין policy, אין שינוי-חתימה, אין כתיבה. **אף מפתח לא נמחק ולא שונה-שם במטען,
--    ואף מספר נמדד לא זז** — הוכחה: השוואת-מטען מלאה לפני ואחרי, פר-פונקציה.
--
...
--    חוזרים כאן מילה-במילה כפי שהמיגרציה הקודמת של כל פונקציה כתבה אותם (נבדק בייט-בבייט,
--    ‏13/13, בסקריפט `shell_check_i1.py`).
--
-- 📐 **מקור הגוף — נמדד, לא זכור:** לכל אחת מהשלוש-עשרה נמשך `md5(prosrc)` מ-`pg_proc`
--    החי ‏16/09/2026 והושווה ל-`md5` של גוף ה-`$function$` בקובץ-המקור העדכני שלה, **לפני**
--    שנגענו בתו אחד. שלוש-עשרה מתוך שלוש-עשרה זהות באורך וב-md5 ⇒ הגוף שלמטה **הוא**
--    הגוף החי:
--      ‏m03 ⇐ `20260916083000_module11_d2_rpcs_executive_fixes.sql`
--      ‏m07 · m08 · m09 · m12 · m14 · m15 · m17 ⇐ `20260916085900_module11_h2_sentence_isolation.sql`
--      ‏m16 ⇐ `20260916065642_module11_f2_rpcs_hostesses_fixes.sql`
--      ‏m19 · m21 ⇐ `20260916082900_module11_g2_rpcs_customers_fixes.sql`
--      ‏m20 · m22 ⇐ `20260916103000_module11_g3_customers_filter_keys.sql`
--    חמש המיגרציות האלה **אינן נערכות** — הן הוחלו, ומיגרציה שהוחלה מתוקנת אך ורק
--    במיגרציה חדשה קדימה (`supabase/migrations/CLAUDE.md §2`).
--
-- 🧾 **שורות `schema_migrations` שהקובץ הזה מייצר (`§9 D-20`) — שלוש-עשרה, ועוד חמש:**
--    ‏`module11_i1_m03` · `_m07` · `_m08` · `_m09` · `_m12` · `_m14` · `_m15` · `_m16` ·
--    ‏`_m17` · `_m19` · `_m20` · `_m21` · `_m22` — **ובנוסף** `module11_i1_m16_units` ·
--    ‏`_m19_units` · `_m20_units` · `_m21_units` · `_m22_units`.
--    🔑 **למה החמש הנוספות:** חמש הפונקציות האלה כבר הוחלו מהטיוטה הזו כשהיא כיסתה עשר
--    פונקציות; פריט ⑬ (יחידות-הציר) הגיע מהמתזמר **אחרי** שהוחלו, והן הוחלו שוב מאותו
--    קובץ — תיקון-קדימה באותה טיוטה שטרם נקמטה, **בדיוק המקרה ש-D-20 מתאר** (וכך בדיוק
--    נהגה H2 עם `module11_h2_m07_cols`). ‏MCP `apply_migration` קוטמת סביב 90KB, ולכן
--    הקובץ מוחל פר-פונקציה (כל מקטע < 60KB). **הקובץ הוא מקור-האמת**, וכל גוף שהוחל אומת
--    ב-`md5(prosrc)` מול גוף ה-`$function$` שבו.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- 🪤 **הפגמים שהמיגרציה קיימת בשבילם — כולם נמדדו על מסך חי או במטען חי** ע"י בוני-הלשוניות
--    ומתקן-המעטפת (16/09/2026), ולא הוסקו מהקוד:
--
--    ① **מ3 · עמודת "שנה" הוצהרה `int`** ⇒ `reportsFormat.FORMATTERS.int` מריץ
--       `Math.round(n).toLocaleString('he-IL')` והמסך הציג **`2,024`**. שנה אינה כמות ⇒
--       ההכרזה היא `text`. **זו העמודה היחידה של שנה בשלוש הרמות** — רמה 1 ממוינת ב-`label`
--       של החודש, ורמה 2 ב-`final_event_date` שכבר הוצהר `date`.
--
--    ② **מ3 רמה 1 · האריח הרביעי** (`price_vs_cost`) הוא `format: 'text'` — שני מספרים
--       בתא אחד — אבל **חצי-ההשוואה שלו הוא כסף יחיד** (`259.7790478394169`).
--       ‏`CompareLine` נופל ל-`tile.format` כשאין `compare.format` (`KpiTile.jsx`) ⇒
--       המספר הגולמי הופיע על המסך. **הכרזה מפורשת `compare.format: 'money'`** היא התיקון,
--       והיא בדיוק תוספת-C8 שהמעטפת כבר קוראת. נסרקו שלוש הרמות של מ3 — זה המקרה היחיד.
--
--    ③ **מ19 · מ20 · מ21 · מ22 — עמודות-תאריך הוצהרו `text`** (`last_event` · `next_event` ·
--       ‏`final_event_date`) ⇒ המסך הציג `"2026-09-15"` במקום `15/09/2026`. הערכים הם
--       ‏`date` של Postgres, ולכן `format: 'date'` מנתב אותם ל-`formatIsraelDate`.
--
--    ④ **מ17 · שני אריחי זמן-התגובה הוצהרו `format: 'days'`** — ו**המדד בשעות**:
--       ‏`cards-hostesses.md` §③ נוקב *"זמן-תגובה חציוני **9.9 שעות**"* ו*"אחוזון-90
--       **20.7 שעות**"*. ‏`FORMATTERS.days` מעגל ומוסיף *"ימים"* ⇒ המסך אמר **"10 ימים"**.
--       ‏`ratio` (ספרה עשרונית אחת, בלי יחידה) הוא מה שעמודת-השעות של `meta.extra_tables`
--       כבר מכריזה. **התוויות לא נגעו:** `spec.md §1.4` כותב אותן בלי יחידה, ולכן היחידה
--       נכנסה ל-`tiles[].sub` — באריח-החציון היא כבר הייתה שם, ובאריח-האחוזון נוספה.
--
--    ⑤ **מ21 · העמודה "ציון אחרון · ממוצעו" הבטיחה שני מספרים והתא נשא אחד.**
--       המוקאפ המאושר (`05_tab_customers_approved.html` `#c3`) מצייר **תא אחד**: הציון
--       האחרון ככוכבים, ואחריו `· 2.90 (n=10)`. ‏**ולמה לא שתי עמודות:** `cards-customers.md`
--       ‏G-ל5 נועל *"טבלת המתרחקים נושאת **9** עמודות — כל אחת נדרשת"* ⇒ עמודה עשירית הייתה
--       סותרת הכרעה רשומה. ⇒ **מפתח-שורה חדש `score_pair`** נושא את שני המספרים כטקסט אחד
--       מבודד, והעמודה מצביעה עליו. ‏`last_score` · `avg_score` · `feedback_n` **נשארים
--       במטען כמספרים** — שום מפתח לא נמחק, ומספר העמודות נשאר תשע.
--
--    ⑥ **מ9 · לגרף-הגיול לא הייתה הערת-פירוש.** ‏`cards-finance.md` ⑧9.8 נולד מביקורת-ישי
--       ‏§👤 6 — *"הסתכלתי עליו חמש דקות ולא הבנתי"* — והתשובה שם היא שלושה דברים, שאף אחד
--       מהם לא היה במטען: כותרת-משנה שאומרת מה השאלה שהעמודות עונות עליה · שורת-פירוש לכל
--       דלי · מקרא-עמודות מלא (`.barkey`). ⇒ `chart.note` נושא את הטקסט הזה, **והמספרים
--       חיים מ-`v_buckets`** ולא מהמוקאפ (שצויר על צילום 10/09), מבודדים ועם מפריד-אלפים.
--
--    ⑦ **מ20 · שני גרפי-הסיבות ("מה משמח" · "מה מכעיס") מצוירים אופקית במוקאפ המאושר** —
--       ‏`aria-label` אומר *"עמודות אופקיות"* ותוויות-הקטגוריה עומדות ב-`x=129
--       text-anchor="end"`. ⇒ `chart.layout: 'horizontal'`, מפתח **תוספתי** שהמעטפת קוראת.
--
--    ⑧ **`tiles[].target.report` נשא את השם העברי של הדוח** (מ14 ×3 · מ16 ×1 · מ19 ×4).
--       ‏§9 D-30 ③ קובע ש**ערך-החוזה הוא שם ה-RPC**; הנתב מקבל את שניהם, ולכן זו יישור-חוזה
--       ולא תיקון-שבר. **אחרי המיגרציה הזו אין ולו יעד עברי אחד** בשלוש-עשרה הפונקציות.
--
--    ⑨ **מחלקת-הכמות במשפטי-השרת עוברת `to_char(x, 'FM999,999,999')` בלי קשר לגודל.**
--       ‏H2 טיפלה ברצפי 4+ ספרות בלבד, ולכן נשארו חשופים: ארבעת מוני-ההוצאה של מ14 ·
--       ספירות-המשמרות של מ15 (‏167 · 43 · 16) · מוני-האוכלוסייה של מ16. **השינוי הוא
--       no-op על הערכים היום** (כולם תלת-ספרתיים ומטה) — וזה מוכח בהשוואת-המטען —
--       והוא מונע מהמספר הבא שיעבור אלף להופיע כ-`1234` באותו משפט שבו `1,234` כבר נכון.
--       **הוחל על ארבעת האתרים שהברִיף נוקב בהם:** `population.label` · `tiles[].sub` ·
--       ‏`meta.notes` · `so_what`.
--
--    ⑩ **מ17 · לעקומת-לורנץ לא היה קו-שוויון במטען.** ‏📐6 מחייב אותו — העקומה **נמדדת
--       מולו**, והשטח ביניהם הוא בדיוק מה שג'יני מודד. ‏`ChartCard` כבר יודע לצייר
--       ‏`axis:'diagonal'` דרך `ReferenceLine segment`. **הקצוות 0–100 ולא 0–1**, כי
--       ‏`v_lorenz` מחזיק אחוזים ו-`domain` נעול `[0,100]` בשני הצירים (📐6 · כרטיס §③).
--       ‏**הקו החדש נוסף ראשון ואינו מחליף** את קו-החציון הקיים — הכרטיס מחייב את שניהם.
--       ‏➕ ובנוסף `chart.note`: פסקת-`p.chart-note` של המוקאפ המאושר, רכיב רמה-0 לפי
--       מבחן-המחיקה של הכרטיס ⑩, שלא היה במטען כלל.
--
--    ⑪ **מ16 · תווית קו-הייחוס הדפיסה `43.27 ₪`** ליד טבלה שמדפיסה שקלים שלמים.
--       ‏📐4: *"₪ בלי אגורות בכל מקום"*. ⇒ **התווית** מעוגלת לשקל שלם; **`value` — המיקום
--       שבו הקו נמתח — נשאר ללא עיגול**, כדי שהקו לא יזוז. ⚠️ המוקאפ המאושר מצייר
--       ‏`43.27 ₪`, ולכן זו **סתירה מדווחת בין רמה 4 (מוקאפ) לבין 📐4**, שהוכרעה לטובת 📐4.
--
--    ⑫ **מ16 · 17 הדיילות ללא דירוג נעלמות מהפיזור בלי שמץ של עקבה.** הן כן ב-`data`
--       עם `rating: null`, אבל נקודה בלי `y` אינה מצוירת — והכותרת אומרת *"50 הדיילות
--       הפעילות"*. המוקאפ צייר להן רצועה נפרדת מתחת לציר; לרכיב אין רצועה כזו, ולכן
--       ‏`chart.note` **אומר את ההעדר במילים**, עם הספירה החיה. **העדר מוצהר, לא השמטה שקטה.**
--
--    ⑬ 🔴 **`chart.unit` הוא שם-פורמט של C8, ולא מחרוזת-תצוגה — והוא היה מחרוזת-תצוגה
--       ברוב המטענים החיים** (נמדד ע"י מתקן-המעטפת על 126 קובצי-מטען, 16/09 18:0X:
--       ‏`'₪'`×51 · `'משובים'`×42 · `'%'`×18), ו-`series[].format` היה `null` ב-296 מתוך
--       342 סדרות. **ולמה זה שובר בפועל:** `ChartCard.valueFormat` מחזיר
--       ‏`series[0].format ?? unit` ומזין את `formatAxisTick`, ו-`HebrewTooltip` מזין את
--       ‏`formatByType` — שניהם מצפים ל-`money|percent|int|ratio|score|days|text|date`.
--       מחרוזת-תצוגה היא **פורמט לא-מוכר** ⇒ הציר נופל לקיבוץ-אלפים חשוף והטולטיפ ל-`text`.
--       **המדידה הנקובה:** ציר "שיעור המרוצים לפי שנה" של מ20 קרא `0/25/50/75/100` בלי `%`.
--       ⇒ בכל גרף של שלוש-עשרה הפונקציות: `unit` הוא שם-פורמט, ו-`series[].format` מוצהר
--       פר-סדרה. **הכיתוב העברי נשמר ב-`chart.unit_label` (תוספתי).**
--       ⚠️ **ולזה אין קורא היום** — אף רכיב אינו מרנדר `unit_label`; הוא נשמר כדי שלא
--       ייעלם מידע, ומדווח כפריט לפזה 4.
--       🪤 **והפגם הנראה ביותר שהפריט הזה תפס:** בפיזור של מ16, `ScatterBody` גוזר את
--       ציר-X מ-`series[0].format ?? unit` ואת ציר-Y מ-`series[1].format ?? unit`; שתיהן
--       היו `null` ⇒ **ציר-הדירוג (טווח 1–5) סומן כשקלים**.
--
-- ─────────────────────────────────────────────────────────────────────────────
-- 🔒 **מה המיגרציה הזו לא עושה, במכוון:**
--    · אינה נוגעת ב-`filter_key` של אף גרף — הוכרעו ב-G3 ו-D-30 ③ ונמדדו שם.
--    · אינה מוסיפה בידוד-כיווניות לשאר משפטי-השרת של מ16 (היחידה ש-H2 לא נגעה בה);
--      נוסף רק הקבוע שההערה החדשה צריכה. **פער מדווח, לא מתוקן כאן.**
--    · אינה מזיזה ערך מספרי, סדר-שורות, מיון או אוכלוסייה באף פונקציה.
-- =============================================================================
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    from (select to_char(date_trunc('month', p.final_event_date), 'YYYY-MM') as ym,
                 v_months_he[extract(month from p.final_event_date)::integer]
                   || ' ' || v_lri || to_char(p.final_event_date, 'YYYY') || v_pdi as he_label,
