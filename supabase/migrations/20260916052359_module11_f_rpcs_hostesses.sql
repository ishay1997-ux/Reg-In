-- =============================================================================
-- מודול 11 · מיגרציה F · ארבע פונקציות-הקריאה של לשונית "דיילות"
-- ‏`report_m14_hostess_overview` · `report_m15_reliability` ·
-- ‏`report_m16_quality_cost` · `report_m17_fairness`
-- =============================================================================
-- 🎯 **מה זה:** ארבע פונקציות-שרת, אחת לכל משטח בלשונית, כל אחת מחזירה `jsonb` אחד
--    בצורת חוזה-המטען של `docs/micro_guides/module-11.md §2ב C8`. **קריאה בלבד.**
--    אין טבלה חדשה, אין עמודה חדשה, אין policy חדשה, אין אינדקס.
--
-- 🔐 **מודל-ההרשאות (הכרעה 2 · ת8 #3 · מדריך-המיקרו §C5):** הלשונית נפתחת לפי
--    **המודול שבעלים על הדאטה — 'דיילות'** ולא לפי 'דו"חות'. ⇒ השורה הראשונה בכל
--    אחת מארבע הפונקציות היא
--    שער-ההרשאה על המודול 'דיילות', ברמות edit/view — ר' השורה הראשונה בכל גוף למטה.
--    ‏**פתוחות:** מנכ"ל · מנהלת גיוס ושיבוץ · **ממוסכות:** מנהלת כספים-ולקוחות ·
--    מנהלת פרויקטים · **חסומה:** מנהלת לוגיסטיקה.
--    🔴 ‏`security definer` עוקף RLS ⇒ **זו ההרשאה היחידה שקיימת**, ולכן היא ראשונה
--    ולא אחרי חישוב. ‏`revoke … from public, anon, authenticated` ואז grant ל-
--    `authenticated` — דפוס `get_dashboard_summary` (מ7), המוקש של 09/08.
--    ⚠️ ‏**מ16 הוא הריכוז הגדול ביותר של נתוני-שכר במודול** (תעריף של 50 דיילות במסך
--    אחד) — ת8 #4 קובע שאין כאן חשיפה חדשה מעבר למסך-מ4, **ולכן הגידור אינו טקס.**
--
-- 📐 **הגדרות שאסור להמציא — כל אחת נקראה במקורה, לא שוחזרה מהזיכרון:**
--    · **בסיס-האמינות = `attendanceCounts` מילה-במילה** (`src/lib/smartMatch.js:234`):
--      פרויקט מבוטל בחוץ · אירוע שטרם התקיים בחוץ · צירוף-נוכחות לא-מוכר **מדולג
--      בשקט** · 🔴 **"חולה" ו"אישור-מראש" מוחרגים משני צדי השבר** — לא במונה ולא
--      במכנה. ‏`approval_withdrawn` נבדק **לפני** עמודות-הנוכחות (הענף הראשון של
--      `resolveAttendanceOutcome`) ושווה 0.5.
--      **ערכי-הנוכחות** (`ATTENDANCE_VALUES`, שורות 56–63): הגיעה 1 · איחור קל 1 ·
--      בינוני 0.75 · רב 0.5 · ביטלה-אחרי-אישור 0.5 · הבריזה 0.
--    · **ציון-אמינות = `reliabilityScore`** ‏`(Σערך + m×C) ÷ (n + m)`, ‏`m` =
--      `params.קבוע_ריסון_m` ו-`C` = ממוצע-החברה **על הסכומים** ולא ממוצע-של-ממוצעים
--      (`companyReliabilityAverage`). **אותו ציון בדיוק ששיבוץ Smart Match משתמש בו —
--      הכרעה 38.**
--    · **ג'יני = גרסת-אוכלוסייה**, בלי תיקון `n/(n−1)` (`spec.md §🔢 3.3`). על
--      `1·2·3·4·10` היא מחזירה **0.40**; גרסת-המדגם מחזירה **0.50** — פער של 25%,
--      ושתיהן נראות נכון על המסך. האורקל: `src/lib/reportsHostesses.test.js`.
--    · **אוכלוסיית-ג'יני (§ח8-1):** כל דיילת עם ≥1 משמרת `finally_approved` בחלון
--      שתאריך-האירוע שלה כבר עבר. **בלי קיפול-שלישייה** — נמדד 16/09/2026: ספירת
--      השורות הגולמיות מחזירה `n=106 · 1,862` בדיוק כמו העוגן.
--    · **סף-מדגם (📐12):** דיילת נכנסת לדוח-האמינות רק עם ≥ `מינימום_תשובות_להצגת_ציון`
--      משמרות מסומנות; השאר **מונה גלוי**, לא אחוז.
--    · **בסיס-ההשוואה (הכרעה 6 · 📐15):** חציון-החברה / התקופה המקבילה שנה אחורה.
--      🚫 **אין יעד ואין צביעה מול יעד.**
--
-- 🔴 **קונבנציית-החלון — הפער היחיד שנמצא מול העוגנים, ומדווח ולא "מתוקן":**
--    החוזה (‏§C8 · `cards-hostesses.md:413` · `spec.md §🔢 3.3`) נועל חלון **חצי-פתוח**
--    `(from, to]`. נמדד 16/09/2026 על החלון `(10/09/2025, 10/09/2026]`:
--    ‏**כל עוגני ג'יני, הריכוזיות, אי-ההגעה, התעריף וזמני-התגובה משתחזרים בו ספרה-בספרה**
--    (‏0.4559 · n=106 · 1,862 · 55.3/16.2/26.0 · 6.5/2.8/0.5 · 14.6/4.9/1.2 · 43.27 ₪ ·
--    ‏9.9/20.7 שעות · 92.6% · 2,299 מתוך 2,483).
--    ⚠️ ‏**ועוגני-הנוכחות של מ14/מ15 משתחזרים רק בחלון סגור** `[from, to]` —
--    ‏`1,730` תצפיות · `C=0.958815` · `1,510` הגעות-בזמן · `2,401` שיבוצים ·
--    `347` סירובים. בחצי-פתוח: `1,725` · `0.958696` · `1,505` · `2,394` · `345`.
--    **חמש תצפיות יושבות בדיוק על 10/09/2025** — וזו ההשערה שכרטיס ⑧17.3 השאיר פתוחה.
--    ⇒ **הפונקציות בנויות לחוזה (חצי-פתוח)**, וההפרש מדווח כממצא. 🔑 **ומה שאינו זז
--    בין שתי הקונבנציות: התור עצמו** — 6 אדומות · 4 ענבר · 87 בדוח · 13 מתחת לסף ·
--    6 מ-6 האדומות `active` · 0 מ-4 הענבר. **ההחלטה זהה; רק המונים זזים.**
--
-- 🧾 **פרמטרים — נקראים בזמן-ריצה, ולעולם לא ברירת-מחדל שקטה** (§7.83 · ת6 · דפוס מ7):
--    `מקדם_אמינות_אדום`(0.87) · `מקדם_אמינות_ענבר`(0.95) · `קבוע_ריסון_m`(3) ·
--    `מינימום_תשובות_להצגת_ציון`(3). שורה חסרה ⇒ שמה נכנס ל-`meta.missing_params`,
--    המסך אומר *"חסר פרמטר מערכת: X"* **ואינו צובע אף אריח שתלוי בה.**
--    🔴 **ושומר-סדר נוסף, שאיש לא היה בעליו:** אם `מקדם_אמינות_אדום` ≥
--    `מקדם_אמינות_ענבר` שני הצבעים **מתהפכים בשקט** — סף-האדום עובר את סף-הענבר,
--    וכל מי שהייתה "ענבר" נצבעת אדום או נעלמת, בלי שגיאה ובלי בדיקה אדומה, על מסך
--    שכל תכליתו להחליט את מי לא לשלוח. שני הפרמטרים ניתנים לעריכה בידי המנכ"ל במסך-מ9
--    (ת6) ⇒ ההיפוך הוא **עריכה אחת**, לא היפותזה. ⇒ מ14 ומ15 מחזירים משפט ב-
--    `meta.notes` **והמסך מציג אותו במקום לצבוע**. התאום ב-JS:
--    `reliabilityParamOrderNote` (`src/lib/reportsHostesses.js`), עם בדיקת-יחידה.
--
-- 🚫 **מה שאין כאן במכוון:**
--    · **אין מסנן-לקוח באף אחת מהארבע.** ‏📑#13 מונה *"חלון · לפי יום-בשבוע"* ·
--      ‏📑#14 *"סטטוס · עיר"* · 📑#14א *"חלון"* — **לקוח אינו מופיע באף אחת**, ושלוש
--      מהארבע נשענות על **ממוצע-החברה** או על **ערך-היום בכרטיס הדיילת**, שאין להם
--      חתך-לקוח כלל. סינון היה מגדיר מחדש בשקט את "ממוצע-החברה". ⇒ `p_customer_id`
--      מתקבל ומוחזר ב-`meta`, **מוכרז ב-`meta.notes`**, ואינו מסנן (‏§C8 מתיר זאת
--      במפורש). 🏷️ `הנחתי` — מדווח.
--    · **אין `0%` באריח אימוץ-ההמלצה.** ‏`assignments.recommended_rank` נוצרה היום
--      ו-**כל 5,741 השורות `NULL`** (נמדד 16/09/2026). ‏📑ב#14א ו-`spec.md §🎯` מורים
--      על **"—" + *"נמדד מ-<תאריך>; N שיבוצים"***. 🚫 `0%` היה נקרא *"אף פעם לא
--      מאמצים את ההמלצה"* — אמירה שקרית על מנוע שעוד לא נמדד.
--    · **אין קידוח בשלוש מהארבע**, ו-📐13 מונה ארבעה דוחות-דריל שאף אחד מהם אינו כאן.
--      ‏`report_m17_fairness` מקבל `p_drill` **כאופציה בלבד** — ברמה 0 הדף זהה לציור,
--      והעקומה אינה לחיצה (‏`cards-hostesses.md` ① מ17). ר' הערת-הפונקציה שם.
--    · **אין אינדקס חדש** — ‏`assignments` מחזיקה 5,741 שורות ו-`hostesses` 186;
--      כל ארבע הפונקציות הן אגרגציות-סריקה על טבלה קטנה.
--    · 🔴 **אין טבלה זמנית באף אחת מהארבע, וזו החלטה ולא סגנון.** ‏`create temporary table`
--      בתוך פונקציה `stable` הוא כתיבה בתוך חוזה שמבטיח שאין כתיבה — והוא **נופל בשקט
--      דווקא במסלול אחד**: ‏PostgREST מתיר `GET` לפונקציה `stable` ומריץ אותה בטרנזקציה
--      ‏`read only`, שם ה-`create` מת ב-`25006`. ‏`supabase.rpc()` שולח `POST` ולכן לא
--      היינו רואים זאת בפיתוח כלל. ⇒ **הכול ב-CTE**, וביטוי-הנוכחות נכתב פעמיים בכוונה
--      (‏`.jscpd.json` סורק `javascript`/`jsx` בלבד — אין כאן שער שנשבר).
--
-- 🔁 **הפיכוּת:** ארבע `drop function` בסוף הקובץ (מוערות). הקובץ אינו נוגע בנתונים,
--    אינו משנה עמודה ואינו נועל טבלה. **כלל-הפריסה:** פונקציות חדשות — תוספת בלבד.
--
-- 🔻 **אימות אחרי ההחלה (קריאה בלבד, בהתחזות):**
--   -- (א) מנכ"ל / גיוס: ארבע הקריאות מחזירות `population.n` > 0.
--   -- (ב) מנהלת פרויקטים / כספים-ולקוחות: `42501` בכל הארבע.
--   -- (ג) `select proacl from pg_proc where proname like 'report_m1%'` ⇒ בלי `anon=`.
--   -- (ד) `report_m17_fairness('2025-09-10','2026-09-10')` ⇒ אריח ג'יני `0.455870`,
--   --     `population.n` = 106, `1862` משמרות — העוגן של `signoff-baseline`.
-- =============================================================================
...
begin
  ...
  with obs as (
    ...
    select p.*, (p.sum_val + v_m * case when p.is_cur then v_c else v_c_prev end)
                / (p.n_shifts + v_m) as score,
           case when p.is_cur then v_c else v_c_prev end as base
      from per p
  ...
  select count(*) filter (where t.is_cur),
    ...
    into v_in_report, v_red, v_amber, v_red_active, v_prev_red, v_rows
  ...
  return jsonb_build_object(
    ...
    'window', jsonb_build_object('from', v_from, 'to', v_to,
      'label', '12 החודשים האחרונים (' || to_char(v_from, 'DD/MM/YYYY') || '–' || to_char(v_to, 'DD/MM/YYYY') || ')'),
    'tiles', jsonb_build_array(
      jsonb_build_object('key', 'on_time', 'label', 'הגעה בזמן', 'value', v_on_time_pct, 'format', 'percent',
        ...
        'target', null,
        'detail', coalesce(v_on_time, 0) || ' מתוך ' || coalesce(v_obs, 0) || ' שיבוצים'),
      jsonb_build_object('key', 'red_hostesses', 'label', 'דיילות אדומות', 'value', v_red, 'format', 'int',
        ...
        'detail', coalesce(v_red, 0) || ' מתוך ' || coalesce(v_in_report, 0) || ' דיילות עם ' ||
      ...
      jsonb_build_object('key', 'gini', 'label', 'ריכוזיות המשמרות', 'value', round(v_gini, 4), 'format', 'gini',
        ...
        'detail', 'מדד ג''יני · רבע הדיילות העמוסות ביותר מקבלות ' || coalesce(v_top_q_pct::text, '—') ||
      ...
      jsonb_build_object('key', 'active_hostesses', 'label', 'דיילות פעילות', 'value', v_active, 'format', 'int',
        ...
        'detail', 'מתוך ' || coalesce(v_registered, 0) || ' רשומות · ' || coalesce(v_worked, 0) || ' עבדו בחלון'),
      jsonb_build_object('key', 'gap_events', 'label', 'אירועים עם חוסר', 'value', v_gap_events, 'format', 'int',
        ...
        'target', null,
        'detail', case when v_upcoming = 0 then 'אין אירועים ב-30 הימים הקרובים'
    ...
    'definitions', 'הגדרות: הגעה בזמן = שיבוצים שסומן בהם "הגיעה" בלי דרגת-איחור, מתוך כל השיבוצים שסומנה בהם נוכחות · ' ||
      ...
      coalesce(round(v_c, 4)::text, '—') || ') — כלומר מתחת ל-' || coalesce(round(v_red_coef * v_c, 4)::text, '—') ||
      '; ענבר = מתחת ל-' || coalesce(trim_scale(v_amber_coef * 100)::text, '—') || '% ממנו (' ||
      coalesce(round(v_amber_coef * v_c, 4)::text, '—') || ') · ' ||
    ...
    'meta', jsonb_build_object(
      ...
      'notes', v_notes,
      'run', null,
      'customer_filter_ignored', p_customer_id is not null,
      'drill_echo', p_drill));
end;
$function$;

comment on function public.report_m14_hostess_overview(date, date, integer, jsonb) is
'מ11 · מבט-על דיילות (בקרה). אוכלוסייה: שיבוצים שסומנה בהם נוכחות, באירוע שכבר התקיים ובפרויקט שלא בוטל. הוצאו: היעדרות באישור-מראש ומחלה (לא במונה ולא במכנה), סירובים, זימונים שלא נענו, ושיבוצים שנוכחותם טרם סומנה. חלון חצי-פתוח (from, to], ברירת-מחדל 12 חודשים בשעון ישראל. ג''יני: גרסת-אוכלוסייה על ספירת המשמרות המאושרות פר-דיילת (§ח8-1). ספי-האמינות נקראים מ-params בזמן-ריצה ואינם קבועי-קוד (הכרעה 35).';
  ...
  return jsonb_build_object(
    ...
    'tiles', jsonb_build_array(
      jsonb_build_object('key', 'on_time', 'label', 'הגעה בזמן',
        ...
        'target', null, 'detail', coalesce(v_on_time, 0) || ' מתוך ' || coalesce(v_obs, 0)),
      jsonb_build_object('key', 'late', 'label', 'איחור',
        ...
        'target', null, 'detail', coalesce(v_late, 0) || ' שיבוצים, מהם ' || coalesce(v_late_heavy, 0) || ' באיחור רב'),
      jsonb_build_object('key', 'no_show', 'label', 'אי-הגעה (הבריזה)',
        ...
        'target', null, 'detail', coalesce(v_ghosted, 0) || ' שיבוצים · ' || coalesce(v_withdrew, 0) || ' ביטלו אחרי אישור'),
      jsonb_build_object('key', 'flagged', 'label', 'דיילות מסומנות', 'value', v_flagged, 'format', 'int',
        ...
        'target', null,
        'detail', coalesce(v_red, 0) || ' אדומות · ' || coalesce(v_amber, 0) || ' ענבר מתוך ' ||
  ...
  return jsonb_build_object(
    ...
    'tiles', jsonb_build_array(
      jsonb_build_object('key', 'no_rating', 'label', 'דיילות בלי דירוג', 'value', v_no_rating, 'format', 'int',
        ...
        'detail', 'מתוך ' || coalesce(v_active, 0) || ' הדיילות הפעילות'),
      jsonb_build_object('key', 'median_rate', 'label', 'תעריף שעתי חציוני', 'value', round(v_median, 2), 'format', 'money',
        ...
        'target', null,
        'detail', 'על ' || coalesce(v_active, 0) || ' הדיילות הפעילות'),
      jsonb_build_object('key', 'expensive_low_rated', 'label', 'יקרות ומדורגות נמוך', 'value', v_expensive, 'format', 'int',
        'window', 'נכון ל-' || to_char(v_today, 'DD/MM/YYYY'),
        'compare', null,
        'target', jsonb_build_object('tab', 'דיילות', 'report', 'אמינות והתייצבות', 'drill', null),
        'detail', 'תעריף מעל החציון (' || coalesce(round(v_median, 2)::text, '—') || ') וגם דירוג 3 ומטה'),
      jsonb_build_object('key', 'blocked', 'label', 'מסומנות "לא לשלוח"', 'value', v_blocked, 'format', 'int',
        'window', 'כל הזמנים',
        'compare', null, 'target', null,
        'detail', 'אצל לקוח אחד לפחות, מתוך ' || coalesce(v_active, 0) || ' הפעילות')),
    'chart', jsonb_build_object(
      'type', 'scatter', 'title', 'תעריף שעתי מול דירוג · הדיילות הפעילות',
      'series', jsonb_build_array(jsonb_build_object('key', 'hourly_rate', 'label', 'תעריף שעתי')),
...
begin
  ...
  ranked as (select hostess_id, shifts, row_number() over (order by shifts desc, hostess_id) rn from cur),
  busiest as (
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
             'drill_key', b.hostess_id, 'hostess_name', h.full_name,
             'status', case when h.status = 'active' then 'פעילה' else 'לא פעילה' end,
             'shifts', b.shifts, 'share_pct', b.pct, 'rank', b.rn) order by b.rn), '[]'::jsonb) j
      from busiest b join public.hostesses h on h.hostess_id = b.hostess_id
  ), speed_rows as (
    select coalesce(jsonb_agg(jsonb_build_object(
             'drill_key', s.hostess_id, 'hostess_name', h.full_name,
             'status', case when h.status = 'active' then 'פעילה' else 'לא פעילה' end,
             'rating', h.rating, 'invites', s.invites,
             ...
             order by s.med_hours desc, h.full_name) j
  ...
  return jsonb_build_object(
    'population', jsonb_build_object(
      'n', coalesce(v_n, 0),
      'label', 'אוכלוסייה: כל דיילת עם משמרת מאושרת אחת לפחות בחלון — n=' || coalesce(v_n, 0) ||
               ' דיילות, ' || coalesce(v_shifts, 0) || ' משמרות · הוצאו: ' ||
               coalesce(v_registered - coalesce(v_n, 0), 0) ||
               ' דיילות רשומות שלא עבדה אף משמרת בחלון (מי שאינה במשחק אינה יכולה להיות מקופחת בו) · אזור המהירות נמדד על אוכלוסייה אחרת: ' ||
               coalesce(v_invites, 0) || ' זימונים שנשלחו בחלון, ולא על דיילות — שני n נפרדים, מוצהרים בכל אזור.',
    ...
    'tiles', jsonb_build_array(
      jsonb_build_object('key', 'gini', 'label', 'ריכוזיות המשמרות', 'value', round(v_gini, 4), 'format', 'gini',
        ...
        'compare', case when v_gini_prev is null then null else jsonb_build_object(
          ...
          'label', 'התקופה המקבילה אשתקד · n=' || coalesce(v_n_prev, 0) || ' מול n=' || coalesce(v_n, 0) || ' היום',
          'direction', case when v_gini > v_gini_prev then 'up' when v_gini < v_gini_prev then 'down' else 'flat' end) end,
        'target', null,
        'detail', 'חלק מהשינוי הוא שינוי-אוכלוסייה: ' || coalesce(v_n_prev, 0) || ' דיילות אז מול ' ||
                  coalesce(v_n, 0) || ' היום'),
      jsonb_build_object('key', 'top_quarter', 'label', 'רבע הדיילות העמוסות', 'value', v_top_q_pct, 'format', 'percent',
        'window', '12 החודשים האחרונים',
        'compare', null, 'target', null,
        'detail', coalesce(v_top_q_n, 0) || ' דיילות · מחצית המאגר מקבלת ' || coalesce(v_bottom_pct::text, '—') ||
                  '% · העשירון העליון ' || coalesce(v_top_dec_pct::text, '—') || '%'),
      jsonb_build_object('key', 'rank1_adoption', 'label', 'אימוץ המלצת Smart Match',
        ...
        'window', case when coalesce(v_rank_total, 0) = 0 then 'טרם נמדד'
                       else 'נמדד מ-' || to_char(v_rank_from, 'DD/MM/YYYY') end,
        'compare', null, 'target', null,
        'detail', case when coalesce(v_rank_total, 0) = 0
                       then 'נמדד מ-— · אין עדיין נתון: המערכת התחילה לרשום היום איזה דרג היא המליצה בזימון'
                       else 'נמדד מ-' || to_char(v_rank_from, 'DD/MM/YYYY') || '; ' || v_rank_total || ' שיבוצים' end),
      ...
      jsonb_build_object('key', 'response_rate', 'label', 'אחוז היענות לזימון',
        ...
        'target', null,
        'detail', coalesce(v_answered, 0) || ' מתוך ' || coalesce(v_invites, 0) || ' · מי שלא ענתה כלל נספרת במכנה')),
    'chart', jsonb_build_object(
      ...
      'refLines', jsonb_build_array(
        jsonb_build_object('axis', 'y', 'value', v_bottom_pct,
          'label', 'מחצית הדיילות = ' || coalesce(v_bottom_pct::text, '—') || '% מהמשמרות')),
      'unit', 'percent'),
    'columns', jsonb_build_array(
      jsonb_build_object('key', 'hostess_name', 'label', 'דיילת',        'format', 'text',    'align', 'start'),
      jsonb_build_object('key', 'status',       'label', 'סטטוס',        'format', 'text',    'align', 'start'),
    ...
    'so_what', case
      ...
      else 'לפתוח את מסך השיבוץ עם רבע הדיילות התחתון — ' || coalesce(v_top_q_n, 0) || ' דיילות מקבלות ' ||
           coalesce(v_top_q_pct::text, '—') || '% מהמשמרות בעוד מחצית המאגר מקבלת ' ||
           coalesce(v_bottom_pct::text, '—') || '%.' end,
    'definitions', 'הגדרות: מדד ג''יני = מדד ריכוזיות בין 0 ל-1, כאן על מספר המשמרות לכל דיילת · ' ||
      ...
      'אחוזון 90 = הערך שתשע מכל עשר תשובות מהירות ממנו · ' ||
      'אחוז היענות = ענו ÷ זומנו; מי שלא ענתה כלל נספרת במכנה.',
    -- 🔴 14א הוא דוח-**בקרה** ואינו אחד מארבעת דוחות-הקידוח של 📐13 ⇒ `drill` תמיד `null`.
    'drill', null,
    'meta', jsonb_build_object(
      'measured_at', now(),
      'missing_params', to_jsonb(v_missing),
      ...
      'extra_tables', jsonb_build_array(jsonb_build_object(
        ...
        'columns', jsonb_build_array(
          ...
          jsonb_build_object('key', 'status',       'label', 'סטטוס',   'format', 'text',  'align', 'start'),
          jsonb_build_object('key', 'rating',       'label', 'דירוג',   'format', 'int',   'align', 'end'),
          jsonb_build_object('key', 'invites',      'label', 'זימונים', 'format', 'int',   'align', 'end'),
...
--   drop function if exists public.report_m14_hostess_overview(date, date, integer, jsonb);
--   drop function if exists public.report_m15_reliability(date, date, integer, jsonb);
--   drop function if exists public.report_m16_quality_cost(date, date, integer, jsonb);
