-- ============================================================
-- Baseline — מצב הסכמה לפני תחילת מעקב המיגרציות (נוצר בעת אימוץ תיקיית המיגרציות).
-- מייצג את 15 הטבלאות + ה-RLS הראשוני כפי שנוצרו ידנית ב-DB לפני שהמיגרציות
-- הממוספרות (20260702+) החלו להירשם. הדלתות של מודול 1 (users_update_self,
-- הקשחת current_user_role_id, נעילת-חשבון) הן קבצי המיגרציה שאחרי baseline זה —
-- ולכן אינן כלולות כאן. הרצת baseline → 6 המיגרציות משחזרת בדיוק את המצב הנוכחי.
--
-- ⚠️ ה-DB בפרודקשן כבר מכיל הכל. אין להריץ קובץ זה מחדש על ה-DB הקיים; הוא נועד
-- לשחזור על DB ריק (branch/staging) ולתיעוד. ראה supabase/README.md.
-- ============================================================
...
-- 2. טבלת מודולים (מודולים במטריצת ההרשאות)
...
-- הערה: ה-check המקורי כלל 'frozen'; מיגרציה 20260702112703 הסירה אותו (מחיקה רכה active/inactive).
create table users (
...
-- ============================================================
-- RLS ראשוני (מודול 1). הגרסה המקורית של current_user_role_id (לפני ההקשחה
-- ב-20260702195258). מדיניות users_update_self ונעילת-החשבון הן מיגרציות נפרדות.
-- ============================================================
