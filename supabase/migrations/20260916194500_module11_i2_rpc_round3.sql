-- =============================================================================
-- מודול 11 · מיגרציה I2 · סבב-3 — יעדי-דלת, בידוד-מספרים, עמודת-בעלים, ספי-אמינות
-- =============================================================================
-- 🎯 **מה זה:** מיגרציית תיקון-קדימה שנייה, חוצת-לשוניות, על **תשע** פונקציות-דוח:
--    ‏`report_m03_trends` (הנהלה) · `report_m09_aging` (כספים) ·
--    ‏`report_m14_hostess_overview` · `report_m15_reliability` · `report_m17_fairness`
--    (דיילות) · `report_m19_customers_overview` · `report_m20_satisfaction` ·
--    ‏`report_m21_drifting` · `report_m22_notes` (לקוחות).
--    היא ממשיכה ישירות את I1 (`20260916114500_module11_i1_rpc_formats_and_notes.sql`)
--    ומכסה את פריטי **סבב-3** שנשארו פתוחים בו — כל פריט שהמדידה סימנה
--    ‏`real: true` ו-`owner: "rpc"`, פלוס תשע ההכרעות של המתזמר.
--    **כל שינוי הוא תוספת, או `format`/תווית/יעד-דלת, או ערך של מפתח חדש.**
...
--    חוזרים כאן מילה-במילה כפי ש-I1 כתב אותם (נבדק בייט-בבייט, 9/9, `shell_check_i2.py`).
--
-- 📐 **מקור הגוף — נמדד, לא זכור:** לכל אחת מהתשע נמשך `md5(prosrc)` מ-`pg_proc` החי
--    ‏16/09/2026 והושווה ל-`md5` של גוף ה-`$function$` בקובץ I1, **לפני** שנגענו בתו אחד.
--    עשר מתוך עשר זהות (התשע שכאן + `report_m16_quality_cost`, שנבדקה ונמצאה כבר-מתוקנת
--    ולכן אינה מופיעה בקובץ הזה) ⇒ הגוף שלמטה **הוא** הגוף החי.
--
-- ⚠️ **שלושה פריטים של סבב-3 כבר נחתו ב-I1 ואינם חוזרים כאן** — נבדק ב-grep על הגופים
--    החיים לפני הכתיבה, לא מהזיכרון: שם-השבב `בלי דירוג בלבד` ו-`chart.x_domain` ב-m16 ·
--    ‏`'value', v_pay_n, 'format', 'int'` ב-m19 · הסתעפות ה-`compare` על
--    ‏`v_run.run_id is null` ב-m22 (אריח 3).
--
-- 🗂️ **שורות הרישום (`supabase_migrations.schema_migrations`), אחת לכל נתח < 60 KB:**
--    ‏`module11_i2_m03` · `module11_i2_m09` · `module11_i2_m14` · `module11_i2_m15` ·
--    ‏`module11_i2_m17` · `module11_i2_m19` · `module11_i2_m20` · `module11_i2_m21` ·
--    ‏`module11_i2_m22`. ‏(D-20: הקובץ מוחל בנתחים כי `apply_migration` קוטם ~90 KB;
--    כל נתח הוא פונקציה אחת שלמה + `comment`/`revoke`/`grant` שלה.)
--
-- 🩹 **הליקויים שהיא מתקנת — כל אחד נמדד על מטען חי, ומסומן 🪤 I2 בגוף:**
--    ‏① **m03, גרף 2:** שלוש סדרות-₪ בשני גדלי-גודל על ציר אחד (מחיר 254–305 מול עלות
--       ‏44–47) ⇒ עקומת-העלות נמעכת לקו של כ-2px. פוצל לשני לוחות מאפסים, כפי שהמוקאפ
--       המאושר אומר במילים (⑧ 3.1). ‏`valueDomain(null)` מחזיר `[0,'auto']` — נמדד
--       ב-`ChartCard.jsx:200-202` — ולכן `domain: null` **הוא** הציר המאפס, ואין מפתח חדש.
--    ‏② **m03, תעריפי-שעה:** 14 הדפסות של `₪` עם ספרה אחרי הנקודה (`305.1 ₪`, `47.4 ₪`)
--       בשעה שהטבלה לצידן מדפיסה שלמים ⇒ 📐4. הערכים הגולמיים ב-`tiles[].value` וב-`rows[]`
--       לא נגעו — רק הטקסט.
--    ‏③ **m09, אריח "מעל 60 יום":** ה-`target` הוביל לקידוח `bucket=d90p` — **מספר אחר
--       מזה שעל האריח**. "מעל 60" הוא צבירה של שני מדרגים, ואוצר-הקידוח הוא חמישה מפתחות
--       יחידים ⇒ אין קידוח שיכול לשאת אותו ⇒ `null`.
--    ‏④ **m09, שמות עם טווח-ספרות:** `מדרג 90+` רונדר `מדרג +90`, וכך גם שמות-הרמות
--       (`מדרג 61–90 יום`) בפירור, באריח-רמה-1 ובשם קובץ-הייצוא. כל טווח מבודד כיחידה אחת
--       ב-`chr(8294)…chr(8297)` — דפוס H2, ולעולם לא שני בידודים לטווח אחד.
--    ‏⑤ **m09, 📐18:** `owner` היה בשורות בשלוש הרמות ולא הוצהר כעמודה, ו-`ReportTable`
--       מרנדר עמודות מוצהרות בלבד ⇒ שורת-חריגה בלי בעל-שם. הוצהר בשלוש הרמות.
--    ‏⑥ **m09, עמודת המדרג:** ערכיה קודי-דלי וכותרתה טווח-ספרות ⇒ `format: 'textLtr'`.
--    ‏⑦ **m14, שתי דלתות מתות:** `on_time` ⇒ `report_m15_reliability` (אותה הגעה-בזמן,
--       מפורקת) · `gap_events` ⇒ `report_m06_staffing` (דלת חוצת-לשונית, תקדים D-18).
--    ‏⑧ **m15, `definitions`:** הדף אמר "אדומה"/"ענבר" שלוש פעמים ולא אמר מעולם במה מדובר
--       במספרים — הם ישבו ב-`meta` ולא הגיעו למסך. נוסף הנוסח של m14 (שתי לשוניות, נוסח
--       אחד) ‏➕ משפט הכרעה 38: הציון הוא `reliabilityScore` של Smart Match מילה-במילה.
--    ‏⑨ **m17, ענף אפס-שורות:** `נמדד מ-— · אין עדיין נתון: …` — מילת-יחס ואחריה קו-מפריד
--       אינה עברית ⇒ `טרם נמדד · אין עדיין נתון: …`.
--    ‏⑩ **m19/m20/m21/m22, תוויות שחוזרות על ערכן:** `KpiTile.CompareLine` מדפיס
--       ‏`{label}: {value}`, וחמש תוויות נשאו בעצמן את אותו המספר (`2.43: 2.43`).
--       🔑 **התיקון שנרשם בממצא היה `compare.value = null` — והוא חסום:** הרכיב מדפיס
--       ‏`{label}: <Ltr>{value}</Ltr>` ללא תנאי, ו-`formatByType(null)` הוא `—` ⇒ המסך היה
--       מקבל `"<משפט>: —"`. יושם **החלופה שהממצא עצמו נוקב בה** — לקצר את התווית לבסיס-
--       ההשוואה בלבד — שאין לה שום תלות במעטפת.
--
-- ✅ **מה שלא זז:** כל `population` · `window` · `meta` · כל מספר ב-`tiles[].value`,
--    ב-`rows[]`, ב-`chart.data` וב-`drill`. ההוכחה היא השוואת-מטען מלאה, עלה-אחר-עלה,
--    לפני ואחרי, פר-פונקציה, עם סיווג נפרד ל-NUMERIC-MOVE.
-- =============================================================================
...
begin
  ...
  return jsonb_build_object(
    ...
    'definitions', 'הגדרות: ציון אמינות = (סכום ערכי-הנוכחות + ' || coalesce(v_m::text, '—') || ' × ' ||
      ...
      'הציון הוא בדיוק reliabilityScore של Smart Match (הכרעה 38) — אותה נוסחה שמדרגת את הדיילות בשיבוץ, ולא מדד שנולד לדוח הזה.',
    'drill', null,
    'meta', jsonb_build_object(
      'measured_at', now(),
      'missing_params', to_jsonb(v_missing),
