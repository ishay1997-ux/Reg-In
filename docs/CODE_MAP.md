...
graph TD
    subgraph UI_Layer ["שכבת ממשק ומשתמש (Presentation Layer)"]
        Modules["src/modules/ (01_auth .. 09_settings)"]
    ...
    subgraph Backend_DB ["בסיס נתונים ואבטחה (PostgreSQL 17 / Supabase)"]
        ...
        RPCs["Database Functions / RPCs (Transactional)"]
        Tables["15 טבלאות ליבה + אינדקסים חלקיים"]
    end
...
| `src/lib/` | לוגיקה עסקית טהורה וחישובים | `quotes.js`, `smartMatch.js`, `pricing.js`, `salaryReport.js` | 72 | 22,957 | • פונקציות טהורות ללא תופעות לוואי.<br>• 100% מכוסות בבדיקות יחידה (`*.test.js`).<br>• שום חישוב כספי אינו מומצא בקומפוננטה. |
...
| `src/modules/09_settings/` | הגדרות מערכת, פרמטרים ותבניות | `ParamsTab.jsx`, `SmartMatchPane.jsx`, `BelowMinWageList.jsx`, `TemplateEditor.jsx`, `MySettingsPage.jsx`, `api.js` | 20 | 4,387 | • SSOT לכל פרמטרי המערכת (`params`).<br>• עריכת תבניות אימייל וניהול התראות. |
| `supabase/migrations/` | היסטוריית מיגרציות ומסד נתונים | 48 קובצי מיגרציה `.sql` | 48 | ~9,800 | • מקור האמת היחיד לשינויי סכמה.<br>• מנוהל תחת שער בלתי-הפיך ופרוטוקול Expand-Contract. |
| `e2e/` | בדיקות קצה-לקצה ונגישות | 19 קובצי `.spec.js`, `smoke.spec.js`, `accessibility.spec.js` | 20 | ~4,200 | • בדיקות Playwright מבודדות רשת (Mocking דרך `page.route`).<br>• אפס הזרקת שורות בדיקה למסד החי. |
