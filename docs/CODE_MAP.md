...
| `src/modules/03_quotes/` | מנוע הצעות מחיר, הנחות ו-PDF | `QuotesPage.jsx`, `QuoteBuilderPage.jsx`, `QuoteDocumentDialog.jsx`, `FollowupDraftDialog.jsx`, `quotePdf.jsx`, `api.js` | 19 | 5,419 | • מנוע תמחור רגיש עם נעילת מע"מ.<br>• המרת הצעה לפרויקט דרך RPC בלבד (`approve_quote_and_create_project`).<br>• תבנית PDF רצה דרך `@react-pdf/renderer` עם הטמעת לוגו כ-Data URI.<br>• ✨ **D2 (24/09/2026):** "נסחי מייל מעקב" בחלון-המסמך ⇐ `draftFollowupEmail` ⇐ `supabase/functions/draft-followup` — **טיוטה בלבד, המערכת לעולם לא שולחת**; הכללים ב-`src/lib/quoteFollowup.js`. *(הספירה כוללת בדיקות — `ls` + `cat … \| wc -l`, 24/09/2026.)* |
...
| `supabase/functions/` | פונקציות-שרת (Edge Functions, Deno) — המקום היחיד שבו חיים סודות | `send-email/` (מ3/מ4/מ6/מ8 — ⇐ Make ⇐ Gmail) · `classify-feedback/` (מ11 · מ22 — סיווג-הערות ב-Gemini, כותבת בשירות-על) · `draft-followup/` (מ3 · D2 — טיוטת מייל-מעקב ב-Gemini, **קריאה בלבד ואינה שולחת**) | 3 + 2 README | ~1,616 | • ‏`deno check` לכל פונקציה **בצעד נפרד** ב-`ci.yml` (צעד שמונה כמה קבצים עוצר בראשון שנופל).<br>• 🔴 **שום דבר ב-CI אינו פורס** — עריכה כאן אינה חיה עד `deploy_edge_function`, והפריסה חיה מיד לכל המשתמשים.<br>• סדר-השער חוזה: סוד ⇐ זהות ⇐ הרשאה ⇐ גוף (403 לפני 400).<br>• 🔒 ל-Gemini יוצאים טקסט וציון (classify) או מצייני-מקום בלבד (draft-followup) — לא שם לקוח, לא כסף, לא אירוע. |
