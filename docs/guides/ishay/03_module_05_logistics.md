...
- [ ] טבלת `projects` קיימת בסכמה — הלוגיסטיקה עושה FK ל-`project_id`. הטבלה כבר קיימת (מ-16 הטבלאות שנוצרו בתשתית, PROJECT_MASTER §2.2), אז **אין תלות-בנייה במודול 6**: הסדר הוא 5 לפני 6 (00_track), ומודול 6 הוא שדורש את מודול 5 מוזג — לא להפך.
