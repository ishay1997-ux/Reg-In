...
- 🗡️ **חדשות מאודיט-הערעור המבני (08/07/2026)** — (המפה: ‏`docs/db_roadmap.md`): רוכב ‏**§7.42↳** — job ניקוי שורות עבשות ב-`login_attempts` (נבנה עם ה-pg_cron במודול 3, או כאן) · ‏**§7.61** תשתית-Storage — השליחות כאן מצרפות קבצים מה-buckets שיוכרעו · ישות **יומן-השליחות** (‏db_roadmap ‏A-25) = טבלת-הליבה שהבלופרינט של §7.60 מסנתז.
