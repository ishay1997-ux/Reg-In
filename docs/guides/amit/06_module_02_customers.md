...
- 🗡️ **חדשות מאודיט-הערעור המבני (08/07/2026)** — פריטים חדשים שנוגעים למודול הזה (רשימה: ‏[PROJECT_MASTER §7](../../PROJECT_MASTER.md); המפה המרוכזת: ‏`docs/db_roadmap.md`): ‏**§7.62** ‏NOT NULL ל-`users.role_id` (להנהון — מצטרף למיגרציית-התשתית של צעד 1.1) · ‏**§7.73** חותמות-זמן `created_at`/`updated_at` לכל הטבלאות העסקיות (להנהון — מועמד לאותה מיגרציה; קודם 09/07 מרוכב-§7.23) · ‏**§7.63** בעלות-עמודות מול RLS שורתי — **כיוון נדרש מישי לפני כתיבת ה-policies** · ‏**§7.64** מדיניות-מפתחות (ח"פ: תיקון-טעות ורב-יחידות-אותו-ח"פ) · ‏**§7.65** ייחודיות `customers.email` (להנהון) · באקט ה-`marketing` של צעד 1.1 = החלוצה של תוכנית-ה-Storage ‏**§7.61**.
