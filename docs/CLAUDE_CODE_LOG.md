...
**פתוח במודול 1:** רק PR+merge ל-`dev` (ישי, ידני; ‏gitleaks על ההיסטוריה נקי, ה-CI צפוי ירוק; §7.24 — סיסמאות לא מוחלפות, החלטה סגורה) · ⚠️ **ליטוש UI לא-בקומיט לצירוף ל-PR:** יישור פריטי ה-Sidebar (`justify-center`→`justify-start` במצב מורחב, `Sidebar.jsx`, 3 קישורים; אומת חי + lint נקי) · 12 תרחישי RLS על `customers` (נדחה ל-M2) · backlog נדחה: שינוי-אימייל עצמי, טבלת העדפות, חיפוש Topbar, UI ל-`params` (מודול 9), שדרוג נעילה ל-Auth Hook (דורש תוכנית Team), חשיפת מודולי-אדמין במטריצה, Error Boundary ברמת Router.
...
**הבא בתור:** ישי — שני קומיטים (Sidebar.jsx · חבילת docs של פתיחת מודול 2) לפי ishay/01 ④3 → PR+merge של מודול 1 ל-`dev`; עמית — אחרי המיזוג, פרומפט 2 (המשך-בנייה) במדריך amit/06 ⑥ (⚠️ לא פרומפט הפתיחה — הבלופרינט כבר קיים ויגיע אליו דרך `dev`). ⚠️ תוקן 07/07: ההנחיה הקודמת "docs של מודול 2 מחכים לענף של עמית" היתה שגויה — עמית על מחשב אחר; docs חייבים לעבור דרך ה-PR של מודול 1.

---

## יומן סשנים (הכי חדש למעלה)

### 📝 סשן 07/07/2026 — "מדריך כיוונון Claude Code חזק לעמית (MCP/סקילים/פלאגינים)" (Claude Code)
**סוג:** תיעוד בלבד (מדריכי עמית). אין שינוי `src/`/DB. מודול 1 נשאר סגור.
**רקע:** ישי ביקש להוסיף למדריכי האונבורדינג של עמית אילו Skills/Connectors/MCP/Plugins להתקין ומה **לא**, כדי שה-Claude של עמית יהיה חזק וממוקד לסטאק. תוכנן ואושר ב-plan-mode.
**מחקר (2 סוכני Explore + claude-code-guide) + אימות חי בפועל:** ה-claude-code-guide ניחש התקנת Supabase MCP דרך `npx @supabase/mcp-server-supabase` + PAT — **התברר כשגוי**. הרצתי `claude mcp list`/`claude mcp get supabase` בסביבה וגיליתי שהמציאות שונה וטובה יותר: **`.mcp.json` כבר מגורסן בשורש הריפו** (מ-04/07) ומגדיר Supabase MCP **מתארח** (`https://mcp.supabase.com/mcp?project_ref=...`, HTTP+OAuth, בלי סוד בקוד). כלומר עמית מקבל את ה-MCP אוטומטית בקלון — רק צריך לאשר+להתחבר. (סוכן ה-Explore הראשון בדק רק בתוך `.claude/` ופספס את הקובץ בשורש — האימות החי תפס את זה.)
**בוצע:** ‏(1) מדריך חדש `docs/guides/amit/04b_claude_code_power_setup.md` (מבנה ①–⑧, RTL, עברית לאדם) — 4 קטגוריות עם "להתקין/לא להתקין": MCP (אשר+התחבר ל-Supabase המגורסן; גבול: שינויי-סכמה עדיין דרך מיגרציות), פלאגינים+סקילים (דרך `/plugin`, **לאמת שמות בתוך המוצר** — לא קובעים שמות/גרסאות שאולי ישתנו), קונקטורים (לא רלוונטי ל-CLI; GitHub דרך `gh`), ורעש-לנתק (Booking/Tripadvisor/Gmail/Calendar/Drive/Make — דוגמאות אמיתיות מ-`claude mcp list`). ‏(2) `00_track.md` — שורת 04b בין 04 ל-05 (בלי לשנות שם של אף קובץ קיים). ‏(3) `05_claude_routines_setup.md` — תנאי-כניסה ל-04b + ריכוך הערת "no MCP access". ‏(4) CHANGELOG.
**למה (עיקרון דיוק):** לא לקבע שמות-פלאגינים/גרסאות/פקודות לא-מאומתים במדריך שאדם זוטר יעתיק — נתתי את המנגנונים היציבים + הדבר הוודאי (Supabase המגורסן), ומוריתי לעמית לאמת ב-`/plugin`/`claude mcp list`. אימות ה-CLI לפני הכתיבה מנע פקודה שבורה (npx+PAT).
**נותר פתוח (אופציונלי, לא בוצע):** להגביל את ה-Supabase MCP המשותף ל-`read_only=true` (קובץ משותף — החלטת ישי) · שורה ב-CLAUDE.md "עובדות סביבה" על Supabase MCP (אדווה בקובץ ברזל — הושאר לישי). קבצי המדריך לא בקומיט — יצטרפו לתיעוד של עמית/הענף המתאים.

### 📝 סשן 06–07/07/2026 — "פתיחת מודול 2: בלופרינט לקוחות + סגירת §7.3/§7.11 + שדרוג טמפלטים" (Claude Code)
**רקע:** ישי הריץ את פרומפט הפתיחה של מודול 2 **במקום עמית** (שעדיין בשלב התקנת כלים). בוצע לפי `create_micro_guide_template.md` עם טריאז' מלא.
**הכרעות ישי:** §7.3 — תוויות לפי האפיון הקפוא (C5 §1.5.3 ממופה 1:1 ל-enum; המוקאפ נדחה) · §7.11 — בלי מיזוג, ח"פ קנוני, חיפוש סלחני (איש קשר/חברה/תחילת ח"פ, כל תוצאה מציגה את שלושתם — לשימוש חוזר ב-M3), חסימת כפילות + הצעת שחזור מארכיון (אושרה ב-07/07 אחרי דחייה ראשונית) · שיווק = bucket ציבורי + mailto‏ (BCC למאושרים; אמיתי ב-M10; Signed-URL נדחה) · כרטיס לקוח מלא עם empty states · תרחישי RLS ‏13–14 (שכבת view) אושרו · מיון עמודות נכלל במודול.
**בוצע:** ‏(1) `docs/micro_guides/module-2.md` — 9 סעיפים, SQL מלא (2 policies §7.21 + constraints + bucket + 4 storage policies + fallback לתקלת בעלות 42501), טבלת 14 תרחישים, טבלת מודל+מאמץ. ‏(2) הבלופרינט עבר **אימות אדוורסרי** (workflow: ציטוטים/טמפלט/עקביות — 18 ממצאים תוקנו; המרכזיים: בעלות storage.objects, קידוד mailto, שדות Goal/Files חסרים, חובות שלא שרדו ל-ledger). ‏(3) PROJECT_MASTER: §7.3+§7.11 סומנו סגורים; §6 קיבל שורות חוב חוצה-מודולים (כרטיס לקוח→M3/M6/M8, שיווק אמיתי→M10). ‏(4) שני הטמפלטים שודרגו במשוב ישי: תקציר יכולות בעברית (מוצג ראשון), הצהרת השתלבות, חובת קריאת §6 בפתיחה + רישום חובות בסגירה, סגנון שאלות הכרעה (רקע+משמעות+הסבר אפשרויות+המלצה), טבלת מודל+מאמץ. ‏(5) מדריך amit/06 §② עודכן (השאלות סגורות; אזהרה לא להריץ שוב את פרומפט הפתיחה).
**למה (החלטות תהליך):** משוב ישי בסשן — הוא רוצה להבין מה כל תוכנית מייצרת במונחי משתמש, לוודא שכלום לא נשבר, ולדעת שחובות נדחים לא יישכחו — עוגן בטמפלטים כדי שיחול על כל מודול עתידי, לא רק על מודול 2. הוכרע גם: **לא** כותבים את כל מדריכי-המיקרו מראש (הכרעות §7 פתוחות + תלות ב-as-built של מודולים קודמים) — מדריך פר-מודול ברגע הפתיחה, עם §6 כזיכרון החובות.
**Tech-debt:** אין חדש בקוד. קבצי הפתיחה לא בקומיט — נכנסים ל-PR של מודול 1 כ-commit ‏docs נפרד (ר' המשך-סשן ג' והנחיות ishay/01 ④3).
**המשך סשן (07/07, א'):** תיקון יישור בסייד-בר לבקשת ישי — `justify-center` → יישור-התחלה (RTL) במורחב, מרכוז רק במכווץ (`Sidebar.jsx`, 3 קישורים + הערת why). אומת חי בסקריפט Playwright חד-פעמי (login של משתמש-הבדיקה, צילומי מורחב+מכווץ — האייקונים על קו אחיד) + lint נקי; הסקריפט נמחק (נגע ב-creds). ⚠️ שינוי קוד על ענף מודול 1 אחרי האודיט — ליטוש UI בלבד, לצירוף ל-PR; **לא לערבב** עם קבצי ה-docs של פתיחת מודול 2 באותו commit.
**המשך סשן (07/07, ג') — עדכון הפרומפטים במדריכים:** ישי שאל אם הפרומפטים דורשים עדכון אחרי שינויי הסשן. ממצא: הפרומפטים מפנים לטמפלטים ("בצע כלשונה") — שדרוגי הטמפלטים זורמים אוטומטית, אין צורך לגעת בגוף הפרומפטים של מודולים עתידיים. כן עודכן: ‏(1) הערת "בחירת מודל לפני הדבקה" מעל ⑥ בכל 11 מדריכי המודול (Opus לפתיחה/סגירה; פרומפט 2 לפי טבלת Model & effort במדריך המיקרו). ‏(2) amit/06 — פרומפט הפתיחה סומן ✅-כבר-בוצע (הרצה חוזרת תדרוס את module-2.md); ④ מתחיל מפרומפט 2. ‏(3) ishay/01 — צעד ④3 חדש: שני קומיטים לפני ה-PR (‏Sidebar.jsx · חבילת docs) + אזהרת regin-pr-gate (עושה commit לכל מה שפתוח); הפרומפט ב-⑥ הוחלף לגרסת "מה שנותר". ‏(4) **תיקון הנחיה שגויה שלי מהיום:** docs של מודול 2 לא "מחכים לענף של עמית" — עמית על מחשב אחר; הם חייבים לעבור דרך ה-PR של מודול 1. תוקן ב-CHANGELOG/STATUS/כאן.
**המשך סשן (07/07, ב') — 2 משתמשים אמיתיים:** ישי הסתבך בהוספת עמית וטל. אובחן ב-SQL חי: לשניהם היתה שורת `users` (מנכ"ל, active), אבל לעמית **לא היה חשבון Auth** (המסך באפליקציה יוצר רק את השורה; חשבון+סיסמה = Supabase Dashboard — פער ידע שכדאי לזכור להסביר). נכתבו הוראות-הדבקה לקלוד-בכרום (יצירת Auth לעמית עם Auto-Confirm; לטל — איפוס רק אם הסיסמה מ-03/07 אבדה, בלי recovery-mail כי אין מסך איפוס עד M10). אומת אחרי ביצוע: עמית התחבר בפועל (`last_sign_in_at` ‏07/07) ✓; טל תקין אך `last_sign_in_at=null` — מעולם לא התחבר, הסיסמה נשארה זו מ-03/07. תועד ב-CHANGELOG.
...
10. **חידוד סעיף הטריאז' בטמפלט הפתיחה** (בעקבות שאלות ישי): שאלות §7 מסוג 🛑 (חוסמות-עכשיו) נשאלות **אינטראקטיבית ב-AskUserQuestion באמצע** (אכיפה של כלל 1 — לא להכריע לבד, לא "לרשום ולקוות"); ⏳ נרשמות בלבד (לא נשאלות); שאלה פתוחה חדשה מוצעת כפריט §7 חדש (כלל 13). נוסף "Persistence rule": כל פריט טריאז' חייב לשרוד לקובץ השמור (§3/§9) — שום דבר לא נשאר רק בצ'אט.
