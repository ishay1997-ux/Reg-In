...
**Written:** `docs/specs/module_06_projects/processes-approved.md` (the stage-1 artifact — decisions, full register sweep, the ⬜ surface list, and an explicit "next item, exactly"). **Deliberately NOT written:** `db_roadmap` rows and any §7 write-back — both wait on the surface that produced them being approved, per the Discovery protocol. **Blocking stop point: Ishay has not approved the 8-surface list**, and stage 2 cannot start without it.
