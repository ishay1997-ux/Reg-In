...
**Not done, deliberately:** Phase-1 compaction (self-update clause (i)) and step 2.0 — both wait on Ishay's 👤 sign-off, which is what the 🔻👤 on step 1.10 means. Nothing broke; no `src/` or DB change this session (the gate is read-only by construction).
