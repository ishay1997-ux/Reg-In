...
#### ⑥.2 · ✅ **מ1 צויר כקובץ — 11/09/2026** *(הפער האחרון שנשאר מ-§⑥)*

**‏`docs/mockups/management-report-screen/drafts/01_shell.html`** — ‏505 שורות. 🔑 **והוא אינו ציור
חדש:** הוא **שלד §⑥ עצמו** *(‏401 שורות, נכתב 06–10/09)* **כקובץ שאפשר לפתוח**, ועליו נוספו ארבעה
דברים בלבד: ‏① **מעבר-לשונית חי** — לחיצה מחליפה את שורת-השבבים, וארבעת הבוררים של §⑥.1 נמצאים
בקובץ · ② **מתג כלי-המוקאפ ומתג מצב-ההטמעה**, באותה מוסכמה של ארבעת המאושרים *(‏`hide-markers`
כברירת-מחדל ⇒ הקובץ נפתח נקי)* · ③ **הלשונית הממוסכת אינה נפתחת בלחיצה** — ‏`aria-disabled`
נבדק בקוד, וזו הכרעה 2 + 15-ה כהתנהגות ולא כצבע · ④ **קופסת-הסבר** שיורדת עם הסימונים ואומרת
מה להסתכל עליו ומה **אינו** חלק ממ1 *(התוכן שבאמצע הוא דף-דוגמה)*.
⚠️ **ומה שהוא עדיין אינו:** ‏**טיוטה ממתינה לעיניך** — ‏`approved/` מקבל עותק רק אחרי אישורך,
כמו ארבעת האחרים. **וחמשת המצבים מוצגים בו כגלריה זה-לצד-זה** *(כפי שהשלד מצייר אותם)*; במסך
החי מוצג **אחד** מהם במקום התוכן, ולא חמישה.
