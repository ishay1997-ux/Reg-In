...
> ✏️ **23/09/2026 — למי שמעתיק מכאן למודול הבא (טיפוגרפיה פזה ב׳):** השלד ב-§⑥ מצייר **שורת-הצהרת-
> אוכלוסייה ושורת-הגדרות-מדדים גלויות** — **זה כבר לא הדפוס.** הבנוי היום: **שבב-היקף אחד** (`population.summary`
> מהשרת, למשל *"35 חשבוניות פתוחות"*) שנפתח בלחיצה להצהרה המלאה ולהגדרות (`src/components/Disclosure.jsx` ·
> `ScopeLine` ב-`src/modules/11_reports/components/ReportSurface.jsx`) · אריח = **שם · מספר · השוואה** בלי תת-שורה
> שחוזרת על המסך · **אין** "אינו מושפע ממסנן…" על המסך — משטח שאינו מגיב למסנן מציג בשורת-המסננים **עובדה**
> (`תקופה  נכון להיום`) במקום פקד (`FiltersBar.jsx`) · רשימת-שיא נושאת כותרת-כנה (*"8 האירועים הגדולים · מתוך 246"*,
> `surface.topN` ב-`reportsCatalog.js`) במקום דפדפן. הנימוק והמדידה: `docs/plans/2026-09-17-module-11-typography.md` §7 פזה ב׳.
...
| טוקן | ערך | עוגן |
|---|---|---|
| `--primary` | ✏️ **`#0F766E`** *(עודכן 11/09/2026)* | `src/index.css`, עוגן-גריפ `--primary: #0f766e` |
...
> ✏️ **11/09/2026 — הטוקן שונה בהכרעת-ישי (10/09) ל-`#0F766E`, והשורה שלמעלה יושרה.** הנימוק המדוד
> יושב ב-`src/index.css` עצמו: לבן על `#0D9488` = **3.74:1**, מתחת ל-4.5 של WCAG 1.4.3 לטקסט 14px,
> ו**כל 129 ה-`<Button>`** יורשים את הטוקן. ⚠️ **והפסקה שמתחת נשארה בנוסחה המקורי בכוונה** — היא
> תיעוד מתוארך של הסתירה כפי שנמדדה ב-06/09, ולא הוראה. **ההוראה המעודכנת:** במקום `#0D9488` שבה
> היא מדברת על הטוקן — קרא `#0F766E`; ‏`#009689`/`bg-teal-600` לא השתנו.
...
> ✅ **אומת 16/09/2026** *(דפדפן אמיתי, Chromium 1280px, `ReportsPage?__chartprobe=1`)*: ‏`<html dir="rtl">`, ה-`<div>` העוטף נושא `dir="ltr"`, וה-`<svg>` **אינו נושא תכונת `dir` כלל** *(`svgHasDirAttr=false`)* אך `getComputedStyle(svg).direction` מחזיר **`ltr`** ⇒ **ה-SVG יורש את הכיוון מה-div העוטף, ואין צורך בהגדרה מפורשת ברמת ה-`<svg>`.** ‏**וזה נמדד גם בתוצאה ולא רק בתכונה:** ציר-הזמן נחת ינואר-משמאל → מרץ-מימין, והפארטו אלפא→בטא→גמא משמאל לימין. ⇒ **עטיפת ה-`dir="ltr"` של סעיף 1 היא המנגנון היחיד שנדרש.** *(המימוש: `src/modules/11_reports/components/ChartCard.jsx`.)*
...
## ④א 🔴 כללי-הניסוח — **`stage2-review/m11-copy-rules.md`. קרא אותו לפני שאתה כותב מחרוזת.** *(נוסף 11/09/2026)*

**‏20 כללים (כ1–כ20), לכל אחד מבחן מכני**, שנגזרו ממעבר-ניסוח מלא על לשונית-הכספים *(‏`90b97e5`, 07/09/2026 15:39)*.
**‏§2.2 שם היא טבלת `לפני | אחרי | הכלל | שורה`** — הדוגמה העובדת שכל ניסוח חדש מחקה.
🔴 **ו-§4 שם הוא החשוב מכולם: "מה לא שיניתי, ולמה".** מונחים שנשמרו **בכוונה** *(גיול · שוטף · חציון ·
פארטו · **ג'יני** · DSO · סטיית-תקציב · מחזור)* לפי בקשת-ישי *"מינוחים מקצועיים ומקובלים"*, ומחרוזות
שיש להן **תקדים-קוד שגובר על כלל-ניסוח** *(`נקה מסננים` · `אין נתונים עדיין` · `נסי שוב`)*.
⇒ **מי שמנסח מחדש בלי לקרוא את §4 "מתקן" דברים שהוכרעו.**

⚠️ **ולמה השורה הזו נוספה, ונמדד:** הקובץ היה מוזכר **אפס פעמים** בקובץ הזה, ב-`screens-approved.md`,
ב-`processes-approved.md` וב-`next-session-prompt.md` — **כלומר בכל מה שסוכן-ציור קורא.** שלוש
הלשוניות האחרות צוירו ב-10/09, **אחרי** שהכללים נכתבו, **ולא הייתה להן דרך להגיע אליהם.** אותו דפוס
בדיוק כמו שכבת-ההטמעה *(`screens-approved.md` §🤖 ראיה 6)*, ואותו כלל-מיקום ב-`docs/CLAUDE.md`:
*"אין מסלול-טעינה ואין מצביע ממסלול כזה ⇒ הוא לא ייקרא"*.
...
> ✅ **אומת 16/09/2026, והתשובה מחזקת את ההוראה:** תיבת-הטולטיפ **שלנו** (`content` מותאם עם `dir="rtl"` מפורש) מחשבת `direction: rtl` ✔, בעוד **מעטפת-הטולטיפ של הספרייה עצמה** (`.recharts-tooltip-wrapper`) מחשבת **`ltr`** — היא יורשת מה-div העוטף ולא מהדף. ⇒ **ברירת-המחדל של Recharts הייתה נוחתת LTR**, ו-`content` מותאם אינו העדפה אלא תנאי. ⚠️ **ו-`dir` על התיבה נכתב מפורשות ואינו נסמך על ירושה** — ירושה כאן פירושה טולטיפ עברי הפוך.
...
**✅ אימות שלושת הפריטים המסומנים בטבלה — נמדד 16/09/2026, אחרי שהגרף הראשון רץ בפועל.**
*(⚠️ הניסוח כאן נמנע במכוון מהביטוי המסומן עצמו: `check:declared-counts` סופר **שורות** שנושאות אותו ב-`design-contract.md`, ושורת-כותרת שמצטטת אותו הייתה מנפחת את הספירה ומפילה את השער — נמדד בפועל בסבב הזה, ‏10 ⇐ 11.)*

> ✅ **אומת 16/09/2026 ב-Node על החבילה המותקנת** *(`Object.keys(require('recharts'))` — **101 ייצואים**)*: ‏**אין ייצוא שמכיל `cumulat`/`lorenz`** ⇒ **אין prop מובנה למצטבר, והחישוב בצד-הלקוח הוא חובה**, כפי שהסעיף שיער. ➕ **ומה שכן נמדד בדפדפן:** קו-השוויון האלכסוני **מצויר** דרך `<ReferenceLine segment={[{x:0,y:0},{x:100,y:100}]}/>` *(נמדד: `x1=64,y1=226 → x2=950,y2=8`)*, ו-`domain={[0,100]}` על שני הצירים שומר את זווית-הבסיס.
>
> ✅ **אומת 16/09/2026 בדפדפן — והתשובה היא "כן, זה עובד":** ב-`ComposedChart` עטוף ב-`dir="ltr"`, ה-`YAxis yAxisId="right" orientation="right"` נחת ב-**x≈931** בעוד הציר השמאלי ב-**x≈99** (ה-SVG משתרע 41–999) ⇒ **`orientation="right"` הוא אכן הצד הימני-החזותי בתוך העטיפה**, ואין צורך בהיפוך. ⚠️ **הסייג שנשאר:** זה נמדד **בתוך** עטיפת-ה-LTR בלבד; בלי העטיפה לא נבדק, ואין סיבה לבדוק — סעיף 1 מחייב אותה בכל מקרה.
>
> ✅ **אומת 16/09/2026 על החבילה המותקנת:** אין ייצוא שמכיל `histogram` (101 ייצואים; גרפי-העל הם `AreaChart · BarChart · ComposedChart · FunnelChart · LineChart · PieChart · RadarChart · RadialBarChart · ScatterChart · SunburstChart`) ⇒ **חישוב-bins בצד-הלקוח הוא חובה**, כפי שהסעיף שיער.
...
// ✅ אומת 16/09/2026 — השלד רץ בדפדפן אמיתי. מה שנמצא **שגוי** בו: `label={{value,position}}` על
// `ReferenceLine` **אינו מרנדר טקסט כלל** (0 צמתי-text); הצורה שעובדת היא **רכיב-ילד `<Label/>`**.
// ומה שאומת כתקין: `dir="ltr"` על ה-div, `accessibilityLayer`, `role="figure"`, `tickFormatter`,
// גובה-הורה מפורש, `<Cell>` לצביעת-סף, ו-`domain`/`ticks` מפורשים.
    ...
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={rows} accessibilityLayer>
        ...
        <YAxis
          ...
          /* ✅ אומת 16/09/2026: `orientation="right"` נוחת בצד הימני-החזותי בתוך עטיפת-ה-LTR (x≈931 מול x≈99 לציר השמאלי) */
        ...
        /* ❌ נמדד 16/09/2026: `label={{...}}` על ReferenceLine **אינו מרנדר טקסט** ב-3.10.1 —
            0 צמתי-`text`, והקו נחת בלי שמו. ⇒ **התיקון שנבנה: תווית כרכיב-ילד** —
            `<ReferenceLine ...><Label value="חציון החברה" position="insideTopRight"/></ReferenceLine>`,
            ואז הטקסט העברי מרונדר ונקרא נכון בתוך ה-SVG ה-LTR (נמדד: "חלוקה שווה" · "קו 80%"). */
...
/* 🔴 ✏️ 10/09/2026 — `display:flex` הוסר, ו**זה היה שורש רב-קבצי**.
   ...
   (ב) `gap:4px` מחליף את רווח-הטקסט ונדחף גם לאן שאין רווח — נמדד `(19.9%)` שמרונדר
   `( 19.9% )`, ו-`2024:` שמרונדר `2024 :`, ו-(ג) כל מספר הופך לנקודת-שבירה חוקית מהמילה
   העברית שצמודה לו — **חמישה פיצולים בפועל נמדדו בלשונית-הכספים.**
  ...
  <div>
    ...
    <nav class="picker" aria-label="דוחות לשונית הנהלה">
      ...
      <!-- ✏️ 10/09/2026 · הכרעה 30 — "ריכוזיות לקוחות" (מ5) **נדחה ואינו נבנה לכנס**.
           ...
           ולמ10 · מ11 · מ13 בלשונית כספים (✏️ 23:5X — הושלם באישרור-ישי). -->
...
> 🔴 **✏️ 11/09/2026 — הבלוק שלמטה יושר למוקאפים המאושרים, והפער היה בדיוק הכשל שההערה הבאה מזהירה ממנו.**
> ‏**מה נמדד היום, מהקבצים המאושרים עצמם:** הנהלה **4** · כספים **7, ושלושה מהם מסומנים ⏸️ נדחה**
> *(`class="deferred"`)* · דיילות **4** · לקוחות **4**. **מה שהיה כתוב כאן:** כספים 7 **בלי סימון-דחייה**,
> ודיילות **6**. ⇒ **בונה שהעתיק מכאן היה מייצר בלשונית-הדיילות שני דפים שאיש לא הזמין**, ובכספים
> היה מציג שלושה דפים נדחים **כאילו הם רגילים.**
> 🔑 **ולמה זה נעלם:** ההערה למטה אומרת *"ארבעה שבבים, לא חמישה או שישה"* — **והקוד שמתחתיה לא שונה
> בהתאם.** ‏**כלל-הבית שנשבר כאן: תיקון שנכתב כפסקה ולא כקוד אינו תיקון** — הצייר מעתיק את הקוד.
> ⚠️ **והסתירה השנייה שנפתרה לטובת המוקאפ:** ההערה קובעת *"השבב הוסר ולא הושאר מנוטרל: דחייה אינה
> מצב-ביניים על המסך"*, בעוד המוקאפ המאושר **כן** משאיר אותם מנוטרלים. **ישי אישר את המוקאפ, ובמילותיו
> *"מראה ⇐ המוקאפ מכריע"*** ⇒ **המוקאפ קובע, וההערה מתוקנת ולא הקובץ המצויר.** *(ומ5/מ18/מ26 אכן הוסרו
> לגמרי — כלומר שני הדפוסים חיים זה לצד זה, וזה מכוון: מה שנדחה **בתוך** לשונית שנבנית מסומן, ומה
> שהוציא לשונית שלמה מהתמונה נעלם.)*

> ✏️ **10/09/2026 · הכרעה 30 — ‏**שישה** שבבים יורדים מהשלד הזה** *(✏️ 23:5X — היה "שלושה"; הושלם באישרור-ישי)*: ‏**ריכוזיות לקוחות** (הנהלה) · **מאגר הדיילות** ו**זימונים שלא נענו** (דיילות) · ➕ **ובלשונית-הכספים שלושה נוספים: תזרים צפוי (מ10) · תוספות-שטח (מ11) · שכר דיילות (מ13)**. 🔴 **ולמה הם נשמטו מכאן:** לשונית-הכספים צוירה **07/09** עם שבעה דפים, וההכרעה נכתבה **10/09** — ההערה הזו נכתבה מהלשוניות שצוירו **אחריה** בלבד. ‏**ולכן הכספים היא הלשונית היחידה שהשלד שלה לא תאם: שבעה שבבים מול ארבעה שהוא עצמו מכתיב.** ⇒ **כל לשונית נושאת מבט-על ועוד שלושה דפים — ארבעה שבבים, לא חמישה או שישה.** ‏🔴 **וזה נכתב כאן ולא רק ב-`screens-approved.md` כי צייר קורא את השלד הזה ומעתיק ממנו**, ושבב עודף בשלד מייצר דף שאיש לא הזמין. הדפים עצמם **אינם יורדים מהאפיון** — הם ברשימת-המוחזקים עם טריגר-החזרה.
...
<!-- לשונית הנהלה — ✏️ 22:1X (מחליף גם את הבלוק שבגוף-השלד למעלה)
     ✏️ 10/09/2026 · הכרעה 30 — "ריכוזיות לקוחות" (מ5) הוסר: הדף נדחה ואינו נבנה לכנס.
     ארבעה שבבים, לא חמישה. 🔴 והבלוק הזה הוא הקנוני (הוא מצהיר שהוא מחליף את גוף-השלד),
     ולכן תיקון גוף-השלד לבדו לא הספיק — נתפס ע"י סוכן-הרענון של לשונית ההנהלה, 10/09. -->
<nav class="picker" aria-label="דוחות לשונית הנהלה">
  <button class="on" aria-current="true">מבט-על הנהלה</button>
  <button>מגמות רב-שנתיות</button>
  <button>הנחות ורווחיות</button>
...
<!-- לשונית כספים — ✏️ 11/09/2026: יושר למוקאפ המאושר. שבעה שבבים, ושלושה מהם **מסומנים ⏸️ נדחה**
     (`class="deferred"` — ‏opacity .55 ומסגרת מקווקוות, `03_tab_finance_approved.html:341`).
     🔴 זו הלשונית **היחידה** שנושאת שבבים נדחים, וכך ישי אישר אותה. -->
<nav class="picker" aria-label="דוחות לשונית כספים">
  ...
  <button class="deferred">⏸️ תזרים צפוי</button>
  <button class="deferred">⏸️ תוספות שטח</button>
  <button>צריכת ציוד</button>
  <button class="deferred">⏸️ שכר דיילות לפי חודש</button>
</nav>

<!-- לשונית דיילות — ✏️ 11/09/2026: **ארבעה** שבבים. היו כאן שישה. -->
...
באנגלית, **`docs/mockups/management-report-screen/drafts/`** ואז `approved/` עם סיומת `_approved.html`.
🔴 **✏️ 11/09/2026 — הנתיב תוקן, והישן פשוט לא קיים:** עמד כאן **`docs/mockups/reports-screen/`**,
ו-`ls docs/mockups/` מחזיר **שלוש-עשרה תיקיות ואף אחת מהן אינה זו.** ‏**התיקייה בפועל היא
`management-report-screen/`**, ובה חמשת הקבצים. ⇒ **צייר שציית לשורה הזו היה יוצר תיקייה חדשה
ומאבד את ארבעת המוקאפים המאושרים מהעין.** *(נמצא במעבר-היסודיות על §⑥.1 בהוראת-ישי.)*
➕ **⑤ חותמת-"היום" משותפת: `10/09/2026`** — ‏**נמדד היום בארבעת הקבצים המאושרים וכולם נושאים
אותה.** *(עמד כאן `06/09/2026` — זו הייתה החותמת בזמן שהשלד נכתב, והמוקאפים עודכנו אחריו.)*
**קובץ חדש נושא את אותה חותמת** — תאריך שונה בין משטחים קורא כמו דאטה משתי תקופות.
...
מוקאפ מגיע מ-`docs/specs/module_11_reports/data-set.md`** — ✏️ **11/09/2026: ‏`grep '^## מ'` מחזיר
**‏26** מקטעים, **מ1–מ26** *(עמד כאן "25 … לפי 25 המשטחים המאושרים" — ספירה מתקופת `M=25`)*.
🔴 **ו"המשטחים המאושרים" אינו המספר הזה בכלל:** ‏**‏16 נבנים · 6 נדחו בהכרעה 30 · 4 לא צוירו** —
‏`screens-approved.md` הוא ה-SSOT היחיד לספירה. ⇒ **‏26 הוא כמה מקטעי-דאטה יש, לא כמה משטחים
נבנים, ואסור לגזור את השני מהראשון.** כל מקטע מתויג 🌱 מסד-חי / 🎭 דמו-מוסכם / 📐 נגזר-בנוסחה,
ותאריך "היום" המשותף הוא **`10/09/2026`** *(ר' כלל-שימוש ⑤)*. **צייר אינו ממציא ערך משלו — גם
לא לשלד-הגלריה שלמעלה** — ואם ערך חסר שם, זו שאלה לשלב הבנייה, לא ניחוש. (`seed-plan.md` נשאר
הקובץ הנפרד לתוכנית-הזריעה **בפועל** ל-DB החי — הכרעות 10–13 — ואינו מקור-הדאטה לציור-המוקאפים.)

---

#### ⑥.2 · 🚫 **מ1 אינו מקבל קובץ-מוקאפ משלו — הכרעת-ישי 11/09/2026**

**מילותיו:** *"המעטפת לא טובה תמחק לדעתי מיותרת גם לא?"* ⇒ ‏`drafts/01_shell.html` **נבנה, הוצג, ונמחק
באותו יום** *(שחזיר מ-`git log` אם אי-פעם ייחשב שוב)*.

🔑 **והנימוק, שנמדד ולא הונח — והוא הופך את זה מוויתור להכרעה נכונה: המעטפת כבר מצוירת ארבע פעמים.**
נמדד בארבעת הקבצים המאושרים: **כל אחד מהם נושא `side` · `tabs` · `picker` · `filters` · `stamp`**,
ו**שלושה מהארבעה נושאים גם את הלשונית הממוסכת.** ⇒ **קובץ חמישי לא היה מראה משטח שישי — הוא היה
מראה את אותו מסך בחמישית.** ➕ **ומחיר-התחזוקה אמיתי:** כל שינוי-רוחבי *(כמו הסרת ספירות-הדאטה
מהרמזים באותו יום)* היה צריך לגעת **בחמישה** קבצים במקום בארבעה, ולהישאר עקבי ביניהם.

✅ **ומה שהתרגיל כן הפיק, והוא נשאר:** ‏**הפריסה של בורר-הדוח בשלד למעלה הייתה שגויה** —
‏`flex-direction:column; width:220px`, כלומר **סרגל-צד שני**, בעוד ארבעת המוקאפים המאושרים מציירים
‏`flex-wrap:wrap; width:100%` — **שורת-שבבים אופקית.** ‏**ישי תפס את זה בעין בתוך דקות מפתיחת הקובץ**,
והשלד תוקן *(§⑥ למעלה)*. 🔴 **וזה מה שהיה באמת בסכנה:** §⑥ מסומן *"שלד-HTML להעתקה"* והוא מה שסשן-בנייה
קורא — **הפגם היה עובר לבנייה בלי סימפטום נראה.** ‏**במילותיו: *"אסור שבפיתוח יפלו בזה גם"*.**

⇒ **מה שהבנייה קוראת עבור מ1: §⑥ למעלה** *(השלד המלא, כולל חמשת המצבים במרקאפ אמיתי)* **+ §⑥.1**
*(ארבעת הבוררים)* **+ כל אחד מארבעת המוקאפים המאושרים, שכולם מציגים את המעטפת החיה.**
...
## ⑧ שכבת-ההטמעה — איך היא מצוירת *(✏️ נוסף 10/09/2026)*

> 🔴 **החוזה המלא: `onboarding-layer-contract.md` בתיקייה הזו. כאן רק מה שצייר חייב לדעת לפני
> שהוא מתחיל, כי הקובץ הזה הוא מה שהוא קורא.**

**המערכת מחזיקה שני מצבים — `0 · נקי` ו-`2 · מודרך`** *(הכרעת-ישי: אין רמה אמצעית, המערכת אקדמית)*.
המנגנון **בנוי במלואו** — `src/components/Hint.jsx` · `src/lib/onboardingCopy.js` · הרמה יושבת ב-
`notification_preferences.onboarding_mode`. **אין להמציא דבר; יש למלא מפתחות.**

| מה | הרמה |
|---|:-:|
| שורת-הצהרת-אוכלוסייה (📐2) · שורת-"אז מה" (📐23) · שורת-הגדרות-המדדים (📐16) | **0 · בסיס** |
| מקור-הנתון פר-אריח · נימוק-הסידור · למה הדוח קיים · מה לעשות בו קודם | **2 · מודרך** |

🚫 **לא מציירים כל דף פעמיים** — דף אחד, **מתג בינארי בכרום של המוקאפ**, בדיוק כפי ש-`<Hint>`
מתנהג בייצור. 🔴 **ועוגני-רמז מסומנים באות עברית בעיגול (א · ב · ג) ולא בספרות** — 📐14ב כבר תפס
ספרות לאינטראקציות, ו-`①②③` תפוסים בשתי משמעויות נוספות בתיקייה הזו.
**וכל משפט-רמה-2 בדוח נפתח במקור ורק אז בנימוק** *(המבחן: כסי את החצי השני — נשאר משפט שאומר
מאיפה המספר)*, כי בהיעדר רמה 1 היא נושאת את שני סוגי-האמירה.

🔴 **ומספר בתוך משפט-רמז — קרא את `onboarding-layer-contract.md §5ב` לפני שאתה כותב אחד**
*(נוסף 11/09/2026)*. בקצרה: ‏`<Hint>` מרנדר **מחרוזת שטוחה**, ולכן `<span class="ltr">` שבמוקאפ
**אינו שורד** את המעבר ל-`onboardingCopy.js` — ההמרה היא ל-**LRI…PDI** (`⁦…⁩`). ‏**נמדד: 28 מספרים
ב-13 רמזים, כולם עטופים היום ב-markup בלבד.** ➕ ובאותו סעיף גם הפער מול קונבנציית-הייצור
*"אפס ספרות"* (‏0/37 בייצור מול 13/67 במ11) — ✅ **הוכרע ובוצע 11/09/2026** *(ישי: "תסיר את ספירות הדאטה גם לפי המלצתך")*: **‏23 ספירות הוסרו, ורמזים-עם-ספרה ירדו 15 ⇐ 9** — התשעה שנותרו הם **המספרים המבניים שהכלל מתיר** (סף · חלון · סולם · קבוע-נוסחה). **המקור המלא: `onboarding-layer-contract.md` §5ג.** ✏️ **השורה כאן כתבה "ממתין להכרעת-ישי" ארבעה ימים אחרי שהוכרע** — תוקן 15/09/2026 בסריקת-סתירות.
