...
> **מה הקובץ הזה:** המקור היחיד לצבע, לטון, לכיווניות ולצורת-הרכיב עבור **שלושת המוקאפים של מודול 5**
...
⚠️ **פח-אשפה מול ארכיון — רלוונטי ישירות לדיאלוג-הסרת-הפריט:** ‏`src/CLAUDE.md`
(עוגן-גריפ: `פח-אשפה מול ארכיון`) — **פח = הסרת שורה מטופס שלא נשמר; רשומה קיימת מארכבים ולא מוחקים.**
✅ **וההכרעה כבר קיימת** *(עודכן 22/08/2026 — כאן נכתב קודם "לא הכרעתי")*:
‏`🔗 מראת ⑰+㊱ — SSOT: processes-approved.md (לא לערוך כאן)` — **הסרה מותרת אך ורק כששני התנאים
מתקיימים יחד: ‏`item_status = 'not_started'` **וגם** `actual_qty = 0`**, והיא מחיקה של השורה
*(שורה שלא הוזמנה ולא הגיע ממנה דבר אינה ראיה לכלום)*. ⇒ **אייקון הפח לגיטימי בדיוק בשורות
שההסרה מותרת בהן.**
🔴 **ולמה הסטטוס לבדו כבר אינו מספיק** *(㊱, ‏22/08/2026 — כאן נכתב קודם תנאי אחד)*: ‏㉟ מחזירה
פריט שכמותו הוגדלה ל-`טרם החל` **בעוד הסחורה שכבר הגיעה נשארת רשומה** ⇒ שורה כזאת נראית
"לא-הוזמנה" **והיא בדיוק ראיית-החיוב** ש-`§7.31` מגן עליה.
🔴 **ובשורות החסומות הפקד *נשאר, מושבת ומנומק*** — ‏`🔗 מראת ㉚ — SSOT: processes-approved.md`.
...
<main>
  ...
  <section class="card">
    ...
    <table>
      <thead>
        <tr>
          <th style="width:46%">פריט</th>
          <th style="width:17%">כמות מתוכננת</th>
          <th style="width:15%">כמות בפועל</th>
          <th style="width:14%">מצב הפריט</th>
          <th style="width:8%"></th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr>
          ...
          <!-- 🔴 ㊱ — והשורה הזאת היא בדיוק מקרה-הנגד, ולכן היא חייבת להיות מושבתת.
               ㉟ מחזירה פריט שכמותו הוגדלה ל-`טרם החל` בעוד `actual_qty` נשאר 8 ⇒ הסטטוס לבדו
               כבר אינו מעיד שלא יצא כסף. ㊱: הסרה דורשת `not_started` *וגם* `actual_qty = 0`,
               שני התנאים תמיד — ושורה זו מקיימת רק את הראשון.
               ⚠️ עד 22/08/2026 עמד כאן פח *פעיל*, והשלד הזה מוצהר "להעתקה" ⇒ הוא היה מטמיע
               את הבאג ישירות בכל מוקאפ שהעתיק ממנו. (עוגן: processes-approved.md,
               גריפ `המבחן המלא, במשפט אחד`.)
               🔑 והנימוק *שונה בכוונה* מזה של `הוזמן`/`מוכן` — היא צריכה לדעת איזה משני
               החוקים חסם אותה. -->
          <td><button class="rowact" disabled
                      title="הגיעו כבר פריטים — לא ניתן להסיר"
                      aria-label="הסרת פריט — לא זמינה: הגיעו כבר פריטים">🗑</button></td>
        </tr>
        <tr>
          <td>
          ...
          <!-- ㉚: הפקד נשאר, מושבת ומנומק — לעולם לא מוסתר. הסרה מותרת רק ב`טרם החל`
               (processes-approved.md, עוגן `🔄ה · הסרת פריט` + `🎓㉚`): הכסף כבר יצא.
               ⚠️ ההסתרה שהייתה כאן עד 22/08/2026 סותרת את ㉚ — אין לשחזר אותה. -->
          <td><button class="rowact" disabled
                      title="הפריט כבר הוזמן — לא ניתן להסירו"
                      aria-label="הסרת פריט — לא זמינה: הפריט כבר הוזמן">🗑</button></td>
        </tr>
        <tr>
          <td>
...
1. **‏`docs/specs/module_05_logistics/screens-approved.md` — ‏🆕 נוצר 22/08/2026, ‏שלב 2 נסגר**
   *(עודכן כאן באותו יום; קודם עמד "טרם נוצר", וזה היה נכון בבוקר ושגוי בערב)*. **בתיקייה שישה
   קבצים** — נוסף `screens-approved.md`, ובו **שלושה כרטיסי-מסך** בדגם תשעת-הסעיפים של מודול 6.
   ‏`processes-approved.md` מצהיר בכותרתו *"שלב 1 סגור"* — **שבעת כרטיסי-התהליך מאושרים · `M = 3`
   משטחים · ‏37 הכרעות ברשם** *(‏①–㊲; קודם נכתב כאן "34" — הרשם גדל ב-㉟ · ㊱ · ㊲)*.
...
3. **‏`docs/mockups/logistics-screen/` — ‏🆕 נוספה `drafts/` ובה שלושת המוקאפים** *(‏22/08/2026)*.
