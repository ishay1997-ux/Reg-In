...
| A-24 | notification-preferences table (+UI toggles wired) | `PROJECT_MASTER.md` §6 (Ishay 07/07) | M9 builds table+UI; M10 wires sending. **📘 m9 blueprint approved 02/09/2026 — migration B (`docs/micro_guides/module-9.md` step 1.2): `notification_preferences(email PK→users on delete cascade, email_new_projects, sms_last_minute, timestamps)` + 3 self-only policies; Ishay R-4: the email toggle goes live, the SMS toggle stays disabled with "אין ערוץ SMS במערכת" (no SMS channel exists)** |
