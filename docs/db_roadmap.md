...
- ⏳ **`20260924210000_module11_m12_measured_past_only` — ממתין להחלה על-ידי הסגן (נכתב 24/09/2026 בענף `ishay/module-12-integration`, סשן 2).** Ishay's ruling via the deputy: in `report_m12_equipment`, "measured rows" = **only events that already happened** (`final_event_date < v_today`, Israel time). Six exactly-once replaces on the live body, same signature: the product table, the "הוזמן מול הגיע" chart and the "נמדד על N שורות" count get the past-only filter; `population.label`, `definitions` and the note under the tiles say so. **Not touched:** the gap tile (window-bound) · cost tiles · the cost-per-product chart · "כמה להזמין לחודש הקרוב" · estimated-price rows. ⚠️ The table's "עלות מוזמנת (₪)" column now sums past events only (same row as "הוזמן"); all-time cost stays in the "עלות ציוד מצטברת" tile. **Measured before (24/09, read-only, live):** 01WEB 12.5% ⇒ 4.5% · "נמדד על" 92 ⇒ 74 of 1,781 rows · three SKUs move up slightly (B-SAT-LAN 2.0⇒2.1 · ECO-TAG 7.6⇒7.9 · SAT-LAN 5.9⇒6.3). Pre-write: all six segments exactly once in the live body (md5 `572f7b502a50dab316d55e963d54b64b`); newest registry row `module11_drop_m16_quality_cost`; a read-only dry-run of all six replaces gave 3 past-only filters and an intact table tail. **After apply, check:** ① md5 of the live body vs the dry-run · `proacl` unchanged (no `anon`) · one CEO-identity call: 01WEB `gap_pct` = 4.5, `meta.measured_rows` = 74 · no `docs/schema.sql` change (signature unchanged) · flip this row to ✅.
