...
|---|---|---|---|
| **M6-1** | 🔴 **NEW table `project_changes`** — `change_id` PK · `project_id` · sku · color · `change_qty` (CHECK ≠ 0) · **`unit_price_snapshot` + `unit_cost_snapshot`** · **`reason` NOT NULL** · `performed_by` → users · timestamps. **Must be written in ONE atomic function together with the `logistics.planned_qty` / `required_hostess_count` update** | ② (closes §7.17 + §7.72) — the approved quote is locked by a DB trigger, so a change has no other home | **absent** |
