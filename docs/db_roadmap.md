...
> `projects_closed_needs_report` CHECK, the survey-link `params` row) are in §5, not here.**

> ### ✅ APPLIED `14/08/2026` — **ALL of Phase 1 is LIVE: ten migrations + the money-gated-reader follow-up.** Each was verified by an independent live read after apply, not by the tool's success flag. *(This header read "seven of nine, 14:0X–14:1X" until the 1.10 gate ran on 18/08/2026 — the last rows below were applied later on 14/08 and the block was never flipped.)*
>
...
> | `module6_rpcs_reads_and_close` | **M6-9 · M6-10 (read/close half)** | 4 functions incl. `list_projects_overview` — **the acceptance oracle passed digit-for-digit** (`#8 → 5,355.00 · 1/6 · 2 pending`) · `set_project_finance_fields` gated **'כספים'**, not 'פרויקטים' (㉘) |
> | `module6_rpcs_writes` | **M6-10 (write half)** | `assert_module_permission` + the 4 operational write RPCs; preconditions refuse once the project is closed (㉙) · `anon` revoked **by name** on all |
>
...
>
> ✅ **SUPERSEDED later the same day** *(this line said "STILL NOT APPLIED — 1.8a/1.8b, M6-9 · M6-10 remain OPEN" until the 1.10 gate, 18/08/2026)*: steps 1.8a/1.8b **were applied 14/08** (`module6_rpcs_reads_and_close` · `module6_rpcs_writes`) — **`M6-9` · `M6-10` CLOSED.** The 1.10 gate itself ran end-to-end **18/08/2026**: **97/100 checks PASS**; the two FAILs are stale expectations, not defects *(triage in the §10 strike entry)*; advisors triaged finding-by-finding.
...
> ✅ **CLOSED `14/08/2026 15:4X` — migration `module6_project_changes_money_gated_reader`.** The reader `list_project_changes(int)` returns the history with `unit_price_snapshot`/`unit_cost_snapshot`/`revenue_delta` as **NULL** for callers without `'הצעות מחיר'`, plus a `money_visible` flag so the screen renders `—` rather than nothing (S-26). 🔴 **And the broad SELECT policy was DROPPED in the same migration** — without that the browser still reads the raw table and the function is decorative. **Proven by impersonation inside a rolled-back transaction:** דנה → `5.00`/`400.00`, `money_visible=true`; מנהלת לוגיסטיקה → all money `null`, `money_visible=false`; **and a direct table read returns 0 rows for both.** *(Was, until now:)* ⏸️ **Open, decided-with-anchor but NOT yet built:** `project_changes_select_by_permission` exposes `unit_price_snapshot`/`unit_cost_snapshot` to every `'פרויקטים'` holder — including מנהלת לוגיסטיקה and מנהלת גיוס, whom `screens-approved.md:809`/`:810` block from financial data. **The table is empty and no screen reads it yet, so there is no live exposure** — but the reader-RPC that nulls the money must land **before step 3.3**.

> ✅ **APPLIED `20/08/2026 23:5X` via MCP `apply_migration` (typed-echo: Ishay typed `module6_closing_rpc_hours_bound` in chat) — `20260820233607_module6_closing_rpc_hours_bound`.** `CREATE OR REPLACE` of the function body of **`close_project_operationally`** only — **no table/column/RLS/trigger change**, fully reversible; body pulled from live `pg_get_functiondef` (not an old file). **ONE surgical fix:** `v_max_hours` is derived from **`p_actual_hours + 2`** (the entered event hours, already validated 0.5–24) instead of the PLANNED `final_start_time`/`final_end_time` — matching the card (`screens-approved.md:1412`) and the client (`src/lib/projectClosing.js:159`). **Post-apply verification (live `pg_get_functiondef`, 20/08): `v_max_hours := p_actual_hours + 2` present · the planned-hours `v_event_hours` block gone.** 🚫 **The empty-rows raise is deliberately UNCHANGED** — an event with zero hostesses does not occur (Ishay's reality ruling 20/08/2026), so no behaviour is built for it (what the first draft of this audit called blocker 1 is reclassified as not-a-blocker). **`docs/schema.sql` NOT changed** — function bodies are pointer-only in the snapshot (DB protocol) and the signature is unchanged. Pending: full regression + the `#12` closing journey red→green.

| # | Change | Why (ruling) | State today |
|---|---|---|---|
| **M6-1** | 🔴 **NEW table `project_changes`** — `change_id` PK · `project_id` · sku · color · **`delta_qty` (CHECK ≠ 0)** *(🔴 **name corrected 14/08/2026: this row said `change_qty`.** Three names were in circulation for one column — `delta_qty` (`spec.md` §14②, the ruling) · `change_qty` (here) · `qty_delta` (`screens-approved.md` surfaces 3/6). **`spec.md`'s ruling wins and is now the only one; caught by a simulated build session.**)* · **`unit_price_snapshot` + `unit_cost_snapshot`** · **`reason` NOT NULL** · `performed_by` → users · timestamps. **Must be written in ONE atomic function together with the `logistics.planned_qty` / `required_hostess_count` update** | ② (closes §7.17 + §7.72) — the approved quote is locked by a DB trigger, so a change has no other home | ✅ **applied 14/08/2026 — `module6_project_changes_table`** *(11 named constraints · 4 indexes · trigger · RLS on. ⚠️ the SELECT policy was later **dropped deliberately** by `module6_project_changes_money_gated_reader` — browser access only via `list_project_changes`, money fields masked)* |
| **M6-2** | `projects.cancelled_at` (timestamptz) **+ a cancellation-TYPE column with THREE values** — 🔴 **`check (cancel_type in ('customer','force_majeure','other'))`**, per `AR-1` and the guide's step 1.1 *(corrected 14/08/2026: this row said **two** values, `standard`/`force_majeure`, and a session building from the register alone would ship a CHECK that **rejects `other`** — the value `AR-1` exists to keep countable)* — **separate from the existing free-text `cancel_reason`, which stays** | ⑤ · ⑩ · §7.16(ב) — without the timestamp the hostess compensation (0/50/100%) **can never be computed**, and the fact is unrecoverable | ✅ **applied 14/08/2026 — `module6_projects_columns_and_constraints`** *(three-value CHECK live; `cancel_reason` stays)* |
| **M6-3** | 🔴 **DROP `projects.project_bonus`** | ㉟ (Ishay: *"we never put a project bonus in the quote"*) — **measured: no bonus line in any quote, 0 references in `src/`** ⇒ no source, no reader. `assignments.personal_bonus` stays, **owned by M8** | ✅ **dropped 14/08/2026 — `module6_projects_columns_and_constraints`** *(all four live rows held `0` — nothing destroyed; verified at the 1.10 gate: column absent)* |
| **M6-4** | `assignments`: `attendance_status` · `lateness_level` · `no_show_reason` | §7.16(א) · `🚧 מ6 ← מ4` — **the only source for the reliability component, weight `0.35`**, the largest one currently switched off | ✅ **applied 14/08/2026 — `module6_assignments_attendance`** *(3 columns + 5 constraints; smartMatch.js alignment = step 2.7)* |
| **M6-5** | 🔴 **Origin pointer on `logistics` — TWO nullable columns, exactly one filled**: → `quote_services.line_id` **or** → `project_changes.change_id`, plus a CHECK enforcing exactly-one | ⑬ + ㉗ — `quote_services` carries `color` and `logistics` does not ⇒ **the same SKU in two colours produces two indistinguishable rows.** ⚠️ **Scope corrected in ㉗: this is a *fulfilment* problem, not a money one** — the money lives in `quote_services` + `project_changes` | ✅ **applied 14/08/2026 — `module6_logistics_policy_and_origin`** *(2 columns · `logistics_origin_exactly_one` · 2 FK + 2 covering indexes; legacy rows stay all-NULL by design)* |
| **M6-6** | 🔴 **Read policy on `logistics`** — standard `permissions × modules` template. **M6 opens it itself; does NOT wait for M5** | Surface 3 · M5-contract item 7 — **measured live: zero policies, deny-all** ⇒ the whole logistics tab loads empty, silently, for everyone | ✅ **applied 14/08/2026 — `module6_logistics_policy_and_origin`** *(`logistics_select_by_permission`, gated **'לוגיסטיקה'** per AR-2; the `rls_enabled_no_policy` finding on `logistics` disappeared as predicted)* |
| **M6-7** | **Operational-closing timestamp** on `projects` | ⑭ — the hand-off moment to M8; without it "how long has this sat unbilled" is unanswerable | ✅ **applied 14/08/2026 — `module6_projects_columns_and_constraints`** *(`operationally_closed_at`/`_by`)* |
| **M6-8** | 🔴 **RE-SCOPED 14/08/2026 — M6 does NOT freeze the profit.** M6 freezes the **inputs it owns**: `assignments.actual_hours` per hostess · the three attendance columns (M6-4) · `projects.actual_hours`/`actual_guests` · the operational-closing timestamp (M6-7). **No `final_gross_profit` column in M6's migration.** | §7.52 (ruled 11/07) — **and clarified by Ishay 14/08/2026 01:17: *"בסגירת-האירוע"* means the **financial** closing (M8), not the operational one (M6).** The system has two closings and the 11/07 wording predates the distinction being named. 🔴 **Why it cannot be M6's, measured:** two cost inputs are unreadable at M6's closing — `assignments.personal_bonus` is **M8-owned by Ishay's 13/08 ruling** (§7.19: *"🚫 מסך-הסגירה של מ6 אינו נוגע בבונוס"*), and `assignments.travel_amount` was measured 14/08 as **a column no code in the repo writes** (§7.69 🟠). `PROJECT_MASTER §5.14` says it of the formula itself: *"אינה כוללת **נסיעות**… **השמטתן מנפחת את הרווח המדווח**"* ⇒ a figure frozen by M6 today would carry bonus=0 and travel=0 — inflated by exactly the amount that warning describes, and frozen forever. | inputs: ✅ **built 14/08/2026** *(attendance columns in `module6_assignments_attendance` · closing timestamp in `module6_projects_columns_and_constraints`)* · the freeze itself: **M8's, not sized here — unchanged** |
| **M6-9** | **Dedicated RPC for the finance-owned columns** (`invoice_sent` · `payment_date` · `feedback_*`) — requires the **'כספים'** module permission, **not `edit` on 'פרויקטים'** | ㉘ (closes §7.63) — **RLS in Postgres is row-level, not column-level**; the system already learned this with `product_costs`, and a second table split on `projects` would be worse. **The RPC is the unit of authorisation, not the column** | ✅ **applied 14/08/2026 — `set_project_finance_fields` in `module6_rpcs_reads_and_close`** *(gated **'כספים'**, verified `G-09` at the 1.10 gate)* |
| **M6-10** | **Preconditions inside the operational RPCs** — they refuse once the project is closed, while the finance RPCs keep working | ㉙ (closes §7.77) — same pattern as the existing `enforce_quote_in_progress_lock`, which already throws `P0001` in Hebrew | ✅ **applied 14/08/2026 — `module6_rpcs_writes` + `module6_rpcs_reads_and_close`** *(preconditions in all four operational RPCs)* |
| **M6-11** | **Daily `pg_cron` job** — flip to `event_finished` once the event date has passed | ㉚ (closes §7.32) — **not a trigger: "the date passed" is a passage of time, not a data event.** `pg_cron` is installed and already runs two jobs | ✅ **applied 14/08/2026 — `module6_status_machine_and_cron`** *(`module6-event-finished @ 0 2 * * *`; proven live — `#7` moved to `event_finished`)* |
| **M6-12** | **Two new `params` mail templates, `param_type='templates'` — ✅ TEXT APPROVED BY ISHAY 14/08/2026 01:2X. Seed exactly as written; this is his approved copy, not a draft.**<br><br>🔴 **`תבנית_מייל_אירוע_בוטל`** — 3 placeholders: `[שם_דיילת]` · `[שם_פרויקט]` · `[תאריך_אירוע]`. Subject (built in code): `` `ביטול האירוע — ${event_name}` ``<br>`היי [שם_דיילת],`<br>`האירוע '[שם_פרויקט]' בתאריך [תאריך_אירוע] בוטל, והמשמרת שלך מבוטלת יחד איתו.`<br>`האירוע כולו לא יתקיים — כל הדיילות ששובצו אליו שוחררו. אין צורך להגיע.`<br>`הזכאות לתשלום על משמרת שבוטלה נקבעת לפי מועד הביטול. מחלקת הכספים תבדוק ותעדכן אותך בהמשך.`<br>`נשמח לשבץ אותך לאירוע הבא. לשאלות ניתן להשיב למייל זה.`<br>`בברכה,`<br>`צוות הגיוס, REG-IN.`<br><br>🔴 **`תבנית_מייל_פרטי_האירוע_השתנו`** — 8 placeholders, **the identical set to `תבנית_אישור_סופי_שיבוץ`** so the builder reuses `resolveShiftContact` unchanged. Subject: `` `עדכון פרטי האירוע — ${event_name}` ``<br>`היי [שם_דיילת],`<br>`חל שינוי בפרטי האירוע '[שם_פרויקט]'. השיבוץ שלך בתוקף ואנחנו מצפים לראותך.`<br>`אלה הפרטים המעודכנים, והם הקובעים:`<br>`תאריך: [תאריך_אירוע]`<br>`שעות: [שעת_התחלה] עד [שעת_סיום]`<br>`מיקום: [כתובת_אירוע_מלאה]`<br>`אם הפרטים החדשים אינם מתאימים לך, עדכני אותנו בהקדם.`<br>`איש קשר: [שם_מנהלת_פרויקט], טלפון: [טלפון_מנהלת_פרויקט]`<br>`בברכה,`<br>`צוות הגיוס, REG-IN.` | §7.31 · ㉑ — **measured: both existing templates are wrong for a full cancellation** (`שחרור` claims the slot was filled; `ביטול` implies the event goes on without her).<br>🔑 **Three copy decisions worth not re-litigating:** ① **no contact placeholders in the cancellation mail, deliberately** — `resolveShiftContact` (`src/lib/shiftEmails.js:72`) returns `null` when name *or* phone is missing and the caller must then refuse to send; a mail that must reach every hostess cannot depend on a nullable `projects.owner_phone`. Reply-to is the house pattern for bad news in both existing templates. ② **`בוטל` carries no attribution** — the cancellation type has three values (⑩), the template is one fixed string, and `fillEmailTemplate` cannot branch or omit a line ⇒ *"בוטל על ידי הלקוח"* would be **false** under force majeure. ③ 🔴 **`אין צורך להגיע` is financially load-bearing in Israel** — under the event-hall extension order a hostess who *arrives* and does not work is owed half a day; one told in time is not.<br>⚠️ **Trigger is narrower than it reads:** a **date** change never sends the second mail — ㉑ resets every final approval and re-invites. Only **location (㉒)** and **hours (㉝)** send an update while her confirmation stands.<br>⚠️ **And the sources say state old→new; we state only the new**, because no old-value placeholder exists in the vocabulary. `והם הקובעים` does the override work. Adding old→new costs two placeholders and a builder that receives the pre-change row. | ✅ **seeded 14/08/2026 — `module6_params_seed`** *(both rows live, `param_type='templates'` — `I-01`/`I-02` at the 1.10 gate)* |
| **M6-13** | **`CHECK` on `projects.negative_feedback_reason`** — the five closed values of `C6 §2.4.4` *(איחור דיילות · תפקוד דיילות · איכות תגים · ניהול לקוי · אחר)* | Found 13/08 by the **reverse-coverage sweep** — the seventh closed list in the module and **the only one nobody had recorded**; `feedback_status` got a CHECK, this column did not | ✅ **applied 14/08/2026 — `projects_negative_feedback_reason_check` in `module6_projects_columns_and_constraints`** |
| **M6-14** | ⚠️ **NOT a schema change, but it belongs with them: fix `OPEN_PROJECT_STATUSES`** in `src/modules/04_hostesses/api.js` — add `'ready'` and move the definition somewhere shared | ⑫ — M4 decided "active" unilaterally in code, **and it is wrong under ⑫**: a `ready` project vanishes from the recruitment manager's screen | ✅ **DONE 18/08/2026 (step 2.6)** — the local constant deleted, `ACTIVE_PROJECT_STATUSES` imported from `src/lib/projects.js` (includes `ready`); verified at close (`grep OPEN_PROJECT_STATUSES src/` → only the guard-test names). *(Was "live, wrong" until the 21/08 close.)* |
...
| # | Change | Why (ruling) | State today |
|---|---|---|---|
| **M5-1** | 🔴 **Write policy on `logistics`** — house pattern `logistics_write_by_permission`, `for all to authenticated`, the identical `exists(...)` in **both** `using` and `with check`, `permission_level = 'edit'` | Module 5's core capability is writing `item_status`, and **measured live 21/08: the only policy is `logistics_select_by_permission` (SELECT).** Today an `INSERT` raises `42501` and an `UPDATE`/`DELETE` returns **0 rows with `error: null`** — a save button that says "saved" and changed nothing | ✅ **applied 26/08/2026 — `module5_logistics_hardening`** *(gate `'לוגיסטיקה'` as required; verified live: דנה(edit) update→1 row, נועה(view)→0 rows, positive control first)* |
| **M5-2** | **`CHECK (actual_qty >= 0)` on `logistics`** | `C6 §2.4.13` requires it (`כמות בפועל … ≥ 0`) and the schema omitted it — **§7.41's module-5 half was nodded by Ishay on 22/08/2026** — his reason, as it was put to him: a negative quantity is not a business state, so there is nothing to weigh. ⇒ **no longer gated; open to build.** **Measured 21/08 via `pg_constraint`: only `logistics_planned_qty_check (planned_qty > 0)` exists** | ✅ **applied 26/08/2026 — `module5_logistics_hardening`** *(`logistics_actual_qty_check` live in `pg_constraint`)* |
| **M5-3** | 🔴 **RIPPLE INTO MERGED M3 CODE — `approve_quote_and_create_project` must fill `logistics.quote_service_line_id`** | ✅ **Ishay-approved 21/08/2026.** The column was created by **M6-5 above** precisely for *"the same SKU in two colours produces two indistinguishable rows"* — **and has no writer, so it is NULL on every row (measured 0/6).** Ishay confirmed the case is real: *"יכול להיות 2 מוצרים לדוגמא תגים בצבע צהוב ותגים בצבע אדום"*. Without it the checklist shows two identical rows and she cannot tell which to order. 🔑 **Closes M6-5's stated purpose AND the reporting debt `🚧 מ11 ← מ6` in one change.** ⚠️ **Positional back-joining is NOT an alternative** — `serial_number` is `row_number()` over the *hostess-filtered* set, and a scope-change row takes `max+1`, so the mapping breaks after the first scope change | ✅ **applied 26/08/2026 — `module5_approve_rpc_origin_backfill`** *(built from the live `pg_get_functiondef`, full-body diff verified; backfill filled 6/6 legacy rows; the seed's approvals birthed new rows with pointers — the live forward proof)*. ➕ **25/08/2026 — same migration also BACKFILLS existing rows:** `update logistics set quote_service_line_id = qs.line_id` for rows where the `(project→quote, sku)` match is **unambiguous** (count=1; measured today: all 6 live rows qualify, all approval-born). Scope-change rows stay NULL by ㉗ — the backfill touches only approval-born rows, and M5-7's price-resolution keeps the existing `(quote_id, sku)` + `count>1 ⇒ raise` path as fallback for any row still NULL |
| **M5-5** | 🔴 **`logistics.color` — עמודה חדשה**, `text` nullable, עם אותו `CHECK` בדיוק כמו `quote_services_color_check` *(לבן·שחור·אפור·טורקיז·כחול)*. ממולאת ע"י אותה אדווה של `M5-3` | ✅ **הכרעת-ישי ⑱, 21/08/2026 — ונרשמה 22/08 אחרי שסוכן-הכתיבה-מחדש תפס שאין לה שורה כאן כלל.** 🔴 **ולמה מצביע לבדו אינו מספיק:** הצבע חי ב-`quote_services`, ששתי המדיניויות עליה מגודרות ב-**'הצעות מחיר'** — ו**מנהלת הלוגיסטיקה `blocked` שם** *(נמדד חי)*. ‏RLS ב-Postgres הוא ברמת-שורה ולא ברמת-עמודה ⇒ המצביע ייתן לה חץ למידע שאינה רשאית לקרוא, **והקריאה תחזור ריקה בלי שגיאה**. **התקדים בבית: ל-`project_changes` כבר יש עמודת-צבע משלה, מאותה סיבה בדיוק.** ⚠️ **וסייג שישי כבר הכריע בו:** הדוגמה שלו *(צהוב ואדום)* **אינה ברשימת-הצבעים המותרת** — הכרעתו 21/08: **נשארים עם הקיימים**, אין מיגרציית-צבעים | ✅ **הוחל 26/08/2026 — `module5_logistics_hardening`** *(‏`logistics_color_check` העתק-בייט, אומת מול `pg_get_constraintdef`)* |
| **M5-4** | 🔴 **RIPPLE INTO MERGED M6 CODE — `apply_scope_change` must reset `item_status` to `not_started` when `planned_qty` INCREASES** | ✅ **Ishay-approved 21/08/2026.** **Measured: `item_status` appears ZERO times in `20260814142440_module6_rpcs_writes.sql`** ⇒ an item marked `ready` at 300 that becomes 350 **stays `ready`**, the readiness formula (which counts `item_status` only, never quantities) keeps returning complete, and **the project stays `מוכן לביצוע` with 50 units missing and nobody informed.** 🔴 **The section that requires the opposite is `C5 §5.6.8`, NOT `§5.5.9`** *(citation corrected 22/08/2026 — the earlier text here quoted a sentence that is not verbatim anywhere in C5, and attributed it to a section that speaks only of adding an item)*. §5.6.8 verbatim: *"במידה ופרויקט שכבר הוכן מקבל שינוי תכולה ממנהלת הפרויקטים (למשל תוספת תגים), הלוגיקה משנה את הסטטוס שלו חזרה ל'בתהליך'"* — **"שינוי תכולה" unqualified, which is exactly the quantity-increase case.** ⚠️ **`§5.5.9` says only *"במידה והלקוח משנה את מפרט המוצרים בהוספת פריט"*** — a NEW row, which already works today. 🔑 **Ishay's own spec was right; the implementation has the hole.** ⚠️ **A decrease resets nothing** — she already has enough. 🔴 **AND THE SCOPE IS EXACTLY ONE COLUMN — reset `item_status`, do NOT touch `actual_qty`** *(module 5 ruling ㉟)*: goods that already arrived did not vanish, so `8 of 8` raised to `12` must sit at `not_started` **with `actual_qty` still `8`**, reading *"8 arrived, 12 needed, order 4 more"*. Zeroing it would destroy a real measurement and contradict ㉕ *("a value she typed is never overwritten")* | ✅ **applied 26/08/2026 — `module5_scope_change_reset_removal`** *(live-body rewrite; `item_status` reset on increase only, `actual_qty` untouched — verified in the live def)* |
| **M5-6** | 🔴 **The checklist write RPC** — the one that sets `item_status`, `actual_qty`, `notes` **and `expected_arrival_date`** on a `logistics` row, **and stamps `actual_arrival_date` on the transition to `ready`** (M5-8) *(scope widened 25/08/2026 at the blueprint: ㊶ made the expected date "hers to fill" and ㉑ routes every write through the RPC — this row previously listed three columns and left the fourth writer-less; caught by the simulated-build pass)* | **Ruling ㉑: every module-5 write goes through an RPC, never direct.** Measured (`🧱④`): the status guard returns **silently, with no error and no effect** when the project is `cancelled`/`event_finished` ⇒ **a direct write "succeeds" and nothing moves.** The database does not enforce module 6's status rules — only module 6's RPCs do, and a new screen bypasses them. In an RPC the guard sits in one place on the server and cannot be routed around. ⚠️ **Its internal permission gate must be `'לוגיסטיקה'`** *(㉞)* — copying `apply_scope_change`'s `'פרויקטים'` gate would lock the logistics manager out of her own module. ⚠️ **And `notes` must be in scope** — `db_roadmap` originally scoped module 5's writes to `item_status`/`actual_qty` only; `notes` was added by ruling ⑮. 📐 **Per-row instant save, not a form with one save button** — `🧱⑤`: the recompute trigger has no `WHEN`/`OF` clause, so every write holds `FOR UPDATE` on the parent project row and bulk saves serialise | ✅ **applied 26/08/2026 — `module5_checklist_rpc`** *(7-assert impersonated round-trip passed inside a rolled-back tx; נועה(view)→42501)* |
| **M5-7** | 🔴 **The item-removal RPC** — deletes the `logistics` row **and** writes the matching `project_changes` row, atomically | **Ruling ⑰ + ㊱, and RPC is mandatory here for two independent reasons** *(not merely preferred)*: ① **two tables in one atomic action** — a half-done removal leaves an item gone with no history; ② **`project_changes` carries money columns the logistics manager is `blocked` from** (she is blocked on `'הצעות מחיר'`), so the write cannot originate in her client. 🔴 **The guard is TWO conditions, both required** *(㊱)*: `item_status = 'not_started'` **AND** `actual_qty = 0`. Status alone is not enough since ㉟ — a raised quantity returns an item to `not_started` while goods already arrived, and that row is exactly the expenditure evidence `§7.31` protects. ⚠️ **`reason` is mandatory free text** *(㉖)* — `project_changes.reason` is already `text not null` **plus** `check (length(btrim(reason)) > 0)`; reuse module 6's existing message rather than writing a new one. ⚠️ **Depends on `M5-3`** — the frozen unit price the `NOT NULL` snapshot columns need is resolved through `quote_service_line_id`, and resolving it by `(quote_id, sku)` instead breaks on a duplicate SKU, which is the very case `M6-5` exists for | ✅ **applied 26/08/2026 — `module5_scope_change_reset_removal`** *(removal proven live by the seed rider: row deleted + history row; both ㊱ refusal strings raised)* |
| **M5-8** | 🆕 **`logistics.expected_arrival_date` + `logistics.actual_arrival_date`** — both `date`, nullable. ➕ **25/08/2026: + `logistics.actual_qty_autofilled boolean not null default false`** — the durable carrier of ㊵'s `מולא אוטומטית` tag: M5-6 sets it `true` when it auto-fills `actual_qty` on the `ready` transition and `false` on any manual `actual_qty` write. *(Why a column and not client state: ㊵'s rationale is financial — an unmeasured number must not read as measured in M8 — and a session-only tag evaporates on refresh, which guts exactly that protection.)* | **Ishay's ruling ㊶, 22/08/2026** — his idea, checked and halved. **`expected_arrival_date` is hers to fill when she orders**, and it is the only thing that makes "late" computable: today the system does not know when delivery was promised, so ⑳ can only warn about *"not ordered yet"*. With it, ⑳ gains a **second, measured trigger** — `ordered` · expected date passed · not yet arrived. 🔴 **`actual_arrival_date` is NEVER written by hand** — `M5-6` stamps it at the moment `item_status` becomes `ready`; that transition *is* the arrival, and a second manual field would be the same fact at the cost of one more keystroke per item. ⚠️ **Declared ceiling on the reporting value, stated to him before he approved:** there is no supplier entity (ruling ⑤), so a report can say *which items ran late*, **not which supplier runs late** — the second being the one worth having. ⏱️ **Priority agreed with him: the columns are built now** (a column must exist before data accumulates); **the date input on screen and the second amber trigger are the first thing to cut if 01/10 gets tight** | ✅ **applied 26/08/2026 — `module5_logistics_hardening`** *(both `date` columns + `actual_qty_autofilled` live; the RPC stamps/clears arrival — proven in the 1.3 round-trip)* |

#### 🆕 Module 11 (Reports) — born in its Discovery, 07/09/2026

> **Source:** `docs/specs/module_11_reports/processes-approved.md` — **the only file in that folder allowed to set a product rule** (everything else there mirrors it). **Ishay-approved: ruling 28, 07/09/2026.**
> ✅ ✏️ **11/09/2026 — the write-back this paragraph was waiting for is below (M11-2…M11-5). Two corrections it forced:**
> **(a) there are FOUR owed items, not three** — card ת2 specifies **two** tables (`feedback_ai_runs` **and** `feedback_ai_insights`), and only the second was named here.
> **(b) two of the four `params` values were stale here.** Ruling **35** (10/09/2026) moved `מקדם_אמינות_אדום` **0.8 ⇒ 0.87** and `מקדם_אמינות_ענבר` **0.9 ⇒ 0.95**, after measuring that at 0.8 the queue was empty and at 0.9 amber had **zero** members. `processes-approved.md` card ת6 already carried the corrected values; **this file did not, and this file is a mandatory blueprint input** — a blueprint reading it would have seeded the rejected numbers.
> *(Original wording, kept so the correction is legible:)* ~~The same Discovery also plans a **new table `feedback_ai_insights`** (card ת2), a **new column `assignments.recommended_rank integer null`** (card ת5) and **four new `params` rows** — `מכפיל_מרווח_מתרחק`=1.5 · `סף_סטיית_תקציב_אחוז`=15 · `מקדם_אמינות_אדום`=0.8 · `מקדם_אמינות_ענבר`=0.9, all `owner_role_id`=מנכ"ל (card ת6). **Those three items are owed by the Discovery's write-back and are deliberately not invented here.**~~
> ⚠️ **And §7's RLS matrix row for module 11 is now stale** — it still reads *"none (5 reports as Views/RPC, read-only)"*, written before the Discovery. m11 writes at least `notification_preferences` (below), `feedback_ai_insights` and `assignments.recommended_rank`. **Flagged, not rewritten:** the other two rows are not on disk yet, and the matrix should be corrected once, with all three.

| # | Change | Why (ruling) | State today |
|---|---|---|---|
| **M11-1** | ✅ **APPLIED 08/09/2026 22:2X — `20260908221959_onboarding_mode` on branch `ishay/ui-copy-rewrite` (independent of m11, per the 28⑤ declared deviation). What landed, verified live against `pg_catalog` after apply:** `notification_preferences.onboarding_mode integer not null default 0` · named check `notification_preferences_onboarding_mode_check (0..2)` — the DO-block probe with `3` and `-1` hit `check_violation` and wrote nothing · `comment on column` (0 clean · 1 pointers · 2 guided) · table comment updated · **fourth policy `notification_preferences_ceo_all` `for all`** (predicate = `users_write_ceo_only`, `(select …)`-wrapped) ⇒ 4 policies. Registry version `20260908192746` (UTC, the documented 3-hour offset; name-matched). Existing row: mode 0. **Gate note:** typed echo **waived once by Ishay, explicitly and in advance** (22:2X: *"ואת המיגרציה ראיתי יש לך אישור ממני להריץ אין צורך שאקליד חד פעמי"*) — same shape as the module-9 A→C waiver; not precedent. **Code:** `src/lib/onboardingCopy.js` · `src/components/Hint.jsx` · `AuthContext.onboardingMode` · `api.saveOnboardingMode`/`listOnboardingModes` · third switch in `ProfileSettingsPage` · CEO column in `UsersManagementPage` (+ its first-ever test file). ✏️ **07/09/2026 17:1X — this row was superseded in four places by ruling 28 ⑨/⑫/⑭ (`processes-approved.md`); text below kept as history:** name **`onboarding_mode`** (not `onboarding_hints`) · type **`integer not null default 0` + named check `notification_preferences_onboarding_mode_check` (0..2) + `comment on column` — a LEVEL, not a boolean** (0 clean · 1 pointers · 2 guided; Tuesday builds 0 and 2 only) · **a fourth CEO write policy IS required** (the three existing ones are self-only) · **default = 0 (off)**, the open sub-item below is closed · independent migration on `ishay/ui-copy-rewrite` (28⑤ deviation). 🆕 **ONE new boolean column on the EXISTING m9 table `notification_preferences`** — proposed name `onboarding_hints boolean not null default …` (see the open sub-item below). **No new table, no new policy, no new index:** the three self-only policies applied by `20260902211550_module9_b_notification_preferences` (`select/insert/update_self` on `email = (select auth.email())`) already cover every column, and the PK on `email` already covers the screen's only query. **The UI half:** a third `Switch` in the **"העדפות והתראות"** tab of `src/components/ProfileSettingsPage.jsx`, cloned from the live `email_new_projects` toggle **including its rollback-on-save-failure** (`handleEmailToggle` ⇒ `setEmailNewProjects(previous)` + `toast.error`) · copy in **one** new file `src/lib/onboardingCopy.js` (key→sentence) · rendered by **one** new component `<Hint id="…" />` that returns `null` when the switch is off. | **Ishay ruling 28, 07/09/2026** — *"חשבתי בהגדרות על כפתור הטמעה שמדליק ומכבה מלא משפטים בעברית שצריך אותם רק בשלב ההטמעה"*, and after seeing a mockup: *"איך היית עושה את זה נכון — מסכים איתך"*. **Per-user and not global** — his settings screen is per-user and the existing toggle is the precedent, so a global flag would let one manager turn the help off for everyone. **The refinement he added, which sets the default posture:** *"בתחושה שלי היא כבר עכשיו די מובנת להטמעה, ורק צריך מתג שיכבה חלק מהם או ינסח מחדש למשהו מקוצר אם בכלל"* ⇒ **the first copy is derived from what is already on screen — turn off / shorten, not add.** | ✅ ✏️ **APPLIED — corrected 15/09/2026.** This status cell still read *"⬜ Not applied"* while **the head of this very row already recorded the apply on 08/09** — the row was updated at the top and never at the end. **Verified this turn, not remembered:** `supabase/migrations/20260908221959_onboarding_mode.sql` exists, and `docs/schema.sql:1820` carries `onboarding_mode integer not null default 0` with its named check at `:1824`. 🔑 **Why it is worth a line: the status column is what a sweep reads**, so a stale cell under a corrected header is invisible in exactly the direction that matters — the build would have opened a migration for a column that is already live. *(Original text:)* ~~⬜ **Not applied.** **Ships inside m11's own migration**~~ — that migration is opening anyway for `feedback_ai_insights` / `recommended_rank`, so no separate migration is opened for one column. 🔴 **RIPPLE INTO MERGED M9 CODE** (`ProfileSettingsPage.jsx` + `src/modules/09_settings/api.js`, whose `select` and `upsert` both name their columns explicitly) ⇒ **full m9 regression required** (`ProfileSettingsPage.test.jsx` + E2E), same discipline as M5-3/M5-4. **Timing (ruling 28 ⑤):** built during m11 ⇒ m11 · m10 · m12 are authored with hints from the start; the nine merged modules get their `<Hint>`s retrofitted **in the buffer before the conference**. **Measured 07/09/2026 01:5X, not assumed:** `information_schema.columns` returns exactly `email, email_new_projects, sms_last_minute, created_at, updated_at` — the column does not exist; and the copy inventory it will draw on is **17 `## ⑥ …הגדרות…` sections across the four `screens-approved.md` files (m4:1 · m5:2 · m6:8 · m8:6) holding 96 table rows and 18 `→` pointers** — *not* the 58 that a naive `grep -c "⑥"` reports (that count includes in-paragraph mentions and seven `⑥` headings that mean something else entirely — six `⑥ הרשאות` and one `⑥ מצבים` in m4). |

✏️ **07/09/2026 17:3X — CLOSED. Ishay ruled the default off (*"1. בסדר"*, 28⑫a); on the level column that is `0` (28⑭). No question remains for the blueprint or for Tuesday — do NOT stop on the paragraph below; it is kept as history.** ~~🔴 **M11-1 open sub-item — DEFAULT VALUE, and it is a product decision, not a naming one.**~~ The table's own documented rule is **"no row ⇒ every toggle off"** (`docs/schema.sql` §32: *"🔴 **אין שורה = שני המתגים כבויים.** … קוד שלא מוצא שורה חייב להציג 'כבוי', לא 'לא ידוע'"*) — written for **notifications**, where silence must never mean "we mailed her". **Onboarding hints run the other way: a brand-new user who has never opened the settings screen has no row, and `default false` would hand her the system with every explanation already switched off** — the exact opposite of what ruling 28 exists for. **Recommendation: `default true`, and code that finds no row shows the hints.** ⚠️ **But this collides head-on with a rule already on disk, so it is Ishay's to rule, not the building session's** — and §10.4 of this file forbids deciding here. **Whoever opens the m11 blueprint must put this question to him before writing the `alter table`.**

**‏M11-2 … M11-5 — the Discovery's DB write-back. ✏️ Written 11/09/2026; source is `processes-approved.md` cards ת2 · ת5 · ת6, which is the only file in that folder allowed to set a product rule.**
✅ ✏️ **16/09/2026 — ALL FOUR ARE APPLIED.** They landed as migrations **A · B · C on 16/09 05:05** under Ishay's one-time typed-echo waiver (his words, recorded in `micro_guides/module-11.md` §9 D-9: *"על כולם — מיגרציות (~8) · חתימת-DoD · ריצת-הזריעה של הסיווג"*). Each status cell below says what actually landed, **measured against `pg_catalog` on 16/09 19:4X and not read off the migration text**; the file-by-file ledger is in **§10ב**. 🔑 **The gate itself did not change** — the waiver was one-time, in his own words, in the same chat, and is not a precedent. *(Original banner, kept so the change is legible:)* ~~🚫 **Nothing here is applied.** All four are blueprint steps for m11's own migration; none may be applied without Ishay's gate (`supabase/migrations/CLAUDE.md`).~~

| # | Change | Why (ruling / card) | State today |
|---|---|---|---|
| **M11-2** | **NEW table `feedback_ai_runs`** — one row per classification run: `started_at` · `finished_at` · `status` (`running`/`done`/`partial`/`failed`) · counts `sent`/`ok`/`failed` · `model` · `run_by` · **`approved_at` · `approved_by`**. RLS: read gated on `'דו"חות'`; the approve transition is an **RPC gated `edit` on `'דו"חות'`**, never a client write | Card **ת2** (approved 06/09, *"מאשר לפי המלצה"*). The approve pair is what makes report 20 show **only approved runs** — ⚖️5-א builds the on-screen run stamp from exactly these two columns | ✅ **APPLIED 16/09/2026 05:05 — `20260916043300_module11_a_feedback_ai_tables.sql` (migration A, through MCP; typed echo waived by Ishay, `micro_guides/module-11.md` §9 D-9).** Live in `pg_catalog` 16/09 19:4X: 13 columns (`run_id bigserial` · `started_at`/`finished_at`/`created_at`/`updated_at timestamptz` · `status text` + `feedback_ai_runs_status_check (running|done|partial|failed)` · `sent_count`/`ok_count`/`failed_count integer not null default 0` + `feedback_ai_runs_counts_nonneg` · `model` · `run_by`/`approved_by text` FK → `users(email)` `on delete restrict` · `approved_at`), RLS **on**, one SELECT policy `feedback_ai_runs_select_by_permission` gated `edit|view` on `'דו"חות'`, four indexes (pkey · `run_by_idx` · `approved_by_idx` · the **H1** partial unique `feedback_ai_runs_one_running_idx` — at most one `running` run), trigger `feedback_ai_runs_set_updated_at` (`extensions.moddatetime`), table + column comments. **Declared deviations, in the migration header:** no write policies — the edge function writes with the service role (`send-email`/`email_log` precedent) · the approve transition is the RPC `approve_feedback_ai_run(bigint)`, which refuses `running`, refuses `failed`, refuses a second approval, and does approve `partial`. Snapshot: `docs/schema.sql` §34 |
| **M11-3** | **NEW table `feedback_ai_insights`** — `project_id` **unique** · `run_id` → `feedback_ai_runs` · `sentiment` (1–5) · ✏️ **`negative_topics text[]`** · ✏️ **`positive_topics text[]`** · `quote` · `red_flag` · `classified_at`. 🔴 ✏️ **Re-specified 11/09 — "one `topics[]` over the ten categories" is not writable.** Measured in `20260904233000`: they are **two lists of five**, and **`'אחר'` appears in both** ⇒ **9 unique strings carrying no sign**, so one array cannot say whether `'אחר'` was a complaint or a compliment — which is exactly what report 20 exists to show. **Two columns, each with its own existing CHECK list, mirror `projects`' own shape and make the agreement matrix a direct per-side comparison.** *(negative: איחור דיילות · תפקוד דיילות · איכות תגים · ניהול לקוי · אחר — positive: מקצועיות הדיילות · עמידה בזמנים · איכות תגים וציוד · ניהול ותקשורת · אחר)* | Card **ת2**. **The taxonomy is deliberately NOT model-updatable** — it is the language customers pick from, and without it there is no human↔model agreement matrix, which is report 20's whole point | ✅ **APPLIED 16/09/2026 05:05 — same file, `20260916043300_module11_a_feedback_ai_tables.sql`.** Live 16/09 19:4X: 11 columns — `insight_id bigserial` · `run_id` FK → `feedback_ai_runs` `on delete cascade` · `project_id integer` **unique** (`feedback_ai_insights_project_id_key`) FK → `projects` `on delete cascade` · `sentiment integer` + `check (1..5)` · `negative_topics text[] not null default '{}'` + `positive_topics text[] not null default '{}'`, each with its own `<@ ARRAY[…]` CHECK **byte-identical to the two `projects` lists** · 🆕 **`free_topic text`** · `quote text` · `red_flag boolean not null default false` · `unclassifiable boolean not null default false` · `classified_at timestamptz not null default now()`. RLS **on**, one SELECT policy gated `edit|view` on `'דו"חות'`, three indexes (pkey · the unique on `project_id` · `run_id_idx`). 🔴 **Two columns this cell never asked for, both deliberate, both recorded as `הנחתי` in the migration header:** **`free_topic`** — card ת2 / ruling 11 require the one-or-two-word free tag behind `'אחר'`, and this register omitted it, so a register-only reader would have shipped report 20 unable to say what `'אחר'` contains · **`unclassifiable`** — the row is kept even when the model failed, because a missing row reads as "the note was never sent". `sentiment` is `integer`, not `smallint` (zero `smallint` columns in this schema). Snapshot: `docs/schema.sql` §35 |
| **M11-4** | **NEW column `assignments.recommended_rank integer null`** — written once, at invite time, by merged m4 code: `insertInviteRow` **only** (`src/modules/04_hostesses/api.js`). 🔴 ✏️ **Corrected 16/09/2026: this cell also named `writeInviteToken`, and that contradicts M11-4's own "not overwritten on resend" — `writeInviteToken` IS the resend path.** `module-11.md` step 1.5 caught it on 11/09 and the register was never corrected, so the two authoritative documents disagreed until now. *(Original wording: "…and `writeInviteToken`, the two sites that already write `invite_sent_at`".)*; **not overwritten on resend**. A failed write must **not** fail the invite — `NULL` + `console.warn` | Card **ת5**. 🔴 **This is a ripple into merged m4 code ⇒ the blueprint step carries a full m4 regression (its E2E), not a unit test alone.** No back-fill: the 5,674 seeded assignments stay `NULL`, and report 14א prints *"נמדד מ-<date>; N שיבוצים"* rather than an invented number | ✅ **APPLIED 16/09/2026 05:05 — `20260916043400_module11_b_assignments_recommended_rank.sql` (migration B).** Live 16/09 19:4X: `assignments.recommended_rank integer null` + `assignments_recommended_rank_check (recommended_rank is null or recommended_rank >= 1)` (tagged `הנחתי` in the file) + a column comment naming `insertInviteRow` as the only writer and "no back-fill" as the reason. The PK triple is untouched and **no row was back-filled — every existing row is `NULL`.** 🔴 ✏️ **And the row count in the middle column of this row is wrong: `assignments` holds 5,741 rows live, not 5,674** — re-measured 16/09 at step 1.3; the docs were stale, the DB was right, and all of them are `NULL` either way (`micro_guides/module-11.md` §9 D-13). The client half landed in **merged m4 code** in the same step: `createShiftInvites({ ranks })` → `insertInviteRow` only, the map built by the new `buildRecommendedRanks(ranked)`; `writeInviteToken` is byte-identical to HEAD and m6's `sendDateChangeReinvites` was not touched; m4's E2E ran as the regression and is green. Snapshot: `docs/schema.sql` §18 |
| **M11-5** | **FOUR new `params` rows**, all `owner_role_id` = מנכ"ל, numeric, **each with a ceiling/floor like every other numeric param** (that pattern is already merged, PR #104): `מכפיל_מרווח_מתרחק`=**1.5** · `סף_סטיית_תקציב_אחוז`=**15** · `מקדם_אמינות_אדום`=**0.87** · `מקדם_אמינות_ענבר`=**0.95** · ➕ a row each in `src/lib/paramsRegistry.js` (label · explanation · unit · group · "affects") | Card **ת6** + 🔴 **ruling 35 (10/09)** for the last two. Ruling **14** is what caps the list at four: a param exists **only** when a threshold appears on screen and someone might want to tune it without a developer | ✅ **APPLIED 16/09/2026 05:05 — `20260916043500_module11_c_report_params.sql` (migration C).** The four rows exist with `param_type = 'control_alerts'` and `owner_role_id` = מנכ"ל, inserted through `on conflict (param_name) do nothing` on the live unique constraint, with a loud guard if the CEO role row is missing: `params` **43 ⇒ 47 rows**. 🔑 **`param_type` is what would have aborted this row as written** — it is `not null` with a six-value CHECK and no source said which value these four take; it was read off the existing rows (display/alert thresholds carry `control_alerts`, while `smart_match` is the weights group), not chosen by taste. 🔴 **And "with ceiling/floor" means a REGISTRY row, not a DB constraint** — `params` has no such columns: four entries in `src/lib/paramsRegistry.js` (`decimal` 1–5 · `percent` 1–100 · `weight` 0–1 ×2), whose own comment says `min`/`max`/`decimals` are enforced nowhere. The missing-row rule is delivered by the new pure helper `src/lib/reportsParams.js` (`REPORT_PARAM_NAMES` · `missingReportParamsMessage`, shaped on `src/lib/quotes.js`'s `missingParamsMessage`) plus `meta.missing_params` in every RPC ⇒ the screen says *"חסר פרמטר מערכת: X"* and paints no tile that depends on it. 🚫 **What was deliberately NOT done, because it is Ishay's and not a build step:** card ת6's side-fix (give `סף_לקוח_רדום_ימים` an owner) changes who may edit an existing row — and live there are **five** owner-less params, not the one the card calls *"the only one"* (§9 D-13 ⇒ `PROJECT_MASTER §6`) |

✅ ✏️ **16/09/2026 — the §7 RLS-matrix row IS corrected, once, with all three writes.** It is in **§7 of this file**, with the old text kept struck through under a `🔴 גובר:` annotation rather than deleted — it was quoted out of here, out of `micro_guides/module-11.md` §5 and §9 D-3, and into the blueprint for a week, so a reader who inherited it needs to see that it was checked and when. ⚠️ **And one pointer to fix while we are here, because it sent the Phase-4 agent to the wrong file:** the correction was briefed against `docs/PROJECT_MASTER_sec7.md`. **The row is not there** — `grep -n 'Views/RPC' docs/PROJECT_MASTER_sec7.md` returns nothing. The matrix that carries the m11 row is **§7 of `db_roadmap.md`** (*"RLS rollout matrix (module × tables it WRITES; base input for §7.63)"*), which is what §9 D-3 means by *"§7's RLS matrix"*. *(Original wording:)* ~~⚠️ **And §7's RLS matrix row for m11 is still stale** — it reads *"none (5 reports as Views/RPC, read-only)"* … **Left for the blueprint step that creates them** — correcting it now would describe tables that do not exist.~~

🆕 🔴 **15/09/2026 — and one thing this whole block does NOT cover, recorded here because this is the file people open when they ask "what state is the database in": the SEED is not in version control at all.** M11-2…M11-5 are *schema* items and they are tracked. **The 20,996 rows of seeded business data that module 11's sixteen surfaces actually read are not** — measured with `git show --name-only`, every `seed(m11):` commit touches markdown and **zero SQL**. ⇒ **`supabase db reset` + every migration in this repo produces a correct, empty schema and not one number the conference demo shows.** **Mitigation applied the same day:** `scripts/seed-snapshot/capture-seed-snapshot.sql` copied 23 tables into a `seed_snapshot` schema (23/23 row-count `MATCH`), **which is an in-database copy and survives a bad write, not a lost project.** The off-database dump needs the database password and is **Ishay's to run** — command in `scripts/seed-snapshot/README.md §4. **The real fix — a re-seed script — is a debt, deferred past the conference by Ishay's ruling 15/09/2026** (`PROJECT_MASTER §6`, `🚧 מ11`).
...
| ✅ ~~A-24~~ | notification-preferences table (+UI toggles wired) | `PROJECT_MASTER.md` §6 (Ishay 07/07) | **✅ TABLE APPLIED 02/09/2026 21:2X — `20260902211550_module9_b_notification_preferences` (§10ב); the UI toggles land at m9 step 3.5; M10 wires sending.** *(The blueprint record, kept:)* **📘 m9 blueprint approved 02/09/2026 — migration B (`docs/micro_guides/module-9.md` step 1.2): `notification_preferences(email PK→users on delete cascade, email_new_projects, sms_last_minute, timestamps)` + 3 self-only policies; Ishay R-4: the email toggle goes live, the SMS toggle stays disabled with "אין ערוץ SMS במערכת" (no SMS channel exists)** |
...
|---|---|---|---|
| §7.64 | natural PKs (email/ח"פ/ת"ז/SKU) vs surrogate — **direction RULED 10/07**: external/PII→surrogate; system-owned (SKU)→natural+ON UPDATE CASCADE; users.email→accept+CASCADE | 🔴🔴 | per-module: customers=**M2 now** · sku=M3 · ת"ז=M4 · email=~~M9~~ **deferred at the m9 blueprint (V-2, 02/09/2026) — see the `users` row in §6** |
...
| §7.68 | salary report as a document: `period` UNIQUE + frozen line snapshots (absorbs §7.46 edge) | ⚪ **RULED 26/08/2026** (Claude-by-delegation, m8 Discovery): full direction adopted — `period` UNIQUE + `salary_report_lines` snapshot table + total. Migration details = m8 blueprint | M8 |
...
| §7.19 | ~~bonus split + per-hostess actual-hours derivation~~ ✅ **CLOSED 26/08/2026** (Claude-by-delegation, m8 Discovery): pay derives directly from the rows — `assignments.actual_hours × hourly_rate_snapshot + personal_bonus`; no event-level "spread" (M6 already writes per-hostess hours live) | M8 |
...
| §7.35 | user off-boarding + `projects.owner_email` reassignment | ~~M9~~ **M12** — verdict `דחה-ל-מ12` at the m9 blueprint (02/09/2026; 0 projects owned by inactive users, measured live; overridable) |
...
| 🆕 **`reports` bucket (private) + `finance` bucket (private) + 8 `storage.objects` policies + per-bucket limits** — same `permissions × modules × current_user_role_id()` pattern already live on `marketing`.<br>🔴 **`file_size_limit` RULED 14/08/2026 (Ishay delegated — *"מה שנראלך"*): `reports` = `2097152` (2 MiB). 🚫 NOT the 10 MiB copied from `marketing`, and the on-screen helper text reads `עד 2MB`, not `עד 10MB`.**<br>**The arithmetic, because "3MB" is not precise enough to build from:** `src/lib/email.js:29` caps an attachment at `MAX_ATTACHMENT_BASE64_CHARS = 4_000_000`. Base64 encodes **3 bytes → 4 chars**, so the hard binary wall is **exactly 3,000,000 bytes ≈ 2.86 MiB**. ⚠️ **A bucket set to 3 MiB (3,145,728) would therefore still admit a file that fails at send** — the limit must sit strictly below the wall, leaving room for the JSON envelope (subject · recipient · body) that travels with it. 2 MiB leaves ~900 KB of headroom. *(Reference point: the closing screen's own drawn example is a 1.4 MB PDF.)*<br>🔴 **Why the ceiling is not raised instead:** the comment at `email.js:26-28` records that Make rejects an oversized payload **on its side — after the user has clicked and been told "נשלח"**, and the ceiling sits deliberately below Make's 5 MB free-plan limit. Raising it trades a caught error for a silent lie, on an action that is **irreversible** (㉙ makes the operational RPCs refuse once the project is closed, and no module-6 control resends).<br>⚠️ **And this is a real reduction, said plainly:** a summary report with many photos will not fit. **That is an infra question for M10 (the email module), not a module-6 workaround** — M6 must not open a second send path.<br>➕ **Twin the value client-side**, per the `marketing` precedent (`MARKETING_MAX_BYTES` / `MARKETING_ALLOWED_MIME`, `src/modules/02_customers/api.js:179-180`, and the 31/07 audit row above): a server-side limit alone is bypassable by a direct REST upload; a client-side limit alone is bypassable by anyone. **`finance` keeps its own limit — M8's to decide.** | ✅ **APPLIED 14/08/2026 — `module6_storage_reports_and_finance`** *(`reports` private · 2 MiB · `finance` private · 10 MiB · 12 `storage.objects` policies total; re-verified at the 1.10 gate 18/08: `E-01`/`E-03`/`D-ST`/`D-SS` all PASS)* · limit ruled 14/08/2026 | `specs/module_06_projects/processes-approved.md` · `src/lib/email.js:26-29` | 6 |
| 🆕 **`check` constraint `projects_closed_needs_report`** — `project_status not in ('awaiting_invoice','awaiting_payment','finished') or summary_report_url is not null`. **Closes §7.36: a project cannot reach billing without its report even if the UI breaks** | ✅ **APPLIED 14/08/2026 — `projects_closed_needs_report` in `module6_projects_columns_and_constraints`** *(verified `B-15` at the 1.10 gate)* | same | 6 |
| ~~🆕 **`params` row — the customer-survey link**~~ 🔴 **SUPERSEDED 14/08/2026 — the row ALREADY EXISTS and must not be seeded again.** It is live as **`קישור_בסיס_סקר_לקוחות`**, seeded with a real URL at `20260723112000_module3_seed_products_tiers_params.sql:63` (נמדד 14/08). ⚠️ **What caused the duplicate: `[לינק_לשאלון_שביעות_רצון]` is the PLACEHOLDER TOKEN inside `תבנית_מייל_משוב_לקוח`, not a param name** — the two were read as one thing. 🚫 **M6 seeds no survey-link row**, and the *"ask Ishay for the value"* instruction is void: the value is on disk. *(Ishay's 13/08 confirmation that the form exists and the link is fixed **stands** — it was already true and already implemented.)* | **RULED 13/08/2026 (Ishay)** · **superseded-as-already-done 14/08** | `…params.sql:63` | 6 |
| `pg_cron` install + jobs (quote expiry, event-passed, login purge) | ✅ **all three live** — quote-expiry + login-cleanup since M3; **event-passed landed 14/08/2026** (`module6-event-finished @ 0 2 * * *`, `module6_status_machine_and_cron`; three distinct hours verified `F-01…F-03` at the 1.10 gate) | §7.42 + §7.75 | 3/6 |
...
| users | NOT NULL role_id (A-14) · email key = **accept + ON UPDATE CASCADE — RULED §7.64 (10/07)**, exec ~~M9~~ **deferred at the m9 blueprint 02/09/2026 (V-2): a cascade without the auth-sync half turns the CEO's SQL-side email change from a loud FK failure into a silent lockout; the UI blocks email edits anyway. M12 or never, with the sync. m9 ADDS a fifth FK to `users(email)` (`notification_preferences`)** · email sync (A-20) · FK index (C-1) |
...
| params | UNIQUE (§7.40) · typed+history (§7.70) · seed (A-12, now 20 rows #1–20) · ghost param (§7.57) · RLS select-all/write-CEO (A-19, §7.83) · write UI for the 2 pricing rows only = M3 "מחירים" tab (§7.84); the other 18 rows still wait for the full M9 params screen. **📘 m9 blueprint approved 02/09/2026 (`docs/micro_guides/module-9.md`): migration A = `owner_role_id integer null → roles(role_id)` + covering index · `param_type` CHECK gains `shift_invites` (6 values) · 6 new rows (`שעות_סף_זימון_לפני_אירוע` 24 · `שעות_תוקף_זימון` 48 · `שעות_אירוע_דחוף` 72 · `סף_שביעות_רצון` 3 · `סף_לוגיסטיקה_ימי_עסקים` 10 · `ימי_אזהרה_הצעה_פגה` 7) · owner map by natural owner (38 rows, Ishay Q-4) with a loud-fail count guard · **2 rows deleted (Ishay Q-2): `תבנית_איפוס_סיסמה`, `קישור_בסיס_סקר_לקוחות` — 0 readers on `origin/main`, checked** · `params_write_ceo_only` (FOR ALL) replaced by 3 command policies (UPDATE settings-edit OR owner · INSERT · DELETE settings-edit) ⇒ 4 policies with the SELECT. Live count at blueprint: 39 rows / 5 types / 2 policies. Migration C: `record_feedback`/`archive_project` read `סף_שביעות_רצון` from `params`; new DEFINER `list_hostesses_below_min_wage()` (§7.66 debt)** · ✅ **A + C APPLIED 02/09/2026 21:2X — live now: 43 rows / 6 types / 4 policies / 38 owned; proofs quoted in §10ב** |
...
| projects | §7.47-mirror ×2 (times — **הוקדם למ3, LOCAL-1 15/07**; ~~cancelled_at~~ ✅ done 14/08/2026, M6-2) · ~~NOT NULL owner/quote_id (A-14)~~ ✅ **done 14/08/2026 (M6)** · ~~finance-column ownership 🔴 (§7.63)~~ ✅ **closed 14/08/2026 — `set_project_finance_fields` gated 'כספים' (M6-9)** · identity snapshot (**§7.76 RULED 11/07; scope ruled 15/07: event_name+customer_id** — inside the §7.49 RPC, columns in M3 mig 1.1) · ~~close-lock (§7.77)~~ ✅ **closed 14/08/2026 — preconditions in the operational RPCs (M6-10)** · profit stored (**§7.52 RULED 11/07 — re-scoped 14/08: the freeze is M8's, M6 built only the inputs**; expected derived live; % display-derived) · ~~coords (§7.55)~~ **✅ DONE 09/08/2026 in `20260809122536` — `lat`/`lng` added** · multi-day (§7.30) · ➕ **`customer_name` snapshot added same migration (local-5, anchor §7.76)** — and `approve_quote_and_create_project` rewritten to populate it, since it is the table's only writer |
| hostesses | ✅ **DONE 09/08/2026 in `20260809122536` (M4 mig A): surrogate `hostess_id` PK · `id_number` → `unique not null` · `rating` → `null` + DEFAULT 3 dropped (§12⑱ב) · `address`/`lat`/`lng`/`has_car`/`languages` added.** Still pending: bank-column protection ⚪ **RULED 26/08/2026 (m8 Discovery, ה19, by-delegation): split to `hostess_bank_details` child table — write/read for 'דיילות' edit (m4's form keeps working) + read for 'כספים' (the salary report); blocked otherwise. Precedent: the `product_costs` split (live). Migration = m8 build** · min-wage rule (§7.66, M4 mig D) ✅ **בוצע בפועל — `20260809134237_module4_rls_and_public_rpc.sql`** *(היא-היא "mig D": `create trigger hostesses_enforce_min_wage … of hourly_rate`. ⇒ **הפריט היחיד שנשאר פתוח בשורה הזאת הוא הגנת-עמודות-הבנק (§7.63).** נמדד 12/08/2026 ב-`regin-docs-sync`)*. ~~email UNIQUE (§7.65)~~ **closed 31/07 — no UNIQUE, soft warning in the form.** *(History: ת"ז → surrogate — RULED §7.64 (10/07), APPROVED-TO-EXECUTE 31/07/2026 as M4's FIRST migration* (`hostess_id bigint identity` PK; `id_number` → `unique not null` column). Re-measured EMPTY (0 rows) immediately before applying, 09/08/2026 12:28.)* |
| assignments | ✅ **DONE 09/08/2026 in `20260809122536` (M4 mig A): PK → `(project_id, hostess_id, assignment_number)` · `id_number` column DROPPED entirely (PII) · FK → `hostesses(hostess_id)` with explicit ON DELETE/ON UPDATE restrict · six-value status CHECK (+`approval_withdrawn`) · `responded_at`/`invite_token`/`invite_sent_at`/`travel_amount`(§7.69)/`is_shift_lead`/`event_date` · `assignments_hostess_id_idx` (C-1 ✓) · `assignments_one_shift_lead_per_project` partial unique.** Still pending: ~~§7.47-mirror (attendance — 🚧 מ6)~~ ✅ **done 14/08/2026 (`module6_assignments_attendance`)** · shift lineage 🔴 (§7.67, ⏸️ deferred) · **partial-unique (A-15) — on `(hostess_id, event_date)`, M4 mig B, STILL OPEN** ✅ **בוצע בפועל — `20260809124327_module4_one_event_per_day_constraint.sql`** *(`create unique index assignments_one_event_per_day … where assignment_status = 'finally_approved'`. 🔴 **וזו סתירה פנימית בקובץ, לא רק שורה שהתיישנה:** שורת `A-15` ב-§2 כבר קוראת `◐ PARTLY DONE 09/08/2026` ומצטטת את אותה מיגרציה, בעוד השורה הזאת אומרת `STILL OPEN`. **הראיה מכריעה לטובת `A-15`:** החצי של אותו-יום/כפל-תשלום **בוצע**; מה שנשאר פתוח הוא רק החצי של §7.54 — שתי שורות פעילות על **אותו פרויקט** — כי §7.88↳ צמצם את נקודת-האכיפה ל-`finally_approved`. נמדד 12/08/2026 ב-`regin-docs-sync`)* · `salary_report_id` FK index (C-1, M8) |
| logistics | actual<planned (§7.22) · ~~lineage/cost (§7.72)~~ ✅ **done 14/08/2026 — origin pointers (`module6_logistics_policy_and_origin`)** · FK index (C-1) — ✅ the two new origin FKs shipped with covering indexes; `logistics_sku_idx` already existed |
| salary_reports | month-id UNIQUE (§7.40/§7.47-mirror) · document model (§7.68) ⚪ **RULED 26/08/2026 (m8 Discovery, by-delegation): `period` UNIQUE + `salary_report_lines` snapshot table + total; travel stamped per-shift from `סכום_נסיעות_למשמרת` at generation (ה14)** · storage (§7.61 — `finance` bucket ready) · RLS policies (deny-all today — m8 opens) |
| projects → **`project_finance` child table (Ishay ruled 26/08: "סבבה לפי המלצתך")** — `final_profit` · `cancellation_fee` · `written_off` live in the child, read-gated 'כספים' (the ה19/product_costs precedent; R1-1: the projects SELECT policy exposes finance columns to logistics/recruitment roles); `invoice_sent_at` stays on projects (process status, not money) | fee persistence = final amount + free-text breakdown note (ה28); suggested components re-derived for display |
| projects (מ8 — דף-משוב ציבורי S4, **Ishay adopted 26/08/2026**) | `feedback_token` column (precedent: `assignments.invite_token`) · public RPC pair get/submit (precedent: `get_shift_invite`/`/shift/:token`, §7.45) — **rate-limiting per the public-RPC precedent (the §7.45 lesson)** · submit writes `feedback_score`+`feedback_notes`+`feedback_status='completed'`, one submission per token · m6's survey-mail link fill moves from the Google-Forms param to the token URL (m8 build ripple into merged m6 code) · seed update: `סכום_נסיעות_למשמרת` 0→**22.60** (ה20 — the legal daily cap, sourced; the §7.69 verify-with-CPA-before-m10 rider stands) · **§7.20 RULED (Ishay 26/08):** `invoice_sent_at` column (stamped by the finance RPC on the V) + param `תנאי_תשלום_ימים`=30 seed + derived days-overdue display; cancelled-after-invoice shows a "credit note needed" flag (display only) · **ה15 amended:** salary aggregation covers BOTH op-closed rows (actual hours) AND cancelled-project rows (§7.16 compensation: 0/50/100% × planned hours × frozen rate) · **P1/P3 additions (Ishay-approved 26/08):** `cancellation_fee numeric` (final billed amount + explicit waive) · `written_off boolean` (close-without-payment, bad-debt path — gate branch) · `final_profit numeric` frozen-at-archive (the §7.52 stored column) · archive RPC extension of `set_project_finance_fields` family: dual gate (payment OR written_off + feedback resolved), one transaction, kills the feedback token |
| notification_preferences | **Applied 02/09/2026 (m9 migration B, `20260902211550`)** — PK `email`→`users(email)` `on update no action / on delete cascade`, RLS on, 3 self-only policies, no DELETE, no extra index. ✅ **08/09/2026 — `onboarding_mode integer` 0/1/2 default 0 + named check + fourth `for all` CEO policy APPLIED (`20260908221959_onboarding_mode`, `M11-1` above) — 4 policies now.** ~~**Pending: one new column from m11 — ✏️ 07/09 17:1X: a level `smallint` 0/1/2 default 0, not boolean, plus a fourth CEO policy (ruling 28 ⑭/⑫)**~~ *(✏️ 08/09: "smallint" here was already wrong on 07/09 17:3X — the ruling says `integer`; zero `smallint` in the schema, measured live)* — `M11-1` above (ruling 28, 07/09/2026): the "מצב הטמעה" switch, per-user, ~~shipped inside m11's own migration; no new policy or index needed~~ *(✏️ 17:3X — independent migration on `ishay/ui-copy-rewrite` per the 28⑤ deviation; a fourth `for all` CEO policy IS required, 28⑫b)*.** ~~🔴 **Its `default` is an OPEN product question** — the table's documented rule is *"no row ⇒ every toggle off"* (`schema.sql` §32), which is right for notifications and backwards for onboarding hints; **Ishay must rule before the `alter table` is written** (see the sub-item under `M11-1`).~~ ✏️ **Closed 07/09: default `0` (off), consistent with the table's rule (28⑫a/⑭).** ⚠️ **Ripple: touching this table touches MERGED m9 code** (`ProfileSettingsPage.jsx`, `src/modules/09_settings/api.js` — both name their columns explicitly) ⇒ full m9 regression. |
| seed_registry | **NEW infra table (not business data), `20260903180958_seed_registry_and_helpers` — module 7's demo-seed generator (`scripts/demo-seed.mjs`).** Registry of rows the generator created (customer/hostess/quote/project), PK `(entity_type, entity_id)`, indexed on `batch_id` for `--reset`. RLS on, **deliberately zero policies** — same deny-all-by-DEFINER pattern as `login_rpc_calls`; only door in is `seed_register`/`seed_backdate_quote`/`seed_backdate_project`/`seed_reset`, all gated `edit` on 'הגדרות מערכת' (CEO only, no role name hardcoded). ⚠️ **One existing-code change riding on this migration:** `enforce_quote_in_progress_lock` (the §7.50 lock trigger) gained a bypass clause — passes only when BOTH the seed-only session key `regin.seed_bypass` is set (settable only inside these 4 DEFINER functions, `is_local=true` so it dies with the transaction) AND the row is present in `seed_registry`; a real quote is never in the registry, so its lock is unweakened. **Live-verified 03/09/2026 post-apply:** table+4 functions exist, `seed_registry` **0 rows** (no seed run yet), `quotes_lock_non_in_progress` trigger still `tgenabled='O'`, no `anon=` in any of the 4 functions' `proacl` (the H5b mine, checked). |
...
| 9 הגדרות | params (`owner_role_id` · 4 policies · +6 rows · −2 rows) · `notification_preferences` (new, self-only) · **no `users` writes** (§7.64 cascade deferred, V-2) · body edits on m8's `record_feedback`/`archive_project` | CEO (edit on 'הגדרות מערכת') **+ the owning role per `params.owner_role_id`** (§7.70↳/§7.21↳, ruled 02/09/2026) | the two m8 DEFINER functions gain a `params` read — signatures unchanged |
| 10 אוטומציות | dispatch-log (new), reminder flags | system (definer jobs) | jobs bypass RLS — SECURITY DEFINER discipline |
| 11 דו"חות | 🔴 **גובר: 16/09/2026 · `micro_guides/module-11.md` §9 D-3.** m11 **does** write, in three places, all applied 16/09: **(a)** `notification_preferences.onboarding_mode` (M11-1, migration `20260908221959`; the row is per-user, the CEO door is policy `notification_preferences_ceo_all`) · **(b)** the two new tables it owns — `feedback_ai_runs` + `feedback_ai_insights` (migration A): RLS on, **one SELECT policy each, gated `edit|view` on `'דו"חות'`, and NO write policy at all** — the edge function `classify-feedback` writes with the service role and the only client-visible write is the RPC `approve_feedback_ai_run(bigint)`, gated `edit` on `'דו"חות'` · **(c)** `assignments.recommended_rank` (M11-4, migration B) — written by **merged m4 code** under m4's own `assignments_write_by_permission` gate (`'דיילות'`), not by anything m11 owns. Everything else m11 shows is read-only through **17 `security definer` functions** (16 report RPCs + the approve RPC), each opening with `assert_module_permission` on its OWNING module — `'כספים'` for the הנהלה and כספים tabs, `'דיילות'`, `'לקוחות'` — never on `'דו"חות'`, which gates the route and the two AI tables only. *(Original cell, kept because it was quoted for a week:)* ~~none (5 reports as Views/RPC, read-only)~~ | per matrix view rights **+ `edit` on `'דו"חות'` for the approve RPC** | ⚠️ **yes, one:** `assignments` is written by m4 and by m8 (`salary_report_id`) and now carries an m11-owned column — additive, nullable, single writer, no policy change |
...
- ⏳ **`20260925000200_module11_m15_band_column` — ממתין להחלה על-ידי הסגן (נכתב 25/09/2026, ענף `ishay/module-12-walkthrough`, סשן 2).** The deputy's ruling on walkthrough finding #4: *"מוסיפים עמודה שמראה את הסימון, מאותו מקור שמ14 סופר ממנו ("אדומות")"*. `report_m15_reliability` rows carried `band` (red/amber), but none of its 8 columns showed it. The hints say "התחילי מהדיילות האדומות". **One source, checked:** m15 `banded` and m14 `tagged` use the same frozen 12-month window, the same score, company average and params (`מקדם_אמינות_אדום`/`ענבר`). Live, as the CEO: m15 5 red · 7 amber of 90; m14 "דיילות אדומות" 5; the same five names. Two exactly-once replaces, same signature: each row gets `band_label` ("אדומה" · "ענבר" · "מעל שני הספים", null when marking is off), and `columns[]` gets "סימון" right after "דיילת". Sort unchanged (score asc already puts red first). No client change: `format: 'text'`, export takes `columns[]` as is. **Pre-write live md5:** `acec24d22f9202dcbd0f33951aab2efc` (29,738). **Expected after apply:** `fa878407a7beef4162de932e2f9c53ff` (30,357). That is the agent's dry-run (pg_temp, always-raise; payload identical except the new column; labels 5/7/78, 0 mismatches vs `band`), re-derived by session 2 with the file's own DO block, `execute` swapped for an md5 probe. **After apply, check:** ① md5 · `proacl` (no `anon`) · one CEO call: 5 "אדומה" rows first · flip this row to ✅.
- ✅ **`20260924234000_seed_logistics_arrived_horizon_rest` — APPLIED 24/09/2026 by the deputy (registry `20260924195346 seed_logistics_arrived_horizon_rest`).** File = applied text (md5 without the final newline = registry `0db9996a…`). **Post-apply proof (the deputy's check ①, re-run by session 2 the same evening, read-only):** 1594 420/420 · 1597 100/100 · both `actual_qty_autofilled = true` · `actual_arrival_date` null in both, as the deputy ruled (#2: *"להשאיר ריק"*) · statuses `event_finished`/`in_progress` unchanged · global autofilled 1,706 · the backup holds 2 rows, no `anon`/`authenticated`/`PUBLIC` grant. *(Was: ⏳ written 24/09, branch `ishay/module-12-reports-review`, session 2.)* The deputy's #1: *"מתקנים עכשיו, לא ב-14/10"*. **Data fix, synthetic data, the same method as `233500`.** The last two equipment rows from the seed-horizon run (10/09 02:23) sit in "מוכן" with "הגיע" = 0: **1594** SAT-LAN 420 (22/09, `event_finished`) · **1597** FAB-LAN 100 (28/09, `in_progress`). Under `measured_held_only`, m12 would count them as a 520-unit shortage from 30/09 and 06/10. The fix sets `actual_qty = planned_qty` and `actual_qty_autofilled = true`. Gate: 2 rows / 520 units, projects exactly {1594, 1597}. Backup `seed_snapshot.logistics_arrived_fix_horizon_rest_20260924` (no `anon`/`authenticated`). Check: 2 rows updated, no project status moved. `actual_arrival_date` stays null, as the deputy ruled (quantity + flag). 1577 is not touched. Restore: `scripts/seed-focused/2026-09-24-logistics-arrived-horizon-rest-restore.sql`. **Dry-run (24/09, session 2, one DO that always ends in `raise`):** gate 2/520/{1594,1597} · updated 2 · statuses `event_finished`/`in_progress` before = after. 1597 is on the active axis, so the trigger did recompute; it reads `item_status` only. Rows after = planned, autofilled. Global autofilled 1,704 ⇒ 1,706 · sum "הגיע" 419,354 ⇒ 419,874 (+520). The backup was held in a variable, not a table, so no DDL ran through `execute_sql`. **Nothing left behind (read-only, before = after):** both rows 0/false with `updated_at` 2026-09-09 23:23:59 UTC · statuses and all status counts identical · 1,704 / 419,354 · max `updated_at` unchanged · 0 `horizon_rest` snapshot tables · m12 md5 `dd3ebac8…`. m12 today: no change (neither event is closed yet). **After apply, check:** both rows = planned and autofilled · the two statuses unchanged · the backup table has no `anon`/`authenticated` grant · flip this row to ✅.
- ✅ **`20260924230000_module11_m12_measured_held_only` — APPLIED 24/09/2026 by the deputy, third of three in this order (registry `20260924192420 module11_m12_measured_held_only`).** The file in the repo = the applied text: md5 of the file without its final newline = md5 of the registry's `statements` (`6d3750a6…`), checked by session 2 the same evening. **Post-apply proof (live, read-only):** md5 `dd3ebac8808d9ac99d1fdb626689a794` = the dry-run · 18,904 chars · `proacl` `postgres/service_role/authenticated`, no `anon`. The deputy's call: "נמדד על" 2 · 01WEB 0.0% · gap tile 0.3% · last-year compare null. In `report_m12_equipment`, "measured" = **closed** events: `final_event_date < v_today - 7` and not cancelled. That follows Ishay's ruling (~21:5X, deputy's chat), quoted in the file: *"…נראלי גג שבוע סוגרים אירוע?…"*. The same population now covers the gap tile and its last-year compare (the deputy's #2: one population per metric). Twelve exactly-once replaces, same signature. Cost tiles, the cost-per-product chart and "כמה להזמין לחודש הקרוב" are untouched. The guided hint `reports.equipment.whyAndFirst` says so (`9af3381a`). Supersedes the past-only filter of `20260924210000` (below). No `docs/schema.sql` change.
- ✅ **`20260924233500_seed_logistics_arrived_2026_09` — APPLIED 24/09/2026 by the deputy, second (registry `20260924192315 seed_logistics_arrived_2026_09`).** File = applied text (md5 without the final newline = registry `e5aedfd2…`). **Data fix, synthetic data:** 15 equipment rows of 8 events (07–16/09) sat in "מוכן" with "הגיע" = 0. That is a state `update_logistics_item` itself prevents. They were left by the seed-horizon run of 10/09 02:23. They now hold the result that function defines for "ready": `actual_qty = planned_qty`, `actual_qty_autofilled = true`. The update is direct, because the function refuses finished events. 1577's ECO-TAG/SAT-LAN (`not_started`, a real shortage of 280) are untouched. **Verified after (session 2, read-only):** all 15 rows in the backup are now autofilled and equal to planned. Backup `seed_snapshot.logistics_arrived_fix_20260924`, no `anon`/`authenticated` grant. Restore: `scripts/seed-focused/2026-09-24-logistics-arrived-restore.sql`; the file's header calls it `restore-logistics.sql` (its source folder). ✅ Two more rows from the same run fell outside its range: 1594 SAT-LAN 420 (22/09) · 1597 FAB-LAN 100 (28/09). They are fixed by `20260924234000_seed_logistics_arrived_horizon_rest` (above), applied the same evening.
- ✅ **`20260924233000_seed_close_events_2026_09` — APPLIED 24/09/2026 by the deputy, first (registry `20260924192225 seed_close_events_2026_09`).** File = applied text (md5 without the final newline = registry `ca94222e…`). **Synthetic data, the seed's own method:** the 20 events of 31/08–16/09 that stood `event_finished` with no hours are closed through `close_project_operationally` + `seed_backdate_project`, as the CEO identity inside the transaction only. That is the same identity that closed 716 of 717 closed events. The formulas come from `scripts/seed-lib/plan.mjs`. Attendance comes from each hostess's own history, a declared approximation. The md5 draws are deterministic. Result: 105 assignments got attendance and hours · 55 preferences overwritten + 29 new · `event_finished` 26 ⇒ 6 · `awaiting_invoice` 5 ⇒ 25. **Verified after (session 2, read-only):** all 20 are `awaiting_invoice` · the global counts are 6 / 25. Backups `seed_snapshot.projects_close_20260924` · `assignments_close_20260924` · `customer_hostess_preference_close_20260924`, no `anon`/`authenticated`/`PUBLIC` grant. Restore: `scripts/seed-focused/2026-09-24-close-events-restore.sql`; the file's header calls it `restore.sql`. The restore was **not** dry-run. **What moved (the adviser's dry-run, `Reg-In-evidence\reports-review-2026-09-25\hours-fix\dry-run.md`):** m2 margin 59.1% ⇒ 57.8% · m4 50.7% ⇒ 50.0% · m7 payroll this month 0 ⇒ 21,539 ₪ · m8 median variance −2.3% ⇒ −1.6% · m14/m15 on-time 87.2% ⇒ 86.9%. Demo script updated. **Same step for 14/10:** step ④ of `scripts/conference-refresh/refresh-before-conference.sql` (17/09–07/10), re-measure `v_expected_*` before it runs.
- ✅ **`20260924221000_module11_m14_overview_no_gini` — APPLIED 24/09/2026 by the deputy (registry `20260924190408 module11_m14_overview_no_gini`).** **Post-apply proof (live, read-only; the deputy's check ①, re-run by session 2 the same evening):** md5 `ff571396a45a0c906f1022c2a5ca916c` = the dry-run · 31,272 chars · `proacl` `postgres/service_role/authenticated`, no `anon` · 0 Gini/Lorenz words in the body · the deputy's call as a 'דיילות'-edit user: `top_quarter` 56 vs 54.6 last year, 0 Gini/Lorenz strings in the payload, the door to `report_m17_fairness` kept. *(Was: ⏳ written 24/09, branch `ishay/module-12-reports-review`, session 2.)* Ishay's ruling (~22:0X, deputy's chat): *"אני מסכים עם השופט האמת והמלצת היועץ... באמת לא היה מובן החלקים האלא"* ⇒ no Gini/Lorenz in anything the user sees. In `report_m14_hostess_overview` the Gini tile becomes `top_quarter` ("רבע הדיילות העמוסות", percent, compare vs last year on the same metric). L7 exactly-once replaces, same signature. **Pre-write live md5:** `2b7b4062a5ba5034867c337565ec92b6` (31,624 chars). **Expected md5 after apply:** `ff571396a45a0c906f1022c2a5ca916c` (31,272 chars) — from the migration agent's read-only dry run, which ran the file's own DO block with `execute` swapped for a probe and ran the new body in a `read only` transaction (it compiles and runs on live data). The tile keeps its door to `report_m17_fairness`. The six remaining `מקדם` hits are the unrelated reliability params `מקדם_אמינות_אדום`/`ענבר`. Today: 56.0% vs 54.6% last year. **After apply, check:** ① md5 · `proacl` (no `anon`) · no user-visible Gini/Lorenz string in the payload · flip this row to ✅.
- ✅ **`20260924220000_module11_m17_fairness_plain_words` — APPLIED 24/09/2026 by the deputy (registry `20260924190300 module11_m17_fairness_plain_words`).** **Post-apply proof (live, read-only; the deputy's check ①, re-run by session 2 the same evening):** md5 `16314e547de0f0a7495eae620e7a3c51` = the dry-run · 23,273 chars · `proacl` `postgres/service_role/authenticated`, no `anon` · 0 Gini/Lorenz words in the body · the deputy's call as a 'דיילות'-edit user: `top_quarter` 56 vs 54.6 last year, the chart is `bar` with 4 quarters, 0 Gini/Lorenz strings in the payload. ⚠️ Not yet seen on screen: the "חלוקה שווה = 25%" line label. *(Was: ⏳ written 24/09, branch `ishay/module-12-reports-review`, session 2.)* Same ruling. In `report_m17_fairness`: the `gini` tile is removed · `top_quarter` gets a last-year compare on the same metric · the Lorenz chart becomes a 4-bar quartile chart ("איך המשמרות מתחלקות בין הדיילות") with a horizontal "חלוקה שווה = 25%" line · Gini/Lorenz definitions replaced with plain ones · `meta.previous.gini` ⇒ `top_quarter_pct`. **Pre-write live md5:** `de862cf19f10437b664b5c2989edff06` (24,756 chars). **Expected md5 after apply:** `16314e547de0f0a7495eae620e7a3c51` (23,273 chars), same dry-run method. Quarter rule `ntile(4)` over shifts desc, which equals the old tile rule `ceil(n/4)` for the busiest quarter. Today: 106 hostesses · 1,895 shifts · quarters 56.0 · 28.4 · 12.5 · 3.1; last year 54.6 · 28.2 · 13.5 · 3.7. The refLine label wraps "25%" in LRI…PDI. **After apply, check:** ① md5 · `proacl` · the busiest-quarter bar = the `top_quarter` tile · the four bars sum to 100 · flip this row to ✅.
- ✅ **`20260924213000_module11_m02_revenue_tile_no_door` — APPLIED 24/09/2026 21:0X by the deputy (registry `20260924180401 module11_m02_revenue_tile_no_door`).** **Post-apply proof (live, read-only):** md5 `e5d48b5a06bde41a48ae465b055f9dd9` = the dry-run · 17,139 chars · `proacl` `postgres/service_role/authenticated`, no `anon` · one call as a 'כספים'-edit user: `revenue.target` = null, `margin`→m04 · `finished_events`→m08 · `top5_share`→m21 intact · no `docs/schema.sql` change (same signature). *(Was: ⏳ written 24/09 in `ishay/module-12-integration`, session 2.)* ⚠️ Left on purpose: an internal comment in the body above `so_what` still names "מגמות רב-שנתיות" — not rendered; fixing it needs a fresh dry-run (deputy: not tonight). The deputy's reversible ruling: the "הכנסות מתחילת השנה" tile in `report_m02_exec_overview` loses its door (`'target', null`). It opened `report_m03_trends`, which `4281d924` (23/09 22:26) turned from "מגמות רב-שנתיות" into "סגירת הצעות" (same rpc, same slug), so a revenue tile opened a quotes report. Rejected: pointing it at another report (none answers the revenue question) · restoring "מגמות רב-שנתיות" (Ishay's product call, asked separately). One exactly-once replace on the live body, same signature. **Pre-write (24/09, read-only, live):** md5 `cdf27b682bb7e58812e81a7e8dc9d3aa`, 17,215 chars; `report_m03_trends` occurs once, as this target; the other three tiles (m04 · m08 · m21) are untouched. **Dry-run:** 17,139 chars, md5 `e5d48b5a06bde41a48ae465b055f9dd9`, 0 `report_m03_trends` left, 3 doors left. The client already renders a tile without `target` as text (`KpiTile.jsx:135`). Code comment (`ExecOverviewSurface.jsx`) and the unit fixture (`ExecutiveTab.test.jsx`) updated in the same commit. **After apply, check:** ① md5 = `e5d48b5a…` · `proacl` unchanged (no `anon`) · one call: `tiles[0].target` is null · no `docs/schema.sql` change · flip this row to ✅.
- ✅ **`20260924210000_module11_m12_measured_past_only` — APPLIED 24/09/2026 by the deputy (registry version `20260924174510`, name `module11_m12_measured_past_only`).** Verified after, re-checked by session 2 the same evening (read-only): live md5 `92315606d0ded3b9c18b231cbf449b21`, 17,558 chars = the dry-run · 3 `< v_today` filters · `proacl` `authenticated`/`service_role`/`postgres`, no `anon`. The deputy's call as a 'כספים'-edit identity: 01WEB 4.5 · `measured_rows` 74 · 9 table rows · gap tile 5.4 (unchanged, window-bound). Decisions (a) "עלות מוזמנת" past-only and (b) today ≠ past — adopted by the deputy. The guided hint `reports.equipment.whyAndFirst` and its mirror in `cards-finance.md` were corrected in the same branch. Was: ממתין להחלה על-ידי הסגן (נכתב 24/09/2026 בענף `ishay/module-12-integration`, סשן 2). Ishay's ruling via the deputy: in `report_m12_equipment`, "measured rows" = **only events that already happened** (`final_event_date < v_today`, Israel time). Six exactly-once replaces on the live body, same signature: the product table, the "הוזמן מול הגיע" chart and the "נמדד על N שורות" count get the past-only filter; `population.label`, `definitions` and the note under the tiles say so. **Not touched:** the gap tile (window-bound) · cost tiles · the cost-per-product chart · "כמה להזמין לחודש הקרוב" · estimated-price rows. ⚠️ The table's "עלות מוזמנת (₪)" column now sums past events only (same row as "הוזמן"); all-time cost stays in the "עלות ציוד מצטברת" tile. **Measured before (24/09, read-only, live):** 01WEB 12.5% ⇒ 4.5% · "נמדד על" 92 ⇒ 74 of 1,781 rows · three SKUs move up slightly (B-SAT-LAN 2.0⇒2.1 · ECO-TAG 7.6⇒7.9 · SAT-LAN 5.9⇒6.3). Pre-write: all six segments exactly once in the live body (md5 `572f7b502a50dab316d55e963d54b64b`); newest registry row `module11_drop_m16_quality_cost`; a read-only dry-run of all six replaces gave 3 past-only filters and an intact table tail. No `docs/schema.sql` change (signature unchanged).
- ✅ **`20260924190500_module11_drop_m16_quality_cost` — APPLIED 24/09/2026 by the deputy (registry name `module11_drop_m16_quality_cost`). Backup inside the same migration: `seed_snapshot.function_backups`, md5 `98c244f91a1c6a8020cf906ec2c30e7a` = the live body before the drop. Verified after: 0 functions left by that name, the backup table has no `anon`/`authenticated` grant.** Was: ממתין להחלה על-ידי הסגן (נכתב 24/09/2026 בענף `ishay/module-12-integration`).** Destructive (contract half of Expand-Contract): a guard that raises if any other function body calls `report_m16_quality_cost(` (plpgsql leaves no `pg_depend` row), then `drop function public.report_m16_quality_cost(date, date, integer, jsonb)` — no `if exists`, no `cascade`. **Precondition met:** the UI stopped calling it and that code is in production (`main` `b5ef4255`, Vercel `6642131150`); grep on `src` · `e2e` · `supabase/functions` · `scripts`, in the branch and in `origin/main`, finds no caller (only comments, `RETIRED_REPORT_TARGETS`, two test fixtures). Ishay: *"מאשר את שינהם"* (24/09, `docs/micro_guides/module-12.md` §5). **Closes** the `hourly_rate`-to-`view` exposure row in `PROJECT_MASTER` §6. **Before apply:** save `pg_get_functiondef(...)` + `proacl` outside the DB (the only exact restore — the files are j1 + three live text rounds). **After apply:** `to_regprocedure('public.report_m16_quality_cost(date, date, integer, jsonb)') is null` · m14 still returns its "דיילות פעילות" tile (the target string stays; the client shows value, no door) · re-sync `docs/schema.sql` (drop the line at the m16 signature) · update the "הפונקציה עצמה נשארת במסד" comment in `src/modules/11_reports/reportsCatalog.js` · flip this row to ✅.
- ✅ **`20260924141000_module5_m12_status_names`** — ✅ **APPLIED 24/09/2026 by the deputy; the gate tester verified it (new text present, old absent, one function per name, no `anon`).** Was: WRITTEN, NOT APPLIED (24/09/2026; the deputy applies).** Fresh-eyes copy fix via the deputy. Text only, two exactly-once replaces on the live body of `report_m12_equipment`, same signature. `definitions`: *"פריט שכבר יצא לספק או הגיע אינו נספר"* ⇒ *"פריט בסטטוס "הוזמן" או "מוכן" אינו נספר"* · `population.label`: *"בלוק ההזמנה למטה חותך לפריטים שטרם יצאו לספק"* ⇒ *"הטבלה "כמה להזמין לחודש הקרוב" למטה כוללת רק פריטים בסטטוס "טרם החל""*. The item statuses are **טרם החל · הוזמן · מוכן** (`LOGISTICS_STATUS_LABELS`, `src/lib/projectLogistics.js`). Both strings are shown to the user (`ReportSurface.jsx` `ScopeLine`). ⚠️ The same paragraph defines *"הוזמן" = הכמות המתוכננת* (a column), so the status is said as **"בסטטוס 'הוזמן'"**. **Pre-write measure (live `pg_get_functiondef`, 24/09):** each old segment occurs exactly once; newest registry row is `20260924130000`. **After apply, check:** the two new strings in the live body, `proacl` unchanged, one CEO-identity call of `report_m12_equipment`.
- ✅ **`20260924140000_module11_m07_singular_invoice`** — ✅ **APPLIED 24/09/2026 by the deputy; the gate tester verified it (new text present, old absent, one function per name, no `anon`).** Was: WRITTEN, NOT APPLIED (24/09/2026; the deputy applies).** Fresh-eyes copy fix via the deputy (a defect inside 0ג-2, `20260924080000`). Text only, one exactly-once replace on the live body of `report_m07_finance_overview`, same signature. The so-what said *"… מעל 60 יום ב-1 חשבונית."* for one invoice ⇒ *"… מעל 60 יום בחשבונית אחת."*; plural unchanged (*"ב-N חשבוניות."*). **Pre-write measure (live, 24/09):** the old segment occurs exactly once. **After apply, check:** the new segment in the live body, `proacl` unchanged, and the singular branch by running the expression with `v_over60_n = 1` (today's data may be plural).

- ✅ **`20260924130000_module11_singular_so_what` — APPLIED 24/09/2026 (1 registry row, same name).** Code-review fix via the deputy (a defect inside 0ג-2, which Ishay approved: *"מאשר הכל לפי המלצתך"*) + the m12 branch the row below made live. Text only, exactly-once replaces on the live bodies, same signatures. ① `report_m14_hostess_overview` so-what: singular/plural for both counts — v_red=1 ⇒ *"דיילת אדומה אחת, והיא פעילה ומוצעת היום בשיבוץ."* / *"…, והיא אינה פעילה היום."*; plural ⇒ *"אחת מהן פעילה…"* / *"אף אחת מהן אינה פעילה היום."* / *"N מהן פעילות…"* (was *"דיילת אדומה אחת, 1 מהן פעילות…"*) ② `report_m12_equipment` so-what: one event ⇒ *"…" לאירוע שבחודש הקרוב"* (was *"ל-1 האירוע"*). **Dry-run:** both segments exactly once. **Proof after (CEO claims):** m14 today unchanged (*"6 דיילות אדומות, 6 מהן…"* — v_red = 6); m12 *"להזמין 997 יחידות של "תג שם אקולוגי - ממותג" לאירוע שבחודש הקרוב — …"*; payload minus so_what md5-identical before/after for both (`58ee1c75` · `da599704`). **The singular branches, exercised from the LIVE body** (the so-what CASE extracted from `pg_get_functiondef`, values substituted, run through `query_to_xml` — no DDL): (1,1) · (1,0) · (6,6) · (6,1) · (6,0) · (0,0) ⇒ all six sentences as above. proacl unchanged (authenticated=X); one function per name. Live md5 after: m14 `2b7b4062` · m12 `cd459b15`. No `docs/schema.sql` change (signatures unchanged).

- ✅ **`20260924120000_module5_upcoming_orders_not_started` — APPLIED 24/09/2026 (1 registry row, same name).** "What to order" = physical items still `not_started` only. Ishay 24/09 (*"מאשר"*, relayed by the deputy with his wording; later *"רק סגרו לי תפינות… בלי קיצורי דרך"*). ① `m11_upcoming_equipment_orders`: + `l.item_status = 'not_started'` + `pr.category is distinct from 'site'` (= `isPhysical` in `src/lib/projectLogistics.js`; an SKU missing from the catalog counts as physical) — exactly-once replace on the live body, same signature ② `report_m12_equipment`: the so-what's three sub-selects (they computed "the largest" with the OLD definition, so the sentence would have said 2,540 above a table saying 997) ⇒ derived from row 0 of the order table itself (SSOT) · `definitions` + population label describe the new rule — four exactly-once replaces. **Dry-run:** all five segments counted exactly once on the live bodies; the new filter as a query ⇒ 2 rows. **Proof after, as CEO (claims injection with `sub`+`email`):** order rows 9 ⇒ **2** (B-ECO-TAG 997 · B-REG-TAG 570); so-what *"להזמין 997 יחידות של "תג שם אקולוגי - ממותג" ל-1 האירוע שבחודש הקרוב…"*; everything else in the payload **md5-identical before/after** (`7c5ff6b1`, payload minus so_what/definitions/population/measured_at/extra_tables[0]; rows `f2109ae9` · tiles `8db46b4f` · chart `0edb6cb6` unchanged); `logistics_upcoming_orders` rows = m12 order rows. **As מנהלת לוגיסטיקה:** the same 2 rows (25/09–24/10). One function per name; proacl unchanged (`m11_upcoming…` no authenticated/anon; `report_m12…` authenticated=X). Live md5 after: `m11_upcoming_equipment_orders` `9ccdfd20` · `report_m12_equipment` `9db992a5`. `docs/schema.sql` pointers updated. ⚠️ Found, not fixed (the m14-singular migration takes it): the so-what's singular branch now fires and reads *"ל-1 האירוע"*.

- ✅ **`20260924110000_data_fix_negative_reason_single` — APPLIED 24/09/2026 14:18 (1 registry row `data_fix_negative_reason_single`, version `20260924111836`).** Pure data fix. Ishay, verbatim: *"מאשר את שינהם"* (24/09), after the deputy showed him the fix (copy the reason from the array as `record_feedback` does, no report moves, with a backup). **Why:** 53 projects (52 `finished` + #1550 `awaiting_payment`) had `feedback_score` below `סף_שביעות_רצון` (3), `negative_feedback_reason` null and `negative_feedback_reasons` filled — residue of the focused seed of 23/09, which wrote the array only. The live `record_feedback` writes `negative_feedback_reason = v_clean_negs[1]` for every score below the threshold, and `archive_project` checks the single column only ⇒ #1550 would have been refused archiving even after payment (*"חסום: הלקוח נתן ציון 1 — נדרש בירור טלפוני…"*). **What:** `negative_feedback_reason = negative_feedback_reasons[1]` for exactly those rows; gate: exactly 53 by condition (threshold read live), and every array is one valid reason, so `[1]` equals what `record_feedback` would write. 🚫 The 66 projects with score ≥ 3 and a negative-reasons array were not touched (they feed M20/M22). Readers of the single column, measured: `archive_project` · `get_project_finance_detail` · `record_feedback` · `submit_feedback` — no `report_m*`, no view; the UI prefers the array (the single column is a fallback) ⇒ no text change on screen. Backup: `seed_snapshot.projects_negative_reason_single_20260924`. **Proof, measured:** dry run in `begin … rollback` · backup md5 `617f4501…` == the pre-apply md5 of the 53 rows · 0 rows left below the threshold with an empty single reason · md5 of the 66 untouched rows `21ce8569…` identical before/after · `get_project_finance_detail(1550)` as the CEO returns *"ניהול לקוי"* · all 39 CEO payloads (16 reports × 2 periods, dashboards, finance/projects overviews, `logistics_upcoming_orders`, #12 detail) identical before/after once `measured_at` is removed (normalized md5 39/39). Side effect: `projects.updated_at` of the 53 moved (moddatetime; no function or client code reads it). **Reversible:** `update … set negative_feedback_reason = b.negative_feedback_reason from` the backup (SQL in the file header). `docs/schema.sql` unchanged.

- ✅ **`20260924100000_data_fix_empty_future_quotes` — APPLIED 24/09/2026 14:09 (1 registry row `data_fix_empty_future_quotes`, version `20260924110937`).** Pure data fix, no DDL on `public`. Ishay, verbatim: *"מאשר"* (on this recommendation) and then *"רק סגרו לי תפינות בבקשה ... קחו תזמן ובלי קיצורי דרך בעבודה אבל תנסו לא להצתרך אותי"* (24/09) — relayed by the deputy with leave to apply without stopping. **Why:** `get_dashboard_summary` for December 2026 and January 2027 raised P0001 *"לא ניתן לחשב כספים לפרויקט 1639 — להצעה 2419 אין שורות."* (`finance_project_money`), so the home screen for those months was a full error screen (`DashboardPage.jsx:122-134`). The approved quotes of 1639 · 1640 · 1642 · 1645 had zero `quote_services` rows — seed skeleton of 09/09, a state `approve_quote_and_create_project` refuses to create. **What:** each of the 4 quotes got the one line its 6 seed siblings carry (1638 · 1641 · 1643 · 1644 · 1646 · 1647: `01WEB` ×1, price 2500.00, cost 1200.00, line 1, color/notes null — read from the siblings, gate: exactly one distinct shape over 6 lines); then `logistics` rows by the approve-quote rule, trigger running. INSERT on `quote_services` is not locked (the lock trigger is `BEFORE DELETE OR UPDATE`). `seed_registry`: sibling lines are not registered (the registry has no line-level type, only customer/hostess/project/quote) ⇒ the new lines are not registered either; the 4 quotes and projects are. Gate by condition (no lines · not cancelled · future event) == `{1639,1640,1642,1645}`. Backups: `seed_snapshot.projects_empty_quote_fix_20260924` (status + `updated_at`) · `seed_snapshot.quote_services_added_20260924` (line ids 7482–7485). **Proof, measured:** dry run in `begin … rollback`, and the header's reversal (delete logistics, then `set local regin.seed_bypass = 'on'` + delete the lines) executed and rolled back on one line · after apply: 0 future active projects with an empty quote · the 4 at 1 logistics row each, still `not_started` · only #11 (hostess-only quote, legal-empty) and #12 (past, untouched) remain without logistics · md5 of the 21 read RPCs unchanged. **Before/after over 39 CEO payloads — 6 changed, all explained:** dashboard December: error ⇒ renders, `monthly_profit` 7,300 over 6 projects (1638 1,300 · 1639 1,000 · 1640 1,225 · 1641 1,300 · 1642 1,300 · 1643 1,175) · January 2027: error ⇒ 4,950 over 4 · current-month dashboard and projects overview: `logistics_total` 0 ⇒ 1 on the 4, `planned_revenue` null ⇒ 2,200 / 2,425 / 2,500 / 2,500 (revenue = 2,500 less the quote's own discount: 1639 12%, 1640 3%) · M12 equipment cost 1,199,008 ⇒ 1,203,808 ₪ (+4,800 = 4 × 1,200), rows 1,777 ⇒ 1,781. **No report revenue moved** (none of the 16 includes not-started future events), so the deputy's pre-apply expectation of "+3 × 2,500 in 2026 revenue" did not materialise — fewer moves, not more. Screen evidence: `data-fix/{before,after}/6-dashboard-2026-12.png`. **Reversible:** the SQL in the file header. `docs/schema.sql` unchanged.

- ✅ **`20260924090000_data_fix_archived_unpaid_and_logistics` — APPLIED 24/09/2026 13:04 (1 registry row `data_fix_archived_unpaid_and_logistics`, version `20260924100427`).** Pure data fix, no DDL on `public`. Ishay, verbatim: *"מאשר לפי המלצתכם..."* and then *"מבחינתי לעשות את זה כמו שצריך שיראה טוב ובלי קיצורי דרך"* (24/09); the deputy read the whole file from disk (md5 `3b5a40e9…`) and said "החל". **Why:** ① 24 projects carried `project_finance.archived_at` while `project_status = 'awaiting_payment'` and `payment_date` was null — a state `archive_project` cannot produce (it needs payment or write-off and writes `finished`). In `bak_projects_20260910` all 24 were `finished` and paid; on 10/09 (11:56–14:47 UTC) their debt was reopened and `archived_at`/`final_profit` were left behind ⇒ Finance ⇐ "ממתין לתשלום" ⇐ the dialog read "התיק נעול — הועבר לארכיון." with no way to record payment (`closingPhase`, `ClosingWindowDialog.jsx:314`). ② 6 future projects (1638 · 1641 · 1643 · 1644 · 1646 · 1647, seeded 09/09) had no `logistics` rows although their quotes carry a product line ⇒ the project card's logistics tab showed `logistics-state-broken`. **What:** ① `archived_at = null`, `final_profit = null` for the 24 — **not** marked paid (that would have erased 145,985.05 ₪ of open debt, measured as their summed revenue). ② `logistics` rows by the exact rule of the live `approve_quote_and_create_project` (non-hostess `quote_services` lines, `serial_number = row_number() over (order by line_number)` per project, `planned_qty = qty`, `quote_service_line_id = line_id`, `color`), with the `logistics_recompute_project_status` trigger running (no `session_replication_role`). #12's logistics untouched (past event). Groups selected by condition and checked against explicit lists; money gate: `final_profit = gross_profit` live in all 24 (so `coalesce(final_profit, gross_profit)` moves 0.00). Backups: `seed_snapshot.project_finance_archived_unpaid_20260924` (24 full rows) · `seed_snapshot.projects_logistics_fix_20260924` (status + `updated_at` of the 6). **Proof, measured:** dry run in `begin … rollback` (and the money gate proven to fire: +0.01 on #1040 ⇒ P0001, rolled back) · after apply: backup md5 over the original columns `3f107e8c…` == the pre-apply md5 of the 24 rows · 0 archived-but-not-finished · the 6 at 1 logistics row each, still `not_started` (`updated_at` moved to 24/09 10:04 UTC — moddatetime on the trigger's same-value write) · logistics 1771 ⇒ 1777 · md5 of the 21 read RPCs identical before/after, so every payload change below is data. **Before/after, as the CEO over 39 payloads (16 reports × UI period and default, dashboard, finance overview, projects overview, `logistics_upcoming_orders`, #12 detail) — 10 changed:** finance overview/#12 detail: only `final_profit` + `archived_at` of the 24 (revenue, gross profit unchanged) · M02 `meta.frozen_count` 206 ⇒ 183 (not rendered) · M08 note *"הרווח סופי ב-206 מתוך 248"* ⇒ *"ב-183"* · dashboard/projects overview: `logistics_total` 0 ⇒ 1 on the 6 · 🔶 **M12 "עלות ציוד מצטברת" 1,191,808 ⇒ 1,199,008 ₪ (+7,200 = 6 × 1,200 ₪, the real cost of `01WEB`)** — the rows quote approval would have created itself; expected and correct (deputy, 24/09), but it was not in the pre-apply expected table, so it was stopped on and explained before continuing; M12's `01WEB` row: ordered 118 ⇒ 124, gap 6 ⇒ 12 (M12 counts future-event rows that were not auto-filled as "measured" — 8 such rows existed before; recorded as a separate finding, not fixed here). Unchanged: M09 aging (236,382 ₪ · 35), M07, dashboard monthly profit (184,141.48 ₪), `logistics_upcoming_orders`, and the other 11 reports. Screen evidence: `Reg-In-evidence/system-polish-2026-09-24/data-fix/{before,after}/` (dialog #12: locked banner ⇒ payment block; project 1638: broken ⇒ one row). **Reversible:** restore from the first backup + delete the 6 rows by their `quote_service_line_id` (the file header carries the SQL). `docs/schema.sql` unchanged (no `public` DDL; the backups live in `seed_snapshot`). `e2e/finance.spec.js` S2 returned to its original form (revert of `473af568`); after the revert, `npm run gate` exit 0 and the full e2e suite against a worktree dev server: 192 passed · 7 skipped · 2 failed that pass alone and as whole files on rerun (`accessibility.spec.js:409` login redirect timeout, `quote-document.spec.js:78` list-button timeout — neither reads the fixed rows); finance S1/S2/S3, `report-density.spec.js` (modes 0 and 2) and all of `reports.spec.js` green.

- ✅ **`20260924081000_module5_upcoming_orders` — APPLIED 24/09/2026 (1 registry row, same name).** System-polish 0ג item 4: "what to order next month" gets a home on the logistics screen. ① `m11_upcoming_equipment_orders(p_to, p_customer_id)` — internal SSOT (security invoker, **no grant** to authenticated/anon) ② `report_m12_equipment`'s order block now calls ① (exactly-once replace) ③ new RPC `logistics_upcoming_orders()` — gate `'לוגיסטיקה'` view/edit. **Proof:** md5 of the m12 order rows (sorted by sku) **identical before/after** (`d3c2b2a6`) and identical to ③ as CEO · as מנהלת לוגיסטיקה ③ returns 9 rows (25/09–24/10) · as מנהלת גיוס ושיבוץ ⇒ `42501` · ACLs as intended. m12 live md5 after: `f8f46960`. `docs/schema.sql` updated (two new signatures + m12 pointers). Same grant as the row below.

- ✅ **`20260924080000_module11_m3_walkthrough_fixes` — APPLIED 24/09/2026 (1 registry row, same name).** System-polish package 0ג items 1+2+5 (`docs/plans/2026-09-24-system-polish.md` §6), approved by Ishay 24/09 (*"מאשר הכל לפי המלצתך"*) and applied under his in-chat grant (*"אני מאשר לך מראש להכיל מיגרציות בלי לעצור"*). Exactly-once text replacements on the live bodies, same signatures: m09 "הגדול שבהם" = the customer with the largest sum over 60 days (was: the single largest invoice) · m07/m14 so_what points to the detailed report (m19 precedent) · m08 (5) / m12 (2) tile `window` = `DD/MM/YYYY–DD/MM/YYYY` instead of "התקופה שנבחרה". **Proof (CEO-identity call, claims injection):** m09 so_what reads *"…הגדול שבהם אלפא סיסטמס בע"מ, 27,186 ₪"* (was 10,163 — the deputy's oracle) · m07 *"לפתוח את "גיול חובות" — 54,415 ₪ ממתינים מעל 60 יום ב-9 חשבוניות."* · m14 *"לפתוח את "אמינות והתייצבות" — 6 דיילות אדומות, …"* · m08/m12 windows *"01/01/2026–24/09/2026"*. proacl unchanged on all five (`authenticated=X`, no anon). Live md5 after: m07 `f61f2ad6` · m08 `25422dda` · m09 `4b545a22` · m12 `3db46be8` · m14 `48167dfb`. No change to `docs/schema.sql` (signatures unchanged).

- ✅ **`20260924007000_module11_m2_feedback_notes_backup`** — the deputy's (backup tables in `seed_snapshot` before replacing 43 contradictory feedback notes); recorded here from its commit `ff8ae71e`, **not re-measured by this session**.

- ✅ **`20260924000000_module11_h1_quote_closing` → `20260924006500_module11_l7b_satisfaction_compare` — 9 files, APPLIED 23–24/09/2026 night** (registry rows under the same names), under Ishay's session grant. ה1/ה2/ה3 = `create or replace` on the SAME signatures of report_m03/m04/m06 (ACL + comments kept) · `m1_feedback_backup` = table `seed_snapshot.projects_feedback_20260924` (552 rows, before the focused seed) · h2b/h_review_fixes/h3_measured_factors/l7/l7b = text + sample fixes (exactly-once guarded replaces). **Proof:** after each, a CEO-identity call (claims injection) returned the expected payload; proacl has no anon on all 16 report_m*; an adversarial reviewer re-derived every decision-line number independently (all matched). Live md5 at `d5ca2adb`: m03 `e93b15ee` · m04 `018d8e33` · m06 `63d92788` · m19 `acfab952` · m20 `2dce46a2`. No change to `docs/schema.sql` (public signatures unchanged; the backup table lives in `seed_snapshot`).
- ✅ **`20260923233000_module11_l6_card_lines.sql` — APPLIED 23/09/2026 night (1 registry row `module11_l6_card_lines`).** Under Ishay's grant for this session (*"לא לבקש אישור להכיל מיגרציות בסשן הזה. תכיל בעצמך ותאמת לבד"*), Hebrew line in chat before applying. 6 text replacements in m07/m08/m17/m21 — the second density-check pass after L5: card lines that wrapped to two lines, a bare 0 with no base (m07), a footnote over the page budget (m08), and the m21 score comparison that L5's value change had re-formatted as days. **Proof:** live md5 of the 4 == `expect-md5.mjs l6`, no `anon` in `proacl`, and `e2e/report-density.spec.js` passes on all 13 non-replaced surfaces (mode 0) and all 16 (mode 2).
- ✅ **`20260923230000_module11_l5_card_standard_copy.sql` — APPLIED 23/09/2026 night (1 registry row `module11_l5_card_standard_copy`).** Same grant. 29 text replacements in 9 functions (m07 · m08 · m09 · m12 · m16 · m17 · m19 · m21 · m22) — the server half of the card standard (plan `docs/plans/2026-09-23-module-11-decision-reports.md` §4ד): claim titles ⇒ metric names, a customer name as a card value ⇒ days (the name moves to the context line), per-year breakdowns and dates out of cards (into the ⓘ window), shorter footnotes and chart notes. ⚠️ Amends one 🔒 dictionary row — *"הכנסת 12 החודשים של הלקוחות המסומנים"* ⇒ *"הכנסת הלקוחות המסומנים"* (recorded in `processes-approved.md`). **Proof:** base == live before apply (md5 13/13 == `bodies.mjs postL4`); after apply, live md5 of the 9 == `expect-md5.mjs l5`, the other 4 unchanged, ACL unchanged.

- ✅ **`20260923220000_module11_l4_frozen_note.sql` — APPLIED 23/09/2026 (1 registry row `module11_l4_frozen_note`).** Under Ishay's standing grant for this session (*"מעכשיו בסשן הזה תכיל לבד"*, 23/09), with a Hebrew explanation in chat before applying. One replacement in `report_m08_profitability`: the frozen-profit footnote now reads by case — all final (*"הרווח סופי בכל 286 הפרויקטים."*) · none final · some final. It was found right after L3: the 2025 call returned "286 מתוך 286 … בשאר", with no "rest"; the pre-L3 sentence had the same flaw. **Proof:** live md5 == `expect-md5.mjs l4` (`cabac4c1…`), and all three cases were called live (2025 · YTD · September).
- ✅ **`20260923210000_module11_l3_compare_notes.sql` — APPLIED 23/09/2026 (Ishay typed the name; 1 registry row `module11_l3_compare_notes`).** **Proof:** live md5 of m08/m14/m17 == `expect-md5.mjs l3`, ACL unchanged, and the compare labels and footnotes are read back as intended. *(Was: ⏳ WRITTEN, NOT APPLIED.)* Text round 3, built on the live bodies after L2 (`bodies.mjs` postL2, md5 16/16): 4 replacements in 3 RPCs — the gini compare label on m14 and m17 ("אשתקד (n=97)" / "אשתקד · n=97 מול n=106 היום", which the browser renders "97=n") ⇒ "אשתקד", and two m08 footnotes ("רצפת-המהותיות", "רווח סופי קפוא" ⇒ plain words). Found by a query over every visible payload field after L2. 🔒 Not included on purpose: m21's "עמודות ואריחי ה-₪ בדף זה כפופים להרשאת מודול כספים." — asked, not cut. ✏️ **Corrected 23/09 (fresh-eyes review):** §7.100 (Ishay 17/09) ruled only that the money masking moves into the DB; **keeping this sentence was the J3 writer's choice, not Ishay's ruling.** The L3 file's own header (line 5) still says "קושרת אותו ל-§7.100 (הכרעת-ישי)". It is applied and append-only, so it is not edited: **this row is the correction.** Post-apply check: `expect-md5.mjs l3`.

- ✅ **`20260923200000_module11_l2_tile_copy.sql` — APPLIED 23/09/2026 (Ishay typed the name; 1 registry row `module11_l2_tile_copy`).** **Post-apply proof:** live `md5(prosrc)` == `expect-md5.mjs l2` for all 16 `report_m*` (8 changed, 8 unchanged incl. m03/m04/m06) · `proacl` unchanged · all 13 called under a real authenticated identity with no error, every tile sub as intended. *(Was: ⏳ WRITTEN, NOT APPLIED.)* Text round 2 of phase B, built on the live bodies after L1 (`phase-b-migration/bodies.mjs`, md5 13/13 == live): 24 replacements in 8 report RPCs (m07 · m14 · m15 · m16 · m17 · m19 · m20 · m22), tile sub-lines only plus one m07 compare note. Why: the live check after L1 showed the tile subs (outside L1's scope) as most of the visible text left — definitions, repeats of the scope chip or of "so what", internal words, and the fragment "לא אותה נוסחה" L1 left behind. Rule: a sub that is the base of the number stays; a definition/repeat/design note goes. Same technique and guards as L1; same signatures. **Post-apply check prepared:** `node phase-b-migration/expect-md5.mjs l2` prints the md5 each function must have (8 changed, 5 unchanged). Ops: `phase-b-migration/ops-l2.mjs`; generator `gen-migration.mjs --ops ops-l2.mjs`.

- ✅ **`20260923180000_module11_l1_report_copy.sql` — APPLIED 23/09/2026 (Ishay typed the name; 3 registry rows `module11_l1_report_copy_p1of3`…`p3of3`).** The file was sent through MCP `apply_migration` in three consecutive verbatim parts (lines 17–261 · 263–635 · 637–948) because it is too large for one call; each DO block is self-guarded and atomic. **Post-apply proof:** live `md5(prosrc)` of all 13 == the md5 computed locally by applying the file's own `replace` pairs to the pre-apply bodies (13/13 match, so no transcription drift) · m03/m04/m06 md5 unchanged · 16 `report_m*` functions, no overload · `proacl` unchanged on all · all 13 called under a real authenticated identity (claim injection) with no error: `population.summary` present on all 13, 0 "אינו מושפע", 0 📐, m15 `period_filter_ignored=true`, m20 = 2 charts, m22 = 3 tiles, m09 datum carries `drill_key` beside `bucket_key`. *(Was: ⏳ WRITTEN, NOT APPLIED.)* Text-only rewrite of 13 report RPCs (M3/M4/M6 untouched — being replaced, Ishay 23/09): new `population.summary` (the scope chip) · shorter `so_what` · tile subs/compare-notes that repeat the screen → `null` · every "אינו מושפע ממסנן…" and 📐13 off the screen · M20 loses the "מה מכעיס" and "שיעור המרוצים לפי שנה" charts · M22 loses tile `other_vs_categories` · M09 chart datums gain `drill_key` · M15 gains `meta.period_filter_ignored` + window label "12 החודשים האחרונים". Technique = K1's: `pg_get_functiondef` + exact `replace`, each of the 86 strings guarded "exactly once" before replacing; same signatures ⇒ `CREATE OR REPLACE` keeps ACL and comments. **Pre-apply evidence:** live `md5(prosrc)` of all 13 == the latest file body (after K1's patches on m02/m07/m19). Generator: `docs/specs/module_11_reports/phase-b-migration/`. Plan: `docs/plans/2026-09-17-module-11-typography.md` §7 phase B.
...
> **(3) שער-ההקלדה של 18 הקבצים האלה נוותר פעם אחת, במילותיו של ישי** *("על כולם — מיגרציות (~8) · חתימת-DoD · ריצת-הזריעה של הסיווג", 16/09/2026)* — **ולא כתקדים.** הכלל ב-`supabase/migrations/CLAUDE.md` לא השתנה. ‏`micro_guides/module-11.md` §9 D-9.

- ✅ **`20260917105300_module11_j3_money_gate_grants_rank_months.sql` — APPLIED 17/09/2026 (9 רשומות, בסדר הרשם: `module11_j3_m02` · `_m03` · `_m07` · `_m14` · `_m19` · `_m21` · `_grants` · `_rank_stamp` · `_m19_detail_fix`).** ארבע הכרעות של ישי מ-06:5X בקובץ אחד. ‏**① §7.100 — מיסוך-הכסף עבר אל תוך המסד** *(`PROJECT_MASTER_sec7.md` פריט 100, אפשרות (ב), *"בצע"*)*: ‏`report_m19_customers_overview` ו-`report_m21_drifting` מחזירות `null` בשדות-הכסף ו-`meta.money_masked = true` לקוראת בלי `view`/`edit` על **'כספים'**; שער 'לקוחות' לא זז. **ולא נוצר עוזר חדש** — התת-שאילתה היא זו של `get_dashboard_summary` מילה-במילה (‏`assert_module_permission` **מעלה** 42501 ולכן אינה מתאימה כאן). **② T2 — `revoke all` מ-`anon`/`authenticated`** על `login_attempts` · `feedback_rpc_calls` · `project_changes` · `seed_registry` *(‏`relacl` לפני/אחרי ב-`docs/schema.sql` §מבוא)*. **③ T9 — `assignments.recommended_rank_set_at timestamptz`** + טריגר `assignments_recommended_rank_stamp`; ‏**אפס שינוי בקוד-הלקוח**. **④ תוויות-החודשים בצירים מתקצרות** (`ינו׳`…`דצמ׳`, גרש `U+05F3`) בחמש הפונקציות שיש בהן ציר-חודשים — מ2 · מ3 · מ7 · מ14 · מ19; **השנה נשארה בדיוק כפי ש-J2 הכריע**. 🔑 **ההוכחות, כולן נמדדו בסבב הזה:** ‏`md5(prosrc)` החי מול גוף ה-`$function$` שבקובץ — **6/6 זהים**, ועל כל 16 פונקציות-הדוח **16/16** · פרוב-מחובר כ-**E2E_PROJECTS** (`לקוחות edit` + `כספים blocked`): ‏`money_masked=true`, כל `rows[].revenue_12m` ‏`null`, אריח-הכסף של מ21 מרוקן (ערך · `sub` · `detail`), שורת-"אז מה" *"…יחד (לא זמין בתפקידך) בשנה האחרונה"* · כ-**E2E_CEO**: ‏`money_masked=false` והמספרים במקומם (‏`643,392.21` · `320,743.08`) · ‏`check_login_lock`/`reset_login_attempts` מחזירות 200 אחרי ה-revoke, וקריאה ישירה ל-`login_attempts` מחזירה **42501** במקום מערך ריק · הטריגר נבדק בטרנזקציה **שגולגלה לאחור** (הכנסה עם דרג ⇒ נחתמה · בלי דרג ⇒ NULL · עדכון-לאותו-ערך ⇒ החותמת לא זזה · דרג⇒NULL ⇒ החותמת מתאפסת). 🪤 **וממצא שהפרוב תפס ושקריאת-הקוד פספסה:** אחרי שעמודת-הכסף של מ19 כבר מוסכה, ‏`tiles[concentration_vs_drifting].detail` עדיין החזיר `top5_revenue: 1224613.9` — **הלשונית אינה ממסכת `detail` כלל** ⇒ תוקן ברשומה `module11_j3_m19_detail_fix`. ⚠️ **שתי תופעות-לוואי מדווחות:** ‏`v_months_he` משמש גם במשפטי-פרוזה (מ7 *"…שהתקיימו בספט׳"* · מ14 *"החודש ספט׳ 2025 אינו בגרף"*), ומשפט-המדיניות של מ21 איבד את חציו השני (*"המיסוך מתבצע בלשונית"*) כי הוא חדל להיות נכון — `cards-customers.md ⑤ מ21` מצטט את הנוסח הישן ודורש יישור.
- ✅ **`20260917021500_module11_j2_rpc_round5.sql` — APPLIED 17/09/2026 (13 רשומות: `module11_j2_m02` · `_m04` · `_m06` · `_m07` · `_m08` · `_m09` · `_m12` · `_m14` · `_m15` · `_m17` · `_m19` · `_m20` · `_m22`).** סבב-5, *"מהיסוד בלי קיצורי דרך"* — **החוזה נושא את האמת והמעטפת רק מרנדרת**. חמישה פריטים: ① **`format:'id'` על שש עמודות-מזהה** (מ4 `quote_id` · מ7 · מ8 · מ9 בשתי הרמות · מ12 ב-`meta.extra_tables`) — ‏`ReportTable.jsx` מחק את כלל-הסיומת `/_id$/` וההצהרה היא מעכשיו המקור היחיד · ② **חצי-ההשוואה בנוסח המוכרע ב-הכרעה 15(ב)/📐1** — `'אשתקד'` במקום ארבעה נוסחים, בתשעה משטחים (34 תוויות) · ③ **ציר-חודשים אחד** — שם-חודש עברי, ‏+ שנה רק כשהציר חוצה שנה קלנדרית (מ7 · מ14 · מ19); מפתח-המכונה `month` נשמר ו-`xKey` עבר ל-`label` · ④ **'חלון קפוא' ⇒ 'חלון קבוע'** (מ14 ×1 · מ15 ×8) — `ui-copy-styleguide.md` שורה 157 נועלת את *"קפוא"* למחיר שנעול בסגירה · ⑤ **מ22 מפסיקה להדהד תקופה שאינה שואלת בה** — `v_from` נמחק, `window.from = null`, ו-`meta.period_filter_ignored` נוסף (התקדים: מ9). ‏ובנוסף: פסקת-הגרף של מ9 עוברת ל-'מדרג' (שלוש ההופעות האחרונות של 'דלי' על המסך). **תוספתי:** אין טבלה, אין עמודה, אין policy, אין שינוי-חתימה, אין כתיבה — טקסט והצהרות-פורמט בלבד. 🔑 **ההוכחה:** ‏`md5(prosrc)` החי מול גוף ה-`$function$` שבקובץ — **13/13 זהים**; `proacl` של כל 16 = `postgres=X | service_role=X | authenticated=X`; פרוב חי כמנכ"ל — 13/13 ב-200, וזהות חסומה (גיוס) ⇒ `42501` על מ7 ומ8. **שלוש שלא זזו ולמה:** `m03` (בסיס-ההשוואה הוא שנה נקובה, ו-*"2025 המלאה"* אינו אותו טווח) · `m16` (השוואה לחציון, 📐15) · `m21` (*"לפני חודש"*).
- ✅ **`20260917005500_module11_j1_rpc_round4.sql` — APPLIED 17/09/2026 (8 רשומות: `module11_j1_m16` · `_m08` · `_m14` · `_m15` · `_m20` · `_m21` · `_m22` · `_m09`).** סבב-4 על **שמונה** פונקציות-דוח — `report_m08_profitability` · `report_m09_aging` · `report_m14_hostess_overview` · `report_m15_reliability` · `report_m16_quality_cost` · `report_m20_satisfaction` · `report_m21_drifting` · `report_m22_notes`. ‏**זהו הקובץ ה-19 של מודול 11, והראשון שנושא תאריך 17/09** — הכותרת שמעל מונה 18 קבצים ב-16/09, והיא נכונה כפי שהיא; הרשימה ארוכה ממנה בשורה אחת מהסיבה הזאת. **מה שנחת:** ① מ8 — שורת-"אז מה" מצביעה על **השורה הראשונה בטבלה כפי שהיא מוצגת** (מיון-₪, 📑ב#5) במקום על בעלת-האחוז הגבוה, ונושאת ₪ ואחוז יחד · ② מ8 — **רצפת-המהותיות מוציאה שורות מ-`rows`** ולא רק ממיינת אותן לסוף, כפי ש-`processes-approved.md §📑ב#5` ו-`cards-finance.md:261` אומרים (*"אינו בדירוג"*) · ③ מ16 — תעריף-חציוני בשקלים שלמים על `sub` של אריח (📐4 · D-34 ⑤) · ④ מ15 — `reliabilityScore` ו-"(הכרעה 38)" ירדו משורת-ההגדרות שעל המסך (כ4 · תקדים D-34 ④) · ⑤ מ14 — שלושת קבועי-האמינות בארבע ספרות כמו במ15, ו-'ענבר' ⇐ 'דיילת ענבר' · ⑥ מ20 — '(📐11)' ו-"N מתוך M" ירדו משורת-ההערות · ⑦ מ22 — `meta.run.approved_by` נושא **שם-אדם** דרך `left join public.users on email` (הדוא"ל נשמר ב-`approved_by_email` החדש), ומזהי-המודלים ירדו ממשפט-ההערות ונשארו ב-`meta.models` · ⑧ מ21 — חצי-ההשוואה של אריח "נתפסים רק בקצב האישי" נושא את מה שכלל-120-הימים **מפספס** (`only_personal`) ולא את מה שהוא מוצא · ⑨ מ9 — "דליי-האיחור" ⇐ "מדרגי-האיחור" בשורת-האוכלוסייה. **תוספתי:** אין טבלה, אין עמודה, אין policy, אין שינוי-חתימה, אין כתיבה. ‏`comment`/`revoke`/`grant` הועתקו מהקבצים החיים ולא הוקלדו. 🔑 **ההוכחה שהקובץ הוא מה שרץ:** ‏`md5(prosrc)` החי מול גוף ה-`$function$` שבקובץ — **8 מתוך 8 זהים**, ‏`proacl` בלי `PUBLIC` ובלי `anon` ב-8/8. ‏📏 **שער חי:** מנכ"ל מחוברת ⇒ **200 על 8 קריאות** עם המחרוזות החדשות במטען · זהות חסומה על 'כספים' (`מנהלת גיוס ושיבוץ`) ⇒ **42501 על מ8 ומ9**. המקור: `<scratchpad>/j1/probe-j1.json`. ⚠️ **שינוי-התנהגות שיש לדעת עליו:** טבלת מ8 ירדה מ-**30** שורות ל-**8** — 22 מן החורגים יושבים מתחת לרצפה. האריח "פרויקטים שחרגו מהתקציב" ממשיך לומר 30, כי הוא מדד-אוכלוסייה ולא מונה-שורות. נמדד בשאילתה עצמאית מול `pg_catalog`/`finance_project_money`: ‏`241` באוכלוסייה · `30` מעל הסף · `22` מהם מתחת לרצפה · `8` בדירוג · ‏`min(planned_hours)` בדירוג = `4.0`. ⇒ **מצביעי הגוף-החי של שמונה הפונקציות ב-`docs/schema.sql §24` עודכנו ל-J1 באותו סבב.**
- ✅ **`20260916194500_module11_i2_rpc_round3.sql` — APPLIED 16/09/2026 (הושלם 20:4X; 9 רשומות: `module11_i2_m17` · `_m21` · `_m20` · `_m22` · `_m14` · `_m15` · `_m19` · `_m03` · `_m09`).** סבב-3 על **תשע** פונקציות-דוח — יעדי-דלת, בידוד-מספרים, עמודת-בעלים (📐18), ספי-אמינות, ותעריפים בשקלים שלמים. **תוספתי:** אין טבלה, אין עמודה, אין policy, אין שינוי-חתימה, אין כתיבה. 🔑 **וההוכחה שהקובץ הוא מה שרץ:** ‏`md5(prosrc)` החי מול גוף ה-`$function$` שבקובץ — **9 מתוך 9 זהים**, ובנוסף חתימה · `comment` · `revoke` · `grant` זהים-בייט ל-I1 (9/9). ‏📏 **ועוד שער שרץ באותה החלה:** מנכ"ל מחוברת ⇒ 200 על 13 קריאות (כולל שלוש רמות-קידוח של מ3 ומ9) · זהות חסומה ⇒ **42501 ב-9 מתוך 9** · דיפף-מטען מלא לפני/אחרי עם **0 תזוזות מספריות**. המקור: `<scratchpad>/results/p2-fixB2.json`. ⚠️ **ולמה השורה הזאת נושאת שלוש שעות:** ב-19:4X הקובץ היה על הדיסק עם **0** רשומות, ב-20:1X וב-20:3X הוא היה על **8 מתוך 9** (בלי `_m09`), וב-20:4X הושלם. **קובץ מיגרציה שמוחל בחלקים עובר דרך מצבי-ביניים שנראים כמו מצב סופי** — ולכן הבדיקה, ולא הציטוט: `select name from supabase_migrations.schema_migrations where name like 'module11_i2%' order by version;` **מה שהוא נושא:** סבב-3 על **תשע** פונקציות — `report_m03_trends` · `report_m09_aging` · `report_m14_hostess_overview` · `report_m15_reliability` · `report_m17_fairness` · `report_m19_customers_overview` · `report_m20_satisfaction` · `report_m21_drifting` · `report_m22_notes` — יעדי-דלת, בידוד-מספרים, עמודת-בעלים (📐18) וספי-אמינות. **תוספתי בלבד:** אין טבלה, אין עמודה, אין policy, אין שינוי-חתימה, אין כתיבה. ✅ **תשע הרשומות נמדדו, כולל `module11_i2_m09`** — הפונקציה שעליה ישב חוסם סבב-3 (עמודות-הגרף כדלת-קידוח), והאחרונה שנחתה. ⇒ **מצביעי הגוף-החי של תשע הפונקציות ב-`docs/schema.sql §24` עודכנו ל-I2 באותו סבב.**
- ✅ **`20260916114500_module11_i1_rpc_formats_and_notes.sql` — APPLIED 16/09/2026 (19 רשומות רשם: `module11_i1_m22 · _m16 · _m20 · _m21 · _m19 · _m08 · _m12 · _m07 · _m17 · _m14 · _m15 · _m03 · _m09 · _m16_units · _m21_units · _m20_units · _m22_units · _m19_units · _m16_axis`).** תיקון-קדימה חוצה-לשוניות על **13** פונקציות-דוח: פורמטים (`score` · `date`), יחידות-מידה ויחידות-ציר, `chart.note`, `compare.format`, פריסת-גרף אופקית, יעדים ממוסכים. **תוספתי:** אין טבלה/עמודה/policy/שינוי-חתימה/כתיבה, ואף מספר נמדד לא זז — ההוכחה היא השוואת-מטען מלאה לפני ואחרי, פר-פונקציה, בכותרת הקובץ.
- ✅ **`20260916103000_module11_g3_customers_filter_keys.sql` — APPLIED 16/09/2026 (2 רשומות: `module11_g3_customers_m20 · _m22`).** מוסיפה `chart.filter_key` ושדות-הצטרפות ל-`report_m20_satisfaction` ו-`report_m22_notes` כדי שהסינון-הצולב של המעטפת יעבוד עליהן. 🔴 **ושני סינונים שהכרטיס דרש נדחו על סמך מדידה ולא תוקנו בשקט** — *"התפלגות הציונים"* (הגרף סופר 163 משובים, הטבלה מחזיקה 19) ו-*"מה משמח"* (אפס משובים נושאים גם סיבה חיובית וגם שלילית) קיבלו `filter_key:false`, **והשאלה הועברה לישי כשאלת-הכרעה** (`micro_guides/module-11.md` §9 D-31). הגופים נבנו מהגוף החי — שני md5 מתוך שניים זהים לקובץ G2 לפני הכתיבה.
- ✅ **`20260916085900_module11_h2_sentence_isolation.sql` — APPLIED 16/09/2026 (9 רשומות: `module11_h2_m08 · _m07 · _m09 · _m12 · _m07_cols · _m09_cols · _m17 · _m14 · _m15`).** **רק מחרוזות זזות** על שבע פונקציות: `n=` נכנס לתוך בידוד ה-LRI…PDI (בלעדיו הוא נקרא `N=n` ב-RTL — נמדד בכרומיום, לא הוסק), וכל מונה בן 4 ספרות ומעלה עובר `to_char(x,'FM999,999,999')`. אף ערך ב-`tiles[].value` לא השתנה. ⚠️ **החוב שנשאר מכאן:** מוני-הוצאה מתחת ל-1,000 (מ14/מ15) אינם מפורמטים ויתפצלו ברגע שיחצו 999 — `PROJECT_MASTER §6`.
- ✅ **`20260916083000_module11_d2_rpcs_executive_fixes.sql` — APPLIED 16/09/2026 (8 רשומות: `module11_d2_executive_m02 · _m03 · _m04 · _m06` + `_m02_counts · _m03_counts · _m04_counts · _m06_counts`).** תיקון-קדימה לארבע פונקציות לשונית "הנהלה" אחרי שהמאמת שלהן נהרג באמצע במגבלת-השימוש (§9 D-24) — הוא הודפס-מחדש כמאמת-מתקן עם רשות לתקן קדימה, תוספתית בלבד. אין טבלה/עמודה/policy/כתיבה.
- ✅ **`20260916082900_module11_g2_rpcs_customers_fixes.sql` — APPLIED 16/09/2026 (5 רשומות: `module11_g2_customers_m19 · _m20 · _m21 · _m22 · _m22b_run_in_progress`).** אותו דפוס בלשונית "לקוחות", כולל שער-הריצה-בתהליך של מ22. אין טבלה/עמודה/policy/כתיבה.
- ✅ **`20260916081500_module11_e3_rpcs_finance_rounding.sql` — APPLIED 16/09/2026 (1 רשומה: `module11_e3_finance_m07`).** שינוי אחד מוכרע בפונקציה אחת: אריח "שכר דיילות החודש" עובר מ-`sum(round(…))` ל-`round(sum(actual_hours × hourly_rate_snapshot))` ⇒ **19,253 ₪ במקום 19,257 ₪**, שהוא המספר שבמוקאפ המאושר וב-`signoff-baseline-2026-09-10.md`. 🔑 **הסדר הוכרע משלושה מקורות מסכימים, לא מטעם:** קו-הבסיס הוא אורקל-הקבלה · ה-SSOT של השכר במ8 (`src/lib/salaryReport.js:32`) מעגל לאגורות **פר-שורה** · הכרטיס אומר רק "₪ בלי אגורות". **המתקן דיווח על הפער ולא "תיקן" את קו-הבסיס** — ההתנהגות הנכונה (§9 D-21). הגוף נבנה מהגוף החי (md5 זהה ל-E2, 23,071 תווים בשני הצדדים).
- ✅ **`20260916065642_module11_f2_rpcs_hostesses_fixes.sql` — APPLIED 16/09/2026 (7 רשומות: `module11_f2_hostesses_m17 · _m16 · _m15 · _m14 · _m15_fix · _m14_winlabel · _m17_winlabel`).** תיקון-קדימה לארבע פונקציות לשונית "דיילות", כולל תוויות-חלון. אין טבלה/עמודה/policy/כתיבה.
- ✅ **`20260916063400_module11_e2_rpcs_finance_fixes.sql` — APPLIED 16/09/2026 (3 רשומות: `module11_e2_rpcs_finance_fixes · _e2b_finance_m07_threshold_comment · _e2c_finance_m12_population_separators`).** תיקון-קדימה ל-12 ממצאי סבב-האימות על E; ארבע הפונקציות נבנו מהגוף החי (4 md5 מתוך 4 זהים לקובץ E). 🔻 **שינוי-חתימה יחיד, ומוצדק בכתב:** `report_m09_aging` ו-`report_m07_finance_overview` מקבלות פרמטר חמישי `p_asof date default null` (שחזור-אורקל; הלקוח אינו שולח אותו). שינוי-חתימה יושב בצד ההרסני של Expand-Contract ומותר רק כשהקוד הפרוס אינו קורא לפונקציה — **נבדק באותו סשן:** `git grep report_m09_aging origin/main -- src e2e` ⇒ 0 קבצים, וכך גם `origin/dev`.
- ✅ **`20260916052600_module11_d_rpcs_executive.sql` — APPLIED 16/09/2026 (1 רשומה: `module11_d_rpcs_executive`).** ארבע פונקציות-הקריאה של לשונית **הנהלה** — `report_m02_exec_overview` · `report_m03_trends` (דוח-קידוח) · `report_m04_discounts` · `report_m06_staffing`. כולן `security definer` · `stable` · `set search_path to ''` · שער `assert_module_permission('כספים', edit|view)` · `revoke … from public, anon, authenticated` ואז `grant … to authenticated`. אין טבלה/עמודה/policy/כתיבה.
- ✅ **`20260916052511_module11_g_rpcs_customers.sql` — APPLIED 16/09/2026 (1 רשומה: `module11_g_rpcs_customers`).** ארבע פונקציות לשונית **לקוחות** — `report_m19_customers_overview` · `report_m20_satisfaction` · `report_m21_drifting` · `report_m22_notes`; שער `'לקוחות'`. אותו דפוס אבטחה והרשאות.
- ✅ **`20260916052500_module11_h1_one_running_ai_run.sql` — APPLIED 16/09/2026 05:2X (1 רשומה: `module11_h1_one_running_ai_run`).** אינדקס-ייחודיות חלקי `feedback_ai_runs_one_running_idx on ((true)) where status='running'` + הערת-אינדקס. 🔑 **מקורו הוא נקודה עיוורת שבונה פונקציית-הקצה הצהיר עליה בעצמו:** שער הלחיצה-הכפולה שלו הוא קריאה-ואז-כתיבה בלי נעילה, ושתי לחיצות באותן מאות-מילישניות היו פותחות שתי ריצות ושורפות מכסה. עכשיו ההכנסה השנייה נופלת ב-`23505` והפונקציה מחזירה את ה-409 הקיים שלה. תוספתי; הפיך ב-`drop index`.
- ✅ **`20260916052359_module11_f_rpcs_hostesses.sql` — APPLIED 16/09/2026 (3 רשומות: `module11_f_rpcs_hostesses_m14 · _m15 · _m16`).** שלוש מארבע פונקציות לשונית **דיילות**; שער `'דיילות'`. `report_m17_fairness` נחתה ברשומה של F2. אין טבלה/עמודה/policy/כתיבה.
- ✅ **`20260916051950_module11_e_rpcs_finance.sql` — APPLIED 16/09/2026 (1 רשומה: `module11_e_rpcs_finance`).** ארבע פונקציות לשונית **כספים** — `report_m07_finance_overview` · `report_m08_profitability` · `report_m09_aging` (דוח-קידוח) · `report_m12_equipment`; שער `'כספים'`. **אורקל-הקבלה של גיול-החובות התקיים ספרה-בספרה** — ‏35 חשבוניות · 10-13-8-1-3 · 236,382 ₪ (`spec.md §🔢`), וימי-האיחור נמדדים מול **מועד-הפירעון** ולא מול יום-השליחה.
- ✅ **`20260916045200_module11_h0_harden_backup_tables.sql` — APPLIED 16/09/2026 04:5X (1 רשומה: `module11_h0_harden_backup_tables`).** 🔴 **הממצא שהוליד אותה, והוא ממצא-אבטחה ולא ניקיון:** שש טבלאות `bak_*` שנוצרו בסשני תיקון-הזריעה של 09–10/09 נמדדו בצעד 1.1 מול `pg_catalog` **בלי RLS ועם SELECT/INSERT/UPDATE/DELETE/TRUNCATE ל-`anon` וגם ל-`authenticated`** — ברירת-המחדל של Supabase לטבלה שנוצרה ב-`public` בלי מיגרציה. הן עותקים מלאים של `projects` (כולל משוב), `assignments`, `hostesses` (ת"ז · טלפון · מייל · כתובת) ו-`project_finance` ⇒ **המידע שה-RLS של טבלאות-המקור מגן עליו היה נגיש דרך מפתח ה-anon הציבורי.** **מה שהמיגרציה עושה, הגנתי ותוספתי בלבד:** `revoke all` משני התפקידים + `enable row level security` בלי policies (deny-all מכוון, דפוס `login_attempts`) + הערות-טבלה. **אומת:** מנכ"ל מחובר ⇒ `42501` על select. **אף קוד אינו קורא מהן** (grep על `src/` · `scripts/` · `supabase/functions/` ⇒ 0). 🚫 **מחיקת הטבלאות עצמן לא נעשתה כאן והיא הכרעת-ישי** — הן העותק היחיד-בתוך-המסד של זריעת מ11 (§9 D-8/D-11), ו`PROJECT_MASTER §6` נושא את השורה. ⚠️ **וסריקת `rls_enabled_no_policy` באודיט-הסגירה תמנה את ששתן — התיוג הוא "deny-all מכוון", לא ממצא.**
- ✅ **`20260916043500_module11_c_report_params.sql` — APPLIED 16/09/2026 05:05 (1 רשומה: `module11_c_report_params`).** ארבע שורות `params` (`מכפיל_מרווח_מתרחק`=1.5 · `סף_סטיית_תקציב_אחוז`=15 · `מקדם_אמינות_אדום`=**0.87** · `מקדם_אמינות_ענבר`=**0.95**), כולן `control_alerts` ובעלות מנכ"ל, `on conflict (param_name) do nothing`, עם שגיאה רועשת אם שורת-תפקיד המנכ"ל חסרה. `params` **43 ⇒ 47**. **הערכים הם הכרעה 35 (10/09) ולא הישנים 0.8/0.9** — ב-0.8 התור היה ריק וב-0.9 לענבר היו אפס חברות. פירוט מלא: **M11-5** למעלה.
- ✅ **`20260916043400_module11_b_assignments_recommended_rank.sql` — APPLIED 16/09/2026 05:05 (1 רשומה: `module11_b_assignments_recommended_rank`).** ‏`assignments.recommended_rank integer null` + CHECK + הערת-עמודה, בלי מילוי-לאחור ובלי נגיעה ב-PK המשולש. 🔴 **אדווה לקוד מוזג של מ4** — `insertInviteRow` בלבד; `writeInviteToken` (מסלול השליחה-החוזרת, המיובא גם ע"י מ6) לא נגע. **ריצת-הרגרסיה המלאה של מ4 ירוקה.** פירוט: **M11-4** למעלה.
- ✅ **`20260916043300_module11_a_feedback_ai_tables.sql` — APPLIED 16/09/2026 05:05 (1 רשומה: `module11_a_feedback_ai_tables`).** **שתי טבלאות חדשות** — `feedback_ai_runs` + `feedback_ai_insights` — כל אחת עם RLS דלוק ו**מדיניות-קריאה באותה מיגרציה שיצרה אותה**, גדורה `edit|view` על `'דו"חות'`. 🔴 **ולמה זה כתוב כאן ולא נחשב מובן-מאליו:** טבלה עם RLS דלוק ואפס policies מחזירה **אפס שורות עם `error: null`** — המסך משקר ואינו נכשל, וזו מלכודת-הליבה של המודול הזה. **+ פונקציית-הכתיבה היחידה:** `approve_feedback_ai_run(bigint)`, גדורה `edit` על `'דו"חות'`; אומתה חי — מנכ"ל אישר, גיוס נדחתה ב-`42501`, אישור שני נדחה ב-`P0001 "ריצת-הניתוח כבר אושרה להצגה."` פירוט: **M11-2 / M11-3** למעלה.

- ✅ **`20260909183000_c01_replace_customer_contacts_feminine_imperative` — APPLIED 09/09/2026 18:4X via MCP `apply_migration` (DB version differs — UTC mine). Typed echo WAIVED A SECOND TIME by Ishay in his own words: *"פשוט תחיל אותה"* (18:4X), after the Hebrew explanation was given and the name offered for typing. Two waivers in two days ⇒ the gate's wording is a question for Ishay (does he want the typing at all?), recorded in the LOG.** One word in the `raise` of `replace_customer_contacts`: `סמן` ⇒ `סמני` (C-01 of the ui-copy night; the address is to the user, all users are women — guide §1; "איש הקשר"/"אחר" unchanged). Body pulled from the LIVE definition (`pg_get_functiondef`, 09/09 18:3X), not from `20260902141451`. No table/column/RLS change; revoke/grant repeated (anon-window mine). Fully reversible. `docs/schema.sql` unaffected (function bodies are not in the snapshot). Client-side byte-twins to align in the same commit after apply: `CustomerFormDialog.jsx` `NO_PRIMARY_MSG` · `api.test.js:194` · `CustomerFormDialog.test.jsx:88` · `02_customers/CLAUDE.md`. Client-side twins aligned in the same commit. Verified after apply: `pg_get_functiondef` contains `סמני`; registered in `schema_migrations`; grants = authenticated · service_role only.
- ✅ **`20260908221959_onboarding_mode` — APPLIED `08/09/2026 22:2X` via MCP `apply_migration`, typed echo WAIVED ONCE by Ishay explicitly and in advance** (his words, 22:2X: *"ואת המיגרציה ראיתי יש לך אישור ממני להריץ אין צורך שאקליד חד פעמי"* — the same shape as the module-9 A→C waiver; **not precedent**, the gate stands). Registered as `onboarding_mode`, DB version `20260908192746` (UTC, the documented 3-hour offset; name-matched). Branch `ishay/ui-copy-rewrite` (cut from `ishay/module-11-reports`; the 28⑤ declared deviation — independent of m11). **Why:** ruling 28 (`processes-approved.md` §🗳️) — the per-user "מצב הטמעה" switch; 28⑭ makes the column a **level** 0/1/2 (`integer`, not boolean, not `smallint` — zero `smallint` in the schema) so level 1 needs no second migration; 28⑫(b)/28⑭(b) require a **fourth `for all` CEO policy** because the three existing ones are self-only and the CEO door of 28⑨ could not read or write another user's row. **Pre-apply, measured:** column absent · 3 policies · 1 row / 8 users · 0 `smallint` columns. **Post-apply proof (all read-only or rolled back):** `onboarding_mode integer NOT NULL default 0` · `CHECK (onboarding_mode >= 0 AND onboarding_mode <= 2)` named `notification_preferences_onboarding_mode_check` · column comment + table comment present · **4 policies** (`ceo_all:ALL` + 3 self) · `ceo_all` USING = `(select current_user_role_id()) = (select role_id from roles where role_name = 'מנכ"ל')` · DO-block probe: inserting `3` and `-1` for a user without a row raised `check_violation` both times, 1 row before and after · existing row mode 0 · security advisors: no new finding on the table. **Reversibility:** `drop policy` + `drop column` (constraint and comment fall with it). **Ripple into merged code, tested:** `09_settings/api.js` (`getNotificationPreferences` now selects `onboarding_mode`; new `saveOnboardingMode`/`listOnboardingModes`) · `AuthContext` (`onboardingMode` + `updateOnboardingMode`) · `ProfileSettingsPage` third switch · `UsersManagementPage` CEO column — unit suites green (api 37 · profile+hint 48 · users 14).

- ✅ **`20260906083345_module8_planned_hours_cross_midnight` — APPLIED `06/09/2026 08:4X` via MCP `apply_migration` after Ishay's typed echo** (`20260906083345_module8_planned_hours_cross_midnight`, typed verbatim). Registered as `module8_planned_hours_cross_midnight`. **Why:** `finance_project_money` derived planned hours as `final_end_time − final_start_time` with no midnight wrap ⇒ a 20:00→01:00 event got **−19 h**, inflating `budget_deviation` and the finance screen's expected profit. **Measured before:** 701 finished projects, **126 negative `planned_hours` — exactly the 126 cross-midnight events, 0 others**; deviation p90 = 480% of labor. **Body:** live `pg_get_functiondef` (md5 `dc02f240…`, identical to H7) with one expression changed to `mod(… + 86400, 86400)` — the same modulo-day rule as `minutesBetweenTimes` (app) and `quotes.estimated_hours` (M3 LOCAL-2). Signature, ACL (`service_role` only) and the rest of the body untouched. **Post-apply proof:** negative `planned_hours` = **0**; project 1177 now 5.0 h / deviation −22 ₪ (was −19 h / +2,162 ₪); deviation p50 −0.1% · p90 14.3% · >15% = 61 (8.7%); anchor #12 `gross_profit` 207.40 unchanged (`gross_profit` does not depend on planned hours); ACL unchanged. Branch `ishay/hotfix-planned-hours-midnight`. Found during the module-11 reports-spec review (`docs/specs/module_11_reports/research/review-2026-09-06.md` §ח9).

- ✅ **`20260903180958_seed_registry_and_helpers` — APPLIED `03/09/2026 18:2X` after Ishay's typed echo** (`seed_registry_and_helpers`, typed verbatim). **Why:** the M7 seed generator (`seed-data-spec.md`) needs ~900 historical projects written through the app's RPCs and reversible — and three live-measured walls made that impossible without DB help: `projects` has **no write policy** for any user (client `UPDATE`/`DELETE` = 0 rows, silently) · `approve_quote_and_create_project` **refuses a past event date** and the quote is locked after approval · the §7.50 lock trigger also blocks `DELETE` ⇒ no `--reset`. **What:** new table `seed_registry` (RLS on, **no policies by design** — same pattern as `login_attempts`/`feedback_rpc_calls`; index on `batch_id`) · 4 DEFINER functions gated on `edit` over 'הגדרות מערכת' — `seed_register` · `seed_backdate_quote` · `seed_backdate_project` (also applies the `module6-event-finished` cron rule to that one project) · `seed_reset` (deletes a batch in FK order; any uncovered restrict-FK fails loudly) · **`enforce_quote_in_progress_lock` re-created from its LIVE body** (`pg_get_functiondef` 03/09 17:4X) with exactly one added `if` block: pass only when session key `regin.seed_bypass` is on **and** the quote is in `seed_registry`. Signature unchanged; the two triggers untouched. **Post-apply proof:** 4 functions in `pg_proc`, ACL `postgres/service_role/authenticated` (no `anon`) · registry 0 rows, RLS on, 0 policies · DO-block `update quotes … where quote_id = 6` still raises *"הצעה נעולה"* (not registered, no key) · advisors: one new `rls_enabled_no_policy` INFO on `seed_registry` (intentional) + the 4 new functions join the standing `authenticated_security_definer_function_executable` WARN class (36 → 40, same pattern as every RPC here). **Declared limits:** `updated_at` still `moddatetime`-stamped (so "פג בקרוב" cannot be seeded on run day) · historical quotes carry today's `product_costs`. **Legacy demo data (customers 46–49, projects 3/7/8/13/14, five named hostesses) was NOT registered by the seed** — Ishay's ruling 03/09: decide on deleting it *after* the full run, with a measured list. ✅ **Decided and done 03/09/2026 21:XX** *("מאשר לפי המלצתך ואם בא לך למחוק הכל לא אכפת לי")*: the July rows were registered as batch `legacy-july-demo` through `seed_register` and removed with `seed_reset` — **13 quotes · 5 projects · 4 customers · 5 demo hostesses**, registry 27 → 0. The five hand-entered customers (213 · 226 · 231 · 240 · 241) and their quotes/projects (incl. #12, the M8 smoke anchor) stay. **Leftovers by design:** 4 `email_log` rows still point at deleted quotes (no FK there; the log is append-only) · the eight approved M6 mockups still draw project #8's `1/6` staffing — documents, not code, untouched. Every E2E/smoke anchor that named those rows was rewritten to a runtime fixture the same evening (`seed-data-spec.md §י׳‑ב` items 23–24).
- ✅ **`pricing_wage_and_catalog_fix` — APPLIED `03/09/2026 ~12:5X`, pure data (no DDL).** Two `UPDATE`s, typed-echo satisfied by Ishay pasting the exact SQL back (stronger proof of comprehension than a name-echo, treated as equivalent): `params.param_value` for `שכר_מינימום_שעתי` **35 → 35.4** (actual Israeli statutory minimum, in force 01/04/2026; measured before applying — 0 active hostesses affected, lowest live rate 38) · `product_costs.cost` for `06ST` **500 → 420** (arithmetic fix — with `04ST` at 300 cost/500 price, a 6h cost ≥450 makes the catalog's implied fixed-cost component negative). Verified live post-apply: both rows read back as expected. `get_advisors` (security) run after — same pre-existing baseline set (4 `rls_enabled_no_policy` + `security_definer` RPCs + `auth_leaked_password_protection`, all previously triaged as intentional), **zero new findings**, consistent with a pure-data change. **Both values were STILL at their old figures when checked live this session** — the two fixes had been analyzed and written up in `pricing-findings.md`/`seed-spec-merged.md §י׳א` earlier the same night but never actually applied (no DB connector available to the session that analyzed them).

- 🔒 **אודיט-סגירת מ7 — 03/09/2026 21:0X, ראש-הענף `1d10eca`, אימות עצמאי אחרי שהזריעה נחתה (832 פרויקטים).** ‏`get_dashboard_summary` נבדק מחדש ולא נסמך על ה-✅ שכבר היה רשום: מטריצת-התחזות בת חמישה תפקידים (מנכ"ל/כספים ⇒ `profit_visible=true`, `monthly_profit=159228.48`, `pending_quotes_count=33` · מנהלת-פרויקטים ⇒ רווח `null` והצעות גלויות · גיוס/לוגיסטיקה ⇒ שני הדגלים `false` ושני השדות `null`; בכל החמישה 75 שורות-פרויקט) · **בקרה שלילית** (אימייל לא-מוכר) ⇒ `42501 אין לך הרשאה לבצע פעולה זו במודול פרויקטים` · ‏`pg_proc`: `prosecdef=true`, `provolatile='s'`, `proconfig={search_path=""}`, `proacl` בלי `anon`, עומס-יתר יחיד · **אורקל-קבלה בשאילתת-יד נפרדת:** `count(*) where project_status in (…)` ⇒ **65** = ‏`active_projects_count` **65**. **ביצועים (הסעיף שהיה החשש):** ‏139 ms קר / 39 ms חם · ‏`list_projects_overview()` 51.98 ms על 827 שורות · `get_finance_overview()` 194.19 ms על 758 — ‏**כולם רחוק מתחת לסף ה-1 שנייה שנקבע ב-`PROJECT_MASTER §6`; אין מיגרציית-תיקון.** ‏advisors: 49, והיחיד שנוגע למ7 הוא מחלקת-ה-DEFINER הצפויה. ⚠️ **וממצא חוצה-מודולים:** `rls_enabled_no_policy` עומד עכשיו על **חמש** טבלאות ולא ארבע — ‏`seed_registry` הצטרפה הלילה כ-deny-all **מכוונת** (המיגרציה נושאת `comment on table` שאומר זאת) — ⇒ אין דליפה, אבל `docs/db_health_checks.md` (בדיקה 3 ושורה 200, "ארבע הטבלאות") טעון עדכון **בענף-הזריעה**.
- ✅ **`20260903184711_module7_dashboard_cancelled_on_calendar_and_profit` — APPLIED `03/09/2026 18:5X` via MCP after Ishay's typed echo** (typed verbatim). DB version `20260903155157`. **Post-apply proof (impersonated CEO, read-only):** September 2026 ⇒ `monthly_profit = 0`, `monthly_profit_project_count = 1`, and the `projects` rows now include `#15` with `project_status = 'cancelled'` (before: absent, profit 3635) · October 2026 ⇒ `12239.2` over 3 projects, **equal to a separate hand sum** (`12239.20`) — non-cancelled months unchanged · ACL still `postgres, service_role, authenticated`, `stable`, `search_path=""`, new body md5 `0e0a6f03…` · security advisors **49 → 49, zero diff** (same function, same class). WRITTEN `03/09/2026 18:4X`. Fix-forward on `get_dashboard_summary(date)` (live body pulled via `pg_get_functiondef`, md5 `2f06bbee…`, both target strings confirmed present; everything else byte-identical). Two Ishay rulings (*"מאשר הכל לפי המלצתך, אהבתי את הכיוון שנהיה בדומה למנדיי"*): ① cancelled projects **stay in the calendar rows** (the `<> 'cancelled'` filter dropped; the client draws them gray/struck-through — Monday/Google Calendar convention) · ② a cancelled project contributes **0** to `monthly_profit` until its fee is resolved (`case when p.project_status = 'cancelled' then 0 else m.gross_profit end` inside the existing `coalesce(pf.final_profit, …)`), overriding §7.96's literal "no special treatment" after measuring #15 → ₪3,635 for an event that will not happen. Signature unchanged; grants restated. Reversible by re-applying the `20260903182735` body. §7.96 ↳ write-back text is parked in `docs/micro_guides/module-7.md` §9 (the §7 file's items 93–97 live only in the seed session's uncommitted diff — rule 16).
- ✅ **`20260903182735_module7_dashboard_summary_rpc` — APPLIED `03/09/2026 18:3X` via MCP `apply_migration` after Ishay's typed echo** (`module7_dashboard_summary_rpc`, typed verbatim). DB version `20260903153856` (UTC, the documented 3-hour offset; name-matched). **Post-apply proof (impersonated via `request.jwt.claims`, positive control first, all read-only):** CEO ⇒ `profit_visible=true`, `monthly_profit=3635`, `quotes_visible=true`, `pending_quotes_count=6`, `active_projects_count=4`, `satisfaction_avg=5` (1 feedback), 7 project rows · finance ⇒ identical · projects-manager ⇒ `profit_visible=false`, `monthly_profit=null`, quotes visible (6) · recruitment and logistics ⇒ both flags `false`, both fields `null`, `pending_quotes` JSON `null`, project rows still 7 · unknown email ⇒ `42501` "אין לך הרשאה … פרויקטים" (negative control). **Hand-count oracle, separate query:** active 4 · in-progress quotes 6 · 90-day satisfaction 5.00 — all three match. ACL `postgres, service_role, authenticated` (no `anon`), `prosecdef=true`, `stable`, `search_path=""`. **Advisors: security 48 → 49, the one new key is exactly `authenticated_security_definer_function_executable_public_get_dashboard_summary_p_month date` (expected class); performance unchanged.** `docs/schema.sql` refreshed (header stamp + section 24 block). 🔴 **Measured surprise, raised to Ishay right after:** the only September project is #15 "ערב השקה — קמפוס צפון", **cancelled 28/08 with the fee unresolved**, and §7.96's literal "no special treatment" makes the RPC count its full live expected profit (₪3,635) as September's monthly profit — an event that will not happen shows as the month's entire profit. Pending Ishay's ruling (together with his own mid-turn question whether cancelled projects should appear on the calendar at all); if he rules, it is a fix-forward migration, not an edit of this one. (Original design note kept below.) WRITTEN `03/09/2026 18:2X` (branch `ishay/module-7-dashboard`, worktree). **One new read-only RPC, `get_dashboard_summary(p_month date default null) returns jsonb`** — the home screen in one call: four KPIs (`active_projects_count` per `ACTIVE_PROJECT_STATUSES` · `satisfaction_avg` over a rolling 90 days anchored to `final_event_date` (§7.95; `הנחתי`: no feedback-answered timestamp exists — verified in the live `record_feedback` body) · `monthly_profit` per §7.96/§7.52 = `final_profit` if frozen else `finance_project_money().gross_profit`, cancelled included, NULL for an empty month · `pending_quotes_count` = `in_progress`), the project rows for the calendar + attention strip (**through the existing `list_projects_overview()`** — same staffing/logistics counts as the projects screen, rule 14), the open quotes for "expiring soon", and the three `params` thresholds (`ימי_אזהרה_קדם_אירוע`=14 **is** §7.94's 14-day window, read live, not hardcoded). **No new table, column, or policy.** Masking (§7.97) happens **in the body**: the same predicate as `project_finance_select_by_permission` ('כספים') and `quotes_select_by_permission` ('הצעות מחיר'), NULL + `profit_visible`/`quotes_visible` flags — never 0. Entry gate: `assert_module_permission('פרויקטים', edit/view)` (all five roles; inactive user ⇒ `42501`). Grants: revoke from `public, anon, authenticated`, grant `authenticated`. Reversible: `drop function`. 🔴 **Corrects the forward-notice row below (`🚧 מ7 ← מ8`) and its 03/09 "מ7's own answer" addendum in the main worktree: מ7 adds NO SELECT policy on `project_finance`** — the micro-guide §4 caught that a second policy with m8's identical predicate would change nothing and would not be what masks anyway (the DEFINER RPC bypasses table RLS). ⚠️ **§7.97 head-vs-tail:** the item opens "מנכ"ל+כספים only" for both money cards but closes with "`quotes` needs no change — the existing policy already does it", and that policy admits מנהלת פרויקטים (`edit` on 'הצעות מחיר'). The RPC follows the existing gate (pending quotes visible to whoever reads quotes elsewhere); raised to Ishay at the typed-echo gate. Security-advisor baseline before apply: **48** findings (42 of them the expected `authenticated_security_definer_function_executable` class) — expect **49** after.
- ✅ **`20260903032212_module9_e_min_wage_rpc_threshold_ceiling` — APPLIED `03/09/2026 03:3X` at module 9’s CLOSING AUDIT.** Closes closing-audit finding **F-3**, measured live and not inferred: impersonated as מנהלת כספים, `list_hostesses_below_min_wage(999999)` returned **26 rows** (every active hostess, name + hourly rate) while her direct read of `hostesses` returned **0** — migration D bounded the argument below and not above. 🔑 **Gate note, so nobody reads it as precedent: the typed echo was WAIVED IN ADVANCE by Ishay, in his own words** (*"מאשר למזג לייצור בלי הקלדה כנ"ל לגבי המיגרציות רק תהיו מסונכרנים בינכם"*, 03/09/2026). ⚠️ **Third waiver of the night — A/B/C waived in advance 02/09, D typed in full, E + the DoD sign-off waived here.** The gate in `supabase/migrations/CLAUDE.md` is unchanged.
  **Body:** migration D’s LIVE body (pulled with `pg_get_functiondef` and diffed line by line before applying) with exactly one change — `if p_threshold < 0` becomes `if p_threshold < 0 or p_threshold > 1000`. Signature, gate (ownership then `assert_module_permission`), `status = 'active'`, ordering, `stable`, `security definer`, `set search_path to ''` and the revoke/grant pair are byte-identical to D.
  **Post-apply proof (impersonated, 10/10):** finance `(999999)` ⇒ `P0001` *"רף לתצוגה-מקדימה חייב להיות בין 0 ל-1000"* · `(1001)` ⇒ `P0001` · `(-1)` ⇒ `P0001` · **`(1000)` boundary ⇒ 26, allowed** · `(40)` ⇒ **2** and `NULL` ⇒ **0** (unchanged) · logistics ⇒ `42501` · 1 overload · ACL `postgres | service_role | authenticated`, no `anon` · `search_path=""`.
  ⚠️ **Known residual, stated rather than glossed:** the highest live rate is **52**, so any value from **53 to 1000 still returns all 26**. The ceiling refuses absurd input; it does not prevent a full read by a caller who is already entitled to the list. **Debt:** align the RPC ceiling with the registry `max` (200). Deliberately not a second migration on the night of the close.
- ✅ **`20260902230500_module9_d_min_wage_rpc_threshold_arg` — APPLIED `02/09/2026 23:1X` after Ishay's typed echo** (`module9_d_min_wage_rpc_threshold_arg`, typed verbatim). Post-apply proof (rolled back): exactly one overload · ACL `authenticated` only · finance default ⇒ 0 · finance `(40)` ⇒ 2 · `(-1)` ⇒ `P0001` · logistics ⇒ `42501`. `list_hostesses_below_min_wage()` ⇒ `(p_threshold numeric default null)`: NULL = the saved `params` value (unchanged behaviour); a value = a preview against the typed threshold, which the approved mockup §4ב draws and which only the DEFINER function can compute (finance is blocked on `hostesses`). The 0-arg version is dropped (an overload would make the bare call ambiguous) — safe under the deploy rule: nothing deployed calls it (created tonight in C; `origin/main` `73c61d5` predates it). Gate, V-10, T13 unchanged. If never applied: the UI keeps the saved-threshold behaviour and the mockup caption is the recorded deviation (m9 guide step 3.6).
- ✅ **Module 9 — all three APPLIED `02/09/2026 21:2X` via MCP `apply_migration`, in the order A → B → C.** 🔑 **Gate note, so nobody reads this as precedent:** the typed-echo gate was **waived once by Ishay, explicitly and in advance** — his words at 21:2X: *"תחסוך לי עבודה בבקשה. תחיל חד פעמי עם אישור מראש. הנה אני מאשר לך להכיל את כולם"* — after the three explanations (tables · RLS · dependencies · reversibility) had been posted. The gate itself (migrations/CLAUDE.md) is unchanged. DB versions (UTC, name-matched — the 3-hour mine): `20260902182441` A · `20260902182448` B · `20260902182524` C.
  **Post-apply verification, quoted:** `params` **43** rows · **6** types · **4** policies (`params_delete_settings_only:DELETE, params_insert_settings_only:INSERT, params_select_all_authenticated:SELECT, params_update_settings_or_owner:UPDATE`) · owned **38** · doomed rows left **0** · `notification_preferences` RLS on, **3** policies, FK actions `a/c` (no action / cascade), trigger function schema `extensions` · `params` FK actions `r/n` (restrict / set null) · `proacl` on all three functions = `postgres, service_role, authenticated` only (no `anon`) · both m8 bodies contain `סף_שביעות_רצון` and no `< 3`. **Impersonation matrix (rolled back via `raise`):** CEO vat=1 · finance vat=**1** / weight=**0** · recruit weight=**1** / vat=**0** · logistics own row=1. **RPC:** finance @35 ⇒ 0 rows, @40 ⇒ **2** (מירב אטיאס@38, ליאת פרץ@39), CEO @40 ⇒ 2, logistics ⇒ **`42501`**. **Threshold contract (rolled back):** param row absent ⇒ `no_response` marking **ok**; scoring with the row absent ⇒ `P0001 סף שביעות-הרצון אינו מוגדר… (פרמטר סף_שביעות_רצון)`; threshold 3 blocks score 2 with "מ-3"; threshold **4** blocks score 3 with "מ-**4**" — the param is read. Advisors: no new unexplained (the new RPC joins the expected `authenticated_security_definer_function_executable` class; `params_owner_role_id_idx` shows as unused — it is minutes old). Health battery + `schema.sql` regen: step 1.4.
  · `20260902211549_module9_a_params_owner_types_seed` — `params.owner_role_id` (FK `roles`, `on update restrict on delete set null`, covering index) · `params_param_type_check` → 6 values (`shift_invites`) · 6 seed rows (24 · 48 · 72 · 3 · 10 · 7) · **deletes `תבנית_איפוס_סיסמה` + `קישור_בסיס_סקר_לקוחות`** (Q-2; deploy-rule check on `origin/main` `73c61d5`: zero executable readers) · owner map 38 rows by LIVE role names with a loud-fail `do $$` guard (43 / 38) · `params_write_ceo_only` dropped → `params_update_settings_or_owner` + `params_insert_settings_only` + `params_delete_settings_only` (4 policies, one per command).
  · `20260902211550_module9_b_notification_preferences` — new table (PK `email` FK `users(email)` **`on update no action on delete cascade`**, two booleans, timestamps, `extensions.moddatetime`), RLS on, 3 self policies, no DELETE policy, no extra index (PK = C-1), no grants. A-24 flips at apply.
  · `20260902211551_module9_c_threshold_functions_and_min_wage_rpc` — `record_feedback` + `archive_project` re-created from their LIVE bodies (`pg_get_functiondef` 21:13) with `< 3` ⇒ `< v_threshold` read from `params.סף_שביעות_רצון` **only on the consulting paths** · new `list_hostesses_below_min_wage()` (DEFINER, stable, `hostess_id bigint`, active only, owner-first gate then `assert_module_permission`) · explicit revoke/grant on all three.
...
- ✅ 27/08/2026 19:3X — **`N1b` APPLIED — THE NORMALIZATION IS COMPLETE** (typed-echo:
  ...
  **Verified after apply:** `hostesses.languages` gone (**0**) · 33 child rows across 20 hostesses ·
  26 hostesses total · a real hostess still reads `אנגלית · עברית` · migration recorded.
  🔑 **And the closing measurement, which is the one worth keeping: `data_type = 'ARRAY'` across the
  whole `public` schema now returns ZERO columns.** The single 1NF violation in the database is
  gone, and that is checkable in one query rather than believed.
  🔴 **Regression after an irreversible drop:** `npm run smoke` **exit 0** · `hostesses.spec.js` +
  `smart-match.spec.js` **32/32 passed**. Nothing broke.
  🌊 `docs/schema.sql` updated the same minute and re-cross-checked: `hostesses` **15 documented /
  15 live, zero divergence**.
  *(The pre-apply plan row this replaces:)* **`N1b` · the drop that finishes the normalization.** `20260827192631_module8_n1b_drop_hostesses_languages.sql`. **Check live:**
...
- ✅ 27/08/2026 — **module-8 migration A APPLIED via MCP `apply_migration`** (typed-echo received
  ...
  **Advisors: 26 = the m5 baseline of 26, zero new** — and `project_finance` does NOT appear under
  ...
  ✅ **מ7's own answer decided 03/09/2026 (`PROJECT_MASTER_sec7.md` §7.97):** its module-gated
  SELECT policy on `project_finance` is scoped to **מנכ"ל+כספים only** — the "monthly profit" and
  "pending quotes" dashboard cards are masked for the other three roles, not shown to everyone.
  `quotes` needs no new policy — its existing one already excludes גיוס/לוגיסטיקה.
...
- ✅ 26/08/2026 — **module-5 Phase 1: all four migrations APPLIED via MCP `apply_migration` + the
  ...
  `…2446_module5_approve_rpc_origin_backfill` (M5-3 + backfill — **6/6 legacy pointers filled**,
  color 0/6 by design (G14); full-body diff vs the live pre-edit `pg_get_functiondef`: the only
  delta is the INSERT's two new columns; `e2e/quote-approval.spec.js` re-run **6 passed**) ·
  `…2447_module5_checklist_rpc` (M5-6 — 7-assert impersonated round-trip inside a rolled-back
  transaction: first-action→`in_progress`, reversibility, autofill 300+flag+arrival stamp,
  G8 revert-to-0, S-5 Hebrew negative raise, ㉕ typed-120-kept, G9 expected-date-on-not_started
  raise; נועה(view)→`42501`) · `…2448_module5_scope_change_reset_removal` (M5-4 + M5-7 — old
  string `להסרת פריט לגמרי` **absent** from live def, both ㊱ refusal strings + G5/G11b raises
  present; behaviour proven by the seed's removal rider). **Typed-echo: WAIVED by Ishay for these
  four, one-time** (quoted in `micro_guides/module-5.md` §10); the tool-permission block was
  cleared by Ishay adding an allow rule to `.claude/settings.local.json`.
  **Seed (`supabase/seed/module5_demo_data.sql`, Ishay-approved plan):** create-run birthed demo
  projects `#13/#14/#15` through the REAL `create_quote`→`approve_quote_and_create_project` path
  (pointers born non-NULL — M5-3's live proof), removal rider ran (row deleted + `project_changes`
  history row, both refusals raised correctly), 9 `finally_approved` assignments, statuses DERIVED
  (`ready`/`in_progress`/`in_progress`); second run = refresh mode, **count delta 0**
  (8/16/27/14/1 before and after). 🕐 **Timezone fix found live:** `current_date` is UTC — a run
  between midnight and ~03:00 Israel seeded "today" one day back; seed now derives
  `(now() at time zone 'Asia/Jerusalem')::date` (§7.56) and a refresh realigned the dates.
  **Advisors:** security 26 = baseline 25 + 1 (`update_logistics_item`, the intentional
  browser-RPC class) · performance 27 = baseline 26 + 1 (`multiple_permissive_policies` on
  `logistics` — the accepted house pattern, C-2/M12). `docs/schema.sql` refreshed and
  cross-checked live: 23 tables / 224 columns / 37+12 policies / 26 functions / 3 cron jobs. — `m6_step_1_10.sql` executed as one read-only query via `execute_sql` (its per-assertion checks had only ever run ad-hoc during the apply round). **Result: 100 checks — 97 PASS · 2 FAIL, both triaged as stale expectations, not defects · 1 INFO.**
  **FAIL triage ①, `D-PC`:** the gate expects exactly one SELECT policy on `project_changes`; the live table has **zero — deliberately**: `module6_project_changes_money_gated_reader` (applied 14/08 **after** the gate script was written) dropped the broad policy so the browser can only read via the masking RPC. The gate's expectation is the stale side.
  **FAIL triage ②, `H-MD`:** the gate greps the trigger definition for `extensions.moddatetime`; `pg_get_triggerdef` prints the function unqualified. **Measured via `pg_trigger→pg_proc→pg_namespace`: the bound function lives in schema `extensions`** — the trigger is correct; the string-match is the false side.
  **Advisor triage (prediction 23/20, measured 25/26 — every delta explained, none open):** security **25** = 17 base + 7 M6 browser RPCs (accepted by design — the real gate is `assert_module_permission` in each body) − 1 `logistics` deny-all resolved **+ 1 `list_project_changes`** (the 8th browser RPC, born in the follow-up migration after the prediction) **+ 1 `rls_enabled_no_policy` on `project_changes`** (the deliberate deny-all above — INFO-level, accepted). performance **26** = the 20-finding baseline unchanged **+ 6 `unused_index`** (INFO) on the six FK-covering indexes this phase created — trivially "never used" on tables with 0–6 rows; **kept deliberately**, dropping them would re-open `unindexed_foreign_keys`.
  **Also at the gate:** `npx deno check` on `send-email` → exit 0 · `🚧 מ6` tokens 17 → 19 occurrences since phase start (grew, none dropped) · `docs/schema.sql` refreshed by query (delta exactly two objects: +`list_project_changes`, −`project_changes_select_by_permission`; all category counts re-verified live: 23/220/118/53/48/25/24) · all M6 registry rows in this file flipped, M6-14 deliberately NOT (it is Phase-2/3 JS work, S-10).
- ✅ 14/08/2026 — **all eleven module-6 Phase-1 migrations APPLIED via MCP** *(entry written at the 1.10 gate, 18/08 — the apply sessions updated the A-M6 block above but never added the strike entry)*: `module6_projects_columns_and_constraints` · `module6_project_changes_table` · `module6_assignments_attendance` · `module6_logistics_policy_and_origin` · `module6_storage_reports_and_finance` · `module6_email_log_accepts_project` · `module6_params_seed` · `module6_status_machine_and_cron` · `module6_rpcs_reads_and_close` · `module6_rpcs_writes` · `module6_project_changes_money_gated_reader`. Typed-echo **waived for Phase 1 only** by Ishay's explicit ruling (14/08, after all nine were measured reversible and the single destructive statement destroys nothing). Evidence anchor: `list_projects_overview` reproduced `spec.md`'s hand-computed numbers digit-for-digit (`#8 → 5,355.00 ₪ · 1/6 · 2 pending`) under impersonation, and the negative control (מנהלת לוגיסטיקה) got `NULL` money with correct metrics. ➕ same day, separately ruled: `module6_logistics_manager_projects_view_only` (Ishay: her permission drops to `view` — measured to cost her nothing) · `demo_data_fix_for_28_08_presentation` (data-only, Ishay's ruling).
...
- 09/08/2026 — migration `20260809125750_module4_tables_params_and_templates.sql`
  ...
  **Forward notice (§10.2): M6** writes `customer_hostess_preference` (`🚧 מ6 ← מ4`) and flips
  ...
  rows (~~`🚧 מ9 ← מ4`~~) — ✅ **built 03/09/2026 (m9 steps 3.1+3.3); the Table-Editor sentence below is superseded and kept only as history.** *(was:)* until it exists, changing any of them means a hand-edit in the Table Editor.
