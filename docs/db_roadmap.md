...
| **M11-3** | **NEW table `feedback_ai_insights`** — `project_id` **unique** · `run_id` → `feedback_ai_runs` · `sentiment` (1–5) · `topics[]` · `quote` · `red_flag` · `classified_at`. 🔴 **`topics[]` values are constrained to the ten categories already in the DB** — the CHECK on `negative_feedback_reasons` + `positive_feedback_reasons` (D14) | Card **ת2**. **The taxonomy is deliberately NOT model-updatable** — it is the language customers pick from, and without it there is no human↔model agreement matrix, which is report 20's whole point | ⬜ **not written** ⇒ **0** in `schema.sql` |
