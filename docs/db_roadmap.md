...
- ⏳ **`20260923200000_module11_l2_tile_copy.sql` — WRITTEN, NOT APPLIED (awaits Ishay typing its name).** Text round 2 of phase B, built on the live bodies after L1 (`phase-b-migration/bodies.mjs`, md5 13/13 == live): 24 replacements in 8 report RPCs (m07 · m14 · m15 · m16 · m17 · m19 · m20 · m22), tile sub-lines only plus one m07 compare note. Why: the live check after L1 showed the tile subs (outside L1's scope) as most of the visible text left — definitions, repeats of the scope chip or of "so what", internal words, and the fragment "לא אותה נוסחה" L1 left behind. Rule: a sub that is the base of the number stays; a definition/repeat/design note goes. Same technique and guards as L1; same signatures. **Post-apply check prepared:** `node phase-b-migration/expect-md5.mjs l2` prints the md5 each function must have (8 changed, 5 unchanged). Ops: `phase-b-migration/ops-l2.mjs`; generator `gen-migration.mjs --ops ops-l2.mjs`.
