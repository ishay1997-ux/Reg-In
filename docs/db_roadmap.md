...
| `params` | **עשרה ערכי `smart_match` חדשים** *(תוקן 07/08/2026 — כאן היה "שבעה" והמנייה נותנת שמונה; ועוד שניים לא נרשמו מעולם)*: שלושת המשקלים המעודכנים (0.40 היענות · 0.35 אמינות · 0.25 קרבה) · שער-מרחק 80 · גולפוסט 40 · ריסון m=3 · חלון 12→24 חודשים · מינימום-3-תשובות · ➕ **`לא_ענתה_ל_N` = 4** *(הכרעה ⑦ ב-`processes-approved.md` — יצרה שורת-params ומעולם לא הגיעה לכאן)* · ➕ **`תקרת_דיילות_מומלצת` = 6** *(הכרעת-ישי 07/08/2026 — `recommended_hostess_count = min(ceil(אורחים ÷ יחס), תקרה)`; **התקרה חלה על ההמלצה בלבד, אין ולידציה שחוסמת שיבוץ מעליה**, ו-`PROJECT_MASTER §5` כבר אמר "ניתנת לדריסה". ⚠️ הנוסחה נגזרת ב-`src/lib/quotes.js:216` — קוד מודול 3 שכבר נבנה ⇒ הביצוע בבלופרינט-מ4)*. 🔴 **ושלוש השורות הקיימות במסד שגויות** (`משקולת_1W_דירוג`=0.4 · `משקולת_2W_קרבה`=0.3 · `משקולת_3W_מהימנות`=0.3) — הן שמות מרכיב (`דירוג`) שההכרעה **הוציאה מהציון**; **מוחלפות, לא משנות שם.** נמדד חי 05/08/2026 | ‏§7.15↳ + `module4_smart_match_research` §11.1 | 4 |
