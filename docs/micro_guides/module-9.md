...
| **Status** | 📘 **BLUEPRINT APPROVED — Ishay, `02/09/2026 20:5X`** (*"מאשר"*, after round 1 R-1…R-4 and round 2 Q-1…Q-4 — all eight = the recommendation — the fresh-context review (≈19:50) and the Phase-1 execution rehearsal (≈20:20)). 🖥️ **BUILD NOT OPENED** — starts on "תמשיך לבנות" at step 1.0. Zero code written. |
| **Last updated** | `02/09/2026 20:5X` *(system clock — approval + rename)* |
| **Active step** | **1.0** — Phase-1 door (re-measure · ledger sweep · Ishay checkpoint), not started |
...
| Step | Title | Status |
|---|---|:--:|
| **1.0** | 🔻👤 Phase-1 door — branch · ledger sweep · live re-measurement · baseline · Ishay checkpoint | ⬜ |
| **1.1** | Migration A — `params` owner column + sixth `param_type` + 6 new rows + owner map (loud-fail guard) + deletion(s) + policy rewrite | ⬜ |
| **1.2** | Migration B — `notification_preferences` table + RLS | ⬜ |
| **1.3** | Migration C — `record_feedback`/`archive_project` read the threshold from `params` · `list_hostesses_below_min_wage()` RPC | ⬜ |
| **1.4** | 🔻👤 Phase-1 gate — advisors · health battery · `schema.sql` regen ⚠️ shared-surface · `db_roadmap` §10 · commit | ⬜ |
| **2.0** | Phase-2 door — ledger sweep; **Q-3 (validation kinds) must be ruled before 2.1** | ⬜ |
| **2.1** | `src/lib/paramsRegistry.js` — label/kind/unit/validation/affects registry + tests · **🗣️ labels brief** | ⬜ |
| **2.2** | `src/lib/emailTemplates.js` — the template⇐placeholders contract (required/optional) + consistency tests | ⬜ |
| **2.3** | ⚠️ shared-surface — the six constants become param readers (per-constant ripple table in §6) + `src/api/params.js` | ⬜ |
| **2.4** | ⚠️ shared-surface — `smartMatch.js`: reliability company-average fix (pays `🚧 מ9 ← מ4/מ6`) | ⬜ |
| **2.5** | `src/modules/09_settings/api.js` — reads · grouped updates · min-wage RPC · notification prefs · "my params" + tests | ⬜ |
| **2.6** | 🔻👤 Phase-2 gate — full unit suite, zero regressions | ⬜ |
| **3.0** | 🔻👤 Phase-3 door — shared-component checkpoint + **the updated mockup (per-role view + the undrawn states) for final approval** (Ishay's 02/09 request) + the Q-1 duplicate-editor ruling applied | ⬜ |
| **3.1** | S1 — `ParamsTab.jsx`: search + group list + generic value pane; route swap `params` ⚠️ shared-surface (`App.jsx`) | ⬜ |
| **3.2** | S2 — `TemplateEditor.jsx`: template list · body · variable chips · required/optional gating | ⬜ |
| **3.3** | S3 — `SmartMatchPane.jsx`: warning · sum-1.00 bar · reliability toggle with live attendance count · distance order rule · partial-row behaviour | ⬜ |
| **3.4** | S4 — `MySettingsPage.jsx` at `/my-settings` + Topbar menu item "ההגדרות שלי" (owner mode of S1–S3) ⚠️ shared-surface | ⬜ |
| **3.5** | S5 — Profile `NotificationsSection` wired: email toggle live, SMS toggle truthful, **both** "(בקרוב)" labels gone ⚠️ shared-surface | ⬜ |
| **3.6** | S6 — "מי מתחת לשכר-המינימום" list beside `שכר_מינימום_שעתי` (pays `🚧 מ9 ← מ4`) | ⬜ |
| **3.7** | 🔻👤 Phase-3 🎨 gate — UX & functional review with Ishay | ⬜ |
| **4.1** | Doc ripples — §6 debts paid · §7.70/§7.83/§7.84/§7.21/§7.64/§7.35 · `db_roadmap` · `products_and_params.md` · the 08/08 STATUS task line · the step guide's ⑥1 pointer · delete the interview file | ⬜ |
