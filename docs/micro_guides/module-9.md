...
| **Status** | 🔒 **Closed — awaiting PR/merge. Closing audit `03/09/2026 04:5X`, verdict [YES] on `c305322`.** *(was:)* 🔨 BUILD OPEN — Ishay's delegation `02/09/2026 21:1X` (*"אני מאציל אותך להתחיל ולסיים את מודול 9… העזר בסוכנים… תפתור בעצמך את הקונפליקטים. אבל שום דבר לא בשקט"*; module-close excluded — fresh session). Orchestrator = this session (Fable 5.1); builders = Opus/Sonnet agents on disjoint files; every 🗣️ brief is still POSTED, but under the delegation it does not block unless Ishay stops it. **The typed-echo gate is NOT delegated** (migrations/CLAUDE.md — the name must be typed). |
| **Last updated** | `03/09/2026 07:1X` *(post-merge — system clock)* |
| **Active step** | **None — merged and approved.** 5.2 closed the module (three fix rounds, five live regression runs, a fresh-context reviewer, a fifth migration); **merged to `dev` as `1ceb09e` and to `main` as `4650293`, approved by Ishay 03/09/2026** *("קראתי ואני מאשר את מודול 9")*. 🔴 **Approved code identity is `ed1240a`, NOT `c305322`** — the audit's first verdict did not ship: **CI went red on `c305322`** (`ProfileSettingsPage.test.jsx` reached the real Supabase client at import time; `.env.local` hides it locally, the GitHub runner has no such file, so **`npm run gate` is structurally blind to that class** — two sessions read 83/2,165 green on a tree CI reported as 82/2,160). The verdict expired by its own rule and was re-issued on the fixed commit. Reproduce with `VITE_SUPABASE_URL= VITE_SUPABASE_ANON_KEY= npx vitest run`. |
| **Post-close work** | 🔨 **In flight, not part of the merged module:** all 22 numeric ceilings now closed (`d14b09f`, branch `ishay/params-last-two-ceilings`, **PR #104 open, all checks green**), and a UX follow-up branch `ishay/m9-ux-followups` held by the other session (the 554px finding — a disabled save button whose explanation sits below the fold). **A future session opening this guide should not read "all steps ✅" as "nothing outstanding".** |
...
| **3.6** | S6 — "מי מתחת לשכר-המינימום" list beside `שכר_מינימום_שעתי` (pays `🚧 מ9 ← מ4`) | ◐ `02/09 22:5X` built by A (`BelowMinWageList.jsx`, three states, `blocked` ⇒ nothing, `refreshKey` after save) **against the SAVED threshold** — the approved mockup §4ב draws the list reacting to the TYPED value before saving, which the C-migration RPC cannot do (finance is blocked on `hostesses`, so the preview must come from the DEFINER function). **Migration D written `20260902230500_module9_d_min_wage_rpc_threshold_arg.sql`** (`p_threshold numeric default null`; the 0-arg version dropped to avoid overload ambiguity — safe, nothing deployed calls it) — **✅ APPLIED `02/09 23:1X` after Ishay's typed echo** (`module9_d_min_wage_rpc_threshold_arg`); proof (rolled back): one overload · ACL authenticated-only · finance default ⇒ 0, `(40)` ⇒ 2, `(-1)` ⇒ `P0001`, logistics ⇒ `42501`. `api.js` `listBelowMinWage(threshold)` passes `p_threshold` for a finite number (orchestrator; test added); the draft-value → preview wiring (debounced, note `תצוגה מקדימה — טרם נשמר`) is in D's wave-2 scope; `schema.sql` signature line updated   ✅ **CLOSED at the audit `03/09 03:3X`:** migration **E** added the missing upper bound to `p_threshold` (D bounded it below only). Measured before: finance `(999999)` ⇒ **26 rows**; after: `P0001`. `(1000)` allowed, `(40)` ⇒ 2 and `NULL` ⇒ 0 unchanged, logistics still `42501`. |
| **3.7** | 🔻👤 Phase-3 🎨 gate — UX & functional review with Ishay | ✅ **APPROVED by Ishay `03/09/2026 01:3X` — *"מאשר"*** (after two of his own catches: the oversized value box, and the unit floating away from it — both fixed and re-measured before the approval). The fresh-context UX/UI review still runs; anything it finds now lands as a post-approval improvement, not as a gate. *(previous state:)* 🔨 `03/09 00:3X` — 12 live screenshots taken (4.2 builder); Ishay saw three (Smart Match · finance my-settings · profile) and ruled the value-column look (centred pair, small uniform box); polish builder applying 6 layout items; **a dedicated fresh-context UX/UI review (Opus) runs right after the polish — Ishay asked *"מישהו עבר על המסכים עשה בדיקות UI UX?"* and the honest answer was "partially" (axe · bidi · states proven; no heuristic journey review yet)**; then the final 12 screenshots go to Ishay for the one approval |
| **4.1** | Doc ripples — §6 debts paid · §7.70/§7.83/§7.84/§7.21/§7.64/§7.35 · `db_roadmap` · `products_and_params.md` · the 08/08 STATUS task line · the step guide's ⑥1 pointer · delete the interview file | ◐ `02/09 23:5X` (Sonnet docs builder) — §6: three inbound `🚧 מ9` lines struck with measurements, the prefs line stays live for מ10 with "חצי שולם" · §7.70/§7.21/§7.83/§7.84/§7.66/§7.90 ↳ sub-notes appended (§7.64/§7.35 already carried the blueprint's deferrals) · step guide ⑥2 refreshed + ①ו block with the four delegated rulings (`הכרעתי, הפיך`) · `check:docs-structure` exit 0. Done earlier this session: `db_roadmap` (§10ב A–D, A-24, `params`/`users`, §7.64 lane) · `products_and_params.md` §16–20 · the tombstone comment · the interview file (blueprint) · ⑥1 pointer (blueprint) · seven closed guides' ripple records. **Still owed at session end (orchestrator): STATUS.md + CLAUDE_CODE_LOG.md**   ✅ **COMPLETE — STATUS + `CLAUDE_CODE_LOG` written at the close (03/09 04:5X), plus the audit's own reverse sweep: 7 paid `🚧 מ9` tokens struck across `module-4`, `module-6`, `db_roadmap`, the m4 spec and two `src` comments (`fefaf18`).** |
| **4.2** | E2E `e2e/settings.spec.js` + smoke anchor + accessibility list ⚠️ shared-surface (`e2e/**`) | ◐ `03/09 00:1X` (Opus E2E builder, ~45 min) — `settings.spec.js` 10 tests (CEO edit intercepted · validation never clears a sibling · template blocked · finance/recruit mirror · **the one real write: finance PATCH on `משקולת_היענות` ⇒ 0 rows, value re-read unchanged** · logistics blocked on the tab + menu item · profile toggle intercepted) · `prices.spec.js` re-pointed (block had drifted to `:170-229`) · smoke: VAT anchor → `settings.vat`, `/system/params` + `/my-settings` anchors, `list_hostesses_below_min_wage` allow-listed (stable RPC, POST), budget 150 s · axe: `/system/params` ×3 states + `/my-settings` as FINANCE (CEO owns nothing — an empty page would scan green). Run: 25 passed / 2 failed — both = **one real axe `label` critical on the template textarea** → `aria-label` added by the orchestrator (`getParamEntry(...).label`); `smoke` 1/1 twice. **12 🎨 screenshots taken** (`scratchpad/screens/`), no value saved. Bidi `Range` (centre + wrap method): 25 pairs · 24 pass · 1 wrap (`ימי עסקים`, layout not bidi). **Layout findings → polish builder (in flight):** hint column starved by the 280 px aside · two-word unit wraps · view-only affordance weak · email input truncates · doubled min-wage heading on `/my-settings`. Pre-existing, app-wide, not m9's: 375 px is unusable because `Sidebar.jsx` has no breakpoint (`11-…-mobile-375.png` is evidence) — recorded for the closing audit, not fixed here. Subtitle singular grammar fixed by the orchestrator (`הגדרה אחת בבעלותך, בקבוצה אחת`). **Ishay's 🎨 ruling `03/09 00:2X` on the Smart Match screenshot: *"שיראה נורמלי למרכז ולהקטין את המלבן בו יושב המספר"* — value+unit pair centred in the `ערך` column, one small uniform box for numbers** → polish builder item 6. **Polish landed `03/09 00:5X`** (Sonnet): uniform `w-24 h-9` centred number box + reserved unit slot (`w-16`, nowrap) · email/url `w-72` · view-only `bg-slate-100` · min-wage card below the table (the `xl:` alternative would re-trigger at the 1280 px test viewport) · doubled heading removed · SmartMatchPane `ערך` header centred (orchestrator). Bidi re-measured: **31 pairs · 0 failures**; m9 units 101/101; `settings` + `accessibility` specs **17/17**. **Fresh-context UX/UI review (Opus) launched 00:5X** — passes 2–5 with first-time-user journeys per role. **Ishay's second 🎨 catch `03/09 01:1X`** (*"אני צריך להגיד לך תבעיה או שאתה מזהה"*): the unit floated far from the number box — the reserved `w-16` slot inherited `text-align:right` from the RTL table; `text-left` added (orchestrator), 32/32; polish builder re-taking `01`/`02` and choosing between the reserved slot and a tight centred pair by looking   ✅ **COMPLETE.** Re-run at the close on `c305322`: `test:e2e` **166 passed / 6 skipped / 0 failed**. One flake resolved by evidence (`projects.spec.js:336` — 1 failure, 3 passes, mechanism traced). |
| **4.3** | Full regression: `gate` · `test:e2e` · `smoke` · DB health battery (13) | ◐ `03/09 01:2X` — **`npm run gate` exit 0** (83 files / 2094 tests, before the UX fixes) · **`npm run test:e2e` exit 0 — 166 passed, 6 skipped, 17.1 min** (incl. the 10 new `settings.spec.js` tests and every pre-existing suite) · **DB health battery after migration D: `נבדקו 12 · נכשלו 0`** (check 10 N/A — no column dropped) · **`npm run smoke` exit 0 — 1/1, 1.3 min** (`03/09 01:2X`, run separately by name because `test:e2e` grep-inverts it) · **final `gate` after the UX fixes: exit 0 — 83 files / 2142 tests** (`03/09 02:1X`) · the two E2E specs re-run after the fixes: **20/20**   ✅ **COMPLETE — re-run five times over the close.** Final on `c305322`: `gate` exit 0 (**83 files / 2165 tests**) · `test:e2e` 166/6/0 · `smoke` 1/1. |
| **4.4** | 🔻👤 Phase-4 gate | ✅ `03/09 02:1X` — every gate named with its number: `gate` exit 0 (83 files / **2142** tests; baseline 67 / 1838) · `test:e2e` 166 passed / 6 skipped · `smoke` 1/1 · `settings`+`prices` 20/20 after the fixes · DB health 12/0 · advisors: zero new unexplained |
| **5.1** | Live acceptance journeys (VAT in an un-saved quote form · owner edits hers · blocked template save · notification toggle round-trip) | ✅ `03/09 02:3X` (Opus, live DB, every change restored through the UI and re-read) — **①** `אחוז_מעמ` 18→17 saved; the **un-saved** quote form computed `מע"מ (17%) = 85 ₪` and `סה"כ 585 ₪` on a 500 ₪ line (18% would be 90/590); restored to 18, `npm run smoke` green afterwards (its VAT anchor is a live-value fixture) · **②** finance edited `תנאי תשלום` 30→31 from "ההגדרות שלי" and back; zero Smart Match weights on her page · **③** deleting the invite link disabled Save with the locked sentence; the chip restored the body **byte-identical (282 chars)**; zero writes left the browser · **④** the notification toggle survived a full reload ON, then OFF · **⑤ the failure path, on screen for the first time** (the UX reviewer's carried blind spot): one PATCH forced to 403 ⇒ `השמירה נכשלה ב"אחוז מע"מ"`, the button offered `נסי שוב`, both typed values survived, exactly one write attempted. **Three findings from the journeys, all handled:** the reason clause read as a second error message and exposed the raw `param_name` ⇒ rewritten (the failed row's name now travels as `err.paramName`, not in the text) · §3.7's example consequence sentence differed from the shipped one ⇒ the guide now marks the PATTERN as locked and the per-token text as living in `emailTemplates.js` · after a byte-identical restore Save is disabled because nothing is dirty — correct, and the acceptance text's "enabled again" was imprecise. **Declared side effect:** journey ④ created the CEO's `notification_preferences` row (born 23:06:49Z); content restored to `false/false`, which §2.8 defines as identical to an absent row, and there is no DELETE policy by design. **Remaining blind spot for the closing audit:** a failure on the SECOND row of a batch (partial write already done) was never put on screen |
| **5.2** | 🔻👤 Closing audit — `module-close` in a FRESH session | ✅ `03/09/2026 02:35–04:5X` — **verdict [YES] on `c305322`.** Two sessions: this audit ruled scope and verified, the build session wrote the code, **and neither decided alone** (Ishay, 03/09: *"אתם לא מחליטים לבד על כלום רק אם שניכם מסכימים"*). Three fix rounds · a fresh-context reviewer (**0 would-hurt-if-merged**) · migration E applied+proven · ⑥3② closed live. 🔴 **The DoD typed echo was WAIVED in advance by Ishay and was NOT obtained; the audit did not sign it and did not merge.** |
...
- Smart Match warning, CEO view (mockup ד׳): `שינוי כאן משנה שיבוצים אמיתיים. הדירוג שמנהלת השיבוץ תראה מחר ייראה אחרת, והיא לא תדע שמשהו השתנה. שינוי כדאי לתאם איתה מראש.` · **owner view (recruitment manager herself):** `שינוי כאן משנה את הדירוג שתראי מחר במסך השיבוץ.` · sum bar: `שלוש המשקולות חייבות להסתכם ל-1.00` + tag `תקין` / amber otherwise. 🔴 **Changed 03/09/2026 at the closing audit (UX-4), and the change of a §3.7-locked string is deliberate:** the old wording, `מסתכמות ל-1.00`, rendered unconditionally beside a tag showing the real sum — so at 1.50 the screen asserted what the number next to it disproved. §3.7 locks strings to stop drift, not to preserve a falsehood.
...
- Template gating sentence pattern (R-3): `בלי [<token>] <consequence>` — **the PATTERN is locked, the consequence text is per-token and lives in `src/lib/emailTemplates.js`.** As shipped: `בלי [לינק_אישור_משמרת] הדיילת לא תוכל לאשר או לדחות את המשמרת` *(the guide's earlier example said "לא יכולה לאשר"; the shipped wording is wider and correct — the link is also how she declines. Difference measured on screen at the 5.1 journeys, 03/09/2026.)*
...
- Success toast: `ההגדרות נשמרו` · save-failed: `השמירה נכשלה ב"<label>" — <reason>` **(the `<reason>` is a reason only — no repeated "נכשלה", no raw `param_name`; corrected 03/09/2026 after journey ⑤ put the line on screen for the first time and it read as two glued messages)** · error/empty: `לא ניתן לטעון את ההגדרות.` + `נסי שוב` · empty "my settings": `אין הגדרות בבעלות התפקיד שלך` · view-only note: `צפייה בלבד` · min-wage empty: `אין דיילות פעילות מתחת לרף הנוכחי`.
...
|---|---|---|
| Unit | registry (names both ways · kinds · sum · order · search) · template verdicts + builder consistency · six param readers (mutation checks) · smartMatch average (dormant + active) · api.js (mixed batch refusal) |  ✅ `gate` on `c305322`: **83 files / 2165 tests**, 0 failures (module open: 67 / 1838) |
| Integration | impersonated RLS: positive control (CEO) · owner ok / non-owner 0 rows (both directions) · prefs self-only · RPC 42501 for logistics · the "paths that never consult the threshold" proof |  ✅ **21/21** impersonated RLS assertions at the close, rolled back, positive control first |
| E2E | `settings.spec.js` · smoke anchor · axe on both screens · re-pointed `prices.spec.js` |  ✅ `test:e2e` **166 passed / 6 skipped / 0 failed** · `settings.spec.js` 10/10 · axe on both surfaces · one flake resolved by evidence |
| Regression | m2/m3/m4/m5/m6/m8 suites · `gate` · full `test:e2e` + `smoke` |  ✅ five full runs across the close; six constants and the hardcoded `3` grep to **0** |
| UAT | Ishay drives journey ① at 5.1 |  ✅ five 5.1 journeys + Ishay's 🎨 approval `03/09 01:3X` + **⑥3② closed live at the audit** (second-row failure on screen) |
| Security/Pen | policies count on `params` = 4, one per command · no policy-less table · RPC ACL · advisors · `search_path` on the 3 functions |  ✅ 4 policies one-per-command · advisors triaged · RPC ACL authenticated-only · `search_path` on all 4 functions · 2 scoped agent scans + a re-scan of the fix diff |
| Performance | N/A — ≤45 rows |  N-A — 43 rows |
| Usability | 3.0 mockup approval + 3.7 🎨 gate + closing UX audit |  ⚠️ partial — mockup + 🎨 gate approved · fresh-context UX review (2 blockers, fixed) · **5 items recorded-not-fixed** |
| Compatibility | Chromium (house); cross-browser = M12 |  ⚠️ partial — Chromium only; **375 px unusable app-wide** (`Sidebar.jsx` has no breakpoint) — pre-existing, M12 |
...
- [x] `npm run verify`/`gate` green · full suite ≥ baseline, zero regressions (numbers recorded)
- [x] 3 migrations applied via MCP after typed echo · `docs/schema.sql` regenerated · committed together
- [x] `db_roadmap.md` rows flipped (A-24 · users · params · §7 matrix) · §6 inbound `🚧 מ9` debts struck against measurement · `products_and_params.md` updated
- [x] §7.70 / §7.21 / §7.83 / §7.84 / §7.64 / §7.35 updated at the item; snapshot counts re-derived
- [x] `CLAUDE_CODE_LOG` + `STATUS` current (incl. the 08/08 task line) · interview file deleted · step guide ⑥1 pointer fixed

### 8.2 Module-specific (each with its measurement)
- [x] `select count(*) from params` = **43** · 6 distinct types · **4 policies** · owned rows = **38**
- [x] impersonated proof: finance updates `אחוז_מעמ` (1 row) and not `משקולת_היענות` (0 rows); recruitment the mirror; positive control first
- [x] `notification_preferences` exists with 3 self policies; a saved toggle survives refresh (live)
- [x] `record_feedback` and `archive_project` contain `סף_שביעות_רצון` and no `< 3` (`pg_get_functiondef`); `no_response` marking works with the param row absent (rolled-back proof)
- [x] `grep` for the six old constants over `src/` ⇒ 0 **and** `grep "< 3\|<3\b\|ל-3" src/modules/08_finance/*.jsx src/lib/customers.js` ⇒ 0
- [x] journey ①: the un-saved quote form computes with the changed VAT (live, restored after, smoke re-run)
- [x] template with the invite link removed ⇒ save blocked (unit + E2E + live)
- [x] `PricingParamsCard` gone from the `מחירים` tab (if Q-1 as recommended) and no testid `param-vat` remains asserted anywhere
- [x] DB health battery: `נבדקו 13 · נכשלו 0`

### 8.3 UX & validation
- [x] 3.0 mockup approved by Ishay (per-role + states) · 3.7 🎨 gate passed · every spec-silent validation was confirmed at a 🗣️ brief
...
### Dated entries
- `08/09/2026 22:4X` — **CROSS-MODULE EDIT by the ui-copy night (branch `ishay/ui-copy-rewrite`; module 9 is closed — required ripple record).** ↳ as-built: `src/modules/09_settings/api.js` — `getNotificationPreferences` now also selects `onboarding_mode` (returned as `onboardingMode`, `?? 0`); two new exports **`saveOnboardingMode(level, { email? })`** (upsert with payload `{ email, onboarding_mode }` ONLY — never through `saveNotificationPreferences`, which sends both notification columns from local state and wraps in `Boolean()`; rejects anything outside integer 0..2 before touching Supabase; `email` given ⇒ the CEO door under policy `notification_preferences_ceo_all`) and **`listOnboardingModes()`** (email → level map). Table change: migration `20260908221959_onboarding_mode` (independent, ruling 28⑤ deviation) — see `db_roadmap.md` §10ב. `api.test.js` 29 → 37 tests (payload shape · `2` not `true` · out-of-range rejected · CEO email path · RLS 0-rows throws · list map). The m9 owner screen `ProfileSettingsPage.jsx` (in `src/components/`) gained the third switch "מצב הטמעה" + `<Hint id="onboarding.self" />`; its suite 5 → 12 tests. Regression of the module's own suites: green (measured in the same session; full CI-parity run logged in `CLAUDE_CODE_LOG.md`).
- `03/09/2026 07:0X` — **the last two numeric ceilings closed, and they are the two Ishay had reserved for himself.** The 03/09 ceilings pass capped 20 of 22 and deliberately left these open because he had kept them; he then closed them **by delegation** (*"תדונו ותחליטו לי וזהו"* · *"מבחינתי מאשר מה שתחליטו"*), decided between the two sessions. **`סף_לקוח_רדום_ימים` ⇒ 730** — a client running one conference a year can legitimately return after more than a year, so 365 would have blocked a real business choice rather than only catching a typo. **`תקרת_שבועות_הוגנות` ⇒ 104** — a plain typo-guard on the same scale as the other twenty. 🚫 **And the alternative that was proposed here and rejected: a cross-field bound on the product `שיעור × תקרה`.** It is the theoretically right guard — the fairness bonus's real danger is out-ranking a hostess's actual record, which a cap on weeks alone does not prevent — **but it is new machinery for a problem nobody has reported**, against Ishay's standing *"אין סיבה להוסיף עבודה גם אם זה שיפור נחמד"*. If the bonus is ever observed outranking a real record in a live match, that is when it earns its place. 🔑 **The lock test's `OPEN_BY_RULING` is now empty, so the mechanism that recorded "these two were his" vanished with it — its comment carries that history instead**, or in three weeks they read as two ceilings that were always there. Commit `d14b09f`, branch `ishay/params-last-two-ceilings`; `gate` exit 0, 83 files / 2,170 tests.
- `03/09/2026 04:2X` — **closing-audit round three (builder + audit sessions as peers; Ishay's rule: neither decides alone).** Two fixes in, four findings recorded-not-fixed, one wrong comment of my own corrected. **A — four hard-coded Hebrew strings still quoted numbers that moved to `params` in step 2.3**, missed by that step: `OverviewTab.jsx:324`+`:339` (`שעות_סף_זימון_לפני_אירוע`) and `SmartMatchPage.jsx:745` + `projectTeam.js:89` (`שעות_תוקף_זימון`). **The measured harm is a screen contradicting itself:** `OverviewTab` computed `cutoffHours` and used it in the KPI tile while `OverviewRow` carried the literal `24` — at threshold 6 the tile reads `בתוך 6 שעות` and the row under it `בתוך 24 שעות`. Fixed by passing the live value down; `teamHeadline` takes `validityHours` and says the sentence **without** the number when the threshold is absent (never `null`). **Three tests pinned the stale literals** (`projectTeam.test.js:76` · `TeamTab.test.jsx:226` · `e2e/hostesses.spec.js:320`) — updated, plus **three mutation tests** that fail the moment a number is frozen again. ⚠️ **`SmartMatchPage` has no unit-test file at all** — that sentence is covered by nothing, before this change or after. **B — the save gate was unscoped across groups.** `useParamsForm.js` gated on `crossFieldErrors.length > 0` while scoping `scopedDirty` one line above: an inverted pair typed in one group killed saving in **every** group, and in `submit` the button could be live while it returned `{ok:false}` with no `saveError` — a click that does nothing and says nothing. `crossFieldErrors` is now `{ message, names }[]` with one shared `crossFieldErrorsIn(errors, names)`; `ParamsTab` filters by the active group, `MySettingsPage` by the visible rows, `submit` by its own scope. 🔴 **And the finding inside the finding: `MySettingsPage.jsx:146` carried the identical unscoped clause *inside the `hasErrors` that R-2 rewrote*, while R-2's own comment claimed `ParamsTab` scoped "את השער ואת הכתיבה" — it scoped the write and the per-field half only.** Third time in one night that a fix shipped with a false claim about its own reach. **Corrected, not fixed: `projectCancellation.js:58` claimed the inverted-ladder guard blocks interpretation "מכל מקור".** It blocks the client only — the same ladder lives in `20260828102653_module8_h6…sql:90-96` and `:105` with no inversion guard, so an inverted pair still returns a wrong percentage into the fee proposal and the salary report. Reachable today by CEO-via-Table-Editor only (the settings screen blocks it) ⇒ **registered as debt; a DB-side guard is a design decision, not a night repair.** **Recorded, NOT fixed (four):** the two dead Smart-Match window params (`חלון_חישוב_חודשים`/`_מורחב` — editable, hinted, read by nothing; hide or re-word is Ishay's call) · the server-side ladder above · `updateParams`'s seven tests never assert the query, so deleting `.eq('param_name', …)` from `api.js:93` — making every save an unqualified UPDATE over all 43 rows — leaves them green (only `e2e/settings.spec.js` catches it, and E2E runs in neither `gate` nor CI) · a first-paint flash of the `טעון בירור` badge on `CustomersPage` · and, from the audit's live failure journey, `SaveRow.jsx:35-52` renders the failure message and the `שינית N מתוך M` counter in the same slot, so with more than two rows nothing on screen says the rows after the failed one were never attempted (**§7 — the wording is Ishay's**). **Live journey by the audit session, both PATCHes intercepted, nothing written:** partial-write behaviour is exactly what `useParamsForm.test.js:185`/`:212` describe — written row keeps its value, failed row keeps its typed draft, friendly label in the message, no false toast. **Gate after: exit 0, 83 files / 2165 tests** (was 2158).
- `03/09/2026 01:3X` — **fresh-context UX/UI review (Opus, 5 real logins, passes 2–5) and its fix round.** The review found **two blockers no automated gate could catch**, both measured live: **A1** — the template editor's variable chip APPENDED the token at the end of the body instead of inserting it at the caret, so following the screen's own instruction ("אפשר להחזיר את המשתנה מהרשימה שלמטה") produced `"היי ,"` plus the token glued to the last sentence — **and cleared the block, enabling Save on a corrupt mail body**; **A2** — `validateParamValue` never read the registry's own `min`/`max`/`decimals`, so `אחוז מע"מ = 17.555` and **`סף שביעות רצון = 99`** were accepted while the error message promised otherwise (99 would make every feedback score require a reason). Fixed with the measured proof: chip restores the body **byte-identical** to the original, caret at 14; bounds enforced from the same numbers the message prints, with an `it.each` over every registry row carrying bounds (it fails again the moment the metadata is ignored). Also fixed: B2 (the unknown-token case told the user to restore instead of delete) · B3 (the min-wage list was ~900 px from its field ⇒ 118 px) · B4 (sticky save row; the recruit page is 2389 px tall) · B5 (`ערך` header left was 424.2 vs 315.8 across group tables ⇒ **286.5 on all five**, via one shared `ParamsTableHead`) · B8 (numeric-input left spread 12 px ⇒ **0.00 px**, header-glyph centre vs input centre **delta 0.0**, using a symmetric invisible unit mirror — the third option, after a reserved slot and a centred pair both left a residue) · pass-4 wording (`קבוע ריסון` → `משקל ממוצע-החברה`, `חלון חישוב` → `טווח חישוב`, the search placeholder's "שם השדה במסד", the singular attendance sentence, `לא שינית כלום` for a one-row group). 218 unit tests in the m9 + registry files (was 170). **Recorded, NOT fixed (closing audit):** the unsaved-changes guard · four card recipes in one screen's four tabs (m9 is `p-4`, its neighbours `p-6`/`p-5`) · the blocked-route page offers no way forward for the two roles m9 newly sends there · the §3.7-locked sum-bar sentence can state "מסתכמות ל-1.00" beside an amber 1.50 · two save models (params explicit, profile toggle auto) · `/my-settings` has no search at 25 rows · the 1024 px hint-column reflow · the stale 375 px screenshot. **Reviewer's blind spot, carried:** no save was ever exercised as a FAILURE on screen — the locked `השמירה נכשלה ב"<label>"` line and `נסי שוב` are unit-tested only.
...
- **`09/09/2026 00:1X` — CROSS-MODULE EDIT by the ui-copy night, stage 2 (branch `ishay/ui-copy-rewrite`; module 9 is closed — this is the required ripple record).** ↳ as-built: address alignment to the locked style guide `docs/plans/ui-copy-styleguide.md` §1 — masculine imperative ⇒ direct feminine, plural ⇒ feminine singular, slash forms removed, a person referred to ⇒ feminine, progress states (`שומר...`) ⇒ feminine participle with a single ellipsis (`שומרת…`, B9; R30 precedent: module 8's `מפיקה ושולחת…`). **Files:** `components/SaveRow.jsx` (`שמרי שינויים`, `שומרת…`), `MySettingsPage.jsx` and `ParamsTab.jsx` (the redundant `retryLabel="נסי שוב"` prop removed — the shared `LoadingOrError` default is now `נסי שוב`). Tests: `SaveRow.test.jsx`. The module's owner screen `ProfileSettingsPage.jsx` (in `src/components/`) went through the shared-components FULL reset: `שמרי שינויים`, `עדכני סיסמה`, `נסי שוב` ×2, `שינוי תפקיד נעשה בניהול המשתמשים` (no trailing period, R14), `הסיסמה הנוכחית אינה תואמת` (R18 — no blame word), the notification helper lost its internal module number (B8: `ההתראות יתחילו להישלח בפועל כשמנוע ההתראות יופעל`), `שומרת…`/`מעדכנת…`. The "מצב הטמעה" switch and its Hint are unchanged (stage 0). No business logic, no `<Hint>`, no layer copy (brief rule ⑥); the module's full copy reset is its own later stage. Full CI-parity suite + `npm run smoke` + the affected E2E specs green at the stage-2 commit (see `CLAUDE_CODE_LOG.md`, 08–09/09).

- **`09/09/2026 05:3X` — CROSS-MODULE EDIT by the ui-copy night, stage 8 (branch `ishay/ui-copy-rewrite`; module 9 is closed — this is the required ripple record).** ↳ as-built: the module's FULL copy reset to the locked style guide `docs/plans/ui-copy-styleguide.md` — writers W12c (screens · components · api · `lib/listWindow`; Sonnet, 334K) and W12d (`lib/paramsRegistry.js`; Sonnet, 274K), merged by session 4 from the saved reports. **Screens (W12c):** search placeholder loses `במערכת` (R9) · `GROUP_LEADS.pricing_timing` first sentence (`הערכים שמנוע התמחור וההצעות קורא.`) ⇒ **C-33** (R10/B8 — internal engine name; H1), the consequence sentence stays (R26) — ⚠️ the old sentence was drawn verbatim in the approved mockup `02_params_tab_roles_and_states.html`; mockups are rank 4, the locked guide wins — declared deviation · `ParamRow.jsx` redundant amber line `שורה זו אינה מוכרת למרשם התצוגה.` removed (R27 — the fallback hint one line above already says it; R10 — `מרשם`), and with it the string-equality detection W12d flagged; an unmapped param still renders `הגדרה ללא הגדרת-תצוגה`, so the never-hidden safety net stands. Everything else in the 11 files stands (all test-pinned or already compliant — the module went through F-1…F-9 · R-2 · R-3 · V-4 · V-7 · V-9 · UX-1…6 before tonight). **Registry (W12d):** 17 field edits across 14 params — label `שכר מינימום שעתי` ⇒ `…לדיילת` (B14) · five `מספר השעות … שמתחתיו` hints ⇒ `כמה שעות …` (R30, one family) · parentheses ⇒ em-dash house style ×4 · `(מודול 6)` / `במודול הדיילות` ⇒ screen names (glossary 3ג) · the raw param-name leak in `תבנית_תזכורת_משמרת` (`טריגר: שעות_תזכורת_לדיילת`) removed (R10/B8); 43 `name` keys unchanged (test-locked); `KIND_RULES` · `rangePhrase` · `numericMessage` · `BLANK_NUMERIC_ERROR` untouched (E2E-pinned); 0 hints quote another param's value as a constant (the guide §4 example-3 mine). `lib/listWindow.js`: 0 changes. **Recorded doubts:** `תנאי תשלום` borderline B14 (pinned by two tests) · the generic `שגיאה בטעינת X.` family (75 hits repo-wide) lacks R17's "how to fix" half — R30 forbids a lone-module fix ⇒ **sixth open guide question for stage 9**. **Registers:** C-33 · C-34 (`קבוע_ריסון_m` rationale clause, kept on the base — recorded per "in doubt ⇒ record"). Tests: `ParamRow.test.jsx` (one assertion removed with the line) — screens 144 green · registry 114 green. Gates at the stage-8 commit: CI-parity 98 files / 2,402 tests green · `vite build` ✓ · eslint 0 errors (1 pre-existing warning, `UsersManagementPage.jsx:75`, untouched line) · prettier ✓ on the 40 changed files · smoke green (1 passed — run twice, before and after the login-button revert) on the session dev server (64343) · E2E logistics · settings · auth · permissions · prices · load-failure-guards · server-messages + smoke-dependent accessibility: 71 passed · 1 failed (`prices.spec.js:218` — pre-night: expects `ערך חוקי: מספר שלם חיובי`, the ceilings PR made it a range) · 1 skipped (`public-confirm.spec.js:76`, fixture-dependent); after the revert, `auth` + `accessibility` re-run on a fresh build: 11 passed.
- **`09/09/2026 12:0X` — CROSS-MODULE EDIT by the ui-copy night, stage 9 (branch `ishay/ui-copy-rewrite`; module 9 is closed — this is the required ripple record).** ↳ as-built: the explanation LAYER (level 2, "מודרך") — copy lives ONLY in `src/lib/onboardingCopy.js` (27 keys, one voice: writer W13/Opus from `stage9-layer-proposal.md`, decided by session 4 in `ui-copy-candidates.md` column "הכרעת שלב 9"); the screens gained `<Hint id=…/>` placements only (no base-string changes except the glossary-form alignments named). `<Hint>` renders nothing at level 0 and is provider-safe (renders nothing outside `<AuthProvider>`, `Hint.jsx` 09/09). **Placement:** `ParamsTab.jsx` — `params.pricingTimingGroup` under the group lead, guarded `{activeType === 'pricing_timing' && …}` (C-33; without the word "מנוע" — the candidate row's draft failed R10). Gates at the stage-9 commit: CI-parity 98 files / 2,403 tests green (+1: Hint outside a provider) · `vite build` ✓ · eslint 0 errors (1 pre-existing warning, `UsersManagementPage.jsx:75`) · prettier ✓ on all changed files · smoke green (one red run while the implementers were still editing — `#quote-ratio` not found under HMR — then green twice on a quiet tree) (session dev server 64343) · full `npm run test:e2e`: 167 passed · 7 skipped · 4 failed — 2 pre-night (`customer-page.spec.js:130` · `prices.spec.js:218`) + 2 load timeouts in `accessibility.spec.js` (login-field fill 10s · `route.fetch` to Supabase) while E2E, screenshots and four evaluators ran concurrently; re-run alone on a fresh build: **7/7 passed** · `npm run gate`: every step green except the pre-existing `check:docs-structure` finding (`cards-finance.md:483`, m11 discovery file, not in CI) — a first run under machine load had 3 vitest worker-start timeouts (`Failed to start forks worker`), the re-run on a quieter machine passed 98/2,403 · screenshots of every screen in both states (55 files, S4 scratchpad `evidence9/`) — whole-document pass by eye: done on the `-on` shots after fixing the evidence spec (it intercepted only the write, so every full navigation reloaded level 0 from the DB — all earlier `-on` shots were effectively `-off`; now the GET is intercepted too and returns level 2). Smart Match, repository, hostess card, logistics, projects: the grey teal-bordered paragraphs sit where the proposal put them, one voice across screens; the two stacked hints above the repository table read as one block and stay..
- **`16/09/2026 19:4X` — CROSS-MODULE EDIT by module 11 (branch `ishay/module-11-build`; module 9 is closed — this is the required ripple record).** ↳ as-built, in two halves:
  **(a) Data, not code.** Module 11's migration `20260916043500_module11_c_report_params.sql` inserted **four new `params` rows** — `מכפיל_מרווח_מתרחק`=1.5 · `סף_סטיית_תקציב_אחוז`=15 · `מקדם_אמינות_אדום`=0.87 · `מקדם_אמינות_ענבר`=0.95 — all `param_type='control_alerts'`, all owned by מנכ"ל, plus **four rows in `src/lib/paramsRegistry.js`** (`decimal` 1–5 · `percent` 1–100 · `weight` 0–1 ×2). **`params` 43 ⇒ 47, measured live.** They surface on module 9's own screen with no code change: `ParamsTab` renders whatever the table holds, and the registry gives each row its label, unit, group and "affects" line. **No schema change to `params`, no policy change, no signature change.**
  **(b) Three comments that named "43" — removed, not updated.** `ParamsTab.jsx:2` · `ParamsTab.test.jsx:49` · `e2e/settings.spec.js:5` each said *"43 שורות"*. 🔑 **The house rule applied here is *a number that rotted gets removed, not corrected*:** none of the three arguments depends on the count (the screen edits every row · the fixture deliberately covers the six types rather than the whole table · the E2E does not count rows), the number has now rotted **twice** (43 was itself a later value), and **no gate can see a stale number inside a comment** — `check:declared-counts` reads declared counts in `docs/`, not JSDoc. Each site now says "כל שורות ה-`params`" with a dated one-line note saying what the number was, why it went, and where the live count comes from (`select count(*) from params`). **Regression measured, not assumed: `npx vitest run src/modules/09_settings` ⇒ 10 files / 134 tests green, and `prettier --check` on the three files ⇒ clean** (16/09/2026 20:0X). **No behaviour changed** — the diff is comments and data.
  ⚠️ **And one thing module 11 deliberately did NOT do inside module 9's territory:** card ת6 asked for `סף_לקוח_רדום_ימים` to be given an owner. It was left alone, because assigning `owner_role_id` changes **who may edit an existing row** under `params_update_settings_or_owner` — and live there are **five** owner-less params, not the one the card calls "the only one". Registered as a `🚧 מ11` line in `PROJECT_MASTER §6`, for Ishay.
- **`09/09/2026 13:1X` — CROSS-MODULE EDIT by the ui-copy night, rulings batch 1 (branch `ishay/ui-copy-rewrite`; module is closed — this is the required ripple record).** ↳ as-built (Ishay's rule, 13:0X: *base = mature product; every friendly explanation = the layer*): `MySettingsPage.jsx` empty state `אין הגדרות בבעלות התפקיד שלך` ⇒ `לתפקיד שלך אין הגדרות משלו — כולן מנוהלות בניהול המערכת` (item 9: R10 · R23 · B13; the locked m9 string was Ishay-approved copy — his 13:0X rule covers it; `MySettingsPage.test.jsx` + the smoke anchor `e2e/smoke.spec.js:190` updated in the same commit, smoke re-run) · `paramsRegistry.js` label `תנאי תשלום` ⇒ `תנאי תשלום ללקוח` (item 13, B14; two pinning tests updated). **Closed without change, with the anchor:** item 12 (`↳ משפיע:` rows) — each is ONE sentence with an em-dash (guide §2 clarification: a dash is not a second sentence); the colon belongs to the `↳ משפיע:` prefix = question 1 · item 6 (`שגיאה בטעינת X.` ×40) — the "how to fix" half of R17 is the `נסי שוב` button `LoadingOrError` renders beside every load error (`LoadingOrError.jsx:3`), so the sentence carries only "what happened" by design.
- **`24/09/2026 01:4X` — CROSS-MODULE EDIT by the system-polish session 1 (branch `ishay/system-polish`, plan `docs/plans/2026-09-24-system-polish.md`; module is closed — this is the required ripple record).** ↳ as-built: A5 — `paramLabel(name)` in `src/lib/paramsRegistry.js` is now the single place a parameter's screen name comes from; all eight "missing parameter" messages use it (`getParamValues`, quotes, reports, smartMatch, closingApi, quotePdf, salary dialog). B — hints `settingsSmartMatch.reliabilityOff` (`SmartMatchPane.jsx`) · `templates.chipInsertPoint` (`TemplateEditor.jsx`, editable templates only).
- **`24/09/2026` — fresh-eyes copy fix (deputy, branch `ishay/system-polish-copy`).** ↳ as-built: `SmartMatchPane.jsx` — hint `settingsSmartMatch.reliabilityOff` repeated the "בפועל כרגע" line above the table (*"מרכיב שכבוי יוצא מהחישוב, והנותרים מתחלקים ביניהם"*) on the same screen. It now shows only when that line does not (`!effectiveWeights` — no gap, i.e. reliability on), and says the real weights *"יוצג אז מעל הטבלה"*. Test: `SmartMatchPane.test.jsx` (+1, with an `AuthContext` mock; mode unset = as before).
  ↳ `24/09/2026` — same branch: `TemplateEditor.test.jsx` gets `vi.mock('@/supabaseClient')` — in CI it crashed at load (*"supabaseUrl is required"*, via `Hint` ⇒ `AuthContext` ⇒ `src/supabaseClient.js`); pre-dates this branch (`9c21b26d`, hint `templates.chipInsertPoint`). Test-only.
  ↳ `24/09/2026` — gate-tester finding (deputy): `settingsSmartMatch.reliabilityOff` shows by the reliability switch itself (on), not by the absence of the "בפועל" line — off with weight 0 made no gap and the hint promised what had already happened. Test in `SmartMatchPane.test.jsx`.
  ↳ `25/09/2026 06:4X` — CROSS-MODULE EDIT by builder m12 (branch `ishay/module-12-walkthrough`, deputy task; source `Reg-In-evidence/reports-review-2026-09-25/night/tonight-copy-review.md` 0-5…0-8 + `params-copy-review.md` #22–#26, #29, #31, #34, #35). Copy only, no behaviour change except #34: validation messages open with the action (`ערך חוקי: X` ⇒ `הזיני X`, E2E `settings.spec.js:170` updated) · group error `תקני את המסומן באדום כדי לשמור` (ParamsTab + MySettingsPage) · template blocked/warning lines lead with the action; "אופציונלי" ⇒ "רשות" (one name with the legend) · SaveRow says `אין שינויים` for any zero · labels `אישור שיבוץ סופי`, `מרכיב-האמינות` · hints: quote validity "פגה" (not "נדחית"), satisfaction = the customer's **average**, `קבוע_ריסון_m` gets `unit: תשובות`, min-wage hint names the list by its title (label kept — the 09/09 B14 edit) · new ↳ on `ימי_אזהרה_קדם_אירוע` (dashboard red + quotes "אירועים קרובים") · #34: `BelowMinWageList` passes `onRetry`, `SmartMatchPane` attendance line gets "נסי שוב" (+1 test each). Hints `settingsSmartMatch.weights` / `params.fairnessWhy` corrected (2-3/2-4/2-5).
  ↳ `25/09/2026 07:3X` — same branch, the explainer's check of 39ceebe2 (`night/copy-fixes-check-39ceebe2.md` #3–#6): template warning singular/plural by the count of missing optional tokens ("בלעדיו" / "בלעדיהם", +1 test) · `קבוע_ריסון_m` hint: m is added to every hostess, in both responsiveness and reliability, and shows mostly for few answers · `settingsSmartMatch.weights`: the urgent-event exception only with response times (213 chars) · SaveRow test title.
  ↳ `25/09/2026 15:0X` — **הכרעת-ישי 25/09** (round 2, deputy chat 14:1X: *"סבב 2 … לפי המלצתך"*): the locked `pricing_timing` group sentence "שינוי כאן משפיע על הצעות חדשות בלבד — הצעה שכבר אושרה שומרת את הערכים שהוקפאו בה." (§3.7 / mockup א׳, line 310 above) moves, verbatim, from the group head to the VAT row only (registry `note`, rendered by `ParamRow`). Why: only VAT is frozen in a quote (`quotes.vat_rate_snapshot`); 5 of the 7 rows affect existing quotes too (`params-copy-review.md` נ1). Guest ratio checked — no snapshot column ⇒ not frozen ⇒ no sentence there.
    ↳ Test: `ParamRow.test.jsx` (+1) — the VAT row carries the sentence verbatim, the validity row does not.
