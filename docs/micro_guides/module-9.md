...
| **Active step** | **3.4 (builder D: `MySettingsPage` + Topbar + pane wiring + min-wage preview + the `TemplateEditor` shape fix) in flight** · 2.3/2.4/2.5, 3.1/3.2/3.3/3.5/3.6 ✅ · migrations A–D applied · **next: 2.6+3.7 combined gate — full unit suite + `npm run gate` + live screenshots for Ishay** |
