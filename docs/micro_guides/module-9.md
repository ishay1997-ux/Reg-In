...
- **`09/09/2026 04:1X` — ⏳ PROVISIONAL (replaced at the stage-8 commit) — CROSS-MODULE EDIT by the ui-copy night, stage 8 (branch `ishay/ui-copy-rewrite`; module 9 is closed — this is the required ripple record).** ↳ as-built: the module's FULL copy reset to the locked style guide `docs/plans/ui-copy-styleguide.md` is IN PROGRESS — four Sonnet writers on disjoint file sets across modules 5+9+1 (settings screens · components · api · `lib/listWindow` · `lib/paramsRegistry`), merged by session 3. Final record, files, tests and gates follow at the commit.
