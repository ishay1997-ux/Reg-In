...
| Branch | `ishay/module-4-hostesses` — **exists; never re-create.** ✅ **PUSHED to `origin` for the first time on 12/08/2026 00:45, on Ishay's instruction** *(this row said "nothing on this branch is pushed" until then — it was true for three weeks and stopped being true here)*. ☠️ **MERGED AND DEAD as of 12/08/2026 16:24 — never commit on it again** (iron rule 10). Evidence, same turn: `git merge-base --is-ancestor ishay/module-4-hostesses origin/dev` → **exit 0**, and `git log --merges -1 origin/dev` → **`2b9c277`**. *(This row said "Still NOT merged" from 12/08 00:45 until the merge; the branch still exists on `origin` — deleting it is Ishay's call.)* 🚫 Do not write a number here — it rotted twice already *("ahead 6" was written on 08/08 and was 93 by 09/08 evening)*. **Measure it: `git rev-list --count origin/dev..HEAD`** |
| Owner | ישי (sole developer) |
| Overall status | ✅ **MERGED to dev 12/08/2026 16:24 — PR #24, merge commit `2b9c277`.** Verified with fresh git/gh output the same turn, **not from anyone's say-so**: `gh pr view 24` → `state: MERGED`, `mergedAt 2026-08-12T13:24:34Z`, `baseRefName dev` · `gh pr checks 24` → **all five `pass`** (Lint·Test·Build 1m25s · Edge-function deno 9s · gitleaks 13s · Vercel ×2) · `git merge-base --is-ancestor ishay/module-4-hostesses origin/dev` → **exit 0**. 🤖 **The PR was opened and merged by Claude in the Chrome extension on Ishay's explicit instruction** (*"תתחבר לכרום ובצע את המיזוג"*, then *"מזג"*) — **a conscious departure from his 24/07/2026 ruling** (`module-close/template.md`: *"the merge decision is his alone, never Claude's"*), which was quoted back to him before the click and re-approved. **Prior state, kept:** 🔒 Closed — awaiting PR/merge. ✅ **ישי חתם על ה-DoD בשער-ההקלדה 12/08/2026 11:2X** (`דיילות + Smart Match DoD`, מוקלד במלואו — לא "מאשר"), אחרי שעבר על דף-הדוח וענה על שלוש שאלות-ההבנה. כל שאר האודיט הושלם. **‏12 ממצאים ⇒ 6 סיבות-שורש · 7 חוסמים תוקנו, כולם בסבב אחד · 0 חוסמים פתוחים.** שערים על `067dad4`+: `gate` **exit 0** (**751 unit** / 26 files) · `test:e2e` **117 passed** · `smoke` **exit 0** · advisors **17** (ללא שינוי) · אפס פער-סכמה (22 טבלאות). **UAT רץ חי עם ישי — שני מסעות, שני מיילים אמיתיים שנחתו בתיבתו, ואישור דרך הקישור הציבורי (19מ׳17ש׳).** |
| Last updated | **12/08/2026 16:32** *(system clock)* — merge absorbed by `post-merge` |
| **Active step** | **NONE — the module is shipped. 5.2 (closing audit) is DONE and the PR is merged.** ✅ Verdict `[YES]` released once Ishay typed the DoD echo (12/08/2026 11:2X); **the "waits only on the PR to `dev`" state ended 12/08/2026 16:24 (PR #24, `2b9c277`) — there is no remaining action on Ishay for this module.** 🔴 **One acceptance item still open, and it is NOT a merge blocker — it was blocked BY the merge:** the public shift-approval page has **never been opened from a phone over the internet**, because the route did not exist on the live site until this merge. Vercel rebuilds `dev` on merge ⇒ **this is now testable and should be done.** *(This row still read "verdict withheld pending Ishay's typed DoD echo" after the signature landed — the Overall-status row above had been updated and this one had not; corrected by `regin-docs-sync` 12/08/2026 12:56.)* **The audit record, kept:** 🔬 **The blocker that mattered was proven and re-proven by fault injection, not by reading code:** with the mail path cut at the network layer, *"שחרר"* claimed `והודעה נשלחה אליה` while `email_log` gained **0** rows — and after the fix, the same cut produced *"לא ידוע אם ההודעה יצאה"*. ⚠️ **Two defects the fix round itself introduced were caught by re-scanning its own diff** (a failed auto-release reported nowhere · the permission message losing to the filter branch) — both fixed. 📌 **Verified against `067dad4`; re-check the head before merging.** |
...
- 🔄 **18/08/2026 — `api.js` edited BY MODULE 6 (its step 2.6, the ⑫/M6-14 debt): `OPEN_PROJECT_STATUSES` deleted, `ACTIVE_PROJECT_STATUSES` imported from `@/lib/projects` — and `ready` projects now appear on the recruitment overview.** This is a deliberate live behaviour change, not a regression: the deleted local constant excluded `ready` with a comment claiming it was deliberate, and ⑫ overrules it (a hostess cancelling on a `ready` event tomorrow must stay visible to the recruitment manager; the stale comment was deleted so no reader re-narrows it). **Zero module-4 tests pinned the old behaviour — because `api.js` has no automated coverage at all** (this file's own coverage-debt note). Suite 870/870 unchanged. Owner of record: `docs/micro_guides/module-6.md` step 2.6 + its §10; screen-level before/after verification is owed at module 6's Phase-2 gate (no live `ready` row exists today to photograph).
...
- ✅ **CLOSED 12/08/2026 15:5X — PROVEN IDENTICAL, mechanically, with no model in the loop.** Ishay supplied a `SUPABASE_ACCESS_TOKEN` into `.env.local` (gitignored; the value never entered the chat), and `supabase functions download send-email --use-api --workdir <scratch>` wrote the **live** function straight to disk. Result: **`md5 83cbf46216f8a99a4e829e1ad587056a` on both sides · 187 lines · 12,620 bytes · `diff` clean, zero differences.** The transcription hypothesis is dead: the deployed v4 **is** the repo file. 📌 Verified the operation was non-destructive — the repo file's md5 was identical before and after, and `--workdir` kept the download out of the tree. 🔑 **And the method is now known and cheap** — one command, no model — so any future deploy can be re-verified the same way; this is the mechanical route the entries below say was missing.
  *(The history is kept below, unedited, because the reasoning is the point: it took three passes — 9 markers, then a 187-line model-mediated comparison, then this — and only the last one is proof.)*
- 🔴 **DEBT, restored 12/08/2026 — the deployed `send-email` was never byte-compared to the repo file, and this module's own close owed that check.** Step 0.3 deployed the shared engine through Supabase MCP `deploy_edge_function` → **v4, ACTIVE**, by **transcribing `supabase/functions/send-email/index.ts` into the MCP call**. ⇒ **repo⇄deployment identity is verified behaviourally, not byte-for-byte** — the 7-case gate matrix and the three real emails opened in Gmail prove the paths that were exercised, not that the live body equals the file. The Phase-0 carry-forward said in as many words *"re-check it byte-wise at module close with `get_edge_function`"*; **the close ran and did not** (measured 12/08/2026: `get_edge_function` appears nowhere in the repo outside the module-4 archives). 📌 **Bounded, not alarming:** the repo file's last commit is `70c739e`, **09/08 08:45 — before the deploy** — so nothing has diverged *since*; the only unknown is the transcription itself. ⇒ **owed as a post-merge action, and it is one MCP read.** ⚠️ **And it is not module-4-only: `send-email` is owned by M3 and consumed by M4 · M8 · M11**, so the generalizable half lives in `CLAUDE_CODE_LOG.md` `## Reference: Operational Gotchas` — do not restate it here, both halves must not drift. *(Restored 12/08/2026 by the compaction audit — removed in the 12/08 guide compaction, found nowhere else.)*
  ...
  ✅ **DOWNGRADED AGAIN, 12/08/2026 ~15:4X — as close to closed as a model-mediated route can get, and the remainder is named rather than buried.** A fresh-context agent that authored neither side pulled the live source with `get_edge_function`, wrote it to disk, and compared: **187 lines both sides · 19 blank lines at identical positions · 66 numeric literals in identical order (incl. the exact pin `@2.112.0`) · the three invisible `U+200F` bidi marks at the same three lines · 8,893 characters each side.** The written artifact was then diffed **mechanically, by me, with no model in the loop**: `diff` clean and **md5 identical — `83cbf462…` on both**.
  🔴 **What is still NOT proven, and it is one narrow class:** the agent could not count the dash-runs in the **seven comment separator lines** off the live rendering, so it transcribed those using the repo's run lengths — **for those seven runs the diff is vacuous.** They sit inside `//` comments; a dropped dash cannot change behaviour. ⚠️ **And the structural caveat stands: a transcription that passes through a model can only ever produce a false clean, never a false difference.** True proof still needs `supabase functions download` + `diff`, which needs an access token nobody has supplied.
  📌 **Two things worth keeping from that pass:** the deployed function carries `verify_jwt: true`, **which does not make the in-code gate redundant** — a request bearing the public anon key passes the platform check and still reaches `getUser()`, so the comment calling the code gate load-bearing is accurate. And the API exposes `ezbr_sha256` (`506fdd54…`): it hashes the built bundle, not the source, so it cannot be reproduced locally — **but if any future deploy goes through the CLI, capturing that value gives a cheap mechanical fingerprint for exactly this check.**
...
- **`19/08/2026 14:4X` — CROSS-MODULE EDIT by module 6 (session D consolidation; module 4 is closed — this is the required ripple record).** `npm run gate`'s blocking jscpd step forced two consolidations into `src/modules/04_hostesses/api.js`, both behavior-identical (verified before unifying, suite 1270 green incl. all M4 tests): **①** the private email-template fetcher (`:257-267`) was byte-identical to the shared `getEmailTemplate` born in `src/api/email.js` (M6 session B) — deleted, now imported from there (`api.test.js` gained mock plumbing only, zero assertion changes). **②** a new export `writeInviteToken(...)` — the `invite_token`/`invite_sent_at` assignments-write block that M6's `sendDateChangeReinvites` had duplicated byte-identically; `assignments` is M4's table so the writer lives here and M6 imports it. Callers keep their own divergent rollback semantics (M4 restores `previousSentAt`, M6 nulls) — deliberately unshared. Full detail: `module-6.md` §10 session-D entry.

- **`02/09/2026 22:0X` — CROSS-MODULE EDIT by module 9 (steps 2.3/2.4; this module is closed — this is the required ripple record).** Six hard-coded business constants became `params` rows (Ishay G-2, 02/09; `docs/micro_guides/module-9.md` §6 step 2.3 carries the per-constant ripple table). Touched here: `src/lib/hostesses.js` (the three invite constants deleted; `HOSTESS_PARAM_NAMES` gains `שעות_תוקף_זימון` · `שעות_סף_זימון_לפני_אירוע` · `שעות_אירוע_דחוף`; the invite functions take the threshold), `src/lib/smartMatch.js` (reliability gets its own company average — pays `🚧 מ9 ← מ4/מ6`; hand anchors unchanged), `SmartMatchPage.jsx` · `HostessViewCard.jsx` · `OverviewTab.jsx` · `api.js` (readers pass the loaded values). Behaviour is unchanged while the seeded values equal the old constants (24 · 48 · 72 · 3 · 10 · 7); the module's suites were re-run green by the m9 builder and again in m9's Phase-2 gate.
 ↳ `02/09 23:0X` follow-up by the same m9 builder: `OverviewTab.jsx`'s filter label `דחוף (עד N שעות)` and its two KPI sub-lines (`בתוך N שעות`) now interpolate the loaded thresholds instead of hard-coding 72/24, and `getHostessScreenParams` fails loud on a missing name (it used to return silently).

- **`04/09/2026 00:15` — CROSS-MODULE EDIT by the list-window work (branch `ishay/list-window-pagination`; this module is closed — this is the required ripple record).** Ishay's ruling 04/09/2026 after the M7 seed filled the ledger: every long list gets the same shared time-window + pager (`src/lib/listWindow.js` · `src/components/ListWindow.jsx` — default "3 months" bounding the past only, future rows never hidden, 50 rows per page, "from–to of total" counter, `?window=&page=` in the URL where the surface has one). Touched here: `RepositoryTab.jsx` — pager only (no event date), page in local state because the tab has no URL of its own; `HostessViewCard.jsx` — window + pager on the history table only ("שיבוצים קרובים" stays a short forward list). Business logic untouched; unit tests extended; the E2E invariant "tab counter = rendered rows" became "rows = min(counter, 50) and the pager says מתוך <counter>".

- **`09/09/2026 00:1X` — CROSS-MODULE EDIT by the ui-copy night, stage 2 (branch `ishay/ui-copy-rewrite`; module 4 is closed — this is the required ripple record).** ↳ as-built: address alignment to the locked style guide `docs/plans/ui-copy-styleguide.md` §1 — masculine imperative ⇒ direct feminine, plural ⇒ feminine singular, slash forms removed, a person referred to ⇒ feminine, progress states (`שומר...`) ⇒ feminine participle with a single ellipsis (`שומרת…`, B9; R30 precedent: module 8's `מפיקה ושולחת…`). **Files:** `HostessFormDialog.jsx` (`הסירי`, `הוסיפי טווח`, `שמרי שינויים`/`שמרי דיילת`, `שומרת…`), `OverviewTab.jsx` (`הציגי חסרים בלבד`, `שלחי שוב למי שפג תוקפן (N)`, `שלחי שוב (N)`, `נקי סינון`, the no-permission empty state now ends `יש לפנות למנכ"ל להרשאה` — unified with the shared components), `RepositoryTab.jsx` (aria `פתחי כרטיס דיילת`, `שחררי מהאירועים`, `השביתי — תשלים…` — `תשלים` is 3rd-person feminine about the hostess and stays), `SmartMatchPage.jsx` (`שלחי אישור סופי…`, `שלחי מייל תיאום`, aria `בחרי את …`). The module's action menu lives in `src/lib/assignmentActions.js` — all ten labels feminine now (`שלחי את הקישור שוב`, `פתחי זימון חדש`, `סמני: …`, `אשרי סופית ושלחי פרטים`, `שחררי …`, `בטלי סימון אחראית משמרת`). E2E updated in the same commit: `smart-match.spec.js` (menu-item filters), `hostesses.spec.js` (prose only). Sort captions untouched (stage 4). No business logic, no `<Hint>`, no layer copy (brief rule ⑥); the module's full copy reset is its own later stage. Full CI-parity suite + `npm run smoke` + the affected E2E specs green at the stage-2 commit (see `CLAUDE_CODE_LOG.md`, 08–09/09).

- **`09/09/2026 01:0X` — CROSS-MODULE EDIT by the ui-copy night, stage 4 (cross-module ripple from module 6; module 4 is closed — this is the required ripple record).** ↳ as-built: `OverviewTab.jsx` sort caption `ממוין: חסרים תחילה, לפי קרבת האירוע` ⇒ `ממוין: חסרים תחילה, ובתוכם לפי קרבת האירוע` — the two captions are the same sort rule and Ishay's ruling 07/09 18:5X unified them on module 6's wording (family template `ממוין: ‹מה קודם›, ובתוכם ‹לפי מה›`). No test asserted the old text; `e2e/hostesses.spec.js` runs green in the stage-4 gate. The module's own full copy reset is stage 6.

- **`09/09/2026 03:4X` — CROSS-MODULE EDIT by the ui-copy night, stage 6 (branch `ishay/ui-copy-rewrite`; module 4 is closed — this is the required ripple record).** ↳ as-built: the module's FULL copy reset to the locked style guide `docs/plans/ui-copy-styleguide.md`, three Sonnet writers on disjoint sets, merged by session 3. **overview + repository + api + `lib/hostesses`** (`OverviewTab` · `RepositoryTab` · `api.js` · `lib/hostesses.js`): two-sentence error/empty texts made one sentence (R13); `יש להזין תעריף שעתי` ⇒ `יש להזין שכר שעתי` (R11 — the field is labelled שכר); `לא נמצאו דיילות התואמות לסינון` ⇒ `אין דיילות התואמות לסינון` (R30 with the overview/customers wording); the deactivate dialog keeps the ⑥ (cancellation email, event back to unstaffed) and drops the why-it-is-safe reasoning (⇒ C-22); progress state `משחרר...` ⇒ `משחררת...`. Sort caption byte-identical to module 6. All 7 param-driven numbers interpolated (checked). **card + form + public page + `lib/shift*`** (`HostessViewCard` · `HostessFormDialog`): section notes that repeated the heading removed (R27); `שכבה 2 של Smart Match` ⇒ heading `העדפות של לקוחות`, empty `טרם נרשמו העדפות` (B8; ⇒ C-24, S-13); edit button `ערכי דיילת` (R12); form: R6 — the required-asterisk mechanism removed, the four optional fields say `(לא חובה)`; `${label} הוא שדה חובה` ⇒ `יש למלא ${label}` (grammar + R19); ID placeholder is an example (R5); dialog descriptions cut to one sentence each, the ⑥ (`נכנסת מיידית למאגר בסטטוס "פעילה"`) kept, the phone-first process sentence ⇒ C-23; `Smart Match` ⇒ `שיבוץ חכם` in the rating hint; the has-car hint reads the live distance gate from `params` (was a hard-coded 40). `PublicConfirmPage` · `lib/shiftInvite` · `lib/shiftEmails`: reviewed, unchanged (already feminine, approved copy). **Smart Match** (`SmartMatchPage.jsx`): `run()` failure texts are full sentences per call site (`לא הצלחנו לשלוח את הזימונים.` …; the old `${label} נכשל` disagreed in gender); every confirm dialog carries an action label (R24: `שחררי` · `סמני שביטלה` · `אשרי בכל זאת` · `אשרי ושחררי`); two stray `🚫` glyphs inside live confirm texts removed; `אנחנו ויתרנו עליה` ⇒ system-voice (R7); reliability banner uses the locked `מרכיב-האמינות`; the rounds paragraph ⇒ guide §4 example 3 (⑥ only; reasoning ⇒ C-25); the five-rule empty state ⇒ `בדקי במאגר הדיילות אם יש דיילת שהסינון האוטומטי לא כלל — אף אחת לא עברה אותו לאירוע הזה כרגע.` (⇒ C-26); the `title=` on the `דיילת חדשה` chip removed (H4; hard-coded `3` was `params.minAnswersForScore`; ⇒ C-27). `lib/smartMatch*` · `sortAngles`: reviewed, unchanged. **Merge fixes by the orchestrator:** `אופס`⇒`איפוס` in four code comments · empty state to the feminine imperative (§1) · `e2e/smart-match.spec.js` negative assertion on the old empty-state text retargeted to `sm-candidates-empty` `toHaveCount(0)` · double dash in the edit-mode ID hint. **Tests:** `lib/hostesses.test.js` (2 assertions). **Raise-coupled strings: 0** (accurate scan of 148 migration texts). **Gates at the stage-6 commit:** CI-parity **98 files / 2,402 tests green** · `vite build` ✓ · eslint 0 · prettier ✓ on changed files · smoke green on this session's dev server (64343) · E2E hostesses + smart-match + public-confirm **37 passed · 1 skipped** (missing live fixture, by design). Registers: C-19…C-27 · S-11…S-14. BEFORE/AFTER screenshots of five screens in both onboarding states (session-3 scratchpad `evidence/`).
- **`09/09/2026 12:0X` — CROSS-MODULE EDIT by the ui-copy night, stage 9 (branch `ishay/ui-copy-rewrite`; module 4 is closed — this is the required ripple record).** ↳ as-built: the explanation LAYER (level 2, "מודרך") — copy lives ONLY in `src/lib/onboardingCopy.js` (27 keys, one voice: writer W13/Opus from `stage9-layer-proposal.md`, decided by session 4 in `ui-copy-candidates.md` column "הכרעת שלב 9"); the screens gained `<Hint id=…/>` placements only (no base-string changes except the glossary-form alignments named). `<Hint>` renders nothing at level 0 and is provider-safe (renders nothing outside `<AuthProvider>`, `Hint.jsx` 09/09). **Placements:** `OverviewTab.jsx` — `projects.sort` (the SAME key as module 6, H6; guarded on a non-empty list) · `RepositoryTab.jsx` — `hostesses.noAnswerPill` then `hostesses.quarterEvents` above `repository-table`, and `hostesses.releaseWhy` in the deactivate dialog after `</DialogHeader>` · `HostessViewCard.jsx` — `hostess.clientPreferences` (settled text, under "טרם נרשמו העדפות"; depends on S-13), `hostess.responsivenessRate` (E2) and a second placement of `hostesses.quarterEvents` under the tiles · `HostessFormDialog.jsx` — `hostesses.addFormWhen` after `</DialogHeader>`, guarded `{!isEdit && …}` · `SmartMatchPage.jsx` — `smartMatch.roundsWhy` (guide §4 ex. 3), `smartMatch.emptyGateRules` (empty branch wrapped in a fragment), `smartMatch.newHostessRanking` (once per screen, guarded on `flags.notEnoughAnswers`), `smartMatch.angles` (E1, guarded on a non-empty list). **Glossary alignment (R11/§3ב):** StatTile label `שיעור היענות` ⇒ `שיעור-היענות`; `במרכיב האמינות` ⇒ `במרכיב-האמינות` in the cancel-after-approval sentence — E2E pins only the hyphenated `שיעור-היענות` (`smart-match.spec.js:250`), so nothing moved under a test. No numbers in any sentence: the gate distance, the no-car distance and the min-answers threshold are `params` and are said as "שנקבע בהגדרות"; the 90-day window is a code constant, said as "רבעון מתגלגל". Gates at the stage-9 commit: CI-parity 98 files / 2,403 tests green (+1: Hint outside a provider) · `vite build` ✓ · eslint 0 errors (1 pre-existing warning, `UsersManagementPage.jsx:75`) · prettier ✓ on all changed files · smoke green (one red run while the implementers were still editing — `#quote-ratio` not found under HMR — then green twice on a quiet tree) (session dev server 64343) · full `npm run test:e2e`: 167 passed · 7 skipped · 4 failed — 2 pre-night (`customer-page.spec.js:130` · `prices.spec.js:218`) + 2 load timeouts in `accessibility.spec.js` (login-field fill 10s · `route.fetch` to Supabase) while E2E, screenshots and four evaluators ran concurrently; re-run alone on a fresh build: **7/7 passed** · `npm run gate`: every step green except the pre-existing `check:docs-structure` finding (`cards-finance.md:483`, m11 discovery file, not in CI) — a first run under machine load had 3 vitest worker-start timeouts (`Failed to start forks worker`), the re-run on a quieter machine passed 98/2,403 · screenshots of every screen in both states (55 files, S4 scratchpad `evidence9/`) — whole-document pass by eye: done on the `-on` shots after fixing the evidence spec (it intercepted only the write, so every full navigation reloaded level 0 from the DB — all earlier `-on` shots were effectively `-off`; now the GET is intercepted too and returns level 2). Smart Match, repository, hostess card, logistics, projects: the grey teal-bordered paragraphs sit where the proposal put them, one voice across screens; the two stacked hints above the repository table read as one block and stay..
- **`09/09/2026 12:1X` — CROSS-MODULE EDIT by the ui-copy night, stage 10 (branch `ishay/ui-copy-rewrite`; module is closed — this is the required ripple record).** ↳ as-built: **`HostessFormDialog.jsx`** — the min-wage helper under `שכר שעתי (₪)` rendered the decimal param (35.4) as `35 ₪` (`Money` whole-shekel default) — an evaluator caught the mismatch against the params screen ⇒ `<Money exact />`; the text itself is unchanged (it already reads the live param, no constant). **Recorded, not changed (approved copy):** the six `תווית — הסבר` field labels (§⑥ copy) breach R1 per E-OFF-2 ⇒ ruling in the night report §3 item 8. **Layer fix pending in this stage:** `OverviewTab.jsx` will swap the shared `projects.sort` for a staffing-only `hostesses.overviewSort` — E-ON-1 proved with the screen's numbers (44 vs 48) that "חסר" here excludes logistics (`hostesses.js` ~414). S-21 (ID-field error on a filled field) registered. ➕ 12:3X: the swap landed — `OverviewTab.jsx` now shows `hostesses.overviewSort` (staffing-only definition of 'חסר', logistics counted on the projects screen; W14's text, source `hostesses.js` ~414/~446); `projects.sort` stays m6-only and its group comment in `onboardingCopy.js` says so. `hostess.responsivenessRate` / `hostesses.quarterEvents` now open with the tile's own label (B14 — two evaluators could not tell which number each explained) and lost their first-person plural (R7).
- **`09/09/2026 13:1X` — CROSS-MODULE EDIT by the ui-copy night, rulings batch 1 (branch `ishay/ui-copy-rewrite`; module is closed — this is the required ripple record).** ↳ as-built (Ishay's rule, 13:0X: *base = mature product; every friendly explanation = the layer*): **`HostessFormDialog.jsx` `Field`** — the hint is no longer glued to the label (`label — hint` in one `<label>`) but a helper line under it (R1 · R3; item 8 of the morning file, two evaluators); four hints shortened to one clause (`נבדקת תוך כדי הקלדה` · `לחישוב הקרבה לאירוע` · `דעתך בלבד — לא נכנסת לדירוג` · `לא משפיע על השיבוץ`), the car hint keeps the live gate distance; the reasons moved to the layer — `hostessForm.whatCounts` (S4, W13's voice; facts: `hostesses.js` ~155 — duplicate email warns, the ID blocks the duplicate person, §7.65) placed after `hostesses.addFormWhen`. **`RepositoryTab.jsx`** header `מצב` ⇒ `סטטוס` (item 10). **`SmartMatchPage.jsx`** release toast: two sentences ⇒ one (item 4, R13). No test pinned the old hints.
- **`09/09/2026 16:5X` — CROSS-MODULE EDIT by the ui-copy night, rulings batch 2 (branch `ishay/ui-copy-rewrite`; module is closed — this is the required ripple record).** ↳ as-built (Ishay 16:4X, *"מאשר את כולם"* on items 16 · 17 · 10א · 10ב): `RepositoryTab.jsx` `+ הוספת דיילת` ×2 ⇒ `+ דיילת חדשה` · `HostessFormDialog.jsx` `+ הוסיפי טווח` ⇒ `+ טווח חדש` (item 10ב — one add-button form across the system). No pins (the `accessibility.spec.js` mention is a comment).
- **`24/09/2026 01:4X` — CROSS-MODULE EDIT by the system-polish session 1 (branch `ishay/system-polish`, plan `docs/plans/2026-09-24-system-polish.md`; module is closed — this is the required ripple record).** ↳ as-built: C1 — `HostessViewCard.jsx` "העדפות של לקוחות" reads `customer_hostess_preference` (new `getHostessClientPreferences` in `api.js`); it was a hard-coded `טרם נרשמו העדפות` for every hostess. Customer name from `projects.customer_name` (recruitment is blocked on customers — RLS measured on both tables 24/09); tri-state with its own retry; grouped by type because live data has 25–37 preferences per hostess. Verified live on #438 as recruitment manager and CEO. B — hint `smartMatch.weightsWhy` (reliability on) — **the key the plan assumed existed did not**.
- **`24/09/2026` — CROSS-MODULE EDIT by the system-polish session 2, package 0ב "return to the report" (approved by Ishay 24/09; plan `docs/plans/2026-09-24-system-polish.md` §6).** ↳ as-built: doors from a report (`ReportsPage.jsx` `DOOR_PATHS`) and from the home-screen attention cards carry `?returnTo=<the report's own URL>` / `?returnTo=/`; `src/lib/returnTo.js` `safeReturnTo` accepts only `/reports…` or `/` (anything else is ignored — no open redirect; test-first, 13 cases); `src/components/ReturnToLink.jsx` shows `חזרה לדוח` / `חזרה למסך הבית`. Closing navigates by push, so the browser Back button still works. e2e `reports.spec.js` "0ב" ×3 (quote dialog · hostess card · project card) green. In this module: `HostessesPage` closing the card returns to `returnTo` (edit from the card stays here — it used to call `closeCard`, which would now navigate away before the form opened); `ReturnToLink` above the title.
...
- **`24/09/2026` — fresh-eyes copy fix (deputy, branch `ishay/system-polish-copy`): hint `smartMatch.weightsWhy` removed.** ↳ It said the first on the list is the one with the highest weighted score. False on the screen: `SmartMatchPage.jsx` renders `sortByAngle(ranked…, activeAngle)` and no comparator in `src/lib/sortAngles.js` reads the score (default angle `proximity`, `defaultSortAngle`); who enters the list is `passesGate`, not the score. The score stays hidden and is used only as the saved recommendation rank (`ranks`, report 14א). The deputy's alternative wording ("הציון קובע מי נכנסה לרשימה") was false too, so the deputy's other option was taken: delete. `smartMatch.angles` above it already says the angles set the order. Key removed from `src/lib/onboardingCopy.js`; no test cited it (`onboardingCopy.budget.test.js` scans cited keys — green).
  ↳ `24/09/2026` — gate-tester finding (deputy): `smartMatch.newHostessRanking` no longer says "מדורגת כמו דיילת ממוצעת" — the score is hidden and the order is the chosen angle's (`sortAngles.js`), the same class as the removed `weightsWhy`. Now: "הסדר ברשימה נקבע לפי הזווית שבחרת, לא לפי הצ'יפ הזה."
- **`25/09/2026 02:0X` — CROSS-MODULE EDIT by the smart-match fix (branch `ishay/smartmatch-score-order`, worker for the deputy; module is closed — this is the required ripple record).** ↳ **Ruling (Ishay, 25/09 ~00:5X, *"ניתוח טוב מאשר הכל לפי המלצתך"*, on the audit `Reg-In-evidence/reports-review-2026-09-25/night/smart-match-audit.md`):** the default order on screen 2 is **"המלצת המערכת"** — a fifth lens that sorts by the score (layer 4 itself), first button in the row; the four angles stay as click-alternatives; "מצוינת אצל הלקוח" stays pinned in every lens. Until then no comparator read `score`, so responsiveness/reliability/leverage moved nobody on screen and the spec's anchor (§3.2, Noa > Michal > Dana) rendered **reversed**. "תענה הכי מהר" stays the default under 72 hours — Ishay's 30/07 ruling (`spec.md` §four angles, `§7.15` round 6), which the 25/09 ruling did not touch (deputy, checked against the transcript).
  **Commits:** `d4b4627c` (lens + window + median + event-date leverage + screen test) · `1bf0e526` (under-72h basis, combined wording, under-72h screen tests) · `eaa6c1ab` (reviewer findings: "worked" as of the event date, Israel-local today via `todayIsoInJerusalem`) · plus this ripple commit.
  **Interpretations taken (spec-silent; review them here, the code is `src/lib/smartMatchCandidates.js`):** ① the 12→24 window is measured **back from today**, by each row's event date — anchoring it on the event date would make `C` differ between events (`§11.3`: *"C is a property of the company"*); a row without a date stays in. ② The widening threshold is the param `מינימום_תשובות_להצגת_ציון` (§11.1 names "3" for both). ③ One window per hostess, chosen by her answers, applies to responsiveness, reliability **and** response time; `C` (both) on the base window. ④ Leverage "worked" = non-cancelled final approval dated **before the event date** (including future bookings); the chip `עבדה לאחרונה לפני N שבועות` stays as of today, past events only. (Its side effect is listed under the known limitations below.) ⑤ "תענה הכי מהר" = **median** hours `responded_at − invite_sent_at` in her window, `null` (last in that lens) below the threshold; the button is enabled only when **this event's candidates** have one.
  **Known limitation (not fixed — needs a schema change or touching invite validity):** `שלח שוב` refreshes `invite_sent_at` on the same row (`api.js` resend paths) and `responded_at` is not reset, so a response after a resend is measured **from the reminder**, not from the first invite ⇒ the median can look faster than she is. Manual marking after a phone call writes no `responded_at` (`markAssignmentStatus`), so only link answers are timed.
  **Known limitation — a booking gives a never-worked hostess a small bonus (deputy's ruling 25/09: no code change):** a hostess who never worked but has a final approval before the event counts as "worked 3 weeks before" ⇒ ×1.06 (1620: Shelly Elkayam), while a new hostess with no booking gets ×1.00. **Why kept:** the gap is small; it follows from Ishay's own spec rule "never worked ⇒ 0 weeks" (`spec.md` layer 4 (ב)); a new hostess is already flagged by the `דיילת חדשה` chip. **Rejected:** a special code path for 6%, and changing "never worked" — that rule is Ishay's.
  **Mockup not redrawn:** `02_smartmatch_approved.html` still draws "קרבה" as default and no fifth button — it is a visual reference only; `screens-approved.md` §⑥ carries the dated note. Settings: the three weight rows' `affects` got the caveat "(באירוע דחוף המסך נפתח על 'תענה הכי מהר')"; the two window hints no longer say "not used yet". Hint `smartMatch.weightsWhy` stays removed (copy decision).
  ↳ `25/09/2026 06:4X` — CROSS-MODULE EDIT by builder m12 (`ishay/module-12-walkthrough`, deputy task; `tonight-copy-review.md` 2-1/2-2): the hint `smartMatch.recommended` shows only when the active lens is "המלצת המערכת" (under 72h the screen opens on "תענה הכי מהר" and the top of the list is not the recommendation), and its text says "מגיעה בפועל" only while the reliability component is on. Test: `SmartMatchPage.order.test.jsx` (+1).
  ↳ `25/09/2026 14:2X` — builder m12 (deputy task, Ishay 14:0X "מאשר הכל לפי המלצתך"; on Inbar: *"נגיד יומיים לפני היא חולה או משו... עשיתי שהמערכת תדע להתמודד עם כל המקרי קצה"*): `approvedButUnavailable` in `src/lib/hostesses.js` — an active assignment (`finally_approved` / `confirmed_available`) whose declared unavailability covers `final_event_date`, event not past — **the gate's own `isUnavailableOn`, not a copy.** Tag "סימנה אי-זמינות" (warn) in the hostess card's assignment table; mode-2 hint `team.unavailableConflict`. Measured (SQL mirror of the rule, 25/09): 4 future cases — 1598 רוני חדד · 1610 אורית ביטון (range starts on the event day) · 1617 + 1620 ענבר שפירא; 8 past cases get no tag. Scale: a tag only, no alert, no flow.
  ↳ `25/09/2026 15:5X` — three error strings say "השיבוץ החכם", not "Smart Match" (`smartMatch.js` ×2 · `smartMatchCandidates.js`); wording otherwise unchanged.
