...
- **`02/09/2026 22:0X` — CROSS-MODULE EDIT by module 9 (steps 2.3/2.4; this module is closed — this is the required ripple record).** Six hard-coded business constants became `params` rows (Ishay G-2, 02/09; `docs/micro_guides/module-9.md` §6 step 2.3 carries the per-constant ripple table). Touched here: `record_feedback` / `archive_project` re-created from their live bodies to read `סף_שביעות_רצון` from `params` (migration `20260902211551_module9_c_…`; `< 3` ⇒ `< v_threshold`, messages interpolate the value); `ClosingWindowDialog.jsx` / `FinancePage.jsx` client gates and the two Hebrew "ל-3" strings read the same row via `src/api/params.js`. Behaviour is unchanged while the seeded values equal the old constants (24 · 48 · 72 · 3 · 10 · 7); the module's suites were re-run green by the m9 builder and again in m9's Phase-2 gate.

- **`04/09/2026 00:15` — CROSS-MODULE EDIT by the list-window work (branch `ishay/list-window-pagination`; this module is closed — this is the required ripple record).** Ishay's ruling 04/09/2026 after the M7 seed filled the ledger: every long list gets the same shared time-window + pager (`src/lib/listWindow.js` · `src/components/ListWindow.jsx` — default "3 months" bounding the past only, future rows never hidden, 50 rows per page, "from–to of total" counter, `?window=&page=` in the URL where the surface has one). Touched here: `FinancePage.jsx` — window chips (on `final_event_date`) applied to every S1 tab, pager under the table; "finished" (701 rows) now opens on the last 3 months by default. Business logic untouched; unit tests extended; the E2E invariant "tab counter = rendered rows" became "rows = min(counter, 50) and the pager says מתוך <counter>".
  ↳ `04/09/2026 00:22` — **deviation, stated on purpose:** `get_finance_overview` returns no `final_event_date`, so on this screen the window runs on the tab's own money date (`invoice_sent_at` / `operationally_closed_at` / `cancelled_at` / `archived_at`) — "closed in the last 3 months" — via the existing `rowDateIso(row, tab)`. Tiles/summary strip stay global (operational alerts, not a list). Reported to Ishay in the same session.
  ↳ `04/09/2026 01:01` — `ScoreCell` moved out of `FinancePage.jsx` into `src/components/ScoreCell.jsx` (shared with the customer card, unchanged markup) — module-8 behaviour untouched; its tests still pass.

- **`09/09/2026 00:1X` — CROSS-MODULE EDIT by the ui-copy night, stage 2 (branch `ishay/ui-copy-rewrite`; module 8 is closed — this is the required ripple record).** ↳ as-built: address alignment to the locked style guide `docs/plans/ui-copy-styleguide.md` §1 — masculine imperative ⇒ direct feminine, plural ⇒ feminine singular, slash forms removed, a person referred to ⇒ feminine, progress states (`שומר...`) ⇒ feminine participle with a single ellipsis (`שומרת…`, B9; R30 precedent: module 8's `מפיקה ושולחת…`). **Files:** `ClosingWindowDialog.jsx` (`שמרי דמי-ביטול`, `שמרי ושלחי`, `שמרי תשלום`, `שני ציון`, `שמרי סטטוס`, `העבירי לארכיון` ×2, progress states `שומרת…`/`שולחת…`/`מארכבת…`; `סגור ללא תשלום` is a locked glossary term and stays), `SalaryReportDialog.jsx` (`ייצאי ושלחי` ×4 incl. the sentence quoting it, `שלחי שוב`, and the redundant `retryLabel="נסי שוב"` prop removed — the shared default is now `נסי שוב`), `PublicFeedbackPage.jsx` (customer-facing public page ⇒ gender-neutral: `שלח` ⇒ `שליחה`; `נסו שוב` stays as the page's neutral plural). Tests: `ClosingWindowDialog.test.jsx`, `SalaryReportDialog.test.jsx`. E2E: no assertion touched (all by `data-testid`). No business logic, no `<Hint>`, no layer copy (brief rule ⑥); the module's full copy reset is its own later stage. Full CI-parity suite + `npm run smoke` + the affected E2E specs green at the stage-2 commit (see `CLAUDE_CODE_LOG.md`, 08–09/09).

- **`09/09/2026 01:2X` — CROSS-MODULE EDIT by the ui-copy night, stage 5 (branch `ishay/ui-copy-rewrite`; module 8 is closed — this is the required ripple record).** ↳ as-built: the module's FULL copy reset to the locked style guide, three Sonnet writers on disjoint groups, merged by session 2. **overview + public feedback + api** (`FinancePage` · `api.js` · `publicApi.js` · `PublicFeedbackPage` · `lib/projectFinance.js` · `lib/feedback.js`): three `api.js` error sentences made one-sentence and action-first (R13/B13/R17); the on-screen invoice-file note kept byte-identical to its migration `raise` **minus the `(כרטיס-P1)` suffix**, which is not in the migration and is an internal card reference (UC30); the customer-facing feedback page keeps its gender-neutral plural voice; the finance page's salary button unified with the dialog title `הפקת דוח שכר דיילות` (R30). **closing window** (`ClosingWindowDialog.jsx`, H2 per sentence — four sentences left the base, each with a written "can she close correctly without it? yes"): `FEE_STORAGE_NOTE` deleted (guide §4 example 2 ⇒ C-02), `CANCEL_ACTIONS_NOTE` deleted (③ ⇒ C-03/C-16; every consequence is repeated in the confirm dialogs), the `title=` hover texts on `סגור ללא תשלום`/`ויתור`/invoice-send removed (research §3 failures 3+4 — duplicates of visible text), the invoice banner reordered action-first in the feminine imperative (B13/§1), `MetaStrip` colons dropped (R2), `במסד` removed (B8), placeholders `לדוגמה:` (R5), busy state `סוגרת…`, confirm label `סגור ללא תשלום` (locked term; `אישור…` fails R12), `עדכני תאריך תשלום` (R12). Every confirm message, gate note, `NO_BILLING_EMAIL_NOTE`/`BILLING_UNKNOWN_NOTE` and the locked terms stand. **salary report** (`SalaryReportDialog` · `lib/salaryReport.js`): B14 on every heading (`הפקת דוח שכר דיילות חודשי`, `היסטוריית דוחות שכר דיילות`), `אישור` ⇒ `בחרי` and `צפייה…` ⇒ `צפי…` (R12), the internal codes `(N-4)` and `ה24/ה29` removed from user-visible text (UC30 — the module's own rule), `במסד`/`תנאי-האיסוף` jargon removed, the formula and the "why no preview" reason moved to candidates (④/⑤ ⇒ C-14/C-15), the hover-only `אין קובץ שמור — לא ניתן לשלוח שוב` made a visible caption with `data-testid="salary-history-no-file-*"` (UC39/R26); the ⑥ irreversibility sentence (`הלחיצה על "ייצאי ושלחי" אינה הפיכה…`) is UNCHANGED. **Untouched by rule:** the salary RPC `raise` texts; `src/lib/projectClosing.js` (module 6's operational closing — wrongly listed in the writer's task file, correctly left alone). Gates at the stage-5 commit: CI-parity מלא 98 קבצים / 2,402 בדיקות — 2,401 ירוקות + 1 שנפלה על timeout של 5 שניות תחת עומס (`CustomerFormDialog.test.jsx`, לא של מ8) ועברה לבדה 4/4 · vite build · eslint · prettier · npm run smoke: **נפל פעמיים על שרת-5173 המשותף** (timeout כללי; לחיצה שנתקעה כשהמסך תקין — דפוס ה-HMR של `e2e/CLAUDE.md`) **וירוק על שרת-פיתוח טרי של הסשן (64342, 1.4 דק') מאותו עץ** ⇒ סביבה, לא קוד · E2E finance + public-feedback 8/8. Structural finding S-10 (generate button's disabled reason only in `title=` while history loads). **Open guide-level questions raised by two writers (for stage 9, not decided here):** inline `label: value` metadata chips vs R2 · noun-form "open dialog" buttons (`+ הוספת X`, `הפקת…`) vs R12 · `— חובה` marking on conditionally-required fields vs R6.
- **`09/09/2026 12:0X` — CROSS-MODULE EDIT by the ui-copy night, stage 9 (branch `ishay/ui-copy-rewrite`; module 8 is closed — this is the required ripple record).** ↳ as-built: the explanation LAYER (level 2, "מודרך") — copy lives ONLY in `src/lib/onboardingCopy.js` (27 keys, one voice: writer W13/Opus from `stage9-layer-proposal.md`, decided by session 4 in `ui-copy-candidates.md` column "הכרעת שלב 9"); the screens gained `<Hint id=…/>` placements only (no base-string changes except the glossary-form alignments named). `<Hint>` renders nothing at level 0 and is provider-safe (renders nothing outside `<AuthProvider>`, `Hint.jsx` 09/09). **Placements:** `ClosingWindowDialog.jsx` — `closing.frozenAmount` where `FEE_STORAGE_NOTE` used to be (guide §4 ex. 2) · `closing.terms` in `FeeActions` under the three buttons (the FIRST `closing-write-off-open`, not the payment block; C-03/C-16 — ויתור · סגור ללא תשלום · חוב-אבוד, accrual said in plain words) · `closing.cancelledProfit` in `CancelledProfitBlock` (C-18, Q-3 in words, negative is normal) · `SalaryReportDialog.jsx` — `salary.formula` in the pre-flight panel (C-14). Tests: unchanged (already mock AuthContext or do not render these paths). Gates at the stage-9 commit: CI-parity 98 files / 2,403 tests green (+1: Hint outside a provider) · `vite build` ✓ · eslint 0 errors (1 pre-existing warning, `UsersManagementPage.jsx:75`) · prettier ✓ on all changed files · smoke green (one red run while the implementers were still editing — `#quote-ratio` not found under HMR — then green twice on a quiet tree) (session dev server 64343) · full `npm run test:e2e`: 167 passed · 7 skipped · 4 failed — 2 pre-night (`customer-page.spec.js:130` · `prices.spec.js:218`) + 2 load timeouts in `accessibility.spec.js` (login-field fill 10s · `route.fetch` to Supabase) while E2E, screenshots and four evaluators ran concurrently; re-run alone on a fresh build: **7/7 passed** · `npm run gate`: every step green except the pre-existing `check:docs-structure` finding (`cards-finance.md:483`, m11 discovery file, not in CI) — a first run under machine load had 3 vitest worker-start timeouts (`Failed to start forks worker`), the re-run on a quieter machine passed 98/2,403 · screenshots of every screen in both states (55 files, S4 scratchpad `evidence9/`) — whole-document pass by eye: done on the `-on` shots after fixing the evidence spec (it intercepted only the write, so every full navigation reloaded level 0 from the DB — all earlier `-on` shots were effectively `-off`; now the GET is intercepted too and returns level 2). Smart Match, repository, hostess card, logistics, projects: the grey teal-bordered paragraphs sit where the proposal put them, one voice across screens; the two stacked hints above the repository table read as one block and stay..
- **`09/09/2026 12:3X` — CROSS-MODULE EDIT by the ui-copy night, stage 10 (branch `ishay/ui-copy-rewrite`; module is closed — this is the required ripple record).** ↳ as-built: `<Hint id="finance.amberRows" />` placed in `FinancePage.jsx` between `<SummaryTiles>` and the `<Card>` — two evaluators: amber is explained in logistics and silent in finance, and 'לא כולל N דמי-ביטול שטרם נקבעו' has no explanation. Text by W14 (source `isActionable` ~954–1001 and its comment · `SUMMARY_OPEN_UNKNOWN` ~100 · ~329–385): the three amber triggers, informative-only/never locks, archived rows never amber, and why the open sum is partial. Opens with the same shape as `logistics.amberWhy` (R30) but a separate key (H6 — different triggers). Also `closing.terms` lost its first-person plural (R7: 'אנחנו' only when admitting a failure — E-ON-2). Gates: at the stage-10 commit — see `CLAUDE_CODE_LOG.md`.
- **`23/09/2026 11:XX` — CROSS-MODULE EDIT by the m11 export session (branch `ishay/export-build`; module is closed — this is the required ripple record).** ↳ as-built: `FinancePage.jsx` wired to the generic export dialog (`src/components/ExportDialog.jsx`) — the third screen outside module 11 and the first with **three core sets**, one per tab (`FINANCE_EXPORT_CORE`), because `COLUMNS` differs per tab and Ishay's 23/09 rule is *"default = what's on screen"*. Rows handed to the dialog are the `entries` of `prepareRows` (filtered → windowed → sorted, never the page), so every column has a `value` that reads the screen's own derivation (`amount` · `amountWithVat` · `dueDate` · `daysOverdue` gated by `debtOpen` · `profitPercent` · `scoreTagText`). Timestamps go through `stampDate` (Israel time) as text because the engine's `formatIsraelDate` rejects timestamps by design. The VAT rate is fetched separately ⇒ `vatState` (`loading`/`ready`/`failed`) keeps the dialog loading, or blocks it with `EXPORT_BLOCKED_VAT` **plus a "נסי שוב" inside the modal** (`error` + `onRetry`). **Two things the Opus adversarial reviewer (read-only, `שומש כפי שהוא`) changed before commit:** ① the on-screen tags — cancel context under the project name, "הסתיים — לא שולם", "נדרשת חשבונית זיכוי" — are now core in every tab (`TAG_CORE`), otherwise a cancelled row whose fee was set read as ordinary revenue · ② tab ① exports **one** date + "מהות התאריך" (the screen shows one, with a sub-line saying which), not two half-empty columns. 📊 Live on 5199 as CEO: tab ② 26 rows / 13 default columns (26 available); tab ③ 48 / 10; tab ① 9 / 10; the downloaded xlsx = 27 rows incl. header, 26/26 amount cells filled; first row equal to the screen's (59 days, 2,377 ₪, 2,804.86 incl. VAT); 0 page errors. **Left to Ishay, not changed:** a null `date`-format cell exports as `—` while a null number exports blank (engine behaviour pinned by `reportsExport.test.js:178`) · `percent` renders `69.0` where the screen says `69%` · blocking the whole export on a VAT failure even when "כולל מע"מ" is unticked. Tests: `FinancePage.export.test.jsx` (9, new). 🚫 Not in scope: `RepositoryTab` (frozen on the `hourly_rate` migration debt).
- **`24/09/2026 01:4X` — CROSS-MODULE EDIT by the system-polish session 1 (branch `ishay/system-polish`, plan `docs/plans/2026-09-24-system-polish.md`; module is closed — this is the required ripple record).** ↳ as-built: A3 — `FinancePage.jsx` `למסנן שבחרת` like the sibling pages (no period added — R14). A7 — the four `סגור ללא תשלום` **buttons** in `ClosingWindowDialog.jsx` → `סגרי ללא תשלום` (button = verb, tag = name; glossary row updated in `ui-copy-styleguide.md` §3ב). A5 — `SalaryReportDialog.jsx` shows parameter labels, not column names. B — hint `finance.paidWhileAwaiting` on the "ממתין לתשלום" tab.
...
- **`24/09/2026` — fresh-eyes copy fix (deputy, branch `ishay/system-polish-copy`).** ↳ as-built: `SalaryReportDialog.jsx` travel note — *"לפי הפרמטר החי 'סכום נסיעות למשמרת' נכון לרגע ההפקה"* (jargon) ⇒ *"נסיעות מחושבות לפי 'סכום נסיעות למשמרת' שבהגדרות, כפי שהיה ברגע ההפקה — הסכום עדיין טעון אימות מול רואה-החשבון."* (the label is the settings row name, `paramsRegistry.js`). No test cited the sentence.
