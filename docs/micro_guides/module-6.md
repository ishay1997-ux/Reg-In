...
| **Active step** | **2.7 + 2.8 — dispatching via build agents (session C).** 2.1–2.6 ✅ all done `18/08`, suite at **879** from Phase-2 work (+13 in-flight from the M4-coverage agent Ishay requested = 892 measured). **Owed at the 2.9 gate:** the ⑫ screen before/after · the `SERVER_CONSTRAINT_RULES` question from 2.5's contradiction ② (§10 entry C) · the M4-coverage agent's findings triage. |
