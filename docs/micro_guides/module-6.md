...
| **Active step** | **2.2 — `src/lib/projectChanges.js`, awaiting its 🗣️ unit brief.** 2.1 ✅ done `18/08` (789 tests green, zero regressions). ⏳ **One nod owed by Ishay from 2.1:** the logistics-only gap sentence `הלוגיסטיקה טרם מוכנה` (§10, `הנחתי` ②). |
...
| **2.1** | `src/lib/projects.js` — status label + tone map, readiness metrics, gap sentences, active-status SSOT | ✅ done `18/08/2026 23:1X` — test-first (37 red→green) · full suite **789 = 752+37**, zero regressions · anchors `1/6`·`0/1`·zero-logistics-complete reproduced · 3 filled gaps logged §10 (one pending Ishay's nod: the logistics-only gap sentence) |
...
- **`הנחתי` ② — the logistics-only-gap sentence `הלוגיסטיקה טרם מוכנה`, and this one is product-flavoured: PENDING Ishay's nod.** When staffing is complete and only logistics is open, no approved sentence exists anywhere I searched (same three sources). **No live row produces this state today**, so nothing on the 28/08 demo can show it — raised in the completion report as a one-line nod, not silently shipped as fact.
