...
(a) Every step transition updates the status header + step table in the same session, before moving on. (b) Any deviation gets an inline "↳ as-built" note on the step + a line in §9. (c) The repo's Stop hook (`.claude/hooks/check-docs-updated.sh`) blocks session end if module code under `src/modules/03_*/` changed but this guide didn't — keep it current, not as an afterthought. (d) End-of-session protocol in `CLAUDE.md` applies (CHANGELOG → CLAUDE_CODE_LOG → STATUS). (e)–(g): per CLAUDE.md iron rules 13/15/16 + end-of-session protocol (new §7 questions → presented in Ishay's question style and registered, never self-answered; migrations/DB gaps ⇒ db_roadmap same session; schema/shared-surface changes name the FUTURE modules they land on in the CHANGELOG line). (h) On ENTERING a phase: sweep this Ledger for OPEN/nod-pending items anchored to this phase's steps and present them to Ishay for a consolidated ruling (P13 style) BEFORE the phase's first step — as of 23/07 **0 OPEN items remain** (LOCAL-6 ruled 23/07: notes block after totals, before terms).

### 9. 📝 Deviations & Tech-Debt Log
- 🔴 **AT M3 CLOSE — harden the quality gates (deferred by Ishay's ruling 23/07/2026 22:35):** the code-quality tooling (jscpd `npm run dup` + `eslint-plugin-sonarjs` in `eslint.config.js`) was introduced in **warn-only / non-blocking** mode so it wouldn't block M3's existing code mid-build. **When this module closes, flip them to blocking:** (1) in `eslint.config.js`, change the `sonarjs/*` rule levels from `'warn'` to `'error'`; (2) in `.github/workflows/ci.yml`, remove `continue-on-error: true` from the "Duplication scan (jscpd)" step; (3) first run `npm run lint` + `npm run dup`, resolve or explicitly waive each existing finding (module 2 already shows 3 cognitive-complexity warnings + 1 jscpd clone between `UsersManagementPage.jsx`↔`CustomersPage.jsx`), so the newly-blocking gate starts green. Tracked also in STATUS reminders + `PROJECT_MASTER §6`.
