...
| Status | ✅ **MERGED to `dev` 05/08/2026 08:03 — PR #10, merge commit `792456f`.** *(Re-verified 12/08/2026 13:5X by `regin-docs-sync`: `git merge-base --is-ancestor 792456f origin/dev` → true. **This row said "CLOSED — awaiting PR/merge" for a week after the merge**: `post-merge` flips `STATUS.md`'s row but never the micro-guide's own header. Same drift found on `module-2.md`.)* **The close itself, kept:** 🔒 **CLOSED 05/08/2026 01:01.** Step 5.4 (closing audit) ran in a FRESH session and re-verified everything independently rather than trusting the 04/08 fix round's report: `npm run gate` **exit 0 on all 8 stages** · **410 unit** · **78/78 E2E green in one run, 0 skips, 5.9m** · **zero DB delta** (10/24/3/3 read before AND after the run) · policies (quotes 2 · quote_services 2 · catalog 3×2 · `email_log` SELECT-only) · both lock triggers (`BEFORE DELETE OR UPDATE`) · 2 active pg_cron jobs · seed 11/40/20 · 10 `module3_` migrations · advisors **0 ERROR** · 6,319 ₪ reconstructed from the live rows. **Verdict [YES]; DoD typed-echo signed by Ishay 05/08/2026 01:0X (`מודול 3 DoD`).** **All DoD boxes are now closed** — the last three were closed by Ishay himself that night: mail arrival + attachment ("נשלחה נראה מעולה") · Make scenario ("עובד") · the 🎨 UX pass, which he asked for and which ran with **0 findings**. *(previous entry: 🔨 Phase 4 CLOSED — gate 4.5 SIGNED by Ishay 01/08/2026 ~11:00 ("מאשר שער 4"), after his own 3-click live-eye pass: rejected-quote display ("עובד") · approved-quote lock ("עובד") · customer-page data flow ("עובד", incl. the open-quotes-value and avg-deal metrics he pointed out). Phase 5: 5.1+5.2+5.3 CLOSED. The 5.4 audit RAN (verdict [NO]-near-YES) — fix round in progress (item 1 of 3 landed, `ed2ca89`); remaining: items 2-3 → gate+E2E rerun → DoD typed-echo → PR.** Phase 1+2+3 closed (see done-tables below).)* |
...
| 🩹 **Post-close fix, 21/08/2026** | The module is closed and merged; the step table below is a historical snapshot and was **not** re-opened for this. Branch `ishay/quote-time-bidi-fix`, not yet committed: (1) BiDi fix on the "משך האירוע" output in `QuoteBuilderPage.jsx` (`dir="ltr"` on a Hebrew-majority string was reversing "8 שעות" → "שעות 8" — measured with `Range.getBoundingClientRect()`, fixed, re-measured correct); (2) `LtrFieldGroup.jsx`'s native time-input picker icon centering (CSS-only, not yet eyeballed live by Ishay). Full account: `docs/CLAUDE_CODE_LOG.md` (21/08/2026 entry). |
...
| 5.4 | Closing audit + PR (FRESH session, final-test template, DoD typed-echo) 🔻👤 | ✅ **CLOSED 05/08/2026 01:01 — verdict [YES], typed-echo signed (`מודול 3 DoD`).** Everything load-bearing was re-run in this session, not taken from the 04/08 report: gate exit 0 (8 stages) · 410 unit · **78/78 E2E, 0 skips** · **DB read back identical before/after** (10/24/3/3) · live policy/trigger/cron/seed reads · advisors 0 ERROR. **Assertion audit of the 4 files `ae78851` touched: 138 assertions examined, 2 green-and-empty found** (`quote-email.spec.js:249` is implied by the line above it; `customer-page.spec.js:91` asserts a 0 that deny-all RLS guarantees), plus 3 weak-but-failable matchers. **Every other spec swept for stale row-ids — 0 found** (#10 approved · #11 rejected · #7 in_progress · 9001-3 synthetic, all still meet their preconditions, verified live). The fix round's own worry — "I caught two green-and-empty assertions by accident, there may be more" — was therefore answered with a number. 🎨 UX pass run on 5 live screens at Ishay's request: 0px overflow ×5 · 124/124 focus stops with a visible ring · 54 money cells consistent · error state explains-not-blames · **0 findings**. ⚠️ Declared boundaries, not silently passed over: `gitleaks` not run locally (not installed; it is a **blocking** CI job at `ci.yml:103`) · `smoke.spec.js` excluded by `--grep-invert` · CI does not run E2E · 390px narrow layout out of scope per Ishay 31/07 (M12 sweep). |
...
| Performance | Index C-6 used by expiry scan (EXPLAIN evidence); no further targets (internal tool) | ✅ **for the one target that was set.** C-6 evidenced in Phase 1. No load testing — deliberate, and the reason is the honest one: this is an internal tool whose whole dataset is 10 quotes and 4 customers, so a load number would be theatre. Re-open only if a real dataset makes a screen slow. |
| Usability | Filled from step 3.7 🎨 review + closing UX audit | ✅ **both halves done.** 3.7 (31/07): 2 sweeps, 4 findings ruled by Ishay, all 4 resolved. Closing audit (05/08, at his explicit request): 5 live screens — **0px horizontal overflow on all five · 124/124 keyboard stops with a visible focus ring · 54 money cells with ₪ on the same side · forced-failure state explains and offers retry instead of blaming the user · 0 unforced console errors · 0 findings.** ⏸️ 390px narrow layout measured (444px of scroll) and **excluded by Ishay's 31/07 ruling**, not by omission. |
| Compatibility | Chromium only now; cross-browser sweep = pre-M5 (§6 ruling) | ⚠️ **Chromium only, stated plainly.** Every E2E run and both live passes used Chromium; Firefox/Safari/Edge and narrow viewports are the M12 sweep. This is a declared coverage boundary, not a claim of cross-browser support. |

### 7. ✅ Definition of Done
Canonical (architecture_and_qa_roadmap.md:32-41) instantiated:
- [x] `npm run verify` green. **(measured 05/08/2026 01:0X in the closing audit — and deliberately
  measured through the STRONGER gate: `npm run gate` (`verify && dup && deadcode && audit &&
  check:context`) exited **0** on all 8 stages. `verify` itself = lint clean · prettier clean ·
  **410 unit tests / 11 files** green in 7.66s · `vite build` 356 modules in 1.23s. The four stages
  beyond `verify`: jscpd **0.42%** (3 small documented clones) · knip **0 findings** · audit-gate
  7 findings with **6 written, reasoned waivers and 0 unwaived criticals** · check:context clean.)**
...
- [x] CLAUDE_CODE_LOG + STATUS updated (end-of-session protocol each session; `CHANGELOG` retired
  23/07/2026). **(done 05/08/2026 01:0X as part of this closing audit: a dated session entry in
  `docs/CLAUDE_CODE_LOG.md`, and `STATUS.md`'s module-3 board row corrected — it had been **stuck five
  phases back** on "פזה 3 — הבא: שער 3.7" while the same file's own header already said 5.4 was the
  last step. ⚠️ **The template's LOG-compaction step (§2b: narrative 1,141 lines vs a 180 threshold)
  was deliberately NOT executed** — Ishay's standing instruction for this session was that
  `STATUS.md` and `CLAUDE_CODE_LOG.md` carry uncommitted, unruled lines from two other sessions and
  must not be cleaned. Compacting is cleaning. Registered as `🚧 מ4` with the measured number rather
  than done silently or skipped silently.)**
...
- [x] **↳ ADDED 30/07 — email send (step 3.4), five separate boxes because each failed at least once:**
  - [x] the mail **arrives**, and the attachment **opens** — verified by size/bytes, not by filename or MIME type; **(CLOSED 05/08/2026 by Ishay's own eye, quoted verbatim: *"נשלחה נראה מעולה"*. This box was brought to him as one of exactly three the audit refused to close on its own, precisely because software can prove a file left at the right size in the right format and cannot prove it opens and reads. The byte-level half was already proven in 3.4 (`REG-IN-quote-6.pdf`, 33KB, base64 decoded) and is pinned permanently by `quote-email.spec.js:134` asserting the `JVBERi0` = `%PDF-` prefix.)**
  - [x] the body is **RTL** and keeps the template's line breaks (direction incident #5 — an artefact leaving the system runs its own direction pass); **(CLOSED 05/08/2026 on two independent legs: the permanent wire assertion `quote-email.spec.js:123` requires `rtl` in the outgoing `body` field on every run — so a regression here fails the suite, it is not a one-time observation — and the 30/07 real-inbox send showed `<div dir="rtl">` with its 4 line breaks intact. Ishay's *"נראה מעולה"* on 05/08 is the human half, on the rendered mail as the customer receives it.)**
  ...
  - [x] the Make scenario answers 200 **only after** the mail module succeeded, and carries a Skip error handler so one bad recipient cannot disable it. **(CLOSED 05/08/2026 by Ishay, verbatim: *"עובד"*, after checking scenario 6759079 himself. This one lives entirely outside the repo — no test in this codebase can reach it, which is exactly why it was brought to him as a question rather than inferred. What it buys: without the Skip handler a single malformed recipient disables the whole scenario silently, and every later send fails with no error anywhere in REG-IN.
    **↳ Independently corroborated the same night** by the parallel ad-hoc session, which read scenario
    6759079 through the Make MCP and reported the structure rather than the outcome: the 200-response
    module sits **after** the mail module in the main flow (so a 200 cannot precede a successful send),
    and the error branch is **502 `{"ok":false}` followed by `builtin:Ignore`** — that `Ignore` **is** the
    Skip handler. Scenario active, `immediately`, `dlqCount: 0`, unchanged since 30/07. Two independent
    confirmations of one box, from a human click and from the scenario definition.)**
...
- [x] UX-&-validation checkbox: 🎨 review passed (design/states/RTL/keyboard) + validation-completeness
  (spec'd implemented, spec-silent confirmed). **(CLOSED 05/08/2026 01:0X. Ishay was asked whether to
  run this before the merge or defer it to M12 and answered *"אם זה לא מסובך תכין"* — so it ran, live,
  on five screens: `/quotes` · `/quotes/new` · `/quotes/8/edit` · `/system/prices` · `/customers/46`.
  **Measured, not eyeballed:** horizontal overflow **0px on all five** · **124 of 124 keyboard stops
  carry a visible focus ring** · **54 money cells with the ₪ on the same side in every one** (the
  defect class that recurred three times) · forced permission-load failure produces an explanation
  **plus a working "נסה שוב"** and never the accusatory "אין לך הרשאה" · **zero unforced console
  errors** across the whole pass. **0 findings — stated as such rather than padded.**
  ⚠️ **One honest correction kept on the record:** the first probe reported two `input[type=time]`
  fields with no focus ring. Re-measured with a settle delay and confirmed by screenshot — the teal
  2px ring is present and clearly visible. It was my measurement reading the computed style before the
  CSS transition finished, not a defect; the "finding" was withdrawn instead of shipped.
  ⏸️ **390px narrow layout measured at 444px of horizontal scroll and deliberately NOT counted as a
  finding** — Ishay ruled on 31/07 that this is a desktop back-office tool and that narrow-screen
  layout is not tested here (`server-messages-and-inactive-product.spec.js:278-281`); the system-wide
  responsive/cross-browser sweep is M12 by the template's own scoping.
  **Validation-completeness:** the spec'd rules are implemented in `src/lib/validators.js` (11 exported
  validators) and each is exercised on both layers — discount >100% shows a Hebrew explanation instead
  of a silently vanishing panel, rejection requires one of 8 reasons, `'אחר'` requires notes and the
  notes are proven *delivered* not merely required, tier boundaries have a negative case, and a quote
  with no hostess line is refused by the RPC itself. What was **not** re-derived here: the spec-silent
  validation choices were surfaced to Ishay in their own rounds during Phase 3 and are recorded in §9;
  this audit did not re-open them, per his instruction not to re-open 3.1–3.7.)**
...
> Dated journal entries are never rewritten (`docs/CLAUDE.md`).
- 05/08/2026 01:01 — **Closing audit (5.4). Verdict [YES], typed-echo signed. Zero blockers, five debts,
  and a number in place of the fix round's open worry.**
  Everything load-bearing was **re-run**, not read: gate exit 0 (8 stages) · 410 unit · 78/78 E2E in one
  run with 0 skips · DB counted before **and** after (10/24/3/3 both times) · policies, both lock
  triggers, 2 cron jobs, seed 11/40/20, 10 `module3_` migrations and advisors all read live · 6,319 ₪
  recomputed from the live rows rather than quoted.
  **(a) The fix round's own stated blind spot, answered with a measurement.** It reported catching two
  empty assertions *by accident* and warned there might be more. There were: **138 assertions examined
  across its 4 files, 2 green-and-empty.** `quote-email.spec.js:249` (`not.toBe(403)` sitting directly
  under `toBe(400)` — strictly implied, cannot fail) and `customer-page.spec.js:91` (`toContainText('0')`
  on the projects tab, where deny-all RLS guarantees the 0 no matter what M3 does, and `'0'` also
  substring-matches `10`). Three more are weak but *can* fail and were not counted as empty. Every other
  spec was swept for stale row-ids and **0 were found** — #10 approved, #11 rejected, #7 in_progress,
  9001-3 synthetic, each precondition re-verified against the live DB.
  **(b) The half-evidence question was decided, not dodged.** The fix round watched 8 tests fail before
  and did not re-break a fixture to watch them fail again. **Ruled sufficient, with the reason:** for 6
  of the 8 it did something stronger than a re-break — it moved each fixture to a row that satisfies the
  *original* precondition, and those preconditions were re-verified here directly against the DB (#22 is
  `in_progress` with a real `email_log` row; #8 is `in_progress` with `04ST` first and a non-hostess
  second line). A re-break only proves a locator resolves; a precondition read from the DB proves the row
  is the right *case*. For the 2 that were genuine contract repairs (a renamed button label, a path that
  returned before the RPC) the two-directional proof is structural: the test requires the mapped message
  to beat the live fallback string at `03_quotes/api.js:204`, which cannot happen unless the path ran.
  **(c) A recorded fact was found stale and corrected.** The 01/08 carry-forward says `db_roadmap` rows
  A-9/A-11/A-14/A-17 lack an "✅ APPLIED" tag. Re-measured: **A-9, A-11 and A-17 all carry it**; only
  A-14 differs, and it reads `⚠️ PARTIALLY APPLIED` **by design**. The item is closed, not carried into M4.
  **(d) 🎨 UX pass run at Ishay's request** (*"אם זה לא מסובך תכין"*) on 5 live screens: 0px overflow ×5 ·
  124/124 focus stops with a visible ring · 54 money cells consistent · forced-failure state explains and
  offers retry · 0 unforced console errors · **0 findings**. ⚠️ **A finding was withdrawn rather than
  shipped:** the first probe flagged two `input[type=time]` fields as having no focus ring; re-measured
  with a settle delay and confirmed by screenshot, the teal 2px ring is plainly there — the probe had read
  the computed style before the CSS transition finished. 390px narrow layout measured at 444px of scroll
  and **excluded by Ishay's own 31/07 ruling**, not by omission.
  **(e) A template step was deliberately not executed, and why.** `module-close` §2b requires this session
  to compact `CLAUDE_CODE_LOG.md` (narrative **1,141 lines** against a 180 threshold). Ishay's standing
  instruction for this session was that `STATUS.md` and `CLAUDE_CODE_LOG.md` hold uncommitted, unruled
  lines from two other sessions and must not be cleaned — and compaction *is* cleaning, which would bury
  another session's work before he ruled on it. **His instruction outranks the template.** Registered as
  `🚧 מ4` with the measured number so it cannot quietly disappear.
  **(f) Boundaries declared rather than rounded off:** `gitleaks` was not run locally (not installed — the
  evidence is a full repo grep plus the fact that it is a **blocking** CI job at `ci.yml:103` with
  `continue-on-error` appearing nowhere) · `smoke.spec.js` is excluded from `test:e2e` by
  `--grep-invert`, so no "78/78" claim covers it · **CI runs no E2E at all** · the PDF and the live mail
  ...
  **(g) The five debts this audit booked, each with its target module — twins live in `PROJECT_MASTER §6`
  ...
  · 🚧 מ12 — 52 `Buffer is not defined` console warnings per run, from the PDF dependency, not from `src/`.
...
- 04/08/2026 — **4 `npm audit` waivers added (Ishay's ruling), and the measurement was redone from
  ...
  **Two independent proofs, not one:** ① `npm ls <pkg> --all` puts every one of them under
  ...
  stylesheet. `reviewTrigger` on all four: next module close, or the submission —
  whichever comes first. *(⚠️ this line said `19/09/2026`; that date was **cancelled by Ishay's
  ruling on 12/08/2026** — the submission is now **01/10** (closing conference) / **20/10** (end),
  `docs/guides/00_roadmap.md §3`. Corrected by `regin-docs-sync` 12/08/2026 12:56.)* Gate proven **in both directions in the same session**: `exit=1` with the
...
- **`02/09/2026 22:0X` — CROSS-MODULE EDIT by module 9 (steps 2.3/2.4; this module is closed — this is the required ripple record).** Six hard-coded business constants became `params` rows (Ishay G-2, 02/09; `docs/micro_guides/module-9.md` §6 step 2.3 carries the per-constant ripple table). Touched here: `src/lib/quotes.js` (`EXPIRING_SOON_DAYS` deleted; `deriveQuoteExpiry` gains a fourth argument; `QUOTE_SCREEN_PARAM_NAMES` gains `ימי_אזהרה_הצעה_פגה`), `QuotesPage.jsx` (passes the loaded value to `deriveQuoteExpiry`); `PricingParamsCard` leaves the `מחירים` tab at m9 step 3.1 (Ishay Q-1, 02/09). Behaviour is unchanged while the seeded values equal the old constants (24 · 48 · 72 · 3 · 10 · 7); the module's suites were re-run green by the m9 builder and again in m9's Phase-2 gate.

- **`04/09/2026 00:15` — CROSS-MODULE EDIT by the list-window work (branch `ishay/list-window-pagination`; this module is closed — this is the required ripple record).** Ishay's ruling 04/09/2026 after the M7 seed filled the ledger: every long list gets the same shared time-window + pager (`src/lib/listWindow.js` · `src/components/ListWindow.jsx` — default "3 months" bounding the past only, future rows never hidden, 50 rows per page, "from–to of total" counter, `?window=&page=` in the URL where the surface has one). Touched here: `QuotesPage.jsx` — window chips in the filter row (on `estimated_event_date`, AND-ed with the existing date-range panel), pager under the table; tab counters count within the window. Business logic untouched; unit tests extended; the E2E invariant "tab counter = rendered rows" became "rows = min(counter, 50) and the pager says מתוך <counter>".
  ↳ `04/09/2026 00:22` — agents' edits on this module's screens landed; coordinator verification (gate · smoke · E2E · screenshots) pending before the PR.

- **`09/09/2026 00:1X` — CROSS-MODULE EDIT by the ui-copy night, stage 2 (branch `ishay/ui-copy-rewrite`; module 3 is closed — this is the required ripple record).** ↳ as-built: address alignment to the locked style guide `docs/plans/ui-copy-styleguide.md` §1 — masculine imperative ⇒ direct feminine, plural ⇒ feminine singular, slash forms removed, a person referred to ⇒ feminine, progress states (`שומר...`) ⇒ feminine participle with a single ellipsis (`שומרת…`, B9; R30 precedent: module 8's `מפיקה ושולחת…`). **Files:** `QuoteBuilderPage.jsx` (`המשיכי לשליחה`, `עדכני ושלחי`/`שמרי ושלחי`, `תקני`), `QuoteDocumentDialog.jsx` (`שלחי שוב`, `המשיכי לשליחה`, `שולחת…`/`מפיקה את המסמך…`), `QuoteSummaryPanel.jsx` (`שומרת…`), `QuotesPage.jsx` (`בדקי`, `לחצי`, `נסי לשנות`). Shared-lib strings of this module changed in `src/lib/quotes.js` (`לחצי … הורידי … ושלחי`). E2E updated in the same commit: `server-messages-and-inactive-product.spec.js` (`getByRole('button', { name: 'עדכני ושלחי' })`), `quotes.spec.js` (describe titles only). No business logic, no `<Hint>`, no layer copy (brief rule ⑥); the module's full copy reset is its own later stage. Full CI-parity suite + `npm run smoke` + the affected E2E specs green at the stage-2 commit (see `CLAUDE_CODE_LOG.md`, 08–09/09).

- **`09/09/2026 04:1X` — CROSS-MODULE EDIT by the ui-copy night, stage 7 (branch `ishay/ui-copy-rewrite`; module 3 is closed — this is the required ripple record).** ↳ as-built: the module's FULL copy reset to the locked style guide `docs/plans/ui-copy-styleguide.md`, four Sonnet writers across modules 2+3 on disjoint sets, merged by session 3. **builder + line editor + picker** (W11b): eight required-asterisks removed and the one optional field marked `(לא חובה)` with its helper split out of the label (R6/R1/R26); `ההצעה לא נמצאה, או שאין לך הרשאה אליה.` (R30 with module 4); `יש לתקן את השדות המסומנים לפני השמירה.` (B13); back link `חזרה לרשימת ההצעות` (R30); `החזירי להמלצה (N)` (R12/B13); hostess-count mismatch stated explicitly (R9); discounts-over-100% error one sentence with the E2E-pinned `חורגות מ-100%` kept; `בחרי מוצר` · `הסירי פריט` · `הוסיפי פריט` · `החליפי לקוח` (R12/R30). **list + dialogs + `lib/quotes` + `pricing` + `catalog`** (W11c): `ApproveQuoteDialog` `אשרי ופתחי פרויקט` + `מאשרת...` · `RejectQuoteDialog` `דחי את ההצעה`, asterisks removed (R6) · `QuotesPage` `נקי תאריכים` (R12/R30) · `lib/quotes.js`: past-date error `בחרי תאריך עתידי` (R17/B13), locked-quote and failed-send messages one sentence (R13) · `lib/pricing.js`: digits not words (R15), a typo (`לללא`), a gender agreement. **Customer-facing outputs** (W11d — formal, gender-neutral voice): PDF title `הצעת מחיר`, `קוד פריט` ⇒ `מק"ט` (R11/R30 with the catalogue screens), `MISSING_VAT_MESSAGE` action-first (B13; `אחוז_מעמ` kept — pinned) · `QuoteDocumentDialog`: `שלחי במייל` · `שלחי שוב` · `הורדה` (R12/R30), `מפיקה את המסמך…`, `שדה` ⇒ `משתנה` for template tokens (R11 with `TemplateEditor`) · `lib/emailTemplates.js`: singular/plural on the token diagnostics (UC37), periods on full sentences (R14). **Raise-coupled, untouched (2):** `ההצעה כבר טופלה (סטטוס …` · `לא מוגדרת עלות למוצר …`. **Merge fixes by the orchestrator:** the failed-send sentence now quotes the renamed button `"שלחי במייל"` (R11 across writers) · `lib/quotes.js` not-found sentence unified with the builder's (R30) · `docs/guides/conference/demo-script-28-08.html` cites the new approve label. Untouched by rule: `QUOTE_TERMS` (legal placeholder), the totals-row labels (byte-twins of `QuoteSummaryPanel`), `NO_CHANGES_SEND_CONFIRM` (Ishay 01/08), `QUOTE_ACTION_LABELS`/`+ הצעה חדשה`/`סיבות דחייה:`/`ביטול` (open guide questions for stage 9 — `ביטול` is the fifth). Tests: none needed. Registers: C-30 · S-15 (PDF terms hard-code `30 יום` while validity is a param) · S-16 (three VAT-missing wordings) · S-18 (optional discount field unmarked inside `LtrFieldGroup`) · S-19 (`פרטי הפרויקט` on the PDF — second instance of S-03). **Gates at the stage-7 commit:** CI-parity **98 files / 2,402 tests green** · `vite build` ✓ · eslint 0 · prettier ✓ · smoke green on the session dev server (64343) · E2E customers · customer-page · quotes · quote-approval · quote-document · quote-email + the cross-tree dependents prices · server-messages: **60 passed · 2 failed · 0 skipped** (both failures pre-date tonight: `customer-page.spec.js:130` counter-vs-window drift, bisected by session 2; `prices.spec.js:218` expects `ערך חוקי: מספר שלם חיובי` while the ceilings PR of 03/09 made the message a range `מספר שלם בין 1 ל-150` — `paramsRegistry.test.js` on `bf1fcfa` already asserts the range form; neither file was touched tonight). BEFORE/AFTER screenshots of four screens in both onboarding states (session-3 scratchpad `evidence/`).
- **`09/09/2026 12:2X` — CROSS-MODULE EDIT by the ui-copy night, stage 10 (branch `ishay/ui-copy-rewrite`; module 3 is closed — this is the required ripple record).** ↳ as-built: `QuotesPage.jsx` tab label `הכל` ⇒ `הכול` (R30: 35 `הכול` vs 7 `הכל` in code; E-OFF-1 measured both forms on one screen; `QuotesPage.test.jsx` clicks by testid — unchanged) · **`QuoteBuilderPage.jsx`** gets `<Hint id="quoteBuilder.ratio" />` under the headcount row (after the headcount error `<p>`): two evaluators measured the quote builder as the only money screen with no layer at all. Text S4-drafted in W13's voice (④ — what 'יחס' is, that it comes from settings and the recommendation can be restored; source: the `paramsRegistry` hint of `יחס_אורחים_לדיילת` + the Ishay-29/07 comment at ~275). The outward action `שמרי ושלחי` is covered at click time by the existing confirm/send flow — not moved. Tests: `03_quotes` suite green in the module set. ➕ 12:3X: `<Hint id="quotes.approvalRate" />` placed inside the rows branch's scroll wrapper, before `quotes-table` — W14's text (source `deriveQuoteMetrics` in `lib/quotes.js`: the denominator is closed quotes only, 'נפתחה בטעות' is not a loss, ✓/✗ record the customer's answer). Not guarded on `canEdit` (the tile explanation carries R28 for viewers too).
- **`24/09/2026 01:4X` — CROSS-MODULE EDIT by the system-polish session 1 (branch `ishay/system-polish`, plan `docs/plans/2026-09-24-system-polish.md`; module is closed — this is the required ripple record).** ↳ as-built: 0 — **H1 report row → empty active edit form** (live #2317): root cause `QuoteBuilderPage.jsx` cleared `loadError` *after* the not-found / not-in_progress branches set it (since `32858cd1`). Now `QuoteUnavailable`: message + way forward (`פתחי את מסמך ההצעה` → `/quotes?view=<id>` / `חזרה לרשימת ההצעות`), never the form. `QuotesPage.jsx` opens `QuoteDocumentDialog` from `?view=` (URL state), in any status; unknown id ⇒ explicit banner. New tests `QuoteBuilderPage.test.jsx` (mutation-checked: 3/4 fail without the guard) · `QuotesPage.view.test.jsx`. A5 — `quotePdf.jsx` `MISSING_VAT_MESSAGE` without `אחוז_מעמ`; `quotes.js` missing-param message via `paramLabel`. B — hints `quotes.expiringVsEventSoon` · `quoteBuilder.priceLocked`, and a visible lucide `Lock` on the line price (it was plain text with no signal at all).
- **`24/09/2026` — CROSS-MODULE EDIT by the system-polish session 2, package 0ב "return to the report" (approved by Ishay 24/09; plan `docs/plans/2026-09-24-system-polish.md` §6).** ↳ as-built: doors from a report (`ReportsPage.jsx` `DOOR_PATHS`) and from the home-screen attention cards carry `?returnTo=<the report's own URL>` / `?returnTo=/`; `src/lib/returnTo.js` `safeReturnTo` accepts only `/reports…` or `/` (anything else is ignored — no open redirect; test-first, 13 cases); `src/components/ReturnToLink.jsx` shows `חזרה לדוח` / `חזרה למסך הבית`. Closing navigates by push, so the browser Back button still works. e2e `reports.spec.js` "0ב" ×3 (quote dialog · hostess card · project card) green. In this module: `QuotesPage` closing the `?view=` document dialog returns to `returnTo`; `QuoteBuilderPage` shows `ReturnToLink` above the title (home-screen quote card).
- **`24/09/2026` — CROSS-MODULE EDIT by the system-polish D2 agent, "AI follow-up draft" (approved by Ishay 23/09 23:15, *"מאשר הכל לפי המלצתך"*; plan `docs/plans/2026-09-24-system-polish.md` §6 D2 · §6ה).** ↳ as-built: `QuoteDocumentDialog.jsx` shows a `נסחי מייל מעקב` row (lucide `Sparkles`) for `canEdit` on an `in_progress` quote (enabled only when `email_log` has a send — same tri-state `previousSend`; otherwise disabled with the visible reason `ההצעה עוד לא נשלחה ללקוח`) or a `rejected`/`פג תוקף` quote. It opens `FollowupDraftDialog.jsx` (a `DialogTrigger`, so focus returns to the button): skeleton under `motion-safe:`, `aria-live` status, labelled editable subject + body (example placeholders — 5-pass, empty-input rule), fixed `טיוטה — בדקי לפני שליחה.` (hidden on failure — nothing to check), `העתיקי` · `פתחי במייל` (`mailto:`; over the marketing mailto threshold the link carries to+subject and the body is copied, said aloud), the sender signature appended client-side from `buildSenderSignature`. 🔒 **The system never sends.** Rules in `src/lib/quoteFollowup.js`; transport `draftFollowupEmail` in `api.js` (own timeout — 35 s since the follow-up entry below, Hebrew error body from `error.context`, one request per open under StrictMode). Server: new Edge Function `supabase/functions/draft-followup` (placeholders-only to Gemini, JWT-only reads, 401/403/400/404/409/429/502 — see its README) + a CI `deno check` step. Tests: `quoteFollowup.test.js` · `FollowupDraftDialog.test.jsx` · `QuoteDocumentDialog.followup.test.jsx` · e2e `quote-followup.spec.js` (keyboard-only) · two new scans in `accessibility.spec.js`. 🔴 **Open:** both live AI calls (24/09 10:58 and 11:05 UTC) timed out with no provider response (18.7 s, 25 s) — the draft path is unverified live; cause and one-call test plan in the function README. Unchanged: `CustomerDetailsPage` passes `canEdit={false}` on purpose, so the row does not show there.
- **`24/09/2026` — 0ב gap closed by the system-polish D2 agent (deputy item; Ishay *"סגרו לי תפינות"*).** ↳ as-built: every target of a home-screen attention card and of a report door now shows `ReturnToLink` when it arrived with `?returnTo=` — audited all `cat.href` targets in `src/lib/dashboard.js` (`/projects/:id` ✓ · `/quotes/:id/edit` ✓ · `/hostesses` ✓ · `/customers/:id` ✓ back-button) and added the three that were missing: `/finance` (`FinancePage` `PageHeader`) · `/logistics` (`LogisticsPage` `PageHeader`) · `/quotes` (`QuotesPage`, also the report door `?view=…&returnTo=/reports…`). Tests: `FinancePage.test.jsx` · `LogisticsPage.test.jsx` (now rendered inside a `MemoryRouter`) · `QuotesPage.view.test.jsx`.
- **`24/09/2026` — code-review fix (deputy): return-on-close only for the item the door opened.** ↳ as-built: `src/lib/returnTo.js` `shouldReturnOnClose({returnTo, closingId, doorId})` — `doorId` is the id that was in the URL on the first render. Wired in `QuotesPage` (`setDocumentQuote(null)`, `doorView`) and `HostessesPage` (`closeCard`, `doorHostess`). Before: an empty home-screen card leads to the module list itself (`/quotes?returnTo=/`), and any item she then opened herself threw her back to the home screen on close. Now: a non-door item closes normally and she stays on the list; the `ReturnToLink` stays while `returnTo` is in the URL. Other `returnTo` consumers checked: `ProjectCardPage` · `CustomerDetailsPage` — the page is the item (back button), not a list, so no change. Tests: `returnTo.test.js` (+3) · `QuotesPage.view.test.jsx` (+2) · new `HostessesPage.returnTo.test.jsx` · e2e new `return-to-home.spec.js` (2/2) + `reports.spec.js` "0ב" ×3 regression green.
- **`24/09/2026` — D2 follow-ups (deputy, after the two live timeouts).** ↳ as-built: `FOLLOWUP_DRAFT_TIMEOUT_MS` 30 s ⇒ **35 s** (`src/lib/quoteFollowup.js`) — the server budget is `BUDGET_MS` 27 s, so the client now waits 8 s past it (was 3 s); the comment that said 24 s is corrected. The loading status is a visible sentence, `מנסחת טיוטה — זה יכול לקחת עד חצי דקה.` (`aria-live` polite, no imperative) instead of a bare skeleton. Clipboard failure: `העתקה נכשלה — סמני את הטקסט והעתיקי ידנית.` (was `ניתן להעתיק ידנית` — told her it is possible, not what to do); same string in `MarketingPanel.jsx` (R30). Server v3 is comment-only (README: deployed source identical to the file). Tests: `FollowupDraftDialog.test.jsx` loading text + new `העתקה שנכשלה` case. 🔴 **Still open:** the ≤10 s target (§6ה-ה) is not met — measured 18.7 s and 25 s with no provider answer; the deputy ruled no model change before the conference.
- **`24/09/2026` — D2 output cap + subtitle (session 2, on the deputy's ruling after the read-only log diagnosis).** ↳ as-built: `draft-followup` sends `generation_config.max_output_tokens` 1024 and `store: false`, and logs one line per answered attempt (`status` · `elapsed` · `output_tokens`); no fallback model (separate ruling). `FollowupDraftDialog.jsx`: "נוסחה בעזרת AI" left the dialog description (it showed while drafting and after a failure) and now heads the draft itself (`followup-ai-origin`); tests cover loading, 429 and ready. Measured live call: `supabase/functions/draft-followup/README.md` (Live verification).
- **`24/09/2026` — D2 button hidden + reviewer fixes (session 2, deputy's ruling after the third live call also timed out).** ↳ as-built: `FOLLOWUP_AI_AVAILABLE = false` (`src/lib/quoteFollowup.js`) gates the row in `QuoteDocumentDialog.jsx`; dialog, server and tests kept; e2e reads the flag from the source and skips the dialog parts while off. Server v5: DB error in the permission gate ⇒ 500 "תקלה זמנית" (was a false 403) · missing AI key ⇒ `unavailable`, final state in the dialog · 401 wording · guard +אחוז/+number-thousands. Dialog: closing mid-draft aborts the request (deferred one tick so StrictMode's remount does not kill it) · "גוף הטיוטה הועתק…" stays · quote number in `<Ltr>`. Privacy hint now states a fact ("לא נשלחים ל-AI"). m12 `upcoming` flag: measured inert (same md5 with/without `p_drill`) and the order rows are already non-clickable (`hasRowDoor` in `ExtraTable`) — locked by a test, no change.
- **`24/09/2026` — fresh-eyes copy fixes (deputy, branch `ishay/system-polish-copy`).** ↳ as-built: ① hint `quoteBuilder.priceLocked` (`src/lib/onboardingCopy.js`) pointed to *"השדה 'הנחה ידנית' שבסיכום"* — that is a display row (`QuoteSummaryPanel.jsx`); the field she types in is **'נוספת'** under the card **"הנחות והערות"** (`QuoteBuilderPage.jsx`, `quote-manual-discount`). Now: *"להנחה ללקוח הקלידי אחוז בשדה 'נוספת' שתחת 'הנחות והערות' — …"*. ② `QuotesPage.jsx` — hint `quotes.expiringVsEventSoon` (it contrasts the two chips) shows only when the "פג בקרוב" chip itself is on screen (`showExpiringChip`; was `showChips`, which also showed it in "מאושרות"/"נדחו", where only "אירועים קרובים" remains). Test: `QuotesPage.test.jsx` (+1).
  ↳ `24/09/2026` — same branch: `QuoteBuilderPage.test.jsx` gets `vi.mock('@/supabaseClient')`. In CI (no `.env.local`) the file crashed at load on *"supabaseUrl is required"* (the page imports `src/api/email.js`, which creates the client at import) — measured with `VITE_SUPABASE_URL= VITE_SUPABASE_ANON_KEY= npx vitest run`; pre-dates this branch (`9a2dbbfd`). Test-only. And the m07 so-what singular (*"בחשבונית אחת"*) is migration `20260924140000_module11_m07_singular_invoice` — ⏳ written, not applied (`docs/db_roadmap.md` §10ב).
  ↳ `25/09/2026 14:2X` — CROSS-MODULE EDIT by builder m12 (deputy task, Ishay 14:0X "מאשר הכל לפי המלצתך"): ① `FOLLOWUP_AI_AVAILABLE = true` — the "נסחי מייל מעקב" button is back; every e2e that clicks it routes `**/functions/v1/draft-followup` first (`quote-followup.spec.js` `routeDraft` ×3 · `accessibility.spec.js` :437), so no test reaches the provider. ② PDF terms: "ההצעה תקפה N יום" takes N from `ימי_תוקף_הצעה` (`quoteToPdfModel` now carries `validityDays`); missing/non-numeric ⇒ 30; 1 ⇒ "יום אחד", 2–10 ⇒ "ימים". `QUOTE_TERMS` ⇒ `quoteTerms(validityDays)` (+3 tests).
  ↳ `25/09/2026 15:5X` — checker finding (via the deputy): the builder's "תוקף ההצעה: N יום מהעדכון האחרון" said "1 יום" at N=1 ⇒ the same `validityDaysPhrase` as the PDF, moved to `src/lib/quotes.js` so the builder does not pull the PDF engine.
