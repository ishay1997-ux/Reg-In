...
| **1.7** | Migration **G** — `params` seed: 2 mail templates + survey link + dormant threshold + cancellation tiers | ⬜ pending |
...
- **A1** — `logistics` policy gate: recommend `'לוגיסטיקה'` (AR-2), show the two-column leak table from §3.2.
...
🚫 **No profit function, no `finalGrossProfit`, no bonus, no travel** (AR-6 · ㉟ · B14).
...
- `getProjectLogistics(projectId)` · `getProjectChanges(projectId)` · `getProjectAssignments(projectId)`.
- One wrapper per RPC.
...
**⚠️ And a real conflict to surface, not paper over:** `MAX_ATTACHMENT_BASE64_CHARS = 4_000_000` (`src/lib/email.js:29`) ≈ **3 MB of raw binary**, while the closing screen's own text promises *"עד 10MB"*. See §10 verification #6 — **this must be reconciled before step 3.5 ships.**
...
- **Section 🅱️ `מה קרה עם כל דיילת`**, lede quoted verbatim from the card: *"נוכחות היא עובדה — היא מזינה את ציון-האמינות בשיבוץ החכם. סימון-איכות הוא שיפוט — הוא קובע את מי נציע לעיריית חדרה בפעם הבאה. שתי שאלות שונות, ושתיהן חובה בכל שורה."*, counter `5 דיילות שובצו · כולן סומנו`, **5 columns:** `דיילת` · `נוכחות` · `סימון-איכות` · `שעות בפועל` · **`עלות בפועל`** *(🚫 **not `שכר`** — cost is M6's, wage is `§7.19` and M8's)*.
  - **One flat 7-option attendance select** (B4) in mockup order.
...
- [ ] All **14** `🚧 מ6` debts closed or explicitly re-targeted with a module number *(not nine — see §10 verification note)*, and the **three** `🚧 מN ← מ6` obligations written into `PROJECT_MASTER §6`.
...
- [ ] Two new private buckets exist with limits set, and `storage.objects` carries **twelve** policies.
- [ ] `smartMatch.js` reads the real column names, its seven weights are **unchanged**, and an unknown shape does not silently skip.
...
- [ ] Every `🌊 אדוות` sweep is closed — with the ripples performed or with the word `אין`.
