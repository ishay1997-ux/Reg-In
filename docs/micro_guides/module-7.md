...
a model that infers a history from a final state.
  ↳ `25/09/2026 15:2X` — CROSS-MODULE EDIT by builder m12 (Ishay in the deputy chat 14:4X, "המשך הכל לפי המלצתך"): the finance card (`unbilled`) counts `awaiting_invoice` only; `event_finished` moves to a fifth card, kind `closing` — "פרויקטים · מנהלת פרויקטים", noun = `PROJECT_STATUS_LABELS.event_finished` ("ממתין לסגירה", no new wording), why = days since the event ("הסתיים לפני N ימים"), tone yellow. Measured 25/09: 33 ⇒ 25 on the finance card, 8 on the new one. Tests: `dashboard.test.js` (fixtures split, +closing singular), `DashboardPage.test.jsx` (the festival now sits in the closing card; empty state ×5). ⚠️ Demo script station 1 quotes "31 אירועים שהסתיימו ולא חויבו" and four cards — flagged to the deputy, not edited.
  ↳ `25/09/2026 15:3X` — deputy ruling: the closing card's empty state reads "✓ אין פרויקטים שממתינים לסגירה" (def `emptyNoun`, AttentionPanel falls back to `noun`); the rows keep the status name. Demo script station 1 now describes five cards and the finance card without a fixed number (re-measured 14/10). +1 assertion in `DashboardPage.test.jsx`.
  ↳ `25/09/2026 15:5X` — checker finding: "8 ממתין לסגירה" was broken Hebrew ⇒ noun/nounOne "פרויקטים ממתינים לסגירה" / "פרויקט ממתין לסגירה" (the same number-then-noun pattern as the other cards); +1 test.
