...
- **`09/09/2026 00:1X` — CROSS-MODULE EDIT by the ui-copy night, stage 2 (branch `ishay/ui-copy-rewrite`; module 5 is closed — this is the required ripple record).** ↳ as-built: address alignment to the locked style guide `docs/plans/ui-copy-styleguide.md` §1 — masculine imperative ⇒ direct feminine, plural ⇒ feminine singular, slash forms removed, a person referred to ⇒ feminine, progress states (`שומר...`) ⇒ feminine participle with a single ellipsis (`שומרת…`, B9; R30 precedent: module 8's `מפיקה ושולחת…`). **Files:** No source string of this module changed (measured: zero masculine address in `05_logistics/`). One test fixed: `ChecklistDialog.test.jsx` asserted the shared `LoadingOrError` loading text `טוען...`, which became `טוען…` (single ellipsis, B9) in the shared-components reset. No business logic, no `<Hint>`, no layer copy (brief rule ⑥); the module's full copy reset is its own later stage. Full CI-parity suite + `npm run smoke` + the affected E2E specs green at the stage-2 commit (see `CLAUDE_CODE_LOG.md`, 08–09/09).

- **`09/09/2026 05:3X` — CROSS-MODULE EDIT by the ui-copy night, stage 8 (branch `ishay/ui-copy-rewrite`; module 5 is closed — this is the required ripple record).** ↳ as-built: the module's FULL copy reset to the locked style guide `docs/plans/ui-copy-styleguide.md` — writer W12a (Sonnet, 362K), merged by session 4 from the saved report. **`LogisticsPage.jsx` `AmberLegend`:** the 254-char legend ⇒ `⏱ שורה בענבר = דורש תשומת-לב. שום דבר לא ננעל.` — the settled example of the plan §4 stage 8 applied verbatim (R9/R13/R27); H2 holds because each amber trigger is still explained per row (`amberGlyphTitle` for the unordered physical item, `lateArrivalReason` for the late delivery); the `amberDays` prop left the component; the two triggers + the `{N}` business-days threshold + the 01WEB exclusion ⇒ **C-31** (🔴 `{N}` is a live `params` value — interpolate, never hard-code). **Bug fix, `ChecklistDialog.jsx` `CANCELLED_QTY_TITLE`:** an internal decision-marker glyph `(㊴)` had leaked into the string and rendered as a browser tooltip — removed (R10); `ChecklistDialog.test.jsx:737` and `e2e/logistics.spec.js:61` updated by the orchestrator. **`SHORTFALL`:** eight string fragments assembled through three nested ternaries ⇒ one `shortfallClause(completion)` with a single singular/plural switch — visible text byte-identical (Ishay's O-5 wording untouched; verified against the 4 `checklist-shortfall` assertions incl. n=1, UC37). **Raise-coupled:** the 4 byte-copies of migration `raise` texts unchanged (`CLOSED_EVENT_SENTENCE` · `CANCELLED_CONTROL_TITLE` · `NEGATIVE_QTY_SENTENCE` · `שדה לא מוכר בבקשה`); the brief's 5th (`לא נשלח אף שינוי לשמירה.`) exists only in a code comment — the live message passes through `error.message`. **E2E:** `logistics.spec.js` re-declares 17 constants byte-for-byte — 16 unchanged, 1 updated (the glyph). Smoke anchor `QUEUE_SORT_LINE` = `ממוין: לפי קרבת האירוע` untouched. **Untouched by rule:** `PILLS` (approved spec) · `DISABLED_PILL_TITLE` (open `screens-approved.md §⑧-13`) · `סגירה` dialog-chrome button (R30 with `ClosingWindowDialog`) · `LEGAL_EMPTY_DETAIL`/`LOAD_FAILURE_DETAIL` (shared with module 6 `LogisticsTab`) · inline `label: value` colons and `לצ'קליסט →` (open guide questions). **Registers:** C-31; no new structural findings. Tests: `LogisticsPage.test.jsx` (amber-legend block: base text asserted; the 5↔12 params-mutation tests trimmed to the icon-`title` channel) · `ChecklistDialog.test.jsx` — 175 green in the module. Gates at the stage-8 commit: CI-parity 98 files / 2,402 tests green · `vite build` ✓ · eslint 0 errors (1 pre-existing warning, `UsersManagementPage.jsx:75`, untouched line) · prettier ✓ on the 40 changed files · smoke green (1 passed — run twice, before and after the login-button revert) on the session dev server (64343) · E2E logistics · settings · auth · permissions · prices · load-failure-guards · server-messages + smoke-dependent accessibility: 71 passed · 1 failed (`prices.spec.js:218` — pre-night: expects `ערך חוקי: מספר שלם חיובי`, the ceilings PR made it a range) · 1 skipped (`public-confirm.spec.js:76`, fixture-dependent); after the revert, `auth` + `accessibility` re-run on a fresh build: 11 passed.
- **`09/09/2026 12:0X` — CROSS-MODULE EDIT by the ui-copy night, stage 9 (branch `ishay/ui-copy-rewrite`; module 5 is closed — this is the required ripple record).** ↳ as-built: the explanation LAYER (level 2, "מודרך") — copy lives ONLY in `src/lib/onboardingCopy.js` (27 keys, one voice: writer W13/Opus from `stage9-layer-proposal.md`, decided by session 4 in `ui-copy-candidates.md` column "הכרעת שלב 9"); the screens gained `<Hint id=…/>` placements only (no base-string changes except the glossary-form alignments named). `<Hint>` renders nothing at level 0 and is provider-safe (renders nothing outside `<AuthProvider>`, `Hint.jsx` 09/09). **Placement:** `LogisticsPage.jsx` — `logistics.amberWhy` inside `AmberLegend`, under the one-line base legend (C-31: the two amber triggers + the registration-site exclusion; the business-days threshold `סף_לוגיסטיקה_ימי_עסקים` is a param ⇒ "שנקבע בהגדרות", never a digit). Gates at the stage-9 commit: CI-parity 98 files / 2,403 tests green (+1: Hint outside a provider) · `vite build` ✓ · eslint 0 errors (1 pre-existing warning, `UsersManagementPage.jsx:75`) · prettier ✓ on all changed files · smoke green (one red run while the implementers were still editing — `#quote-ratio` not found under HMR — then green twice on a quiet tree) (session dev server 64343) · full `npm run test:e2e`: 167 passed · 7 skipped · 4 failed — 2 pre-night (`customer-page.spec.js:130` · `prices.spec.js:218`) + 2 load timeouts in `accessibility.spec.js` (login-field fill 10s · `route.fetch` to Supabase) while E2E, screenshots and four evaluators ran concurrently; re-run alone on a fresh build: **7/7 passed** · `npm run gate`: every step green except the pre-existing `check:docs-structure` finding (`cards-finance.md:483`, m11 discovery file, not in CI) — a first run under machine load had 3 vitest worker-start timeouts (`Failed to start forks worker`), the re-run on a quieter machine passed 98/2,403 · screenshots of every screen in both states (55 files, S4 scratchpad `evidence9/`) — whole-document pass by eye: done on the `-on` shots after fixing the evidence spec (it intercepted only the write, so every full navigation reloaded level 0 from the DB — all earlier `-on` shots were effectively `-off`; now the GET is intercepted too and returns level 2). Smart Match, repository, hostess card, logistics, projects: the grey teal-bordered paragraphs sit where the proposal put them, one voice across screens; the two stacked hints above the repository table read as one block and stay..
- **`09/09/2026 12:2X` — CROSS-MODULE EDIT by the ui-copy night, stage 10 (branch `ishay/ui-copy-rewrite`; module 5 is closed — this is the required ripple record).** ↳ as-built: `<Hint id="logistics.amberWhy" />` moved out of `AmberLegend` (page bottom — an evaluator measured it at pixel 3,333 of 3,418) to the top of `QueueTable`, guarded `entries.length > 0`, wrapped in a fragment (H4: inline next to the rows it explains). Text unchanged. **Recorded for Ishay, not changed (approved §⑧-3 copy):** the outbound-window paragraph (`החלון: … ⇒ יום העסקים הבא הוא חמישי 10/09.`) — three OFF/ON evaluators flagged it (R3 · R13 · R2 · R10 · R28); night report §3 item 7. ➕ **13:0X — Ishay ruled ("1. מאשר")**: the outbound-window paragraph (§⑧-3 ①) and the legend (②) left the base; heading only; the why moved to `logistics.outboundWindow` (level 2). `OUTBOUND_LEGEND`, `nextBusinessDayIso`, `dayMonth`, `addDaysIso` removed from `LogisticsPage.jsx`; the test 'אין בסעיף שום פקד-רישום' now asserts the heading. C-35 registered; ✏️ in `screens-approved.md §⑧-3`. Tests: module set 15 files green.
- **`09/09/2026 13:1X` — CROSS-MODULE EDIT by the ui-copy night, rulings batch 1 (branch `ishay/ui-copy-rewrite`; module is closed — this is the required ripple record).** ↳ as-built (Ishay's rule, 13:0X: *base = mature product; every friendly explanation = the layer*): `LogisticsPage.jsx` queue header `מצב` ⇒ `סטטוס` (item 10). *(Ruling 1 — the outbound paragraph — is the 13:0X line above.)*
- **`24/09/2026 01:4X` — CROSS-MODULE EDIT by the system-polish session 1 (branch `ishay/system-polish`, plan `docs/plans/2026-09-24-system-polish.md`; module is closed — this is the required ripple record).** ↳ as-built: A4 — `ChecklistDialog.jsx`: the §3.7 five-line explainer (with "מודול 8" on screen) → base `כל שינוי נשמר מיד.` + hint `checklist.autoSave`; cancel banner = fact + exception (`אין לעדכן…` and `הנעילה חלה על כל המשתמשות` dropped; ordered-item reason → hint `checklist.cancelledOrdered`); `checklist-locked-note` shortened. Spec write-back: `docs/specs/module_05_logistics/screens-approved.md` §5 as-built note. B — visible lucide `Lock` beside a quantity locked by item state (the reason was a hover `title` only) + hint `checklist.qtyLocked`.
- **`24/09/2026` — CROSS-MODULE EDIT by the system-polish session 2 (same branch/plan; package 0ג item 4, approved by Ishay 24/09).** ↳ as-built: "להזמין לחודש הקרוב" on the logistics screen, under the queue — `UpcomingOrders.jsx` (own tri-state load; a failure shows `נסי שוב` and never "nothing to order", and does not take the queue down) + `api.js` `getUpcomingOrders` ⇒ new RPC `logistics_upcoming_orders()` (gate `'לוגיסטיקה'` view/edit), migration `20260924081000_module5_upcoming_orders.sql`. ⚠️ **Declared exception to AR-3** (three direct reads, no read-RPC): the calculation is module 11's (m12 equipment report), and one internal function `m11_upcoming_equipment_orders` now serves both (iron rule 14 — m12's order rows md5-identical before/after). Columns פריט · כמות · אירועים, exactly as the plan. Not built, reported to Ishay: a "כבר הוזמן" column (the quantity is the total for upcoming events, whatever the item's status).
- **`24/09/2026` — CROSS-MODULE EDIT by the system-polish D2 agent ("להזמין" = what is still unordered; Ishay *"מאשר"*, relayed by the deputy).** ↳ as-built: migration `20260924120000_module5_upcoming_orders_not_started` — the shared `m11_upcoming_equipment_orders` now counts only `item_status = 'not_started'` rows of physical items (`category <> 'site'`, the same rule as `isPhysical` in `src/lib/projectLogistics.js`). 📏 The logistics table "להזמין לחודש הקרוב" went from 9 rows led by "שרוך סאטן - ממותג 2,540" (all of it already `ready`) to **2 rows: תג שם אקולוגי - ממותג 997 · תג שם רגיל - ממותג 570**, verified as the logistics manager. No UI code change (only the `UpcomingOrders.jsx` header comment); the empty sentence `אין ציוד להזמין לאירועים של החודש הקרוב.` still fits. Proof rows: `db_roadmap.md` §10ב.
...
- **`24/09/2026` — "להזמין" in the first screen height (deputy item; 0ג item 4 said *"בגובה-המסך הראשון, מתחת לתור"*).** ↳ 📏 measured as the logistics manager (13 queue rows): the block started at y=1441 at both 1536×864 and 1024×768. Fix, the smallest that leaves the queue untouched: at viewport ≥1360px the queue card and `UpcomingOrders` share a grid (`minmax(0,1fr) 20rem`, the block on the left in RTL); below 1360 — stacked, as before. After: 1536×864 ⇒ block at y=432–626, queue 878px wide (it already works at 702 in 1024), same rows, same order, no inner scroll. 1024×768 ⇒ unchanged, still below the fold — no option there without changing the queue itself (reported, not built). `UpcomingOrders` gained a `className` prop (placement only). Screenshots `Reg-In-evidence\system-polish-2026-09-24\orders\{before,after}-fold-{1536,1024}.png`.
  ↳ 24/09/2026 — the comment above that block said the window size as digits joined by the multiplication sign, which `check:bidi` (in `npm run gate`) flags as a digit glued to × — the fold commit `6b32f0d0` broke the gate and it was caught only on the final gate run. Reworded to "ברוחב 1536 ובגובה 864"; no code change.
- **`24/09/2026` — code-review fix (deputy): `checklist.autoSave` shows only for `canEdit`** (`ChecklistDialog.jsx`), like `checklist.qtyLocked` under it — for `view` it explained typing into controls she does not have. Test: `ChecklistDialog.test.jsx` "מצב 2: … ל-edit ולא ל-view". ⚠️ Not changed, noted: the base line `כל שינוי נשמר מיד.` still shows to `view` too.
- **`24/09/2026` — fresh-eyes copy fixes (deputy, branch `ishay/system-polish-copy`).** ↳ as-built, `ChecklistDialog.jsx`: ① cancelled project — the bottom line `checklist-locked-note` (*"הפרויקט בוטל — הפקדים נעולים, חוץ מרישום כמות שהגיעה."*) is **removed** (it repeated the banner; "נעול" appeared three times in one dialog) — this supersedes the `checklist-locked-note` clause in the "Cancel-banner ㊴ sentence" row above; the banner line is now *"אפשר עדיין לרשום כמות שהגיעה — שאר השדות נעולים."* ("פקדים" was jargon). ② hint conditions — `checklist.autoSave` and `checklist.qtyLocked` show only while the project is active (`canEdit && !locked`; before: also on a finished event, and `autoSave` also on a cancelled one, where no status can change); `checklist.cancelledOrdered` shows only for `canEdit` (it says "רשמי כאן"). ③ `checklist.autoSave` wording (`src/lib/onboardingCopy.js`) told a lie: *"רק כשהשדה ריק… ומה שהקלדת לעולם אינו נדרס"*. The live RPC `update_logistics_item` (only definition: `20260826002447_module5_checklist_rpc.sql`, read to the end) fills on the move to `ready` when `not v_has_qty and v_row.actual_qty = 0` — a typed 0 is filled too, by design (㉕: *"0 פירושו 'עדיין לא נרשם כלום'"*). The hint now says so; the RPC is unchanged. Tests: `ChecklistDialog.test.jsx` (+2: cancelled edit/view, finished event) · `e2e/logistics.spec.js` (banner string, locked-note count 0).
  ↳ `24/09/2026` — same branch: the m12 report's scope text (*"פריט שכבר יצא לספק או הגיע"* · *"בלוק ההזמנה … חותך"*) ⇒ the item status names — migration `20260924141000_module5_m12_status_names`, ⏳ written, not applied (`docs/db_roadmap.md` §10ב). ⚠️ Not changed (outside the item list): the frontend hint `reports.equipment.orderCeiling` (`src/lib/onboardingCopy.m11.finance.js`) still says *"מה שכבר יצא לספק או הגיע לא נספר"*.
  ↳ `24/09/2026` — gate-tester findings (deputy): the cancelled-project banner line "אפשר עדיין לרשום כמות שהגיעה…" shows only with `edit` (a `view` user was promised an action she lacks); `checklist.cancelledOrdered` shows only when an ordered/ready item exists (the inverse of `checklist.qtyLocked`). Tests in `ChecklistDialog.test.jsx`.
  ↳ `25/09/2026 15:2X` — builder m12: data fix `20260925150000_data_fix_1588_autofill_flag` (⏳) — two 1588 logistics rows carried `actual_qty_autofilled = true` with actual≠planned, a state `update_logistics_item` cannot produce (10/09 seed run). Flag ⇒ false; nothing else touched. Record: db_roadmap §10ב, module-11 §10.
