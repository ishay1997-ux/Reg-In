...
- 21/08/2026 — **`MODULE_META`/`GROUPS` string duplication closed (a §6 backlog item from this file, target-triggered when the `modules` table schema was next touched — that already happened 10/07/2026 in M2 and sat undischarged since).** Cross-module edit made from a docs-freshness/debt-audit session (M1 is closed+merged; this is an expected M1 file in a future diff). `Sidebar.jsx`'s `MODULE_META` and `PermissionsMatrixPage.jsx`'s `GROUPS` each independently hard-coded the same 7 Hebrew business-module names/paths/icons/groups — a silent-drift risk the backlog line named explicitly. Extracted to one source, `src/lib/constants.js` → `BUSINESS_MODULES` (an array of `{name, path, icon, group}`), following the existing `SYSTEM_MODULES` pattern in the same file. Both call sites now derive their local shape from it (`MODULE_META` via `Object.fromEntries`, `GROUPS` via a fixed group-label order filtered against `BUSINESS_MODULES`) instead of maintaining their own copy. **Verified behavior-identical, not assumed:** full `npx vitest run` before and after — **1,271/1,271 tests, byte-identical pass count** — plus `vite build` green and a manual re-read of both edited files confirming the derived structures produce the same keys/values as the originals. **Zero M1 screen/behavior change** — the permissions matrix renders exactly the same groups, order, and cell content as before; only the source of the underlying strings moved.

- **`02/09/2026 22:5X` — CROSS-MODULE EDIT by module 9 (Phase 3; module 1 is closed — this is the required ripple record).** (a) `src/components/ProfileSettingsPage.jsx` `NotificationsSection`: the two toggles are LIVE — email preference persists to the new `notification_preferences` table (migration `20260902211550_module9_b_…`, Ishay R-4 02/09); the SMS toggle stays disabled with the truthful label `אין ערוץ SMS במערכת`; both "(בקרוב)" suffixes and the stale "אין עדיין טבלת העדפות" comment are gone (rule 13ח); new `ProfileSettingsPage.test.jsx` (5 tests). (b) `src/modules/01_auth/PricesManagementPage.jsx` loses `PricingParamsCard` (the file and its `pricesApi` param functions are deleted) — Ishay Q-1 02/09: the two params (`אחוז_מעמ`, `יחס_אורחים_לדיילת`) are edited only in the `פרמטרים` tab and in "ההגדרות שלי"; §7.84's reopen trigger fired. `docs/micro_guides/module-9.md` steps 3.1/3.5 carry the evidence.

- **`09/09/2026 00:1X` — CROSS-MODULE EDIT by the ui-copy night, stage 2 (branch `ishay/ui-copy-rewrite`; module 1 is closed — this is the required ripple record).** ↳ as-built: address alignment to the locked style guide `docs/plans/ui-copy-styleguide.md` §1 — masculine imperative ⇒ direct feminine, plural ⇒ feminine singular, slash forms removed, a person referred to ⇒ feminine, progress states (`שומר...`) ⇒ feminine participle with a single ellipsis (`שומרת…`, B9; R30 precedent: module 8's `מפיקה ושולחת…`). **Files:** `LoginPage.jsx` (7 strings: `נסי שוב` ×5 forms, `הזיני`, `בדקי` ×2, and the two account-state sentences unified with `MainLayout`/`AuthContext` to `יש לפנות למנכ"ל…`), `PermissionsMatrixPage.jsx` (1), `PriceTiersDialog.jsx` (`הסירי מדרגה`), `ProductFormDialog.jsx` (`בחרי…` ×3, `נסי שוב`, `שמרי שינויים`/`הוסיפי מוצר`), `UsersManagementPage.jsx` (`נסי שוב` ×4, `בחרי תפקיד`, `שמרי שינויים`/`הוסיפי משתמש`, `ערכי משתמש`, `השביתי משתמש`, `הפעילי משתמש מחדש`, the deactivate confirm now speaks of `המשתמשת … היא לא תוכל`, and the add-dialog description became the guide-§7 settled sentence `היא לא תוכל להתחבר עד שתיצרי לה חשבון-כניסה בנפרד`); progress states `שומרת…`/`מתחברת…` (single ellipsis, B9). Tests: `UsersManagementPage.test.jsx` (14 green). The E2E `title` contract of this module (`צפייה ועריכה`/`צפייה בלבד`/`אין גישה`) and the `כתובת דוא״ל` placeholder were NOT touched. No business logic, no `<Hint>`, no layer copy (brief rule ⑥); the module's full copy reset is its own later stage. Full CI-parity suite + `npm run smoke` + the affected E2E specs green at the stage-2 commit (see `CLAUDE_CODE_LOG.md`, 08–09/09).

- **`09/09/2026 05:3X` — CROSS-MODULE EDIT by the ui-copy night, stage 8 (branch `ishay/ui-copy-rewrite`; module 1 is closed — this is the required ripple record).** ↳ as-built: the module's FULL copy reset to the locked style guide `docs/plans/ui-copy-styleguide.md` — writer W12b (Sonnet, 367K), merged by session 4 from the report saved verbatim in `docs/plans/ui-copy-stage8-reports.md` (kept — Ishay 09/09 05:2X, *"מה למה שתמחק קובץ דוחות?"*). **LoginPage:** nine two-sentence messages merged into one sentence with an em-dash (R13): temporary lock · 5-attempt lock (`— נסי שוב בעוד כ-15 דקות.`) · `מייל או סיסמה שגויים — נסי שוב.` · unauthorised / inactive account (`— יש לפנות למנכ"ל…`; no bypass route exists on the login screen, so the role name stays — value changed 0 times) · Google failure · reset-mail sent/failed (`בדקי את…`, R7 admission). **Buttons: W12b wrote `התחברי` / `התחברי עם Google` (R12/B2) — REVERTED to `התחברות` / `התחברות עם Google` at the merge.** Two independent sessions (S3, then S2 with a guide anchor) argued the other way, and the anchor holds: guide §1 allows a gender-neutral action noun *where natural* and names `כניסה` — and the login screen is the only pre-identity surface (Ishay himself, the CEO, logs in on stage). The guide's R12 line (*button = feminine verb*) did not consider a pre-identity screen; **Ishay ruled 09/09 05:2X: *"תשאיר התחברות"*** — recorded as a dated ✏️ note in the locked guide §1 (a primary button on a pre-identity screen keeps the gender-neutral action noun). The E2E entry locator `name: 'התחברות', exact: true` (24 spec files, 28 call sites + 2 absence-assertions) is untouched; `auth.spec` + smoke re-run after the revert. **UsersManagementPage:** four failure messages gain `— נסי שוב.` (R13/R17) · `דוא"ל` ⇒ `דוא״ל` U+05F4 (R11 — same glyph as the login placeholder; 0 E2E hits on the straight form) · edit-dialog description without a period (R3) · placeholder `שם פרטי ומשפחה` ⇒ `לדוגמה: דנה כהן` (R5; **S-05 closed** — the twin in `src/components/ProfileSettingsPage.jsx` unified in this commit and its stale "until module 1 is rewritten" comment removed). **PermissionsMatrixPage · PricesManagementPage:** one message each (R13). **ProductFormDialog:** `שדה חובה` ⇒ `יש להזין מק"ט` / `יש להזין שם פריט` (R19) · `(רשות)` ⇒ `(לא חובה)` ×2 (R6) · duplicate-SKU error names the fix. **PriceTiersDialog:** the long tier-rule paragraph ⇒ base `ללא מדרגות, ההצעה מתומחרת לפי מחיר הבסיס`, the rule ⇒ C-32. **pricesApi:** six failure messages get `— נסי שוב.` (R17). `lib/permissions.js` · `lib/constants.js`: 0 changes. **E2E contracts untouched:** `title` triad `צפייה ועריכה`/`צפייה בלבד`/`אין גישה` · placeholder `כתובת דוא״ל` U+05F4 · h1 `כניסה למערכת` · h2 `מטריצת הרשאות`; `auth.spec.js:33` regex still matches the merged sentence. **Registers:** C-32 · S-20 (login fields have no visible `<label>`). **Open for stage 9 / Ishay:** `שכחת סיסמה?` (R12 vs the world convention) · `משתמש` in category-form dialog titles vs the feminine everywhere else (precedent `הוסיפי לקוח`). Tests: `UsersManagementPage.test.jsx` (placeholder · U+05F4 · deactivate wording) — 20 green. **Manual pass on the four module-1 screens (login · users · permissions · prices) in both onboarding states:** done by eye on the AFTER shots (S4 scratchpad `evidence/after-m1-*`, both states): users — `דוא״ל` header with U+05F4, `+ הוספת משתמש חדש`, the `מצב הטמעה` column, `לא פעיל` row greyed; permissions — legend triad `צפייה ועריכה`/`צפייה בלבד`/`אין גישה` intact, four groups; prices — `ללא מדרגות`/`5 מדרגות` tags, `+ מוצר חדש`, 11 rows; login — the shot predates the revert (shows `התחברי`), the reverted `התחברות`/`התחברות עם Google` is proven by `auth.spec` (locator `name: 'התחברות'`, 11 passed) — S-20 (no visible `<label>`) confirmed on screen. Off/on identical by design: no `<Hint>` on these screens before stage 9, and the evidence spec intercepts the onboarding write, so the users column shows the DB value. Gates at the stage-8 commit: CI-parity 98 files / 2,402 tests green · `vite build` ✓ · eslint 0 errors (1 pre-existing warning, `UsersManagementPage.jsx:75`, untouched line) · prettier ✓ on the 40 changed files · smoke green (1 passed — run twice, before and after the login-button revert) on the session dev server (64343) · E2E logistics · settings · auth · permissions · prices · load-failure-guards · server-messages + smoke-dependent accessibility: 71 passed · 1 failed (`prices.spec.js:218` — pre-night: expects `ערך חוקי: מספר שלם חיובי`, the ceilings PR made it a range) · 1 skipped (`public-confirm.spec.js:76`, fixture-dependent); after the revert, `auth` + `accessibility` re-run on a fresh build: 11 passed.
- **`09/09/2026 12:0X` — CROSS-MODULE EDIT by the ui-copy night, stage 9 (branch `ishay/ui-copy-rewrite`; module 1 is closed — this is the required ripple record).** ↳ as-built: the explanation LAYER (level 2, "מודרך") — copy lives ONLY in `src/lib/onboardingCopy.js` (27 keys, one voice: writer W13/Opus from `stage9-layer-proposal.md`, decided by session 4 in `ui-copy-candidates.md` column "הכרעת שלב 9"); the screens gained `<Hint id=…/>` placements only (no base-string changes except the glossary-form alignments named). `<Hint>` renders nothing at level 0 and is provider-safe (renders nothing outside `<AuthProvider>`, `Hint.jsx` 09/09). **Placements:** `UsersManagementPage.jsx` — `users.addLoginAccount` after `</DialogHeader>` of the add/edit dialog, guarded `{!editingUser && …}` (guide §7 round-2 settled text) · `PriceTiersDialog.jsx` — `prices.tierRule` after `</DialogHeader>` (C-32). Tests: `UsersManagementPage.test.jsx` unchanged (already mocks AuthContext). Gates at the stage-9 commit: CI-parity 98 files / 2,403 tests green (+1: Hint outside a provider) · `vite build` ✓ · eslint 0 errors (1 pre-existing warning, `UsersManagementPage.jsx:75`) · prettier ✓ on all changed files · smoke green (one red run while the implementers were still editing — `#quote-ratio` not found under HMR — then green twice on a quiet tree) (session dev server 64343) · full `npm run test:e2e`: 167 passed · 7 skipped · 4 failed — 2 pre-night (`customer-page.spec.js:130` · `prices.spec.js:218`) + 2 load timeouts in `accessibility.spec.js` (login-field fill 10s · `route.fetch` to Supabase) while E2E, screenshots and four evaluators ran concurrently; re-run alone on a fresh build: **7/7 passed** · `npm run gate`: every step green except the pre-existing `check:docs-structure` finding (`cards-finance.md:483`, m11 discovery file, not in CI) — a first run under machine load had 3 vitest worker-start timeouts (`Failed to start forks worker`), the re-run on a quieter machine passed 98/2,403 · screenshots of every screen in both states (55 files, S4 scratchpad `evidence9/`) — whole-document pass by eye: done on the `-on` shots after fixing the evidence spec (it intercepted only the write, so every full navigation reloaded level 0 from the DB — all earlier `-on` shots were effectively `-off`; now the GET is intercepted too and returns level 2). Smart Match, repository, hostess card, logistics, projects: the grey teal-bordered paragraphs sit where the proposal put them, one voice across screens; the two stacked hints above the repository table read as one block and stay..
