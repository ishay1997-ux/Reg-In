...
**מה עושים:** שכפול תבנית ה-RLS על טבלת `customers` (בסביבת הבדיקה המאובטחת שלכם) והרצת 12 תרחישי קצה מלאים כדי לוודא שאין אף חור אבטחה במערכת:
...
* ☐ ניסיון כניסה עם משתמש `inactive` ← חוסם ומציג את ההודעה הייעודית ("חשבון זה אינו פעיל במערכת. פנה למנכ"ל.") ומנתק את הסשן מיד.
* ☐ מסך הגדרות פרופיל ← שני טוגלי ההתראות מנוטרלים חזותית (disabled) ומציגים בצורה נקייה את התווית "(בקרוב)" או "(פיתוח עתידי)".
* ☐ לוגו נטען תקין.
...
* ☐ RLS פעיל ואוכף על 4 טבלאות הליבה; משתמש בסטטוס `inactive` חסום לחלוטין ברמת ה-DB (דרך חוקי ה-RLS וה-constraint החדש), ולא רק בפרונטנד.
* ☐ מסך ניהול משתמשים (כולל מנגנון המחיקה הלוגית באייקון פח אדום וסינון השורות המחוקות מה-UI) עובד מקצה לקצה.
* ☐ מסך מטריצת הרשאות (במבנה המקובץ ל-4 קבוצות, עם חסימת עמודת מנכ"ל ועריכת Auto-Save) עובד פיקס.
* ☐ סימולציה: 2 משתמשים מתפקידים שונים — כל אחד רואה בדיוק את מה שהוגדר לו במטריצה העסקית.
* ☐ 12 התרחישים המורחבים על טבלת `customers` ירוקים כולם (צעד 12).
* ☐ CAPTCHA — הוחלט/בוצע/נדחה במפורש (לא בשקט).
* ☐ עודכן `docs/CHANGELOG.md` ו-`CLAUDE_CODE_LOG.md` (כולל תיעוד מיגרציית ה-SQL של ה-constraint ושינויי הקוד).
