...
> ⚠️ **DEV DATA PRESENT — read before Phase 3/5 (added 10/07 per Ishay's request; he later flagged it may have been unnecessary):** **`customers` is NOT empty** — 5 fictitious customers live in the shared DB (`customer_id` 7–11; reproducible via `supabase/seed_dev_customers.sql`; covers the 4 types + one `inactive`). Steps 3.x UI-verifies and step 5.1 E2E **must not assume a 0-row table** — use relative / count-delta assertions and create rows with distinct ח"פ. **To restore the clean empty state:** `delete from customers;` (the seed file keeps them one command away). See §9 (17:00 entry) + CHANGELOG.
