...
| **Status** | **🔨 Phase 2 ✅ complete + `🔻👤` gate SIGNED (Ishay, 10/07 evening). Phase-3 P13 sweep RULED (all six items — see Ledger) + fresh-context audit run, 4 findings fixed. READY for Phase 3 in a fresh session.** |
| Last updated | 10/07/2026 19:52 (P13 sweep ruled + adversarial audit: gross-profit `🚧 מ7`→`🚧 מ8` retarget (+§6 tombstone, §7.79 fix) · `getCustomer` wired into 3.6 · M3/M6 read-path requirement in §6 · M8 feedback past-events note. Full-check earlier 19:26: 32/32 · lint 0 · build ✓. Prior — see §9) |
| **Active step** | **3.1 — CustomersPage (list) + route swap. All Phase-3 rulings are in the Ledger (do NOT re-ask); free-hand grant on search/filter/sort details. Start with the 🗣️ build-narration.** |
...
| **OPEN — deferred (surfaces at step 3.2; M2-scope ruled 10/07 P13 ↑)** | §7.65 (08/07 audit) — `customers.email` uniqueness unruled (spec silent; shared contact across two companies may be legitimate). M2 ships without UNIQUE (confirmed); the hostesses half stays for an M4 nod | — | — | Step 3.2 note |
| **OPEN — deferred (surfaces at step 3.4; M2-scope ruled 10/07 P13 ↑)** | §7.34 — archiving a customer with live obligations (active quotes/projects) is unruled; M2 ships the plain status toggle with NO guard (confirmed). Ruled direction-leaning for M3: warning-not-block. When M3 exists this becomes real | — | — | Step 3.4 note |
...
| **OPEN — deferred (placeholder in step 3.6; retargeted M7→M8 10/07 evening)** | §7.79 — per-customer cumulative gross-profit definition (population/formula): C6 §2.4.1 lists it as a derived customer attr (citation corrected — C5 §5.6.3 names only revenue+feedback), but no formula. M2 ships a placeholder ("אין נתונים עדיין"); **M8** wires it (owns the profit formula — spec 5.14, with §7.78 exec `מ3/6/8`) | — | — | Step 3.6 (placeholder) |
