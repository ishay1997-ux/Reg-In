...
| `src/lib/validators.js` | `EMAIL_REGEX`, `ISRAELI_MOBILE_REGEX` — extend with `COMPANY_ID_REGEX` (9 digits) + `ISRAELI_PHONE_REGEX` (landline+mobile: mockup shows `03-1234567`). |
...
**🔻👤 gate — MANDATORY: shared Supabase project. Ishay/Amit approves the SQL text, and this approval explicitly covers its application in step 1.2.**
...
```sql
begin;
select set_config('request.jwt.claims', '{"email":"<test-user-email>","role":"authenticated"}', true);
set local role authenticated;
-- scenario query here (auth.email() resolves from the claims)
rollback;  -- no data persists; test inserts vanish
```
Run order: 4 → 1 inside one transaction (scenario 1 needs the row inserted in 4), then the rest. 5 real test users exist (one per role). Scenarios 13–14 run in one rolled-back transaction that first grants `view` on 'לקוחות' to a blocked role.
...
**What:** `CUSTOMER_TYPE_LABELS` (§7.3 ruling — the 4 spec labels keyed by enum) · `matchesCustomerFilters(customer, filters)` (type/consent/min-discount + free-text that matches company_name alone, contact_name alone, or a ח"פ **prefix** — the §7.11 forgiving-search ruling; written here so M3's quote-flow customer picker reuses it) · `deriveCustomerMetrics(projects)` → `{totalRevenue: null, avgFeedback: number|null}` (feedback avg from `projects.feedback_score` when data exists; totalRevenue returns null with a comment pointing to M3 pricing SSOT — NO revenue formula here) · validators: `COMPANY_ID_REGEX = /^[0-9]{9}$/`, `ISRAELI_PHONE_REGEX` (landline `0X-XXXXXXX` incl. mobile), discount 0–100 helper.
...
**Verify 🤖:** `npm run lint` → 0 errors; `grep -rn "from('customers')" src/ --include=*.jsx --include=*.js` → matches ONLY inside `src/modules/02_customers/api.js`.
**🔻👤 end-of-phase gate.**
...
**Verify 🤖:** preview — create a customer → row appears; re-enter same ח"פ → the friendly duplicate error names the existing customer + edit link; archive that customer, re-enter its ח"פ → the restore offer appears and restoring re-activates it (screenshot evidence for both flows).
...
**What:** archive/restore action: `status` toggle, archived rows dimmed (not hidden), "לא פעיל" pill, no "delete" wording anywhere.
...
**Verify 🤖:** preview — upload a small PDF, then `select count(*) from storage.objects where bucket_id='marketing';` → ≥1; the rendered send-button `href` starts with `mailto:` and its bcc list equals exactly the consented+active customer emails.
...
**What:** row click → card: full details + project history (via `getCustomerProjects` — empty state "אין פרויקטים עדיין") + metrics (סה"כ הכנסות / ממוצע משוב — "אין נתונים עדיין" placeholders via `deriveCustomerMetrics`).
**Verify 🤖:** preview — card opens showing details + both empty states.
...
|---|---|---|
| Unit | `customers.js` (labels, filters, metrics), validators (ח"פ/phone/discount) — Vitest | |
...
- [ ] Customer card renders with both empty states; no pricing formula anywhere in module-2 code (`grep -rn "0.18\|vat\|מע\"מ" src/modules/02_customers src/lib/customers.js` → 0 business-formula hits).
...
- [ ] Guide ⑦ acceptance: finance creates+sees customer · logistics fully blocked · filter works · archive reversible · `npm run verify` green · PR opened.
...
- [ ] PR to `dev` opened; CI green; merged.
