...
- **`02/09/2026 22:0X` — CROSS-MODULE EDIT by module 9 (steps 2.3/2.4; this module is closed — this is the required ripple record).** Six hard-coded business constants became `params` rows (Ishay G-2, 02/09; `docs/micro_guides/module-9.md` §6 step 2.3 carries the per-constant ripple table). Touched here: `src/lib/customers.js` (`SATISFACTION_ATTENTION_MAX` deleted; `needsSatisfactionAttention` takes the threshold) and `src/modules/02_customers/api.js` (`getCustomerScreenParams` loads `סף_שביעות_רצון`). Behaviour is unchanged while the seeded values equal the old constants (24 · 48 · 72 · 3 · 10 · 7); the module's suites were re-run green by the m9 builder and again in m9's Phase-2 gate.

- **`04/09/2026 00:05` — CROSS-MODULE EDIT by the list-window work (branch `ishay/list-window-pagination`; this module is closed — this is the required ripple record).** Ishay's ruling 04/09/2026 after the M7 seed (60 customers, one customer with 262 quotes): every long list gets the same shared time-window + pager (`src/lib/listWindow.js` · `src/components/ListWindow.jsx` — default "3 months" bounding the past only, 50 rows per page, "from–to of total" counter, state in the URL). Touched here: `CustomersPage.jsx` (pager only — customers have no event date) and `CustomerDetailsPage.jsx` (window chips above the tabs, a pager under the quotes tab and under the projects tab; tab counters count within the window). Business logic untouched; unit tests extended; `e2e/customer-page.spec.js` invariant became "rows = min(counter, 50)".
  ↳ `04/09/2026 00:16` — the two customer surfaces were wired by two parallel Sonnet agents (list page · customer page); both reports received, unit tests green in scope. Verification (gate · smoke · E2E · screenshots) follows in the coordinator session before the PR.
  ...
  ↳ `04/09/2026 01:01` — **"שביעות רצון" column on the customer card's projects tab (Ishay's request):** the score the customer gave each finished event, rendered by the new shared `src/components/ScoreCell.jsx` (stars + label tag, `—` until the survey is answered; only `feedback_status = completed` counts, same population as the "ממוצע משוב" tile). Test: `CustomerDetailsPage.score.test.jsx`.
  ↳ `04/09/2026 01:30` — **"רווח גולמי מהלקוח" tile replaces "סה"כ הצעות מאושרות" on the customer card (Ishay's ruling: *"מה אכפת לי כמה הסכום הצעות מאושרות יש לו"*).** Value = sum of frozen `project_finance.final_profit` over the customer's finished projects (`customerProfitTile` in `src/lib/customerProjects.js`, LEFT-embedded in `getCustomerProjects`); without the "כספים" permission the tile says "דורש הרשאת כספים", never 0 (§7.97). The approved-quotes sum stays as a column on the customers list. Tests: `customerProjects.test.js` + `CustomerDetailsPage.score.test.jsx`; E2E `customer-page.spec.js`/`smoke.spec.js` now assert `metric-profit`.

- **`09/09/2026 00:1X` — CROSS-MODULE EDIT by the ui-copy night, stage 2 (branch `ishay/ui-copy-rewrite`; module 2 is closed — this is the required ripple record).** ↳ as-built: address alignment to the locked style guide `docs/plans/ui-copy-styleguide.md` §1 — masculine imperative ⇒ direct feminine, plural ⇒ feminine singular, slash forms removed, a person referred to ⇒ feminine, progress states (`שומר...`) ⇒ feminine participle with a single ellipsis (`שומרת…`, B9; R30 precedent: module 8's `מפיקה ושולחת…`). **Files:** `CustomerDetailsPage.jsx` (`בדקי`, `נסי שוב`, `נקי חיפוש`, aria `פתחי כרטיס פרויקט`), `CustomerFormDialog.jsx` (`רענני`, `נסי שוב` ×2, `ערכי את הכרטיס הקיים`, `שחזרי מהארכיון ופתחי לעריכה`, `בחרי סוג לקוח`, `הסירי איש קשר`, `הפכי לראשי`, `הוסיפי איש קשר`, `שמרי שינויים`/`הוסיפי לקוח`, `שומרת…`), `CustomersFilterSheet.jsx` (`נקי הכל`), `CustomersPage.jsx` (`העבירי לארכיון`, `שחזרי מהארכיון`, `ערכי לקוח`, `נסי שוב` ×2, `לחצי`, `נסי לשנות`, `העלי חומר ושלחי`), `MarketingPanel.jsx` (plural ⇒ feminine singular throughout: `העלי`/`בחרי`/`שלחי`, `החליפי/העלי קובץ`, `הסירי קובץ`, `נקי הכל`/`בחרי הכל`, the too-long banner). ⚠️ **One string deliberately left masculine:** `NO_PRIMARY_MSG` (`אי אפשר למחוק את איש הקשר הראשי. סמן קודם אחר כראשי.`) is byte-coupled to the `raise exception` text of the `replace_customer_contacts` RPC (migration `20260902141451`) and is Ishay's own wording (`02_customers/CLAUDE.md`) ⇒ changing it needs a migration under the typed-echo gate — **stage 7 item**, recorded in `ui-copy-candidates.md`. Tests: `CustomerDetailsPage.projects.test.jsx`, `CustomerFormDialog.test.jsx`. E2E updated in the same commit: `customers.spec.js` · `customer-page.spec.js` · `load-failure-guards.spec.js` (`getByTitle('העבירי לארכיון')`/`'שחזרי מהארכיון'`). No business logic, no `<Hint>`, no layer copy (brief rule ⑥); the module's full copy reset is its own later stage. Full CI-parity suite + `npm run smoke` + the affected E2E specs green at the stage-2 commit (see `CLAUDE_CODE_LOG.md`, 08–09/09).

- **`09/09/2026 04:1X` — CROSS-MODULE EDIT by the ui-copy night, stage 7 (branch `ishay/ui-copy-rewrite`; module 2 is closed — this is the required ripple record).** ↳ as-built: the module's FULL copy reset to the locked style guide `docs/plans/ui-copy-styleguide.md`, four Sonnet writers across modules 2+3 on disjoint sets, merged by session 3. **list + details + filters + marketing + `lib/customers*`** (W11a): banner term `עמודת ההתרשמות` ⇒ `עמודת שביעות הרצון` (R11 — the column is named so everywhere else); table header `תוכן שיווקי` ⇒ `מאושר לדיוור` (R30 — the same field's name on the detail page and in the marketing panel); marketing-file helper without a trailing period (R3); `COMPANY_ID_ERROR` `שגיאה: מספר ח.פ. …` ⇒ `מספר ח"פ חייב להכיל 9 ספרות בדיוק` (R18/R19/R30 — the only field error in `lib/customers.js` that opened with `שגיאה:`; the old C5 wording no longer binds, `docs/CLAUDE.md` 03/09). **form + api** (W11b): dialog descriptions cut to one short line (R3/R27), the add-mode one became `sr-only`; contact-loading and duplicate-company errors made one sentence (R13/R17); archived-customer notice drops its question (R27 — the button below carries it); file-upload errors aligned with module 8's `— המגבלה היא 10MB` (R30). **Raise-coupled, untouched (3):** `NO_PRIMARY_MSG` · `ניתן לסמן איש קשר ראשי אחד בלבד.` ×2 (form + api). Tests: `lib/customers.test.js` (2) · `CustomersPage.satisfaction.test.jsx` (1). Untouched by rule: `CUSTOMER_TYPE_LABELS` · `archiveWarningMessage` (§7.34 wording) · `feedbackCountNote` (already carries the ④ qualifier) · `MarketingPanel`'s `!embedded` branch (dead code — sole call site passes `embedded`; noted, not removed). Registers: C-28 (projects-list sort captions ⑤) · C-29 (list satisfaction cell without the response count ④) · S-17 (satisfaction filter disabled while the column is live — product call). **Gates at the stage-7 commit:** CI-parity **98 files / 2,402 tests green** · `vite build` ✓ · eslint 0 · prettier ✓ · smoke green on the session dev server (64343) · E2E customers · customer-page · quotes · quote-approval · quote-document · quote-email + the cross-tree dependents prices · server-messages: **60 passed · 2 failed · 0 skipped** (both failures pre-date tonight: `customer-page.spec.js:130` counter-vs-window drift, bisected by session 2; `prices.spec.js:218` expects `ערך חוקי: מספר שלם חיובי` while the ceilings PR of 03/09 made the message a range `מספר שלם בין 1 ל-150` — `paramsRegistry.test.js` on `bf1fcfa` already asserts the range form; neither file was touched tonight). BEFORE/AFTER screenshots of four screens in both onboarding states (session-3 scratchpad `evidence/`).
