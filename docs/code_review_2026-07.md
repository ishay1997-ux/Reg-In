...
|---|---|---|---|
| 3 | **RLS על 11 הטבלאות העסקיות** — כולן RLS-on בלי policies (deny-all מכוון). ✅ **המודל הוכרע (06/07/2026, §7.21) — תבנית ה-policy המדויקת כתובה שם.** נותר רק היישום בפועל (מודול 2, הטבלה הראשונה) | ר' `docs/PROJECT_MASTER.md` §7 פריט 21 (סגור) | amit/06 מודול 2 (יישום ראשון לפי התבנית) |
| 4 | ~~אין Error Boundary~~ — ✅ **נסגר (06/07/2026):** נוצר `src/components/ErrorBoundary.jsx` ועטף את כל עץ ה-Routes ב-`App.jsx` | `src/App.jsx`, `src/components/ErrorBoundary.jsx` | ✅ סגור |
| 5 | ~~שינוי סיסמה לא מחובר ל-Backend~~ — ✅ **נמצא ממומש בפועל (06/07/2026)**: `SecuritySection` ב-`ProfileSettingsPage.jsx` כבר מאמת מחדש עם הסיסמה הנוכחית (`signInWithPassword`) ואז קורא ל-`supabase.auth.updateUser({password})`. הממצא המקורי היה לא-מדויק/מיושן. | `src/components/ProfileSettingsPage.jsx` | ✅ סגור — אין פעולה נדרשת |
| 6 | ~~E2E למטריצת הרשאות חלקי~~ — ✅ **נסגר (06/07/2026):** נוספו 2 בדיקות — שינוי תא מלא (edit→view→blocked, כולל אימות ששרד רענון דף) והגנת self-lockout (עמודת המנכ"ל מנוטרלת) | `e2e/permissions.spec.js` | ✅ סגור |
| 7 | ~~`current_user_role_id()` — הערת תיעוד חסרה~~ — ✅ **נסגר (06/07/2026):** נוספה הערה מפורשת שזו הפונקציה ה-load-bearing לכל RLS עתידי, עם הפניה לתבנית ב-§7.21 | `supabase/migrations/20260702195258_harden_current_user_role_id.sql` | ✅ סגור |
...
| 10 | ~~סיסמאות בדיקה בטקסט גלוי ב-`docs/CHANGELOG.md`~~ — ✅ **נסגר (06/07/2026):** הבלוק הוסר מהקובץ והוחלף בהפניה לערוץ פרטי. הסיסמאות הישנות נשארו בהיסטוריית git — **ישי הכריע (06/07) שלא מחליפים אותן**: פרויקט אקדמי, ריפו פרטי, משתמשי-בדיקה בלבד — סיכון מקובל ומתועד ב-[PROJECT_MASTER §7.24](PROJECT_MASTER.md). ממצא gitleaks על ההיסטוריה (אם יופיע) מטופל ב-`.gitleaksignore`, לא ברוטציה | `docs/CHANGELOG.md` | ✅ סגור (§7.24) |
| 11 | ~~`<html lang="he" dir="rtl">`~~ — ✅ **נסגר (06/07/2026):** היה `lang="en"` בלי `dir` כלל; עודכן ל-`lang="he" dir="rtl"` (RTL היה מוחל רק פר-`<div>`, לא גלובלית) | `index.html` | ✅ סגור |
...
| P2 #10 | ✅ נסגר 06/07 — הבלוק הוסר מ-CHANGELOG; רוטציה בוטלה במודע (PROJECT_MASTER §7.24) |
