...
- Fix-round progress (8c format), updated 20/08 23:4X: **blocker 3 — FIXED** (client: `ScopeChangeDialog.jsx:536` `.catch(()=>null)` removed; comment updated; 15/15 tests green) · **blocker 2 — migration WRITTEN, awaiting Ishay's typed-echo** (`20260820233607_module6_closing_rpc_hours_bound.sql`: `v_max_hours := p_actual_hours + 2`) · **blocker 1 — RECLASSIFIED, NOT a blocker** (Ishay's reality ruling 20/08: "אין דבר כזה במציאות אירוע ללא דיילות" ⇒ the empty-close path guards a case that does not occur; the server raise is left untouched, nothing is built for it) · regression rerun — טרם (blocked on the typed-echo + apply). B-1(amount) debt registered (`PROJECT_MASTER §6`, `🚧 מ8 ← מ6`).
- Migration is now a ONE-line fix (hours bound only) — CREATE OR REPLACE of the function body (pulled from live `pg_get_functiondef`), no table/column/RLS change, fully reversible. Cannot apply without Ishay's typed-echo (DB protocol).
- ⚠️ Open caveat surfaced to Ishay (not yet ruled): the nightly cron flips ANY past-date project to `event_finished` regardless of staffing, so the zero-hostess state is technically reachable even though it does not occur operationally; today the server would error + delete the uploaded report if reached. Also: the CLIENT already carries a signed Phase-3 "legal-empty" render (ClosingTab legal-empty + save-enabled) that, under this ruling, now guards a non-case — harmless (never renders when events always have hostesses), pending Ishay's call to leave/annotate vs. remove. Spec annotation on `screens-approved.md:1303` owed at final persistence.
