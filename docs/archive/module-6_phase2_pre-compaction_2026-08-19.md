...
| **Last updated** | `18/08/2026 23:51` *(system clock, `Get-Date`)* — 2.1–2.4 ✅ committed; 2.5 + an M4-coverage agent (Ishay's request) mid-write; 2.6 landed, uncommitted pending joint verification. *(The Stop hook re-fires on every turn-end while agents write module-6 src — this stamp line is the sanctioned no-op refresh; the substantive update lands when the agents do.)* |
| **Active step** | **3.0 — Phase-3 door (NOT yet opened). Open in a FRESH session with "תמשיך לבנות".** 🔒 **Phase 2 CLOSED — Ishay signed the phase report `19/08/2026 00:4X` ("מאשר").** The fresh session owes, in order: ① Phase-2 compaction (self-update clause (i) — archive copy first) · ② step 3.0's shared-component checkpoint + the `_design-contract` UNSOURCED rule (§10 nine-point #9) · ③ the ⑫ screen before/after at the first live screen run. **Phase-3 session plan, recommended `19/08/2026` and liked by Ishay (same agent-build + orchestrator-verify + his visual sign-off per surface):** **A** compaction + 3.0 + 3.1 (overview, the demo flagship; includes the ⑫ screenshot) · **B** 3.2 shell + dialogs 3.6/3.7 · **C** tabs 3.3/3.4/3.5 (closing screen last — it births the attendance data) · **D** 3.8 (LAST — shared surface with M2/M3, the E3 label rename) + full regression + the end-of-phase 🎨 UX/a11y pass + a small panel. ⚠️ Every manufactured DB state (a `ready`/`cancelled` project for screenshots) is checked against `docs/guides/do-not-touch.md` FIRST — the 28/08 demo data is live. |
...
**🌊 אדוות —** `18/08/2026`: the sweep's one ripple — **5 stale `⬜` tokens in §3.5 flipped to 🟢** (A5·A9·A10·B7·B11; each was already ruled in the blocks above its row). Nothing else touched.
**🗣️ אושר —** 🤖 confirmation `18/08/2026 22:5X` — full §3.5 re-read, result **אין** (no OPEN item anchored to Phase 2); per this step's own rule, no Ishay stop is required when the sweep is empty.
...
🔴 **↳ as-built `14/08/2026` — the live status board, measured with `select project_id, project_status from public.projects`. Four projects exist, and only THREE of the eight statuses are represented:**
...
> ⚠️ **`#8` moved off `not_started` and `#11` moved off `ready` when Phase 1's status machine first ran** — any step whose expected screen names a project's status must use the four above, not an earlier draft's.
...
🔴 **↳ as-built `18/08/2026` — three filled gaps, all logged in §10 (dated 18/08) and each with an owner:** ① `gapSentence`'s parametric wording beyond the approved examples (`כולן` when pending≠2; the short form when pending covers the gap) — `הנחתי`, no approved wording exists for those shapes, the approved sentences reproduce verbatim for every live anchor · ② the logistics-only-gap sentence `הלוגיסטיקה טרם מוכנה` — ✅ **nodded by Ishay `18/08/2026`** (was `הנחתי`; no live row produces it today) · ③ `confirmed_available` is an **optional client-side field** — the live RPC does not return it, so surface 1 must derive it from assignment rows or the #11 sentence never renders (**owned by steps 2.5/3.1**, noted in 2.5's as-built block). Also: `resolveProjectTone` throws on an unmapped label — the module's screens resolve tones through it, never through `StatusTag`'s silent fallback.
**🌊 אדוות —** `18/08/2026`: §10 entry (three `הנחתי`) · a pointer line in step 2.5's as-built block (`confirmed_available`) · step table + status header. Checked and NOT needed: no DoD checkbox pins the unit-test count; no §3.5 row and no approved-spec section describes this step differently.
**🗣️ אושר —** `18/08/2026 23:1X` — Ishay approved the unit brief ("מאשר"), incl. the long phrasing `התקיים לפני N ימים` for both tabs. Built test-first: 37 new tests seen red (module absent), then green; full suite **789 = 752 + 37**, zero regressions; eslint clean.
...
🔴 **↳ as-built `14/08/2026` — what `apply_scope_change` already computes server-side, read from `pg_get_functiondef`. Build the pure functions AROUND this, not over it:**
> **‏① The RPC returns `hours_to_event`** — `floor(epoch(( final_event_date + coalesce(final_start_time,'00:00') ) at time zone **'Asia/Jerusalem'** − now())/3600)`. ⇒ **`isLateChange` must use the same anchor** *(event date **+ start time**, Asia/Jerusalem, floored hours)*. A client function that keys on midnight, or on the browser's zone, disagrees with the server by up to a day — and both numbers reach the same screen.
> **‏② `revenue_delta_total` and every line's `revenue_delta`/`unit_price_snapshot` come back `null` when the caller lacks `'הצעות מחיר'`** *(the response carries `can_read_revenue` as the discriminator)*. ⇒ **the money block must render `—` on `null`, never `0.00 ₪`** — same rule as S-2, now on the dialog.
> **‏③ 🔴 The RPC's `revenue_delta` is PRE-DISCOUNT and PRE-VAT** — it is exactly `round(delta_qty × unit_price_snapshot, 2)`, and the comment in the migration says the snapshot is stored *"שמור טרום-הנחה כמו כל שורה אחרת"*. ⇒ **`סכום השינוי` is the RPC's figure; the four rows beneath it (discount → pre-VAT → VAT → `תוספת לחיוב`) stay this step's arithmetic.** 🚫 **Never display `revenue_delta_total` as `תוספת לחיוב`** — that skips the discount and the VAT and renders a wrong number that passes every schema check.
> **‏④ A NEW line's price is resolved from `price_tiers`**: the highest `min_qty <= target_qty`, `max_qty` **not** participating, falling back to `products.base_price` when no tier matches. **It also requires `products.status = 'active'` and a `product_costs.cost` row — absent either, the RPC refuses the whole call.** ⇒ the `+ הוספת פריט` path can fail for reasons the dialog cannot see in advance; surface those two Hebrew messages rather than a generic failure.
> **‏⑤ ⚠️ A line whose `delta_qty` computes to `0` is accepted and writes NOTHING** — no `project_changes` row, no `logistics` update — and still comes back inside `lines` with `delta_qty: 0`. **The DB will not tell you nothing happened.** ⇒ step 3.6's state ③ *("לא שינית אף כמות")* is the only guard, and it is load-bearing, not cosmetic.
...
🔴 **↳ as-built `14/08/2026` — the server-side validation this file's pure functions must MIRROR, not contradict. Read from `close_project_operationally`'s live body:**
> **‏① The attendance vocabulary is ENGLISH in the DB and the CHECKs are live** — `attendance_status ∈ {arrived, late, no_show}` · `lateness_level ∈ {light, medium, heavy}` · `no_show_reason ∈ {sick, approved_absence, ghosted}`, held together by `assignments_attendance_shape`, which permits **exactly** the four tuples `ATTENDANCE_OPTIONS` produces. ✅ **The seven options in this step already map correctly — this note pins them, it does not change them.**
> **‏② 🔴 The quality mark is NOT a column on `assignments`.** It travels as the payload key **`preference`** and lands in `customer_hostess_preference.preference`, whose CHECK holds the **Hebrew** trio `מצוינת · בסדר · לא_לשלוח`. **The upsert is `on conflict on constraint customer_hostess_preference_unique do update` — the overwrite B13 ruled, implemented.** ⚠️ **And a guard that is server-side only: when `projects.customer_id` is NULL the upsert is SKIPPED and the closing still completes** — the response reports `preferences_saved: false` and `preferences_written: 0`. **A screen that says "5 סימונים נשמרו" without reading those two fields can be lying.**
> **‏③ `hostessActualCost` has an upper bound the server enforces and this step never mentions: per-hostess `actual_hours` must be `0 ≤ h ≤ (event hours + 2)`**, where the event hours come from `final_end_time − final_start_time` *(cross-midnight handled; falls back to `24` when either time is NULL)*. **Event-level `p_actual_hours` is separately bounded `0.5 … 24`, and `p_actual_guests ≥ 0`.** ⇒ **`closingValidationSummary` should catch these before submit**, or the user meets a Hebrew server error after pressing save.
> **‏④ `no_show` is coerced server-side, not merely validated:** the RPC sets `actual_hours := 0` **and** `preference := null` **and** `preference_reason := null`. ✅ **That is ט4-א enforced in the DB** — the disabled-not-empty UI and the server now agree, and a form that sends a preference on a `no_show` row has it silently discarded rather than rejected. **Do not rely on that discard; disable the control as ט4-א says.**
...
**🗣️ אושר —** `18/08/2026` — inside the free-rein grant; anchor **1,085.00 ₪** reproduced with Keren's row forced to 0 · `לא_לשלוח` underscore asserted · no-show disables quality per ט4-א. ✅ **The three invented Hebrew validation-category strings NODDED by Ishay `18/08/2026 23:5X`** (`סימוני-נוכחות` · `סיבות ל"לא לשלוח שוב"` · `שעות בפועל שגויות` + singular forms + the list-joining grammar) — presented with a recommendation and approved; now his ruling (§10 18/08 entry B).
...
🔴 **↳ as-built `14/08/2026` — THE LIVE RPC SURFACE. Everything below was read from `pg_proc` this turn, not from step 1.8's draft. Where the two disagree, this block is what shipped.**
> ➕ **`18/08/2026`, from step 2.1:** `list_projects_overview` does **NOT** return a `confirmed_available` count — and `gapSentence`'s #11 sentence (*"הדיילת היחידה אישרה זמינות…"*) needs one. ⇒ **this step's wrapper (or step 3.1) derives it from the assignment rows via the `MAX(assignment_number)` fold and passes it as the optional `confirmed_available` field**; without it the sentence never renders and the column silently falls back to `חסרות N`.
> **‏⓪ `getProjectChanges` is an RPC, NOT a table read — and getting this wrong is silent.** ⇒ **`supabase.rpc('list_project_changes', { p_project_id })`.** 🚫 **Never `.from('project_changes').select()`:** the table is RLS-on with **zero policies** *(measured: `pg_policies` ⇒ 0 rows)*, so a `.from()` read returns `{data: [], error: null}` **for every role including מנכ"ל** — the empty-list-that-looks-like-no-data failure this module documents on three other surfaces. **It returns 13 columns:** `change_id · change_group_id · change_target · sku · color · delta_qty · unit_price_snapshot · unit_cost_snapshot · revenue_delta · money_visible · reason · performed_by · created_at`, ordered `created_at desc, change_id desc`. 🔴 **The three money columns come back `NULL` without `'הצעות מחיר'`, and `money_visible boolean` is the explicit discriminator** — **use the flag, never `price === null`**, because a genuine `NULL` and a denied `NULL` are otherwise indistinguishable. ⚠️ **`revenue_delta` is `round(delta_qty × unit_price_snapshot, 2)` — PRE-discount and PRE-VAT.**
> **‏① `list_projects_overview()` returns SEVENTEEN typed columns, in this order** — `project_id · event_name · customer_name · final_event_date · final_start_time · final_end_time · final_location · project_status · **required_hostess_count** · hostesses_confirmed · **pending_invites** · assignments_row_count · logistics_ready · logistics_total · cancelled_at · cancel_type · planned_revenue`. 🔴 **Two corrections against step 1.8's list: the required-count column is `required_hostess_count` (NOT `hostesses_required`), and `pending_invites` is missing from 1.8 entirely.** ‏`pending_invites` is the same de-dup fold counted on `assignment_status='pending'` — **it is what surface 1's `מה חסר` sentence *"2 זימונים ממתינים למענה"* is built from**, and without it that sentence cannot be produced without a second round-trip.
> **‏② The payload key lists, verbatim from the live function bodies.** `apply_scope_change(p_project_id, p_lines, p_reason)` reads per line: **`target` · `sku` · `serial_number` · `target_qty`** — and nothing else. `close_project_operationally(p_project_id, p_actual_hours, p_actual_guests, p_report_path, p_rows)` reads per row: **`hostess_id` · `assignment_number` · `attendance_status` · `lateness_level` · `no_show_reason` · `actual_hours` · `preference` · `preference_reason`**. 🔴 **The quality key is `preference`. There is no `quality_mark` field anywhere in the module** — it is not on `assignments` at all; the value lands in `customer_hostess_preference.preference`.
> **‏③ 🔴 The step's own banner above is now half-wrong and must be read with this exception: *"do not omit `serial_number`"* is TRUE for an existing logistics line and FALSE for a new one.** **Omitting `serial_number` is how a NEW line is requested** — the RPC allocates `coalesce(max(serial_number),0)+1` **across the whole project** and refuses if that `sku` already has any row on the project. 🚫 **The client never invents a serial number.** *(`assignment_number` carries no such exception — it is always required.)*
...
> **‏⑤ AR-10's constraint-name mapping is NOT needed for the date-move conflict, and wiring it there is dead code.** **Measured: `update_project_details` runs the pre-query itself and raises `P0001` carrying the finished Hebrew sentence** — the named variant *"`{full_name}` כבר מאושרת סופית ל…"* when the caller can read `'דיילות'`, and the anonymous variant when she cannot. ⇒ **the client displays the server's message; it does not reconstruct it.** *(Extending `SERVER_CONSTRAINT_RULES` may still be right for genuine `23505`/`23514` leaks — but AR-10's case is no longer one of them.)*
> **‏⑥ `assert_module_permission` is REVOKED from `authenticated`** *(measured: `has_function_privilege('authenticated', …)` ⇒ `false`; only `service_role`)*. 🚫 **No client code path can call it, ever** — it is reachable only from inside the `SECURITY DEFINER` functions that already own it. **Same for `recompute_project_status`.** **All seven client-facing RPCs are `authenticated`-executable and `anon`-revoked.**
...
🔴 **↳ as-built `14/08/2026`: the before/after screenshot this step demands has NO subject in the live database.** **Measured: zero projects sit on `ready`** *(the four live rows are `#3 not_started · #7 event_finished · #8 in_progress · #11 in_progress` — step 2.1's as-built)*, **and zero sit on `cancelled`.** ⇒ **the "a `ready` event now appears where it did not before" screenshot requires manufacturing that state first.** **The cheapest honest route: `#11` needs `required_hostess_count = 1` met — one `finally_approved` assignment and its single logistics gap closed would carry it to `ready` through the live trigger, with no hand-written status UPDATE.** 🚫 **Do not force `project_status` with a direct UPDATE to produce the screenshot** — the status machine is the single writer (⑫/㉚), and a hand-set status is exactly the drift this module was built to stop. **Whatever you do, say in the step which state you created and how.**
...
🔴 **↳ as-built `14/08/2026` — the three column names are final, and one verification instruction below is currently IMPOSSIBLE to satisfy:**
> **The columns to map onto are `assignments.attendance_status` / `lateness_level` / `no_show_reason`, English values as listed in step 2.3's as-built.** ✅ **`WITHDREW` confirmed: `approval_withdrawn` is a member of `assignments_assignment_status_check`, a different column entirely** — the separate branch this step demands is correct, not defensive.
> 🔴 **But: `select count(*) from assignments where attendance_status is not null` ⇒ ZERO today.** The columns exist and are empty, because **no project has been closed yet** — `#7` is the only project on `event_finished` and its `operationally_closed_at` is `null`. ⇒ **the instruction *"assert `attendance` is non-empty for a hostess with closed assignments"* cannot pass until surface 5 has actually closed `#7`.** **Two honest options, and pick one out loud rather than quietly weakening the assertion:** ① write the test against a fixture and defer the live assertion to step 3.5's journey, or ② run this step after 3.5. 🚫 **Do not soften it to "returns an array"** — `[]` is the exact bug this item exists to kill, and an assertion that `[]` satisfies is worse than none.
...
✅ **↳ as-built `14/08/2026` — both template rows and the CHECK are live, and this step's counts are CORRECT. Confirming rather than correcting, so nobody re-derives them:** `תבנית_מייל_אירוע_בוטל` carries exactly **3** placeholders — `[שם_דיילת] · [שם_פרויקט] · [תאריך_אירוע]` — and its body contains no contact-person token, which is why `resolveShiftContact` must not be called. `תבנית_מייל_פרטי_האירוע_השתנו` carries exactly **8** — the three above plus `[שעת_התחלה] · [שעת_סיום] · [כתובת_אירוע_מלאה] · [שם_מנהלת_פרויקט] · [טלפון_מנהלת_פרויקט]`. **And `email_log`'s CHECK is live at four values — `{quote, shift, project, project_report}`** ⇒ **AR-8's ordering obligation is discharged on the DB side; the Edge-Function half is the part still to confirm at deploy time.** ⚠️ **Read the bodies from `params` at build time anyway — a placeholder renamed in the row and not in the builder leaves an unfilled `[…]` in a customer-facing mail, and only the "no unfilled token" assertion below catches it.**
...
> | מנהלת פרויקטים | **edit** | edit | view | view | edit | ➖ |
