...
#     אחרי הקובץ האחרון *שהסשן הזה* שינה *בתיקייה הזו* (✏️ 24/09/2026 — ר' למטה).
# ‏stop_hook_active אמת ⇒ לא חוסמים פעם שנייה (✏️ 24/09/2026 — מניעת לולאה).
...
# --- הגנת-לולאה (נוסף 24/09/2026, לוח מ12 §7 "חוב קטן") ---
# ‏`stop_hook_active: true` = ‏Claude כבר ממשיך את התור *בגלל* חסימה קודמת של Stop hook. חסימה
# נוספת כאן אינה אכיפה אלא לולאה: ההודעה הראשונה כבר נמסרה, וסשן שלא תיקן אחריה לא יתקן אחרי
# השנייה. (תיעוד ה-hooks של Claude Code: לבדוק את השדה הזה כדי שה-Stop hook לא ירוץ בלי סוף.)
# ⚠️ המחיר, ביודעין: אחרי חסימה אחת, הסיום השני עובר גם אם התיעוד עדיין חסר.
...
# משווים מול זמן העריכה של *הסשן הזה*, לא מול NEWEST הגלובלי — כך שינויים לא-מקומיטים של
# סשן מקביל לא נספרים. בלי session_id → נשארים על NEWEST.
# ✏️ 24/09/2026 — "זמן העריכה של הסשן" נמדד עכשיו **רק על קבצים בתיקייה הזו שמופיעים בסימון**:
# ה-mtime החדש ביותר מבין שורות CHANGED (git status של התיקייה הזו) שהנתיב שלהן רשום בסימון.
# זה אותו חיתוך בדיוק שאכיפה 0 עושה למעלה.
# 🔑 **למה:** קודם נמדד ה-mtime של קובץ-הסימון עצמו, והסימון רושם גם נתיבים שאינם עבודה בתיקייה
# הזו. 📊 **נמדד 24/09/2026:** הסימון של סשן-הסגן החזיק רק
# `.claude/worktrees/agent-abb466…/docs/db_roadmap.md` ושתי מיגרציות של אותה תיקיית-עבודה מקוננת.
# ‏git של התיקייה הראשית אינו רואה את הקבצים האלה, אבל קובץ-הסימון התעדכן ⇒ כל סיום-תור נחסם
# על LOG/STATUS של התיקייה הראשית, שאיש לא נגע בה. (העקיפה אז: `touch` לשניהם בלי שינוי-תוכן.)
# ⚠️ **'*' בסימון** (היקף לא-ידוע — נתיב שלא נורמל) ⇒ ההתנהגות הישנה, mtime הסימון: אין דרך
# לדעת לאיזו תיקייה הכתיבה ההיא שייכת, ולכן נשארים בצד המחמיר.
# ⚠️ **גבול ידוע:** קובץ שהסשן ערך וכבר קימט אינו ב-CHANGED ולכן אינו נספר. זו אותה פשרה
# שהיציאה המוקדמת למעלה כבר עושה (הכרעת-ישי 12/08/2026) — מעדכנים LOG/STATUS לפני הקומיט.
if [ -n "$SESSION_ID" ] && [ -s "$MARK" ]; then
  if grep -qxF '*' "$MARK" 2>/dev/null; then
    ...
    while IFS= read -r line; do
      ...
      [ -f "$f" ] || continue
      f_l=$(printf '%s' "$f" | tr '[:upper:]' '[:lower:]')
      grep -qxF "$f_l" "$MARK" 2>/dev/null || continue
      m=$(stat -c %Y "$f" 2>/dev/null || echo 0)
      [ "$m" -gt "$EDIT_TS" ] && EDIT_TS=$m
    done <<< "$CHANGED"
  fi
else
  EDIT_TS=$NEWEST
