...
| **10** | ✏️ **Added 10/09/2026 — conditional, and it fires far more often than ❸ does: if the product has an onboarding/help layer, every hint anchor on this surface, each with its copy key, its level-2 text, and a column saying what the screen still shows with the layer off.** 🔴 **The last column is the deletion test in disguise** — fill it and discover a screen that is no longer operable, and an essential sentence has been demoted into the layer. **Why this row exists at all:** REG-IN's module 11 was the first module drawn after its layer shipped, and `design-contract.md`, `screens-approved.md` and this template together held **zero** mentions of it (measured). Four drawing agents produced sixteen surfaces without a layer and **not one of them could have known**. A capability the drawing contract does not name is a capability the drawer cannot draw. |
