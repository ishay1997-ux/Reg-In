...
3. **Semantic duplication jscpd CANNOT see:** hunt for functions/components/validators that duplicate the *intent* of code elsewhere without being textual copies — a formatter, a picker, a permission check, a money/date helper re-derived instead of imported from `src/lib/` (iron rule 14 says each such concept has exactly ONE home). This is where the real "should-be-shared but isn't" findings live.
