...
| Security, authorization, secrets | `code-modernization:security-auditor` — also run `npm run audit` and read its output for known dependency CVEs (a different signal from code-level OWASP review: this is third-party package vulnerabilities, not code the module wrote) |
