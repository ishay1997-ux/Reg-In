...
- 🔴 **The gap protocol — never silent-fill.** The blueprint is not always detailed enough, and the
  ...
  ואז הוא מנחש לפעמים"*).
  🆕 🔴 **Before you classify anything as a gap — look in the approved spec.** `docs/specs/module_NN_*/`
  (its `spec.md`, `processes-approved.md`, `screens-approved.md`, the approved mockups) is **tier 2** and
  ...
  not an impression from the guide. **Why this step is first and not optional:** without it a weight, a
...
🔴 **The approved spec is tier 2 in the truth hierarchy; this micro-guide is tier 5. Where they disagree on something the spec actually covers, the spec wins — per item, not per folder** — and the divergence is reported to Ishay + logged as a `↳ as-built`/§9 line, never resolved silently in the guide's favour. Its `spec.md` **§① is the module's own reading list** (which of `processes-approved.md` / `screens-approved.md` / the approved mockups / a research section to open — **and what NOT to read**): follow it instead of exploring. **Why this line exists at all:** measured 07–08/08/2026, `docs/specs` appeared **zero** times in this file, so the locked vocabulary, the word-for-word screen labels and the spec's hand-computed oracle reached the builder only if the micro-guide happened to copy them.

**Resolving the folder — derive it, do not guess it.** Same name as the step guide (`docs/CLAUDE.md` convention): `docs/guides/modules/module_NN_name.md` ⇒ `docs/specs/module_NN_name/`. ⚠️ **The leading zero is load-bearing** — `module_4_*` does not exist, `module_04_*` does, and a wrong path returns nothing and reads exactly like "this module has no spec". ⇒ **run `ls docs/specs/` and look, before concluding there is none.** **Genuinely no folder ⇒ say so out loud once** ("אין אפיון מאושר למודול הזה — מדריך-המיקרו הוא התוכנית היחידה") **and the micro-guide is the only plan** — that is the correct state for modules built before the Discovery flow, and it must be a stated finding rather than a silent assumption.
...
- 🆕 🔴 **An APPROVED mockup is not a tier-4 mockup — do not re-draw it and do not re-approve it.** The `מהמוקאפ — לאישורך` flag and the "limited-liability reference" stance were written for `docs/mockups/**` at **tier 4** (`CLAUDE.md`: *"וחלק מפרטיהם שגויים"*). **The test, not a list of paths:** does the module's approved spec **name this mockup as approved** (in practice, the `approved/` sub-folder its `spec.md` §① points at)? **Yes ⇒ tier 2 — Ishay already signed it off** ⇒ **build it as drawn.** The 🗣️ brief then shows *what you will build* and *where the spec was genuinely silent*; it does not re-open a settled screen. **No ⇒ tier 4, and everything above applies unchanged.** **Why re-asking is a failure and not diligence:** it makes Ishay re-decide what he already decided, which teaches him his rulings are not recorded — the same reasoning that retired the PM interview in `module-blueprint/template.md`.
  ⚠️ **And `אפיון-שותק` is a claim you must earn, not a default:** flag a detail that way only after checking `spec.md`, `processes-approved.md`, `screens-approved.md` **and that surface's own screen card**. Un-verified silence is a gap you did not look for, wearing the label of one you did.
  🔒 **What this does NOT subtract — the STATES rule above stands in full.** For an approved mockup the states are usually **already answered in its screen card** (*«מצבים · ולידציות»*) ⇒ **read them there and present them**, rather than re-deriving them or dropping them. A state the card does not cover is spec-silent by the test above, and goes to Ishay.
