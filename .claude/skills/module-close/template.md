...
- 🆕 🔴 **And read the spec set's PROCESS document too — the line above named only the acceptance chapter and the screen cards, and that is not the whole approved spec** *(added 12/08/2026, Ishay's question at a live close: the spec was authored **processes → screens → integration between them**, so the two can disagree — and only one of them was being audited)*. **Ask ONE bounded question, and do not re-derive the spec:** *for each built surface, does the process document describe a behaviour the system does not do, or does differently?*
  ...
  ⚖️ **Resolution order — and the audit does NOT rule, it reports:** ‏① both agree ⇒ nothing to do · ② they differ **and one is later + more specific to that item** ⇒ the later one governs and the divergence is recorded as a **dated** note (a spec set carrying such notes is normal, not a defect) · ③ a genuine collision on the same question with no later ruling ⇒ 🛑 **a finding for Ishay**, never resolved by the audit alone. *(The mockup-vs-spec split is a different rule and already lives in the spec set's own `§⚖️` arbitration section — don't restate it here.)*
