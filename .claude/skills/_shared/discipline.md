...
- 🔴 **A finding that contradicts a RECORDED decision is not a fix instruction — it is a conflict question.** Before a reviewer/verifier reports a defect, it searches the decision registers in its field of view (the module's rulings registry · the ledger · declared limitations · why-comments); a contradiction is reported as the TWO sources side by side, never patched. ⚠️ **The boundary this must not blur** — the `a ruling that exists is not a ruling that works` row above: a MECHANISM measured broken is a real defect even with a recorded ruling behind it; what may not be "fixed" on a reviewer's own authority is the recorded PRODUCT decision. 📍 **Load-path caveat (measured 26/08/2026): subagents do not load this file** — an orchestrator dispatching verifiers copies this rule VERBATIM into each verifier's task file (`module-build/SKILL.md`, the army block). *(Born from Ishay's question — "יגיד שמשהו ממצא אבל פיספס את התמונה הגדולה?" — and fired the same day: 4 of 15 module-5 panel findings were this class; one "defect" was a drawn legend that his own later ruling ㊶ had made incomplete — patching the code would have broken the ruling. Sibling rules: root `CLAUDE.md` rule 1's contradiction protocol · `quality-audit`'s owner-wins-on-operational-reality.)*
- 🔴 **The ask-first boundary (Ishay's instruction, 26/08/2026: *"לשים לב אילו דברים צריך לשאול אותי לפני ולא לעשות ישר ולבצע בשקט"*) — NOT a closed list:** even under a broad "continue to the end" mandate — ‏(א) a session-scoped directive of his is not a standing rule; the scope question goes back to him (the רעיון≠הכרעה family, root rule 1) · ‏(ב) an anchored self-closure that amends an APPROVED document is permitted only with the dated note written in the SAME session and the disclosure in the SAME message, phrased as overridable · ‏(ג) live-DB writes — the narrow demo-row exception and its rules live in `e2e/CLAUDE.md`; everything else waits · ‏(ד) anything a user will see or feel with no prior nod — his, per rule 1's anchor test. **Not on this list ≠ proceed silently — the list records the measured cases, not the boundary itself.**
