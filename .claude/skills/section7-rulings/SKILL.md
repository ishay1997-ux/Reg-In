...
## Discipline
The shared discipline blocks of the module skills apply here in full — **block A** (no citation without a same-turn check + the resume-after-interruption rule: a rulings session cut mid-batch re-derives position from §7's own markers, never from the transcript) and **block B** (how to talk to Ishay) — read them in `.claude/skills/module-build/SKILL.md` if not already loaded. Skill-specific instances: a §7 number is cited only after opening the registry this turn; NEVER close an item on your own authority — including "obviously stale" ones (those are *proposals* with evidence; his word closes them); an existence-answer ("לא קורה") is attributed to Ishay with its date like any ruling.
