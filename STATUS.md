...
|---|-------|-------|:-----:|-----|-----------|
| 0 | תשתית | ישי | ✅ | — | — |
| 1 | משתמשים והרשאות | ישי | 🔒 סגור (אודיט עבר) — ממתין ל-PR+merge של ישי; ליטוש סייד-בר (07/07) ממתין ל-commit על הענף | `ishay/module-1-permissions` | [ishay/01](docs/guides/ishay/01_close_module_1.md) |
...
## תזכורות תפעוליות
- 📘 **מדריך חדש לעמית (07/07):** [04b_claude_code_power_setup.md](docs/guides/amit/04b_claude_code_power_setup.md) — כיוונון Claude Code חזק (MCP/סקילים/פלאגינים). ‏`.mcp.json` כבר מגורסן ומספק Supabase MCP; עמית רק מאשר+מתחבר. קבצי המדריך לא בקומיט עדיין.
...
- ⚠️ **לפני PR של מודול 1 — שני קומיטים (הוראות מדויקות: [ishay/01 ④3](docs/guides/ishay/01_close_module_1.md)):** ‏(1) `Sidebar.jsx` (יישור הניווט, 07/07) ‏(2) כל קבצי ה-docs (חבילת פתיחת מודול 2 + מדריך 04b) — **חייבים להיכנס ל-PR הזה**: עמית על מחשב אחר, docs שלא עוברים דרך `dev` לא יגיעו אליו. לא להריץ `regin-pr-gate` לפני הקומיטים (היא מקמטת הכל בערימה אחת).
