...
## שאלות פתוחות
הרשימה הממוסמרת חיה **רק** ב-[PROJECT_MASTER §7](docs/PROJECT_MASTER.md). כרגע פתוחים: 1, 3, 4, 5, 6, 7, 9, 11–20. ‎**#21 נסגר (06/07/2026)** — ראו התבנית ב-§7.
...
- **נותר לפני פתיחת PR:** לאמת ב-Supabase Table Editor ש-`modules` = 9 שורות (ייתכן ש-10 הוזנו — §7 פריט 10) ו-`permissions` = 45. (אומת עקיפות ע"י ה-E2E: המטריצה נטענת עם שמות תפקידים תקינים ושינוי-תא נשמר ב-DB בפועל — אך לא אושרה ספירת השורות המדויקת.)
...
- ⚠️ נמצאו 4 קבצים לא-מזוהים ב-`docs/reference_spec/` (שמות עבריים, לא בגיט) — כנראה שמירות ידניות שלך של פלטי-סוכנים; לא נוגעו, לא נוספו ל-git.
