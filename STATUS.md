...
> עודכן לאחרונה: 07/07/2026 19:10
...
|---|-------|-------|:-----:|-----|-----------|
| 0 | תשתית | ישי | ✅ | — | — |
| 1 | משתמשים והרשאות | ישי | 🔒 סגור (אודיט עבר) — כולל מיגרציית initplan ‏07/07 (advisors נקיים); **קומיטים מקומיים ממתינים ל-push** (‏deps · initplan+snapshot · docs ×2, ‏16:37–16:56), ואז PR+merge של ישי | `ishay/module-1-permissions` | [ishay/01](docs/guides/ishay/01_close_module_1.md) |
...
## המסלול של ישי
**השלב הנוכחי:** [docs/guides/ishay/01_close_module_1.md](docs/guides/ishay/01_close_module_1.md) — להדביק את **פרומפט הסגירה המעודכן** (⑥, Opus): אודיט חוזר על מדריך המיקרו המסונכרן + **push של 3 הקומיטים הממתינים** (ה-docs כבר קומטו, העץ נקי); ואז הצעד הידני — פתיחת PR (base: `dev` ← compare: `ishay/module-1-permissions`, יש בלוק 🧩 מוכן במדריך), CI, ‏Merge. אחרי המיזוג → [ishay/02](docs/guides/ishay/02_module_04_hostesses.md) (מודול 4).
...
## שאלות פתוחות
הרשימה הממוסמרת חיה **רק** ב-[PROJECT_MASTER §7](docs/PROJECT_MASTER.md). כרגע פתוחים: 6, 7, 9, 14–20, 22, 23, 29–37, 39–41, 43–46. ‎**נסגרו באודיט 07/07/2026 (ישי):** ‎**#4** (ערכי מטריצה=האפיון; קיבוץ=תצוגה) · **#5** (ניווט = רשימה שטוחה של 9; 4-קבוצות רק כתצוגה בהרשאות; ‏11-בדשבורד = שאלת מודול 7) · **חבילת הכסף #1+25+26+27+28+42** (מע"מ 18% · עיגול אגורות-בחישוב/שקל-בתצוגה · הנחות בחיבור עם CHECK ≤100% · מדרגות tie-break-בקוד · הקפאת עלות-רכש · pg_cron למעברי-זמן). ‎**#12** (PDF = הפקה-בזמן-אמת, בלי אחסון) · **#38** (חשבונית = העלאה-ידנית-ושליחה, המערכת לא מפיקה). ‎#13 עדיין ממתין-לביצוע (Seed, חסם המע"מ הוסר). ‎**#25–46 נוספו (07/07/2026, אודיט פריטים פתוחים)** — סריקה שיטתית של C5+C6 מול הסכמה בשלושה מעברים (שתיקות-כלל · פערי-דאטה · פערי מנגנון-ביצוע); ‏#40–41 = "להנהון" (אישור קצר, לא דיון). הקרובים ביותר: ‏#40(א) ייחודיות roles/modules — **לפני ה-policies של מודול 2**; חבילת הכסף ‏#1+25+26+27+28 — לפני SSOT התמחור של מודול 3. ‎**#3 + #11 נסגרו (06–07/07/2026, פתיחת מודול 2)** — תוויות `customer_type` לפי האפיון; בלי מיזוג לקוחות (ח"פ קנוני + חיפוש סלחני). ‎**#21 נסגר (06/07/2026)** — ראו התבנית ב-§7. ‎**#10 נסגר בפועל** (modules=9 בוצע 02/07). ‎**#22–23 נוספו (06/07)** — פערי אפיון שאותרו בסריקה (לוגיסטיקה actual<planned; audit-trail). ‎**#24 נוסף ונסגר (06/07)** — סיסמאות משתמשי הבדיקה לא יוחלפו (סיכון מקובל, פרויקט אקדמי).
**חוסמים את הצעדים הקרובים:** ‎#1 מע"מ **נסגר** (18%). מודול 2 — אין חסמי §7; חסם יחיד: merge של מודול 1 ל-`dev`. לפני policies מודול 2: להנהן על §7.40(א) ייחודיות roles/modules.
...
- 📚 **ספריית פרומפטים מצבית חדשה (07/07/2026 15:45):** [docs/guides/prompt_library.md](docs/guides/prompt_library.md) — P1–P11 לפי סיטואציה (חילוץ, סטייה, undo, קונפליקט, רגרסיה, "עשיתי ידנית", §7 ועוד). מהיום: סיטואציה מיוחדת ← פותחים שם, לא ממציאים פרומפט.
...
- ✅ **הקומיטים לפני ה-PR בוצעו (07/07, עודכן 16:56):** ‏`594c26b`+`ec408d4` נדחפו; אחה"צ נוספו ‏`088cb39` (deps patch) · ‏`3da7baf` (**מיגרציית initplan** — advisors נקיים; תבנית §7.21 + בלופרינט מודול 2 עודכנו בנעילה) · ‏`c792c5d` (docs) · קומיט סנכרון-התיעוד של 16:56 — **קומטו, טרם נדחפו**. ‏push בפרומפט הסגירה או `regin-pr-gate`; רק אז לפתוח PR. אחרי ה-merge: להריץ `regin-docs-sync` ‏(Run now).
