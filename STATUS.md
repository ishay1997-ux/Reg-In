...
> עודכן לאחרונה: 29/07/2026 18:57 (ללא שינוי-מצב — עדיין ממתין להכרעתך על בעלות 3.3)
...
> **לא-מקומטים, מהסשן האחר**. **אל תיגע בהם ואל תריץ `git add -A`.** לוודא עם ישי מי בונה את 3.3
> לפני שממשיכים בו כאן.
...
**➡️ הצעד הפעיל: 3.3 — מסך ניהול ההצעות. 🗣️ המוקאפ אושר 29/07 18:15 — הבנייה טרם החלה
(נגמר מקום בסשן; נעצר במכוון ולא באמצע קוד).** המשך: **"תמשיך לבנות"** בסשן חדש.
המוקאפ המאושר: `scratch.local/mockup_3_3.html` (מחוץ ל-git). **7 הכרעות-ישי שנכללות בו:**
2 מדדים בלבד ליד הכותרת ("שווי הצעות פתוחות" · "שיעור אישור") · לשוניות **קו-תחתון** בסגנון
`01_overview_reworked.html` + לשונית `הכל` · "פג בקרוב" הוא **כפתור-מסנן** בשורת-הסינון (מושבת ב-0)
ולא אריח · מיון ברירת-מחדל **"הקרוב לפוג ראשון"** · עמודת איש-קשר עם **mailto** (כפתור-חיוג ירד —
במחשב הוא לא מחייג) · **"אחרי X% הנחה"** מתחת לסכום (רק כשיש הנחה) · פילוח **סיבות דחייה** בלשונית
נדחו. **נדחו במפורש:** שכפול-הצעה · עמודת רווח-גולמי (עמודה 8) · תאריך-שליחה · מספר-אורחים.
⚠️ **הכרעה שדורשת מיגרציה (שער-👤 typed-echo, טרם בוצע): סיבת דחייה שמינית "נפתחה בטעות"**
— היום הצעה שנפתחה בטעות מזהמת את שיעור-האישור. דורש CHECK חדש + החרגה מחישוב-השיעור +
write-back ל-§7.82/F2.
**שכפול-הצעה ירד** (הכרעתך).
